...
/**
 * Xml解析类 v1.13 @edit date 2013-06-21 @auth by wtr
 */
class Xml {
    ...
    public function _open(&$parser, $tag, $attributes)
    {
        $tag = $this->case_mode ? $tag : strtolower($tag);
        $this->_data_tmp['data'] = ''; // stores temporary cdata
        $this->_data_tmp['last_opened_tag'] = $tag;
        if ( is_array($this->_data_tmp['parent']) && array_key_exists( $tag, $this->_data_tmp['parent'] ))
        {
            if ( is_array( $this->_data_tmp['parent'][$tag]) && array_key_exists( 0, $this->_data_tmp['parent'][$tag]))
            {
    ...
    {
        if ( $this->_data_tmp['last_opened_tag'] != NULL )
        {
            $this->_data_tmp['data'] .= iconv('utf-8', $this->_encoding, $data);
        }
    }

    public function _close(&$parser, $tag)
    {
        $tag = $this->case_mode ? $tag : strtolower($tag);
    ...
    // 编码转换函数
    public function iconv_str($str, $code = 'gb2312') {
        $str = iconv('utf-8', $code, $str);
        return $str;
    }
    // 数组编码转换
    public function iconv_arr($arr, $code = 'gb2312') {
        if (! $arr)
            return false;
     ...
     */
    public function setPath($path, $flag = true) {
        if ($flag || $this->_path == "")
            $this->_path = $path;
        else
            $this->_path .= '->' . $path;
     ...
     * false:不存在 true存在
     */
    public function isNode($namepath, $flag = false) {
        if ($this->error)
            return false;
     ...
     * return null:不存在
     */
    public function getAttr($namepath, $flag = false)
    {
        if ($this->error)
            return false;
        $namepath = $this->case_mode ? $namepath : strtolower($namepath);
     ...
     */
    public function getAll($namepath = null, $flag = false)
    {
        if ($this->error)
            return false;
    ...
    public function getValue($namepath, $flag = false)
    {
        if ($this->error)
            return false;
        $namepath = $this->case_mode ? $namepath : strtolower($namepath);
    ...
    // 获取值列表
    public function getValueList($namepath, $flag = false)
    {
    ...
    public function error($show = false)
    {
        if ($show === false)
            return $this->error;
        if ($show === true)
    ...
    // 数组转xml
    public function getXMLFromArray($data, $encoding, $root = null)
    {
    ...
    // 数组转xml
    public function getXMLFromArrayNoHeader($data, $encoding, $root = null)
    {
        $xml = $this->_getXMLFromArray($data);
        if ($root != null) {
            $xml = sprintf($root, $xml);
    ...
    // 数组转xml
    public function _getXMLFromArray( $data )
    {
        $lineFeed = "";
        $str = "";
        foreach ($data as $key => $value) {
            if (! is_array($value)) {
                ...
                if (isset($value[0])) {
                    foreach ($value as $v) {
                        $str .= "<{$key}>" . $this->_getXMLFromArray($v) .
                            "</{$key}>{$lineFeed}";
                    }
                } else {
    ...
    public function getArrayNoRoot()
    {
        $arr = $this-> getAll();
