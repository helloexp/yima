...
class UploadFileExtension {
    ...
    private function save($file) {
        ...
        if (in_array(strtolower($file['extension']), 
            ...
            if (false === $info || ('gif' == strtolower($file['extension']) && empty(
                $info['bits']))) {
                $this->error = '非法图像文件';
                return false;
            }
        ...
        if (! move_uploaded_file($file['tmp_name'], 
            $this->autoCharset($filename, 'utf-8', 'gbk'))) {
            $this->error = '文件上传保存错误！';
            return false;
        }
        if ($this->thumb && in_array(strtolower($file['extension']), 
            array(
                'gif', 
                'jpg', 
                'jpeg', 
            ...
            if (false !== $image) {
                ...
                $thumbPath = $this->thumbPath ? $this->thumbPath : dirname(
                    $filename) . '/';
        ...
        foreach ($files as $key => $file) {
            ...
            if (! empty($file['name'])) {
                ...
                if (function_exists($this->hashType)) {
                    ...
                    $file['hash'] = $fun(
                        $this->autoCharset(
                            $file['savepath'] . $file['savename'], 'utf-8', 
                            'gbk'));
                }
                // 获得宽高
                $file['width'] = $this->getWidth($filePath);
                $file['height'] = $this->getHeight($filePath);
