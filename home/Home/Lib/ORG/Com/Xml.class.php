...
class Xml {
    ...
    public function getXMLFromArray($arr, $includeHeader = true) {
        ...
        if (! $includeHeader)
            return $str;
        $str = $this->xmlHeader . $lineFeed . $this->xmlRoot . $lineFeed . $str .
             $this->xmlFoot;
        if ($this->encodeType != self::ENCODE_TYPE_UTF8) {
            $str = mb_convert_encoding($str, $this->encodeType, 
                self::ENCODE_TYPE_UTF8);
        }
        return $str;
    }
    ...
    function getArrayFromXml($xml) {
        ...
        if (preg_match_all($reg, $xml, $matches)) {
            ...
            if ($this->encodeType != self::ENCODE_TYPE_UTF8) {
                $xml = mb_convert_encoding($xml, self::ENCODE_TYPE_UTF8, 
                    $this->encodeType);
            }
            return $xml;
        }
