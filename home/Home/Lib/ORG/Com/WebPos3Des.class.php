...
class WebPos3Des {
     ...
     * 主密钥和工作密钥从终端表中获取 签到 $i_mac_str = ''.$pos_id.$user_id.$pos_seq; 验证
     * $i_mac_str = ''.$pos_id.$user_id.$encode_type.$valid_info.$pos_seq; 冲正
     * $i_mac_str =
    ...
    function login_mac($i_mac_src, $master_key) {
        ...
        $real_master_key = $this->mdecrypt_3des_cbc(pack("H*", $master_key), 
            $root_key);
        $real_master_key = $real_master_key . substr($real_master_key, 0, 8);
        $md5_mac_src = pack("H*", md5($i_mac_src));
        return strtoupper(
            bin2hex($this->mcrypt_3des_cbc($md5_mac_src, $real_master_key)));
    }
    ...
    function trans_mac($i_mac_src, $master_key, $work_key) {
        ...
        $real_master_key = $this->mdecrypt_3des_cbc(pack("H*", $master_key), 
            $root_key);
        $real_master_key = $real_master_key . substr($real_master_key, 0, 8);
        $real_work_key = $this->mdecrypt_3des_cbc(pack("H*", $work_key), 
            $real_master_key);
        $real_work_key = $real_work_key . substr($real_work_key, 0, 8);
        $md5_mac_src = pack("H*", md5($i_mac_src));
        return strtoupper(
            bin2hex($this->mcrypt_3des_cbc($md5_mac_src, $real_work_key)));
    }
