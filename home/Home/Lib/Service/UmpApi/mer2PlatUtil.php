...
class ReqDataUtil {
    ...
    public function getReqData($appname, $map, $funcode, $method) {
        ...
        if ($method == method_get) {
            ...
            // $reqData->setUrl($url);
            $log->logInfo(
                $funcode . " url=" . $url . "?" . $param . '&sign=' . $sign);
            // 获取POST方式请求数据对象
    ...
    public function makeRequestData($appname, $map, $method) {
        ...
        if ($method == method_get) {
            ...
            $log->logInfo(
                "The service " . $funcode .
                     " request platform to get the address url=" . $url . "?" .
                     $param . '&sign=' . $sign);
            // 获取POST方式请求数据对象
        } else if ($method == method_post) {
            $url = $this->getUrlForV4($appname);
            $log->logInfo(
                "The service " . $funcode .
                     " request platform to post the address url=" . $url);
            $reqData->setUrl($url);
            $map->put("sign", $sign);
    ...
    private function getSignData($map) {
        ...
        $log->logInfo(
            "Participate in the signature parameters：[" . $plain .
                 "],Merchant number is：[" . $merId . "]");
...
class WebReqPayData {
    ...
    public function getData($map) {
        ...
        if ("" == $merId || strlen($merId) > 8)
            die("merId为空或者长度超过限制");
        $orderId = StringUtil::trim($map->get("orderId"));
        if ("" == $orderId || strlen($orderId) > 32)
            die("orderId为空或者长度超过限制");
        $merDate = StringUtil::trim($map->get("merDate"));
        if (! DateUtil::checkData($merDate))
            die("merDate为空或者长度不符合要求");
        $amount = StringUtil::trim($map->get("amount"));
        if ("" == $amount || strlen($amount) > 13)
            die("amount为空或者长度超过限制");
        $amtType = StringUtil::trim($map->get("amtType"));
        if ("" == $amtType || strlen($amtType) != 2)
            die("amtType为空或者长度不符合要求");
        $retUrl = StringUtil::trim($map->get("retUrl"));
        if ("" == $retUrl || strlen($retUrl) > 128)
            die("retUrl为空或者长度超过限制");
...
class MerToPlatFactory {
    ...
    public static function getData($map) {
        ...
        if ("pay_req_split_front" == $service || "pay_req_split_back" == $service ||
             "pay_req_split_direct" == $service) {
            ...
            return MerToPlatDataProcessor::getData($map, $fields1, $fields2);
        } elseif ("pay_req_ivr_call" == $service ||
             "pay_req_ivr_tcall" == $service || "pay_req" == $service) {
...
class DirectPayData {
    ...
    public function getData($map) {
        ...
        if ("" == $merId || strlen($merId) > 8)
            die("merId为空或者长度超过限制");
        $goodsId = StringUtil::trim($map->get("goodsId"));
        if ("" == $goodsId || strlen($goodsId) > 8)
            die("goodsId为空或者长度超过限制");
        $mobileId = StringUtil::trim($map->get("mobileId"));
        if ("" == $mobileId || strlen($mobileId) != 11)
            die("mobileId为空或者长度不符合要求");
        $orderId = StringUtil::trim($map->get("orderId"));
        if ("" == $orderId || strlen($orderId) > 32)
            die("orderId为空或者长度超过限制");
        $merDate = StringUtil::trim($map->get("merDate"));
...
class MerRefundData {
    ...
    public function getData($map) {
        ...
        if ("" == $refundNo || strlen($refundNo) > 16)
            die("refundNo为空或者长度超过限制");
        $orderId = StringUtil::trim($map->get("orderId"));
        if ("" == $orderId || strlen($orderId) > 32)
            die("orderId为空或者长度超过限制");
        $merDate = StringUtil::trim($map->get("merDate"));
        ...
        if ("" == $payAmount || strlen($payAmount) > 13)
            die("payAmount为空或者长度超过限制");
...
class TransBillData {
    ...
    public function getData($map) {
        ...
        if (! DateUtil::checkData($payDate))
            die("payDate为空或者长度不符合要求");
        $version = StringUtil::trim($map->get("version"));
        if ("" == $version || strlen($version) != 3)
            die("version为空或者长度不符合要求");
        $data->put("merId", $merId);
...
class SettleBillData {
    ...
    public function getData($map) {
        ...
        if (! DateUtil::checkData($settleDate))
            die("settleDate为空或者长度不符合要求");
        $version = StringUtil::trim($map->get("version"));
        if ("" == $version || strlen($version) != 3)
            die("version为空或者长度不符合要求");
        $data->put("merId", $merId);
...
class NotifyResData {
    ...
    public static function getNotifyResDataPlain($map) {
        ...
        if ("" == $retCode || strlen($retCode) != 4)
            die("retCode为空或者长度不符合要求");
        $retMsg = StringUtil::trim($map->get("retMsg"));
        if ("" == $retMsg || strlen($retMsg) > 128)
            die("retMsg为空或者长度不符合要求");
        $version = StringUtil::trim($map->get("version"));
        if ("" == $version || strlen($version) != 3)
            die("version为空或者长度不符合要求");
        $data->put("merId", $merId);
