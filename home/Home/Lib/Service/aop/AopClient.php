                ...
                } else // 文件上传用multipart/form-data，否则用www-form-urlencoded
{
    ...
    protected function logCommunicationError($apiName, $requestUrl, $errorCode, 
        $responseTxt) {
        log_write(
            'apiName:' . $apiName . '==requestUrl:' . $requestUrl .
                 '==errorCode:' . $errorCode . '==responseTxt:' . $responseTxt);
        /*
         * $localIp = isset ($_SERVER["SERVER_ADDR"]) ? $_SERVER["SERVER_ADDR"]
         * : "CLI"; $logger = new LtLogger; $logger->conf["log_file"] =
         * rtrim(AOP_SDK_WORK_DIR, '\\/') . '/' . "logs/aop_comm_err_" .
         * $this->appId . "_" . date("Y-m-d") . ".log";
         * $logger->conf["separator"] = "^_^"; $logData = array ( date("Y-m-d
         * H:i:s"), $apiName, $this->appId, $localIp, PHP_OS,
         * $this->alipaySdkVersion, $requestUrl, $errorCode, str_replace("\n",
    ...
    public function execute($request, $authToken = null) {
        ...
        try {
            ...
            // $resp = HttpRequest::sendPostRequst($this->gatewayUrl,
            // $paramsArr) ;
        } catch (Exception $e) {
        ...
        /*
         * if (isset ($respObject->code)) { $logger = new LtLogger;
         * $logger->conf["log_file"] = rtrim(AOP_SDK_WORK_DIR, '\\/') . '/' .
         * "logs/aop_biz_err_" . $this->appId . "_" . date("Y-m-d") . ".log";
    ...
    public function exec($paramsArray) {
        ...
        $requestClassName = ucfirst(
            $inflector->camelize(substr($paramsArray["method"], 7))) . "Request";
    ...
    public function checkSignAndDecrypt($params, $rsaPublicKeyPem, 
        $rsaPrivateKeyPem, $isCheckSign, $isDecrypt) {
    ...
    public function encryptAndSign($bizContent, $rsaPublicKeyPem, 
        $rsaPrivateKeyPem, $charset, $isEncrypt, $isSign) {
        // 加密，并签名
        if ($isEncrypt && $isSign) {
            $encrypted = $this->rsaEncrypt($bizContent, $rsaPublicKeyPem, 
                $charset);
