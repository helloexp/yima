...
class WeixinRedPackService {
    ...
    public function init($nodeId) {
        ...
        if (empty($wxConfig['apiclient_cert']) ||
             empty($wxConfig['apiclient_key'])) {
    ...
    public function sendPack($openId, $bId, $mId, $requestId) {
        // 获取红包信息
        $redPackInfo = M('tbatch_info')->where(
            "id='{$bId}' and batch_class='22'")->find();
        if (! $redPackInfo) {
            $this->error = "未找到红包信息";
        ...
        $sendData = array( // 接口参数全必填
            ...
            'mch_billno' => $this->getMchBillno(),  // 商户订单号（每个订单号必须唯一组成：mch_id+yyyymmdd+10位一天内不能重复的数字。接口根据商户订单号支持重入，如出现超时可再调用
            'mch_id' => $this->mchId,  // 微信支付分配的商户号
            'wxappid' => $this->wxappId,  // 微信分配的公众账号ID
            'send_name' => $redPackInfo['material_code'],  // 红包发送者名称
            're_openid' => $openId,  // 接受红包的用户用户在wxappid下的openid
        ...
        if ($result) {
            ...
            $traceData['status'] = '1';
            $traceData['wx_ret_msg'] = "[{$this->errCode}]{$this->error}";
    ...
    public function sendBegin($data) {
        $sendData = array( // 接口参数全必填
            ...
            'mch_billno' => $data['mch_billno'],  // 商户订单号（每个订单号必须唯一组成：mch_id+yyyymmdd+10位一天内不能重复的数字。接口根据商户订单号支持重入，如出现超时可再调用
            'mch_id' => $this->mchId,  // 微信支付分配的商户号
            'wxappid' => $this->wxappId,  // 微信分配的公众账号ID
            'send_name' => $data['send_name'],  // 红包发送者名称
            're_openid' => $data['re_openid'],  // 接受红包的用户用户在wxappid下的openid
            'total_amount' => $data['total_amount'],  // 付款金额，单位分
            'total_num' => $data['total_num'],  // 红包发放总人数
            'wishing' => $data['wishing'],  // 红包祝福语
            'client_ip' => $data['client_ip'],  // 调用接口的机器Ip地址
    ...
    protected function curlPostSsl($url, $vars, $second = 30, $aHeader = array()) {
        ...
        if ($data) {
            ...
            if ($arr['xml']['result_code'] == 'SUCCESS') {
                return $arr['xml'];
            } else {
                $this->errCode = $arr['xml']['err_code'];
                $this->error = $arr['xml']['return_msg'];
                return false;
            }
        } else {
            $this->error = "接口请求失败,errorNo:{$errorNo} errorStr:{$errorStr}";
            log_write(
                "WeixinRedPackLog:{$this->nodeId}接口请求失败,errorNo:{$errorNo} errorStr:{$errorStr}");
            log_write("url:{$url} var: {$vars} 请求失败 errorNo:{$errorNo} errorStr:{$errorStr}", 'WARN', 'curlPostSsl');
            curl_close($ch);
            return false;
        }
    }
    ...
    public function getMchBillno() {
        return $this->mchId . date('Ymd') . substr(time(), - 5) .
    ...
    protected function createCertFile($apiclientCert, $apiclientKey) {
        ...
        if (! is_file($fileName) ||
             ceil((time() - filemtime($fileName)) / 3600) > 24) { // 24小时重新生成文件
