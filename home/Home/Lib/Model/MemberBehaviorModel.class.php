 ...
 */
class MemberBehaviorModel extends BaseModel {
     ...
     * type=13关注微信公众号 type14活动参与中红包 type15活动中奖并中积分卡券 type16参与图文编辑营销活动 type=17
     * 为小店商城抵扣积分 type=1,type=2 $relationId
     */
    public function addBehaviorType($memberId, $nodeId, $type, $point = '', $relationId = '', $amt = '') {
        if (empty($memberId) || empty($type) || empty($nodeId)) {
            log_write(
                "会员号或商户号或类型或者备注为空" . "会员号：" . $memberId . "类型：" . $type . '机构号：' .
                     $nodeId, '关联ID：' . $relationId);
            return false;
        }
        $behaviorArr = array(
            'member_id' => $memberId, 
        ...
        if ($type == 1) {
            ...
            $behaviorArr['remark'] = "活动名称：" .
                 $this->getActivityName($relationId);
        ...
        if ($type == 5) {
            ...
            $this->addBehaviorData($memberId, $nodeId, '', '', '', '', 1);
        }
        // type=6手动减积分
        if ($type == 6) {
            $behaviorArr['change_num'] = $point;
        ...
        if ($type == 14) {
            ...
            $behaviorArr['relation_id'] = $relationId;
            $behaviorArr['remark'] = "红包名称:" . $bounsArr['bonus_page_name'] .
                 "红包金额：" . $bounsArr['bonus_amount'];
        }
        // type15活动中奖并中积分奖品
        if ($type == 15) {
            $behaviorArr['relation_id'] = $relationId;
        ...
        if ($type == 17) {
            $behaviorArr['relation_id'] = $relationId;
            $behaviorArr['remark'] = "订单号：" . $relationId;
            $behaviorArr['change_num'] = $point;
        }
        $res = M("tintegral_behavior_trace")->add($behaviorArr);
        if ($res === false) {
            log_write("Mysql：".M()->getLastSql());
    ...
    public function addBehaviorData($memberId, $nodeId, $joinTotal = 0, 
        $sendTotal = 0, $verifyTotal = 0, $shopTotal = 0, $shopLineTotal = 0) {
        if (empty($memberId) || empty($nodeId)) {
            log_write($memberId . "会员ID或者nodeId为空！");
            return false;
        }
        $BehaviorInfo = M("tmember_activity_total")->where(
            array(
                'node_id' => $nodeId, 
                'member_id' => $memberId))->find();
        log_write('YYYY:'.M()->getLastSql()."行为数据total:".print_r($BehaviorInfo,true));
        $BehaviorData = array(
            'node_id' => $nodeId, 
    ...
    public function _addNewBehavior($BehaviorData) {
        $activityInfo = $this->behaviorDataMerge($BehaviorData['member_id'], 
            $BehaviorData['node_id']);
        if ($activityInfo) {
            if ($activityInfo['join_total']) {
                $BehaviorData['join_total'] = $BehaviorData['join_total'] +
                     $activityInfo['join_total'];
            }
            if ($activityInfo['send_total']) {
                $BehaviorData['send_total'] = $BehaviorData['send_total'] +
                     $activityInfo['send_total'];
            }
            if ($activityInfo['verify_total']) {
                $BehaviorData['verify_total'] = $BehaviorData['verify_total'] +
                     $activityInfo['verify_total'];
            }
    ...
    public function _updateBehavior($BehaviorData, $BehaviorInfo) {
        ...
        $res = M("tmember_activity_total")->where(
            array(
                'member_id' => $BehaviorInfo['member_id']))->save($data);
        if ($res === false) {
            log_write($BehaviorData['member_id'] . "新增行为数据失败！");
            return false;
        }
    ...
    public function behaviorDataMerge($memberId, $nodeId) {
        ...
        if ($mergeFlag) {
            $activityInfo = M("tmember_activity_stat")->where(
                array(
                    'node_id' => $nodeId, 
                    'member_id' => $memberId))->getField(
                'sum(join_cnt) join_total,sum(send_cnt) send_total,sum(verify_cnt) verify_total,sum(shop_cnt)');
            if ($activityInfo) {
                return $activityInfo;
            }
        }
    ...
    public function getActivityName($mId) {
        $activityName = M("tmarketing_info")->where(
            array(
                'id' => $mId))->getField('name');
        if ($activityName) {
            return $activityName;
        }
    }
    ...
    public function getPosName($posId) {
        $posName = M("tpos_info")->where(
            array(
                'pos_id' => $posId))->getField('pos_name');
        if ($posName) {
            return $posName;
        }
    }
    ...
    public function getBounsName($id) {
        $getBonusName = M()->table("tbonus_use_detail a")->join(
            'tbonus_info b on b.id=a.bonus_id')
            ->where(array(
            'a.id' => $id))
