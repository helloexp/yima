 ...
 */
class TwoFestivalCjSetModel extends CjSetModel {
    ...
    public function verifyReqDataForWeel($requestedValue, $nodeId) {
        ...
        try {
            ...
            $phoneDayPart = M('tcj_rule')->where(
                array(
                    'batch_id' => $batchInfo['id']))->getField('phone_day_part');
            if ($batchInfo['click_count'] > 0 &&
                 $result['phone_day_part'] != $phoneDayPart) {
                throw_exception('已经有人访问过活动,不能修改日参与次数');
            }
            ...
            if ($result['join_mode'] == '0') {
                ...
                $result['phone_total_count'] = 1;
                $result['phone_day_count'] = 1;
            }
