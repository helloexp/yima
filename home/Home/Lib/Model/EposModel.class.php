...
class EposModel extends Model {
    ...
    public function get_batch_info_list($pos_id, $like = null) {
        $where = "pos_id = '" . $pos_id . "' ";
        ...
        if (! $pos_info) {
            $this->_log(
                "get_batch_info_list find tpos_info error " . M()->_sql());
            return $this->return_json("1001", null);
        }
        if ($like != null) {
            $like_where = "and (b.material_code like '%" . $like .
                 "%' or b.batch_short_name like '%" . $like . "%' )";
        } else {
            $like_where = '';
        }
        ...
        // $end_time;
        $where = "(store_id_list LIKE '%" . $pos_info['store_id'] .
             "%' OR (c.node_id = '" . $pos_info['node_id'] .
             "' AND TYPE = '1')) AND b.status ='0' AND b.node_id='".$pos_info['node_id']."' AND g.status='0' " . $like_where . $end_time;
        import('ORG.Util.Page'); // 导入分页类
        $count = M()->table('tbatch_info b')
            ->join(
            'left join tgoods_info g on b.goods_id = g.goods_id inner join tbatch_info_tostore_exp c on b.id = c.b_id')
            ->where($where)
            ->count();
        ...
        if (! $batch_info_list) {
            $this->_log(
                "get_batch_info_list find tbatch_info error " . M()->_sql());
    ...
    public function get_batch_info($pos_id, $batch_info_id) {
        ...
        $pos_info = M()->table('tpos_info')
            ->where($where)
            ->find();
        log_write('$pos_info sql:'.var_export(M()->_sql(),1));
        log_write('$pos_info:'.var_export($pos_info,1));
        if (! $pos_info) {
        ...
        $batch_info = M()->table('tbatch_info')
            ->where($where)
            ->find();
        log_write('$batch_info sql:'.var_export(M()->_sql(),1));
        log_write('$batch_info:'.var_export($batch_info,1));
        if (! $batch_info) {
        ...
        $goods_info = M()->table('tgoods_info')
            ->where($where)
            ->find();
        log_write('$goods_info sql:'.var_export(M()->_sql(),1));
        log_write('$goods_info:'.var_export($goods_info,1));
        if (! $goods_info) {
        ...
        $data['batch_short_name'] = $batch_info['batch_short_name'];
        $data['custom_no'] = $batch_info['material_code'];
        $data['img_url'] = get_upload_url(
            './Home/Upload/' . $goods_info['goods_image']);
        if ($batch_info['storage_num'] == - 1)
            $data['remain_num'] = - 1;
        else
            $data['remain_num'] = $batch_info['remain_num'];
        ...
        $rs = M()->table('tpos_day_count')
            ...
            ->find();
        log_write('$rs sql:'.var_export(M()->_sql(),1));
        log_write('$rs:'.var_export($rs,1));
        if (! $rs) {
        ...
        log_write('$data:'.var_export($data,1));
        return $this->return_json("0000", $data);
    }

    function send_reqid() {
        $data = M()->query(
            "SELECT _nextval('epos_send_seq') as reqid FROM DUAL");
        if (! $data) {
    ...
    public function batch_info_send($pos_id, $epos_acount, $batch_info_id, 
        $phone_no, $send_num) {
        $this->_log(
            "batch_info_send find tpos_info error $pos_id, $epos_acount, $batch_info_id, $phone_no, $send_num");
        $where = "pos_id = '" . $pos_id . "' ";
        $pos_info = M()->table('tpos_info')
            ->where($where)
            ->find();
        ...
        if (! $marketing_info) {
            $this->_log(
                "batch_info_send find marketing_info error " . M()->_sql());
            ...
            if ($batch_info['storage_num'] != - 1) // 非不限库存
{
                if (($batch_info['remain_num'] - 1) >= 0) {
                    ...
                    if ($rs === false) {
                        ...
                        $this->_log(
                            "batch_info_send update tbatch_info error " .
                                 M()->_sql());
                    ...
                    $data['err_info'] = '库存不足，发送成功' . $data['succ_num'] . '个';
                    $this->_log(
                        "batch_info_send remain_num not enough " .
                             $data['err_info']);
                    return $this->return_json("1007", $data);
                }
            }
            $TransactionID = $this->send_reqid();
            $req_data = "&node_id=" . $batch_info['node_id'] . "&phone_no=" .
                 $phone_no . "&batch_no=" . $batch_info['batch_no'] .
                 "&request_id=" . $TransactionID . "&data_from=" . $data_from .
                 "&batch_info_id=" . $batch_info_id . "&batch_type=" .
                 $marketing_info['batch_type'] . "&store_id=" .
                 $pos_info['store_id'] . "&channel_id=&user_id=" . $epos_acount .
                 "&pos_id=" . $pos_id;
    ...
    public function get_send_list($pos_id, $page, $like = null) {
        $where = "pos_id = '" . $pos_id . "' ";
        $pos_info = M()->table('tpos_info')
            ->where($where)
            ->find();
        ...
        if ($like != null) {
            $like_where = "and (b.phone_no like '%" . $like .
                 "%' or i.batch_short_name like '%" . $like . "%' )";
        } else {
            $like_where = '';
        }
        $where = "b.pos_id = '" . $pos_id . "'  and b.trans_type = '0001' " .
             $like_where;
        
        import('ORG.Util.Page'); // 导入分页类
        $count = M()->table('tbarcode_trace b')
        ...
        $show = $Page->show();
        $barcode_trace_list = M()->table('tbarcode_trace b')
            ->join('left join tbatch_info i on b.b_id = i.id')
            ->where($where)
            ->field(
            'b.req_seq as req_seq,  i.batch_short_name as batch_short_name, b.phone_no as phone_no, b.trans_time as trans_time')
            ->order('b.id desc')
            ->page($page . ',20')
            ->select();
        ...
        if (! $barcode_trace_list) {
            $this->_log(
                "get_send_list find tbarcode_trace error " . M()->_sql());
    ...
    public function get_send_info($pos_id, $req_seq) {
        ...
        $pos_info = M()->table('tpos_info p')
            ->join(
            'LEFT JOIN tstore_info t ON p.`store_id` = t.`store_id`  LEFT JOIN  tnode_info n ON p.`node_id` = n.`node_id` ')
            ->where($where)
            ->find();
        if (! $pos_info) {
        ...
        $where = "b.pos_id = '" . $pos_id .
             "' and b.trans_type = '0001' and req_seq = '" . $req_seq . "'";
        $barcode_trace = M()->table('tbarcode_trace b')
            ->join('left join tbatch_info i on b.b_id = i.id')
            ->where($where)
            ->field(
            'b.req_seq as req_seq,  i.batch_short_name as batch_short_name, b.phone_no as phone_no, b.trans_time as trans_time, b.status as status, b.user_id as user_id')
            ->find();
        if (! $barcode_trace) {
            $this->_log(
                "get_send_info find tbarcode_trace error " . M()->_sql());
            return $this->return_json("1004", null);
        }
    ...
    public function resend($pos_id, $req_seq, $epos_acount) {
        ...
        // 校验原流水
        $where = "b.pos_id = '" . $pos_id .
             "' and b.status = '0' and b.trans_type = '0001' and req_seq = '" .
             $req_seq . "'";
        $barcode_trace = M()->table('tbarcode_trace b')
            ->join('left join tbatch_info i on b.b_id = i.id')
            ->where($where)
            ->field(
            'b.req_seq as req_seq,  i.batch_short_name as batch_short_name, b.phone_no as phone_no, b.trans_time as trans_time, b.status as status, b.user_id as user_id, b.node_id as node_id, b.request_id as request_id')
            ->find();
        if (! $barcode_trace) {
            $this->_log(
                "get_send_info find tbarcode_trace error " . M()->_sql());
        ...
        // 调用重发接口
        $data['err_info'] = '';
        $req_data = "a=CodeResendReq&node_id=" . $barcode_trace['node_id'] .
             "&request_id=" . $barcode_trace['request_id'] . "&user_id=" .
             $epos_acount;
    ...
    public function cancel($pos_id, $req_seq, $epos_acount) {
        ...
        // 调用撤销接口
        $data['err_info'] = '';
        $req_data = "a=CodeCancelByReqId&node_id=" . $barcode_trace['node_id'] .
             "&request_id=" . $barcode_trace['request_id'] . "&user_id=" .
             $epos_acount;
        $resp_array = $this->requestWcAppServ($req_data);
        if (! $resp_array || ($resp_array['resp_id'] != '0000' &&
    ...
    public function get_send_stat($pos_id) {
        ...
        $data['total_send_num'] = $rs['send_num_sum'];
        // 取该终端发送统计数 当日
        $where = "pos_id = '" . $pos_id . "' and trans_date = '" . date('Y-m-d') .
             "'";
        $rs = M()->table('tpos_day_count')
            ->where($where)
            ->field('ifnull(SUM(send_num), 0) as send_num_sum')
        ...
        $data['day_send_num'] = $rs['send_num_sum'];
        return $this->return_json("0000", $data);
    }
     ...
     * 获取发布到旺财APP上的活动列表
     */
    public function getWcAppBatchChannelList($pos_id, $currentPage = 1, $limit = 8) {
        $where = "pos_id = '" . $pos_id . "' ";
        $pos_info = M()->table('tpos_info')
            ->where($where)
            ->find();
        ...
        $where = "b.node_id = '" . $pos_info['node_id'] . "' AND b.status = '1' AND m.status = '1' AND c.status = '1'
 and c.sns_type = '62' and m.end_time >=".date('YmdHis');
        $count = M()->table('tbatch_channel b ')
            ->join(
            'LEFT JOIN tmarketing_info m ON b.batch_id = m.id LEFT JOIN tchannel c ON b.channel_id = c.id')
            ->where($where)
            ->count();
        ...
        $batch_channel_list = M()->table('tbatch_channel b ')
            ...
            ->where($where)
            ->field(
            "b.id, m.name, m.start_time, m.end_time, b.batch_type")
            ->order('b.id desc')
            ->limit($limit)
            ->page($currentPage)
            ->select();
        ...
        if (! $batch_channel_list) {
            $this->_log(
                "getWcAppBatchChannelList find tbatch_channel error " .
                     M()->_sql());
            return $this->return_json("1004", null);
        }
        $data_arr = array();
        $data_arr['count'] = 0;
        $data_arr['page'] = ceil($count / $limit);
        $batch_channel_name = C('BATCH_TYPE_NAME');
        foreach ($batch_channel_list as $batch_channel) {
            $data['batch_name'] = $batch_channel['name'];
            $data['start_date'] = date('Y-m-d', 
                strtotime($batch_channel['start_time']));
            $data['end_date'] = date('Y-m-d', 
                strtotime($batch_channel['end_time']));
            $data['batch_type'] = $batch_channel['batch_type'];
            $data['batch_type_name'] = $batch_channel_name[$batch_channel['batch_type']];
            $data['batch_url'] = $url = C('PAYGIVE_DOMAIN') . U(
                'Label/Label/index', 
                array(
                    'id' => $batch_channel['id']));
            $data_arr['send_list'][] = $data;
     ...
     * @param type $storeId
     */
    public function getPosId($nodeId, $storeId){
        $posArray = M('tpos_info')->where(array('store_id'=>$storeId, 'node_id'=>$nodeId))->field('pos_id, pos_type')->find();
        return $posArray;
    }
}