 ...
 */
class DrawLotteryModel extends BaseModel {
     ...
     */
    public function initParam($labelId, $mobile, $fullId = null, $ticketSeq = null, 
        $otherParam = array()) {
        ...
        if (! empty($otherParam) && is_array($otherParam)) {
    ...
    public function getLabel() {
        ...
        $where = array(
            'id' => $this->labelId);
        $this->batchChannelInfo = $this->getBatchChannel($where, 
        ...
        $this->cjWhiteBlack = $this->getNodeInfo(
            ...
            BaseModel::SELECT_TYPE_FIELD, 'cj_white_black');
     ...
     * @return mixed
     */
    public function getCodeVerify($where, 
        $selectType = BaseModel::SELECT_TYPE_ONE, $field = '') {
    ...
    public function getGoodsInfoAndBounsInfo($where) {
        $return = M()->table('tcj_batch a')
            ->field(
            'g.goods_name,g.goods_id,g.id,g.goods_type,g.bonus_id,b.link_url,i.node_name,a.b_id,g.goods_image,g.node_id')
            ->join('tgoods_info g ON a.goods_id=g.goods_id')
            ->join('tbonus_info b ON b.id=g.bonus_id')
            ->join('tnode_info i ON i.node_id=g.node_id')
            ->where($where)
            ->find();
        return $return;
    }
     ...
     * @return int
     */
    public function getDrawLotteryLeftChances($labelId, $batchType, $batchId, 
        $mobile, $totalPart) {
     ...
     * @return int
     */
    public function getDrawLotteryLeftChancesPerDay($labelId, $batchType, 
        $batchId, $drawLotteryId, $totalPartPerDay) {
    ...
    public function getCjInfo($marketInfo = array()) {
        ...
        $cjRuleWhere = array(
            ...
            'status' => '1');
        $cjRule = $this->getCjRule($cjRuleWhere, BaseModel::SELECT_TYPE_ONE, 
        ...
        if ($cjRule) {
            $cjCateList = $this->getCjCate(
                array(
                    ...
                    'id' => array(
                        'in', 
                        $cjCateIds)), BaseModel::SELECT_TYPE_ALL, 'id,name');
        }
        
        // 处理页面奖项奖项
        $cjCateId = array();
        $cjCateName = '';
        foreach ($cjCateList as $vv) {
            $cjCateId[] = $vv['id'];
            $cjCateName .= '"' . $vv['name'] . '",';
        }
        return array(
     ...
     * @return mixed
     */
    public function getWeixinInfo($where, 
        $selectType = BaseModel::SELECT_TYPE_ONE, $field = '', $order = '') {
        return $this->getData('tweixin_info', $where, $selectType, $field, 
            $order);
    }
    ...
    public function getAwardList($condition) {
        ...
        $normalAwardList = M()->table('tcj_trace t')
            ...
            ->join('tintegal_get_detail integal ON integal.request_id=t.request_id')
    ...
    public function getAwardListold($condition) {
        ...
        $normalAwardList = M()->table('tcj_trace t')
            ...
            ->join('tcj_batch b ON t.rule_id=b.id')
            ->join('tgoods_info i ON b.goods_id=i.goods_id')
            ->join('tbatch_info c ON c.id=b.b_id')
            ->join('tbonus_info ti ON ti.id=i.bonus_id')
            ->join('tnode_info ni ON ni.node_id=t.node_id')
            ->where($where)
            ->select();
        
        $bonusCondList = array();
        $wechatCardCondList = array();
        $bonusList = array();
        $wechatCardList = array();
        ...
        if ($normalAwardList) {
            foreach ($normalAwardList as $idx => $award) {
                if ($award['bonus_id']) { // 红包
                    ...
                    $tmpWechatCardCond = array(
                        'open_id' => $award['mobile'], 
                        'card_batch_id' => $award['card_batch_id']);
        ...
        if ($wechatCardCondList) { // 获取微信卡券
            ...
            $wechatCardList = M('twx_assist_number')->where($wechatCardCondList)
                ->order('id desc')
                ->select();
            ...
            if ($award['bonus_id']) { // 红包
                if ($bonusList && is_array($bonusList)) {
                    foreach ($bonusList as $bidx => $bonus) {
                        if ($bonus['phone'] == $award['mobile'] &&
                             $award['bonus_id'] == $bonus['bonus_id']) {
                ...
                foreach ($wechatCardList as $tidx => $wechatCard) {
                    if ($wechatCard['open_id'] == $award['mobile'] &&
                         $award['card_batch_id'] == $wechatCard['card_batch_id']) {
     ...
     * @return mixed
     */
    public function getBatchChannel($where, 
        $selectType = BaseModel::SELECT_TYPE_ALL, $field = '', $order = '') {
        return $this->getData('tbatch_channel', $where, $selectType, $field, 
            $order);
    }
     ...
     * @return mixed
     */
    public function getBatchInfo($where, 
        $selectType = BaseModel::SELECT_TYPE_ALL, $field = '', $order = '') {
        return $this->getData('tbatch_info', $where, $selectType, $field, 
            $order);
    }
    ...
    public function getMarketInfo($where) {
        return $this->getData('tmarketing_info', $where, 
            BaseModel::SELECT_TYPE_ONE);
    }
    ...
    public function getCjWhiteBlackList($where) {
        return $this->getData('tcj_white_blacklist', $where, 
            BaseModel::SELECT_TYPE_ONE);
    }
    ...
    public function getBatch() {
        ...
        if ($this->today . $this->time > $this->marketInfo['end_time'] ||
             $this->today . $this->time < $this->marketInfo['start_time']) {
    ...
    public function getMarketActivityNodeNameByBid($bId) {
        ...
        ->alias('b')
        ->field('m.node_name')
        ->join('inner join tmarketing_info m on b.m_id = m.id')
