 ...
 */
class LaborDayAction extends BaseAction {
    ...
    public $img_path;
    // 旺币有效期，旺币值
    public $wbValidDate = '20150531';

    public $wbNum = 200;

    const END_LABORDAY_TIME = 20150510;
    // 劳动节限制c0,c1用户添加时间
    const APPLYEPOS_LABORDAY_TIME = 20150531;
    // 初始化
...
//             // 限制c0,c1用户登录时间限制
//             if (! $this->hasPayModule('m1')) {
//                 if (intval(date('Ymd')) > self::END_LABORDAY_TIME) {
//                     $this->error(
//                         '对不起，本次活动免费时间已过。<br/>您只要购买营销工具礼包或单独支付该营销工具使用费即可创建该活动！', 
//                         array(
//                             '点击开通' => U('Home/Wservice/marketToolVersion')));
//                 }
//                 // 参与过春节打炮限免活动的非c2用户无法参加此活动
...
//                 if ($result) {
//                     $this->error(
//                         '对不起，本次限免活动只针对未参与过春节打炮活动的企业认证用户。<br/>您只要购买营销工具礼包或单独支付该营销工具使用费即可创建该活动！', 
//                         array(
//                             '点击开通' => U('Home/Wservice/marketToolVersion')));
//                 }
//             }
...
//         }
    ...
    public function index_pop() {
        if ($this->nodeQsCheckStatus != 2) {
            ...
            $this->assign('pop_ac_url', 
                'index.php?g=LabelAdmin&m=LaborDay&a=index');
    ...
    public function index() {
        ...
        if ($wc_version == 'v0.5' || $wc_version == 'v0') {
            if (intval(date('Ymd')) <= self::APPLYEPOS_LABORDAY_TIME) {
                $model = M('tmarketing_info');
                $LaborDay_map = array(
                    'node_id' => $this->node_id, 
                    'batch_type' => '45');
                $LaborDay_time = $model->where($LaborDay_map)->find();
                if (! empty($LaborDay_time)) {
                    $this->assign('LaborDay_apply_Epos', '1');
                }
            }
        }
        node_log("首页+劳动最光荣");
...
//         }
    ...
    public function addSubmit() {
        ...
        if (! empty($data['fans_collect_url'])) {
            if (substr($data['fans_collect_url'], 0, 4) != 'http') {
                $data['fans_collect_url'] = "http://" . $data['fans_collect_url'];
            }
        }
...
//         }
        ...
        $this->success('添加成功！', 
            array(
                '设置抽奖' => U('LabelAdmin/LaborDayCjSet/index', 
                    array(
                        'batch_id' => $resp_id)), 
    ...
    public function edit() {
        ...
        if ($wc_version == 'v0' || $wc_version == 'v0.5') {
            if (intval(date('Ymd')) > self::END_LABORDAY_TIME) {
                $Labor_time_edit = "disabled='disabled'";
                $this->assign('Labor_time_edit', $Labor_time_edit);
            }
        }
        $model = M('tmarketing_info');
        $id = I('get.id');
        ...
        $query_arr['join_i'] = $query_arr['join_mode'] .
             $query_arr['member_join_flag'];
        $this->assign('row', $query_arr);
        $this->assign('join_model_arr', 
            array(
                '00' => '手机号', 
                '10' => '微信参与', 
                '11' => '微信粉丝'));
        $this->assign('join_model_notice', 
            array(
                '00' => '手机号', 
                '10' => '微信参与', 
                '11' => '微信粉丝'));
