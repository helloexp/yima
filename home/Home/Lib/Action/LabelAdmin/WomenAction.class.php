<?php
// 38妇女节
class WomenAction extends BaseAction {
    // 活动类型
    public $BATCH_TYPE = '15';
    // 图片路径
    public $img_path;
    ...
    public function addSubmit() {
        ...
        if (! $resp_id) {
            $this->error('系统错误！', 
                array(
                    '返回女人我最大' => U('MarketActive/Activity/MarketList')));
        }
        ...
        $ser->init($arr);
        $ser->sendBatch();
        node_log('女人我最大添加|活动名:' . $data['name']);
        // 顺便发布到多乐互动专用渠道上
    ...
    public function editStatus() {
        ...
        if ($result) {
            node_log('女人我最大状态更改|活动id:' . $data['batch_id']);
            $this->success('更新成功', 
                array(
                    '返回' => U('News/index')));
        } else {
