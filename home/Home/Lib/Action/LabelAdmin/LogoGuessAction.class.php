<?php
// 真假大冒险
class LogoGuessAction extends BaseAction {
    // 活动类型
    public $BATCH_TYPE = '16';
    // 图片路径
    public $img_path;
    ...
    public function _initialize() {
        ...
        $this->assign('batchTv', $this->BATCH_TYPE);
    }
    ...
    public function addSubmit() {
        ...
        $ser->init($arr);
        $ser->sendBatch();
        node_log('真假大冒险添加|活动名:' . $data['name']);
        // 顺便发布到多乐互动专用渠道上
    ...
    public function editStatus() {
        ...
        $result = $baseActivityModel->changeTheActivityStatus($data, 
            $this->BATCH_TYPE, $this->nodeIn());
        if ($result) {
            node_log('真假大冒险状态更改|活动id:' . $data['batch_id']);
            $this->success('更新成功', 
                array(
                    '返回' => U('News/index')));
        } else {
