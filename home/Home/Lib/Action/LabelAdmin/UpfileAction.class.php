...
class UpfileAction extends BaseAction {
    ...
    public function _initialize() {
        ...
        // 校验是否登录
        if (ACTION_NAME == 'editoImageSave' ||
             ACTION_NAME == 'editoImageMobileSave') {
            if (! $userService->isLogin()) {
                echo "{'url':'','title':'','original':'','state':'请重新登录'}";
                exit();
            }
        }
        $this->_checkLogin();
        $this->type = I('get.type');
        $this->batch_type = I('get.batch_type');
        $this->node_id = $userInfo['node_id'];
    }

    public function index() {
        if ($this->type == 'img' || $this->type == 'audio') {
            $this->setAstrict($this->type);
        } else {
            if (empty($this->batch_type)) {
                $arr = array(
                    'msg' => '-111',  // 通信是否成功
                    'error' => "未知活动类型",  // 返回错误
                    'imgurl' => $this->type); // 返回图片名
    ...
    public function editoImageSave() {
        ...
        import('@.ORG.Net.UploadFile');
        $upload = new UploadFile(); // 实例化上传类
        $defaultPath = 'Editor/' . $this->node_id . '/';
        $uploadPath = I('get.uploadPath');
        // 写死上传目录
        if (strpos($uploadPath, '.') !== false ||
             strpos($uploadPath, '/') !== false) {
            echo "{'url':'','title':'','original':'','state':'uploadPath error!'}";
            exit();
        }
        if ($uploadPath) {
            $defaultPath = $defaultPath . $uploadPath . '/';
        }
        $defaultPath .= date('Y/m/d') . '/';
        
        $upload->maxSize = $this->maxSize;
        $upload->allowExts = $this->allowExts;
        $upload->savePath = C('UPLOAD'); // 设置附件,一定要以/线结束
        $upload->autoSub = true; // 自动子目录
        $upload->subType = 'custom'; // 自动子目录
        $upload->subDir = $defaultPath; // 子目录名
        $upload->saveRule = time() . sprintf('%04s', mt_rand(0, 1000));
        if (! $upload->upload()) { // 上传错误提示错误信息
            $this->errormsg = $upload->getErrorMsg();
            ...
            $info = $upload->getUploadFileInfo();
            $this->imgurl = $info[0]['savename'];
            echo "{'url':'" . $this->imgurl .
                 "','title':'','original':'','state':'SUCCESS'}";
            exit();
        }
    }
