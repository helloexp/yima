...
class SpecialAction extends BaseAction {
    ...
    public function add() {
        ...
        $mem_batch = M('tmember_batch')->where(
            ...
            ->select();
        node_log('码上有红包点击添加|商户名:' . $nodeInfo['node_name']);
        $this->assign('is_cj_button', '1');
    ...
    public function addSubmit() {
        ...
        if (! $resp_id) {
            $this->error('系统错误！', 
                array(
                    '返回码上有红包' => U('MarketActive/Activity/MarketList')));
        }
        // 更新奖品规则
        ...
        node_log('码上有红包添加|活动名:' . $data['name']);
        $this->success('添加成功！', 
            array(
    ...
    public function editStatus() {
        ...
        if ($result) {
            node_log('码上有红包状态更改|活动id:' . $data['batch_id']);
            $this->success('更新成功', 
                array(
                    '返回' => U('News/index')));
        } else {
    ...
    public function winningExport() {
        ...
        $sql = "SELECT mobile,add_time,
			    	...
			    	FROM
			    	tcj_trace WHERE batch_id='{$batchId}' AND batch_type=11 AND node_id = '{$node_id}'
