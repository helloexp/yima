...
class RegulationAction extends MemberAction {
        ...
        if ($this->isPost()) {
            ...
            $req_array = array(
                'ActivityModifyReq' => array(
                    ...
                    'ActivityID' => $cardsData['batch_no'], 
                    'ActivityStatus' => '0', 
                    'ActivityInfo' => array(
                        'ActivityName' => $levelName, 
                        'ActivityShortName' => $levelName, 
                        'BeginTime' => $cardsData['add_time'], 
            ...
            // 特权添加
            $result = $this->privilegeEdit($cardsData['batch_no'], $printInfo);
            if (! $result)
                $this->error('系统出错,特权信息编辑失败');
            $data = array(
            ...
            // 门店信息
            $batchInfo = M('tgoods_info')->field('pos_group,pos_group_type')
                ->where(
                "batch_no='{$cardsData['batch_no']}' AND node_id='{$this->nodeId}'")
                ->find();
            if ($batchInfo['pos_group_type'] == '2') { // 获取终端门店
                $shopData = M()->table('tgroup_pos_relation r')
                    ->field('r.store_id,s.store_name')
                    ->join("tstore_info s ON r.store_id=s.store_id")
                    ->where(
                    "r.group_id='{$batchInfo['pos_group']}' AND r.node_id='{$this->nodeId}'")
                    ->select();
                foreach ($shopData as $k => $v) {
                    $cardsData['shopList'][] = $v;
                }
            }
            $cardsData['node_pos_type'] = $batchInfo['pos_group_type'];
    ...
    public function openLevel() {
        ...
        // 支撑创建活动
        $batchNo = $this->getBatch_no($levelName, $groupId, 1);
        // 特权添加
        $result = $this->privilegeEdit($batchNo, $printInfo);
        // 添加tgoods_info表信息
        $data = array(
