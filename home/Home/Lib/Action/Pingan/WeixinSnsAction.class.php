...
class WeixinSnsAction extends BaseAction {
    ...
    public function index() {
        ...
        $labelId = I('post.label_id', '');
        $channelId = I('post.channel_id', '');
        if (is_null($labelId) || is_null($channelId)) {
            $this->error('缺少必要参数');
        }
        $labelInfo = M('twx_qrchannel')->field("id")
            ->where(
            "id='{$labelId}' and channel_id='{$channelId}' and node_id='{$node_id}'")
            ->find();
        if (! $labelInfo) {
            $this->error('未找到该推广渠道');
        }
        ...
        $batchInfo = M('tweixin_info')->where("node_id='{$this->node_id}'")->find();
        
        $ajaxData = array(
            'status' => '1', 
            'batchName' => $batchInfo['weixin_code'], 
            'channelId' => $channelId, 
            'batchUrl' => $wapUrl, 
            'titleName' => $titleName);
    ...
    public function checkBinding($channelId) {
        ...
        $result = M('tchannel')->where(
            "node_id='{$this->node_id}' AND id='{$channelId}'")->find();
        if (! $result)
            $this->error('未找到该渠道');
        if ($result['type'] != '1')
    ...
    public function putOut() {
        ...
        switch ($snsType) {
            ...
            case 5: // 开心
                ...
                $data = array(
                    'content' => $content, 
                    'picurl' => CURRENT_HOST . '/Home/Upload/' .
                         basename($filename)); // 这里写生产上的路径,测试上调试完要改回来
