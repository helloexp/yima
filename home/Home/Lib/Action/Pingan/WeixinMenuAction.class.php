...
class WeixinMenuAction extends BaseAction {
    ...
    public function index() {
        ...
        foreach ($result as $v) {
            if ($v['response_class'] == '4') { // 卡券
                $v['response_info'] = D('TweixinInfo')->getCardInfo($v['response_info']);
            } elseif ($v['response_class'] == '6') { // 活动
                $v['response_info'] = explode("\r\n", $v['response_info']);
                $v['response_info']['batch_id'] = $v['batch_id'];
            }
            if(in_array($v['response_class'],array(0,1,5))){
                $v['type'] = 0;
            }else{
                $v['type'] = 1;
            }
		...
		//dump($v['response_info']); exit;
        // echo '<pre>';
        //dump(array_values($menuArr));exit;
        $this->display();
    }
    ...
    public function menuSubmit(){
        ...
        if ($response_class == '6') {
            $tmarketing_info = $this->_getLabelUrl($response_info);
            $tmarketing_info = M('tmarketing_info')
                ->where('id='. $response_info)
                ->field("id batch_id,batch_type,name")
                ->find();
            $labelId = $this->getLabelIdAndTryAddLabelInfo($tmarketing_info['batch_id'], $tmarketing_info['batch_type']);
            $m = $tmarketing_info['batch_type'] == '4' ? 'MemberRegistration' : 'Label'; // 会员粉丝招幕(特殊处理,不需要跳到标签)
            $url = U('Label/' . $m . '/index/', array('id' => $labelId), '', '', true);
            $batch_id = $response_info;
            $response_info = $tmarketing_info['name'] .
                 "\r\n<a href='" . $url . "'>参与活动</a>";
        } else {
            $batch_id = '';
        }
        ...
        if(!$id){
            ...
            if (! $result) {
                ...
                $this->ajaxReturn(array('id'=>$result,'sort_id'=>$sort_id),'添加成功',1);
            }
        }else{
            ...
            if ($result === false) {
                ...
                $this->ajaxReturn(array('id'=>$id,'sort_id'=>$sort_id),'编辑成功',1);
            }
        }
    }
