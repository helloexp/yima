...
class PinganAction extends BaseAction {
    ...
    public function interact() {
        ...
        if ($this->isPost()) {
            ...
            $end_time = I('end_time', '');
            $actType = I('actType', 'add');
            
            if (dateformat($begin_time, 'YmdHis') >
                 dateformat($end_time, 'YmdHis') && $end_time != '')
                $this->error('活动时间错误');
            ...
            if($this->nodeId == C('new_year_node_id')){
                ...
                $respId = I('respId', array());
                foreach($hb_num as $k=>$v){
                    if(!preg_match('/^\d+$/i', $v)) $this->error('红包数量不能小于1！');
                    if($actType == 'edit'){
                        ...
                        $goodsInfo = M()->table("tbatch_info t2")
                            ...
                            ->field('t1.remain_num + t2.remain_num as total')
                            ->find();
                        if($v > $goodsInfo['total']) $this->error('红包数量不能大于库存量！');
                    }else{
                        if($v > $remain_num[$k]) $this->error('红包数量不能大于库存量！');
                    }
                    if($v < 1) $this->error('红包数量不能小于1！');
                }
                $result = $TweixinInfo->interact2($this->nodeId);
            }else{
                $result = $TweixinInfo->interact($this->nodeId);
            }
            if ($result['status'] == 1) {
                $this->error($result['info']);
            } else {
                $message_id = $result['info'];
            }
            ...
            $this->assign('hdyltc', $_COOKIE['hdyltc']);
            $this->display();
        }
    }
    ...
    public function interact_created() {
        ...
        if($this->nodeId == C('new_year_node_id')){
            $list = M()->table("twx_message t2")
                ->join('twx_bonus_send_trace t1 on t1.m_id = t2.m_id and t1.node_id=t2.node_id')
                ->field("t2.m_id,SUM(CASE WHEN t1.STATUS = '0' THEN 1 ELSE 0 END) AS receive_num,SUM(CASE WHEN t1.STATUS <> '0' THEN 1 ELSE 0 END) AS fail_num,t2.id,t2.message_name,t2.add_time,t2.begin_time,t2.end_time")
                ->limit($Page->firstRow . ',' . $Page->listRows)
                ->where("t2.node_id = '".$this->nodeId."' and t2.response_type = '6' and t2.status = '0'")
                ->group('t1.m_id')
                ->select();
        }else{
    ...
    public function interact_detail() {
        ...
        if ($down) {
            ...
            $title_ .= "总领取量 \r\n";
            if($this->nodeId == C('new_year_node_id')){
                echo iconv('utf-8', 'gbk', "奖品名称,奖品领取数\r\n");
                echo iconv('utf-8', 'gbk', $message_info['message_name']) . ", ";
                echo iconv('utf-8', 'gbk', $count) . "\r\n";
                echo iconv('utf-8', 'gbk',"\r\n");
                echo iconv('utf-8', 'gbk',"\r\n");
                echo iconv('utf-8', 'gbk', "发放粉丝（昵称）,openid\r\n");
                if ($fans_list) {
                    foreach($fans_list as $val){
                        echo iconv('utf-8', 'gbk', $val['nickname']) . ", ";
                        echo iconv('utf-8', 'gbk', $val['open_id']) . "\r\n";
                    }
                }
            }else{
                ...
                echo $title_;
                echo iconv('utf-8', 'gbk', $message_info['message_name']) . ", ";
                echo iconv('utf-8', 'gbk', $message_info['time']) . ", ";
                foreach ($list as $val) {
                    echo iconv('utf-8', 'gbk', 
                        $val['name'] . '(' . $val['num'] . ')') . ", ";
                }
                echo iconv('utf-8', 'gbk', $message_info['receive_num']) . "\r\n";
            }
            exit();
        } 
        if($this->nodeId == C('new_year_node_id')){
    ...
    public function hpyb() {
        ...
        if ($this->isPost()) {
            // $this->error(111);
            $saveType = I('saveType', 'js_submit');
            // 插入消息表、消息回复表
            $result = $TweixinInfo->hpyb($this->nodeId);
            if ($result['status'] == 1) {
                $this->error($result['info']);
            } else {
    ...
    public function changjingma() {
        ...
        $count = M()->table("twx_qrchannel wq")->join('tchannel c ON wq.channel_id=c.id')
            ->field(
            "wq.id,wq.flag_insert,c.name,DATE_FORMAT(c.add_time,'%Y-%m-%d') as add_time,wq.click_count,wq.subscribe_count,wq.img_info")
            ->where($where)
            ->count();
        $Page = new Page($count, 10); // 实例化分页类 传入总记录数和每页显示的记录数
        $pageShow = $Page->show(); // 分页显示输出
        ...
        if ($down) {
            $list = M()->table("twx_qrchannel wq")->join(
                'tchannel c ON wq.channel_id=c.id')
                ->field(
                "wq.id,c.name,wq.`flag_insert`,DATE_FORMAT(c.add_time,'%Y-%m-%d %H:%m:%s') as add_time,wq.click_count,wq.subscribe_count,wq.img_info")
                ->where($where)
            ...
            header("Pragma: no-cache");
            header("Expires: 0");
            $title_ = "场景码名称, 创建时间, 创建方式, 扫码累计数, 累计关注数\r\n";
            $title_ = iconv('utf-8', 'gbk', $title_);
            echo $title_;
        ...
        $list = M()->table("twx_qrchannel wq")->join('tchannel c ON wq.channel_id=c.id')
            ->field(
            "wq.id,c.name,wq.`flag_insert`,DATE_FORMAT(c.add_time,'%Y-%m-%d %H:%i:%s') as add_time,wq.click_count,wq.subscribe_count,wq.img_info")
            ->where($where)
            ->order('c.add_time Desc')
            ->limit($Page->firstRow, $Page->listRows)
            ->select();
    ...
    public function changjingmaStore() {
        $store_id = I('store_id', '');
        $storeName = M('tstore_info')->where(array('node_id'=>$this->node_id))->getField('store_id,store_short_name');
        if ($store_id) {
    ...
    public function changjingma_response() {
        ...
        // 响应时间保存后取值展示
        $info = M()->table("twx_message wm")->join(
            'twx_msgresponse wms on wm.id=wms.message_id')
