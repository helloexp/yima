...
class LoginAction extends Action {
    ...
    public function _before_showLogin() {
        ...
        if ($userService->isLogin()) {
            $this->redirect('CnpcGX/Index/index');
        }
    }
    ...
    public function ssologin() {
        ...
        if ($email == "") {
            if ($autologin_flag)
                redirect(U('CnpcGX/Login/showLogin'));
        ...
        if ($reqResult === true) {
            ...
            if ($fromurl != '') {
                ...
                if ($lname == "wxyx") { // 微信营销
                ...
                } else if ($lname == "weibozhushou") { // 新浪微博
                    $this->ajaxReturn(
                        array(
                            'success' => 1, 
                            'redirectUrl' => "index.php?g=LabelAdmin&m=Weibo&a=index"));
                } else if ($lname == "zhidahao") { // 直达号
                    $this->ajaxReturn(
                        array(
                            'success' => 1, 
                            'redirectUrl' => "index.php?g=LabelAdmin&m=Med&a=index"));
                } else if ($lname == "weiguanwang") { // 微官网
                    $this->ajaxReturn(
                        array(
                            'success' => 1, 
                            'redirectUrl' => "index.php?g=MicroWeb&m=Index&a=index"));
                } else if ($lname == "xuanma") { // 炫码
                    $this->ajaxReturn(
                        array(
                            'success' => 1, 
                            'redirectUrl' => "index.php?g=VisualCode&m=Index&a=index"));
                } else if ($lname == "zfb") { // 支付宝线下收单
                    $this->ajaxReturn(
                        array(
                            'success' => 1, 
                    ...
                    $this->ajaxReturn(
                        array(
                            'success' => 1, 
                            'redirectUrl' => "index.php?g=CnpcGX&m=Index&a=index"));
                }
            }
            exit();
            // }
