...
class BatchTraceAction extends BaseAction {
    ...
    public function index() {
        ...
        $list = M()->table('tbarcode_trace c')
            ->field('c.*,g.goods_name,n.node_name')
    ...
    public function downTrace() {
        ...
        $sql = "SELECT a.store_id ,a.store_name,SUM(s_count) as s_count , SUM(p_count) as p_count ,c_count
             ...
             $where .
             " trans_type='0001' and data_from='8' and status='0' GROUP BY pos_id ) d
             ...
             $this->nodeId .
             "' and pt.trans_type='0' AND pt.STATUS='0' and bt.data_from='8' GROUP BY pt.pos_id)  e
             ...
             $this->nodeId .
             "' and  a.trans_type='1' AND a.STATUS='0' and abt.data_from='8' GROUP BY a.pos_id)  g
             ...
             $where . " trans_type='0001' and status='0' and data_from='8' and " .
    ...
    public function posTrace() {
		...
		$cities = I('city');
		$towns = I('town');
		$statuss = $status = I('status');

		$transTypes = $transType = I('trans_type');
        ...
        $hasLocation = false;
		if ($provinces !='') {
			$map['ti.province_code'] = $provinces;
            $hasLocation = true;
		}
		if ($cities !='') {
			$map['ti.city_code'] = $cities;
            $hasLocation = true;
		}
		if ($towns !='') {
			$map['ti.town_code'] = $towns;
            $hasLocation = true;
		}
       //var_dump($map);exit;
	    ...
	    if (isset($condition['start_time']) && $condition['start_time'] != '') {
           $map['p.trans_time'] = "p.trans_time >='{$condition['start_time']}00'";
        }
        if (isset($condition['end_time']) && $condition['end_time'] != '') {
           $map['p.trans_time']= "p.trans_time <= '{$condition['end_time']}59'";
        }
		...
		if ($transType !== ''){
			$map['p.trans_type'] = $transType;
		}
        // $map['g.goods_type']='6';
		//地点筛选
		//省市区				
		$where = ['node_id' => $this->node_id];
		$select = M()->table('tstore_info ti')->where($where)->distinct(true)->field('province_code')->select();
		$provinceCodeList  = [];
		if (is_array($select)) {
			foreach ($select as $provinceCode) {
				$currentProvinceCode = get_val($provinceCode, 'province_code');
				if ($currentProvinceCode > 0) {
					$provinceCodeList[] = $currentProvinceCode;
				}
			}
		}
		$province_code_list = '';
		if ($provinceCodeList) {
			$province_code_list = implode(',', $provinceCodeList);
		}
		
		$this->assign('province_code_list', $province_code_list);
       //时间筛选
        ...
        if ($hasLocation == true) {
            $mapcount = M()->table('tpos_trace p')
                    ->field('p.*,n.node_name,g.goods_name')
                    ->join('tnode_info AS n ON p.node_id=n.node_id')
                    ->join('tbarcode_trace AS b ON b.req_seq=p.req_seq')
                    ->join('tpos_info AS pi ON pi.node_id=n.node_id AND pi.`pos_id`= p.`pos_id`')
                    ->join('tstore_info AS ti ON ti.store_id=pi.store_id')
                    ->join('tgoods_info AS g ON g.batch_no=p.batch_no')
                    ->where($map)
                    ->count();
        }else {
            $mapcount = M()->table('tpos_trace p')->join('tbarcode_trace b ON b.req_seq=p.req_seq')->where($map)->count(
                    );
        }
        $Page = new Page($mapcount, 10); // 实例化分页类 传入总记录数和每页显示的记录数
        ...
        $this->assign('transType', $transType);
		$this->assign('transTypes', $transTypes);
        $this->assign('list', $list);
