...
class OrderListAction extends BaseAction {
    ...
    public function export() {
        ...
        $count = M()->table("ttg_order_info o")->join(
            "tmarketing_info m ON o.batch_no=m.id")
            ->join("tbatch_channel b ON b.id=o.batch_channel_id")
            ->join("tchannel c ON c.id=b.channel_id")
            ->where($filter)
            ->count();
        ...
        $sql = "SELECT o.order_id,m.b_name AS batch_name,d.name AS 'from_batch_name',c.name AS from_channel_name				,o.order_phone,o.add_time,o.delivery_company,o.delivery_number,o.receiver_name,o.receiver_phone,o.receiver_addr,IFNULL(o.freight, '0.00') freight,IFNULL(bd.bonus_amount,0.00) AS bonus_amt,
				...
				JOIN ttg_order_info_ex m ON m.order_id=o.order_id
				JOIN tbatch_channel b ON b.id=o.batch_channel_id
				JOIN tchannel c ON c.id=b.channel_id 
    ...
    public function editStatus() {
        ...
        if ($this->isPost()) {
            ...
            $ret = M('ttg_order_info')->where("order_id='" . $orderId . "'")->save(
                array(
                    'delivery_status' => $status));
            if ($ret !== false) {
                $message = array(
    ...
    public function orderPrint() {
        ...
        $orderInfo = M('ttg_order_info')->where($map)->find();
        ...
        $orderInfoExList = M()->table("ttg_order_info_ex a")->join(
            "tbatch_info b on a.b_id=b.id")
            ->where($map)
            ->field("a.*,b.batch_no")
            ->select();
