...
class StoreWebAction extends BaseAction {
    ...
    public function _initialize() {
        ...
        $this->allowExts = array(
            '');
        parent::_initialize();
        C(include (CONF_PATH . 'LabelAdmin/config.php'));
        $this->upload_path = './Home/Upload/StoreWebImg/' . $this->node_id . '/';
    ...
    public function index() {
        ...
        if ($list) {
            // redirect(U('Df/StoreWeb/add',array('store_id'=>I('store_id'),'mw_batch_id'=>$id)));
        } else {
            redirect(
                U('Df/StoreWeb/add', 
                    array(
        ...
        $this->display('StoreWeb/index'); // 输出模板
    }
    ...
    public function add() {
        ...
        /* $map_select['link_type']=2;/*链接类型 0营销活动1手工地址2图文链接 */
        /* $map_select['title']="门店介绍"; */
        $count = $model->where($map_select)->count();
        if ($count == 0) {
            $map_select['link_type'] = 2;
            $map_select['title'] = "门店介绍";
            ...
            $map_select['link_type'] = 1;
            $map_select['title'] = "优惠活动";
            $map_select['field_id'] = 2;
            $map_select['field_type'] = 2;
            $map_select['field_img_name'] = "df_active_ico.png";
            $map_select['field_img_color'] = "colorVal-4";
            $map_select['image_name'] = C('TMPL_PARSE_STRING.__PUBLIC__') .
                 '/Image/df/df_store_default.png';
            $map_select['tpl_id'] = $batch_arr['tpl_id'];
            $map_select['add_time'] = date('YmdHis');
            $map_select['edit_time'] = date('YmdHis');
            $map_select['link_url'] = U('Df/DfWap/ListBatch_index');
            $query = $model->add($map_select);
        }
        /*
         * $map_select=$map; $map_select['is_fixed']=1;
         * $map_select['link_type']=1; $map_select['title']="优惠活动"; $count =
         * $model->where($map_select)->count(); if($count==0) {
         * $map_select['field_id']=2; $map_select['field_type']=2;
         * $map_select['field_img_name']="df_active_ico.png";
         * $map_select['field_img_color']="colorVal-4";
         * $map_select['image_name']=C('TMPL_PARSE_STRING.__PUBLIC__').'/Image/df/df_store_default.png';
         * $map_select['tpl_id']=$batch_arr['tpl_id'];
         * $map_select['add_time']=date('YmdHis');
         * $map_select['edit_time']=date('YmdHis');
         * $map_select['link_url']=U('Df/DfWap/ListBatch_index'); $query =
         * $model->add($map_select); }
         */
        ...
        $this->display('StoreWeb/add'); // 输出模板
    }
    ...
    public function addSubmit($data) {
        ...
        if ($data['field_type'] == '1') { // 1号位 3个张图轮换
            ...
            if ($field1_count >= 3) {
                M()->rollback();
                $this->error("本机构1号位置已存在3条图片记录，无法添加新的记录");
            }
            // 获取图片跳转url
        ...
        $message = array(
            ...
            'id' => $query);
        if ($data['field_type'] == '0') {
            $message['url'] = U('Df/StoreWeb/addEditSuccess', 
                array(
                    'mw_batch_id' => $data['mw_batch_id'], 
    ...
    public function edit() {
        ...
        $this->assign('is_df', 
            (C('df.node_id') == $this->node_id) ? true : false);
        $this->display('StoreWeb/edit'); // 输出模板
    }
    ...
    public function editSubmit($data) {
        ...
        if (in_array($data['field_type'], 
            ...
            } elseif (($data['tpl_id'] == '3' || $data['tpl_id'] == '4' ||
                 $data['tpl_id'] == '9' || $data['tpl_id'] == '10' ||
                 $data['tpl_id'] == '11' || $data['tpl_id'] == '12') &&
                 ($data['field_type'] == '2')) {
                if ($data['tpl_id'] == '10') {
            ...
            if ($query) {
                ...
                node_log('门店首页模版编辑成功|机构号:' . $this->node_id);
            } else {
                M()->rollback();
                $this->error('门店首页模版编辑失败');
            }
        } else {
            M()->rollback();
            $this->error('模版位置编号错误');
        ...
        $message = array(
            ...
            'respStr' => '编辑成功');
        if ($data['field_type'] == '0') {
            $message['url'] = U('Df/StoreWeb/index', 
                array(
                    'store_id' => $data['store_id']));
        }
        $this->success($message);
    ...
    private function addbatch() {
        ...
        if (! $batch_data) {
            ...
            $batch_arr = array(
                ...
                'end_time' => date('YmdHis', strtotime("+100 year")), 
                'add_time' => date('YmdHis'), 
                'click_count' => 0, 
                'cj_count' => 0, 
                'send_count' => 0, 
