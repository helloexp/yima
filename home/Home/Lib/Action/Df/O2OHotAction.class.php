...
class O2OHotAction extends BaseAction {
    ...
    public function firstIndex() {
        ...
        $this->display('O2OHot/firstIndex');
    }
    ...
    public function singleOrder() {
        ...
        $this->display('O2OHot/singleOrder');
    }
    ...
    public function exportOrder() {
         ...
         * tbatch_channel b ON b.id=o.from_channel_id JOIN tchannel c ON
        ...
        if (in_array($this->node_id, C('fb_tongbaozhai.node_id'), true)) {
            $sql = "(SELECT o.order_id,m.name AS batch_name,m.group_goods_name,d.name AS 'from_batch_name',c.name AS from_channel_name ,o.order_phone,o.add_time,o.parm1,sc.name as petname,o.receiver_name,o.receiver_phone,o.receiver_addr,ifnull(o.freight, '0.00') freight,
			...
			JOIN tmarketing_info d ON d.id=o.from_batch_no {$whereNew} AND o.order_type='0')
			UNION
			(SELECT o.order_id,m.b_name AS batch_name,m.b_name AS group_goods_name,d.name AS 'from_batch_name',c.name AS from_channel_name ,o.order_phone,o.add_time,o.parm1,sc.name as petname,o.receiver_name,o.receiver_phone,o.receiver_addr,ifnull(o.freight, '0.00') freight,
			...
			LEFT JOIN(SELECT order_id,GROUP_CONCAT(code_trace) AS send_seq FROM torder_trace GROUP BY order_id) tt ON tt.order_id=o.order_id
			LEFT JOIN tmember_info sc ON sc.id=o.parm1 and sc.node_id=o.node_id {$where} AND o.order_type='2')";
        } else
            $sql = "(SELECT o.order_id,m.name AS batch_name,m.group_goods_name,d.name AS 'from_batch_name',c.name AS from_channel_name ,o.order_phone,o.add_time,o.receiver_name,o.receiver_phone,o.receiver_addr,ifnull(o.freight, '0.00') AS freight,
			m.group_price,o.buy_num,o.order_amt,IFNULL(bd.bonus_amount,0.00) AS bonus_amt,
			CASE o.receiver_type WHEN '0' THEN '凭证自提订单' WHEN '1' THEN '物流订单' ELSE '其他' END receiver_type,
			CASE o.pay_status WHEN '1' THEN '未支付' WHEN '2' THEN '已支付' ELSE '其他' END pay_status,o.pay_seq,
			CASE WHEN o.pay_channel='1' AND o.pay_status='2' THEN ROUND(o.order_amt*{$fee_rate},2) WHEN o.pay_channel<>'1' AND o.pay_status='2' THEN ROUND(o.order_amt*0.02,2) ELSE 0.00 END fee_cost,
			...
			JOIN tmarketing_info d ON d.id=o.from_batch_no {$whereNew} AND o.order_type='0')
			UNION
			(SELECT o.order_id,m.b_name AS batch_name,m.b_name AS group_goods_name,d.name AS 'from_batch_name',c.name AS from_channel_name ,o.order_phone,o.add_time,o.receiver_name,o.receiver_phone,o.receiver_addr,ifnull(o.freight, '0.00') AS freight,
			m.price AS group_price,m.goods_num AS buy_num,o.order_amt AS order_amt,IFNULL(bd.bonus_amount,0.00) AS bonus_amt,
			CASE o.receiver_type WHEN '0' THEN '凭证自提订单' WHEN '1' THEN '物流订单' ELSE '其他' END receiver_type,
			CASE o.pay_status WHEN '1' THEN '未支付' WHEN '2' THEN '已支付' ELSE '其他' END pay_status,o.pay_seq,
			CASE WHEN o.pay_channel='1' AND o.pay_status='2' THEN ROUND(o.order_amt*{$fee_rate},2) WHEN o.pay_channel<>'1' AND o.pay_status='2' THEN ROUND(o.order_amt*0.02,2) ELSE 0.00 END fee_cost,
			...
			JOIN tchannel c ON c.id=b.channel_id 
			LEFT JOIN(SELECT order_id,GROUP_CONCAT(code_trace) AS send_seq FROM torder_trace GROUP BY order_id) tt ON tt.order_id=o.order_id
			JOIN tmarketing_info d ON d.id=o.from_batch_no {$where} AND o.order_type='2'
			)";
        $cols_arr = array(
            'order_id' => '订单号', 
            'batch_name' => '活动名', 
    ...
    public function dataAnalyse() {
        ...
        $this->assign('today', $today);
        $this->assign('yesterday', $yesterday);
        $this->display('O2OHot/dataAnalyse');
    }
    ...
    public function channelChart() {
        ...
        foreach ($list as &$vo) {
            $count = M()
                ->table("ttg_order_info t")
                ->field('sum(t.buy_num)')
                ->join('tbatch_channel b ON b.id=t.from_channel_id')
                ->where(
                array(
                    't.batch_no' => $batch_id, 
                    't.pay_status' => '2', 
        ...
        $this->display('O2OHot/channelChart'); // 输出模板
    }
    ...
    public function clickChart() {
        ...
        $this->display('O2OHot/clickChart');
    }
    ...
    public function newedit() {
        ...
        $row = M()->table("tmarketing_info g")->field(
            'g.*,i.info_title,i.use_rule,i.verify_end_date,i.verify_end_type,u.id as goods_id ,i.storage_num,u.goods_name,u.remain_num,u.storage_type,u.goods_type')
