...
class ShopOptionAction extends BaseAction {
    ...
    public function index() {
        ...
        $logoInfo = M('tfb_df_pointshop_banner')->where(
            array(
                'node_id' => $this->node_id, 
                'm_id' => $marketInfo['id'], 
                'ban_type' => 1))->find();
        $banInfo = M()->table("tfb_df_pointshop_banner b")
                ->field('b.*,m.name as link_batch_name')
        ...
        $proInfo = M()->table("tfb_df_pointshop_goods_ex g")
            ->field('g.*,m.name as m_name,b.batch_img')
    ...
    public function logo_add() {
        if ($this->isPost()) {
            ...
            if ($img) {
                ...
                if ($logo_id) // 更新
{
                    $ret = M('tfb_df_pointshop_banner')->where(
                        array(
                            'id' => $logo_id))->save(
                        array(
                            'img_url' => $img));
...
{
                    $ret = M('tfb_df_pointshop_banner')->add(
                        array(
                            'm_id' => $m_id, 
                            'node_id' => $this->node_id, 
                            'ban_type' => 1, 
    ...
    public function ban_add() {
        if ($this->isPost()) {
            ...
            // 循环处理
            for ($i = 1; $i < 4; $i ++) {
                $banInfo = M('tfb_df_pointshop_banner')->where(
                    array(
                        'node_id' => $this->node_id, 
                        'm_id' => $m_id, 
                        'ban_type' => 0, 
                ...
                if ($banInfo) { // 更新
                    ...
                    if ($img) {
                        ...
                        $ret = M('tfb_df_pointshop_banner')->add($data);
                    }
                }
                if ($ret === false) {
                    M()->rollback();
    ...
    public function product_add() {
        if ($this->isPost()) {
            ...
            for ($i = 1; $i < 5; $i ++) {
                ...
                if ($product_mid) {
                    $result = M('tfb_df_pointshop_goods_ex')->where(
                        array(
                            'node_id' => $this->node_id, 
                            'is_commend' => $i))->save(
                        array(
    ...
    public function ban_del() {
        ...
        $result = M('tfb_df_pointshop_banner')->where(
            array(
                'm_id' => $m_id, 
                'ban_type' => '0', 
                'node_id' => $this->node_id, 
    ...
    public function product_del() {
        ...
        if (! $id)
            $this->error('数据错误');
        $proInfo = M('tfb_df_pointshop_goods_ex')->where(
            array(
                'node_id' => $this->node_id, 
                'id' => $id))->find();
        if (! $proInfo)
