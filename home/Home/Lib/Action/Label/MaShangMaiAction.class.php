<?php
// 码上买
class MaShangMaiAction extends MyBaseAction {
    ...
    public function _initialize() {
        ...
        $isAdressFb = in_array($this->batch_id, C('fishsush.address'));
        // 非标用户特定 【非标V1.1.0_C15】爱蒂宝 redmin 15348
        $isADBFb = in_array($this->batch_id, C('aidibao.mid'));
        
        $this->saler_id = $saler_id;
        $this->assign('isADBFb', $isADBFb); // 爱蒂宝非标
        $this->assign('isFb', $isFb); // 鱼旨寿司非标
        $this->assign('isAdressFb', $isAdressFb); // 鱼旨寿司非标
    ...
    public function index() {
        ...
        // 活动
        $row = M('tmarketing_info as m')
                ->join('tbatch_info as b on b.m_id = m.id')
                ->where(['m.id' => $this->batch_id])
                ->field('b.begin_time, b.end_time as b_end_time, m.*')
                ->find();
        ...
        if ($logoInfo['biaoti'] == null)
            $logoInfo['biaoti'] = $this->node_short_name;
        
        $this->assign('id', $id);
        $this->assign('row', $row);
        $this->assign('channel_id', $this->id);
    ...
    public function newproduct() {
        ...
        $ecshopm_id = M('tmarketing_info')->where(
            array(
                'node_id' => $this->node_id, 
        ...
        if ($logoInfo['biaoti'] == null)
            ...
            // 查询是否有小店
        $InfoModel = M('tmarketing_info');
        $Info = $InfoModel->where(
            array(
                'node_id' => $this->node_id, 
                'batch_type' => $this->store_batch_type))->find();
        // 查询店铺是否有商品并且是上架的
        
        $nodeWhere = " e.node_id='" . $this->node_id .
             "' AND b.status='0' AND b.end_time>='" . date('YmdHis') . "'";
        $goodscount = M('tecshop_goods_ex e ')->join(
            "tbatch_info b ON b.id=e.b_id")
            ->where($nodeWhere)
            ->count();
        
        if (! empty($Info) && $goodscount > 0) {
            // 查询店铺访问地址
            ...
            $label_id = get_batch_channel($Info['id'], $channel_id, 
                $this->store_batch_type, $this->node_id);
        }
        ...
        // 查询分享图片
        $len = strlen($row['share_pic']) - 2;
        $tmpic = substr($row['share_pic'], 2, $len);
    ...
    public function goodsInfo() {
        $m_id = I('m_id', null, 'mysql_real_escape_string');
        $id = I('id', null, 'mysql_real_escape_string');
        // 非标 定点时间开始
        if ($m_id == 17750) {
            if (date('YmdHis') < '20150308000000')
                $this->error('该活动将于3月8日凌晨0点开始');
            if (date('YmdHis') > '20150308003800')
                $this->error('该活动已于3月8日凌晨0点38分结束');
        }
        if ($m_id == 17751) {
            if (date('YmdHis') < '20150308003800')
                $this->error('该活动将于3月8日凌晨0点38分开始');
            if (date('YmdHis') > '20150308063800')
                $this->error('该活动已于3月8日6点38分结束');
        }
        if ($m_id == 17752) {
            if (date('YmdHis') < '20150308063800')
                $this->error('该活动将于3月8日6点38分开始');
            if (date('YmdHis') > '20150309000000')
                $this->error('该活动已于3月8日结束');
        }
        // 活动
        $goodsInfo = M()->table('tmarketing_info m')
            ->field(
            "m.*,b.remain_num,b.storage_num,b.begin_time, b.end_time as b_end_time, b.batch_name,g.goods_image,g.goods_id, g.config_data, g.is_sku, g.is_order")
        ...
        unset($map);
        $skuInfoList = $skuObj->getSkuEcshopList($m_id, $this->node_id, true);
        // 判断是否有sku数据
        $goodsSkuTypeList = '';
        $goodsSkuDetailList = '';
        $skuDetail = '';
        ...
        if (!$skuInfoList) {
            ...
            $goodsInfo = $skuObj->makeGoodsListInfo($goodsInfo, $m_id, '');
            // 分离商品表中的规格和规格值ID
            ...
            if ($goodsInfo['is_order'] == '2') {
                ...
                $goodsInfo['end_days'] = (int) ((strtotime(
                    $goodsInfo['end_time']) - time()) / 86400);
            }
            // 取得规格表信息
            if (is_array($goodsSkuDetailList)) {
                if (isset($goodsInfo['config_data']['cycle_type'])) // 取得订购类型
                    $goodsSkuTypeList = $skuObj->getSkuTypeList(
                        $goodsSkuDetailList, 
                        $goodsInfo['config_data']['cycle_type']);
                else
                    $goodsSkuTypeList = $skuObj->getSkuTypeList(
                        $goodsSkuDetailList);
            }
        ...
        if ($goodsInfo['member_join_flag'] == '1' && $this->wx_flag == 1) {
            ...
            if ($wxnodeinfo['app_id'] && $wxnodeinfo['app_secret'] &&
                ...
                if (! $wxBatchOpen['openid']) {
                    $surl = U('Label/MaShangMai/goodsInfo', 
                        array(
                            'id' => $id), '', '', true);
                    redirect(
        ...
        if ($goodsInfo['begin_time'] > date('YmdHis')) {
            ...
            $due = '0'; // 未过期
            $goodsInfo['js_start_time'] = strtotime($goodsInfo['begin_time']) *
                 1000;
        ...
        if (session('groupPhone')) {
            ...
            $totalBuyNum = M('ttg_order_info')->where(
                "batch_no ={$goodsInfo['id']} and order_phone ='" .
        ...
        if ($buyNum > 0) {
            ...
            if ($goodsInfo['buy_num'] > 0) {
                $lastBuyNum = $goodsInfo['buy_num'] - $daySaleNum; // 剩余可买数量
            ...
            // 限单个手机的购买数量
            if ($goodsInfo['defined_three_name'] >= 0) {
                $leftBuyNum = $goodsInfo['defined_three_name'] - $totalBuyNum;
                if ($leftBuyNum <= 0)
                    $buyNum = 0;
                else {
        ...
        if ($this->isPost()) {
            ...
            if ($receiverType != '0') {
                ...
                if ($receiver_addr != '' && $receiver_name != '') {
                    // $count =
                    // M('tphone_address')->where(array('user_phone'=>session('groupPhone'),'user_name'=>$receiver_name,'phone_no'=>$receive_phone,'address'=>$receiver_addr,
            ...
            if ($orderList) {
                foreach ($orderList as $v) {
                    ...
                    $str .= '<li>
                         ...
                         $v['group_goods_name'] . $payStr . '</div>
								<div class="orderList-con">
									<p>
									   <span>下单时间:' .
                         dateformat($v['add_time'], 'Y-m-d H:i:s') . '</span>
    ...
    public function showBatchList() {
        ...
        // 该商户团购列表渠道id
        $channelId = M('tchannel')->where(
            "node_id={$result['node_id']} AND type=4 AND sns_type=47")->getField(
            'id');
        // 处理剩余日期
