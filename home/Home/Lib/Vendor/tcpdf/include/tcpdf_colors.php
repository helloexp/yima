...
/**
 * @file PHP color class for TCPDF
 *
 * @author Nicola Asuni
...
/**
 * @class TCPDF_COLORS PHP color class for TCPDF
 *
 * @package com.tecnick.tcpdf
 * @version 1.0.004
...
class TCPDF_COLORS {
    ...
    /**
     * Array of WEB safe colors @public static
     */
    ...
    /**
     * Array of valid JavaScript color names @public static
     */
    ...
    /**
     * Array of Spot colors (C,M,Y,K,name) Color keys must be in lowercase and
     * without spaces. As long as no open standard for spot colours exists, you
     * have to buy a colour book by one of the colour manufacturers and insert
     * the values and names of spot colours directly. Common industry standard
     * spot colors are: ANPA-COLOR, DIC, FOCOLTONE, GCMI, HKS, PANTONE, TOYO,
     * TRUMATCH. @public static
     */
    public static $spotcolor = array(
        ...
        'blue' => array(
            ...
            0, 
            'Blue'));
    // Add here standard spot colors or dynamically define them with
    // AddSpotColor()
    // ...
     ...
     * @since 5.9.125 (2011-10-03) @public static
     */
    ...
    /**
     * Returns an array (RGB or CMYK) from an html color name, or a six-digit
     * (i.e. #3FE5AA), or three-digit (i.e. #7FF) hexadecimal color, or a
     * javascript color array, or javascript color name.
     *
     ...
     *         static
     */
    public static function convertHTMLColorToDec($hcolor, &$spotc, 
        ...
        } elseif ((substr($color, 0, 4) != 'cmyk') and
            ...
            case 8:
                {
                    ...
                    $returncolor['C'] = max(0, 
                        min(100, 
                            round(hexdec(substr($color_code, 0, 2)) / 2.55)));
                    $returncolor['M'] = max(0, 
                        min(100, 
                            round(hexdec(substr($color_code, 2, 2)) / 2.55)));
                    $returncolor['Y'] = max(0, 
                        min(100, 
                            round(hexdec(substr($color_code, 4, 2)) / 2.55)));
                    $returncolor['K'] = max(0, 
                        min(100, 
                            round(hexdec(substr($color_code, 6, 2)) / 2.55)));
                    break;
                }
            default:
                {
     ...
     * @since 5.9.137 (2011-12-01) @public static
     */
     ...
     *
     * @param $color (string) color name or "#RRGGBB" @protected
     * @since 2.1.002 (2008-02-12) @public static
     */
    public static function _JScolor($color) {
        if (substr($color, 0, 1) == '#') {
            return sprintf("['RGB',%F,%F,%F]", 
