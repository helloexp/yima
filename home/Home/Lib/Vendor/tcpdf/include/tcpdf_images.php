...
/**
 * @file This is a PHP class that contains static image methods for the TCPDF
 * class.<br>
 *
...
/**
 * @class TCPDF_IMAGES Static image methods used by the TCPDF class.
 *
...
class TCPDF_IMAGES {
    ...
    /**
     * Return the image type given the file name or array returned by
     * getimagesize() function.
     *
     * @param $imgfile (string) image file name
     * @param $iminfo (array) array of image information returned by
     *            getimagesize() function.
     * @return string image type
     * @since 4.8.017 (2009-11-27) @public static
     */
    public static function getImageFileType($imgfile, $iminfo = array()) {
        $type = '';
        if (isset($iminfo['mime']) and ! empty($iminfo['mime'])) {
            $mime = explode('/', $iminfo['mime']);
            if ((count($mime) > 1) and ($mime[0] == 'image') and
                 (! empty($mime[1]))) {
        ...
        if (empty($type)) {
            ...
            if (isset($fileinfo['extension']) and
                 (! TCPDF_STATIC::empty_string($fileinfo['extension']))) {
     ...
     * @param $image (image) GD image object. return GD image object.
     * @since 4.9.016 (2010-04-20) @public static
     */
    public static function setGDImageTransparency($new_image, $image) {
        ...
        $tid = imagecolorallocate($new_image, $tcol['red'], $tcol['green'], 
            $tcol['blue']);
    ...
    /**
     * Convert the loaded image to a PNG and then return a structure for the PDF
     * creator. This function requires GD library and write access to the
     * directory defined on K_PATH_CACHE constant.
     *
     * @param $image (image) Image object.
     * @param $tempfile (string) Temporary file name. return image PNG image
     *            object.
     * @since 4.9.016 (2010-04-20) @public static
     */
    ...
    /**
     * Convert the loaded image to a JPEG and then return a structure for the
     * PDF creator. This function requires GD library and write access to the
     * directory defined on K_PATH_CACHE constant.
     *
     * @param $image (image) Image object.
     * @param $quality (int) JPEG quality.
     * @param $tempfile (string) Temporary file name. return image JPEG image
     ...
     * @return array structure containing the image data @public static
     */
    public static function _parsejpeg($file) {
        ...
        if (count($icc) > 0) {
            ksort($icc);
            $icc = implode('', $icc);
            if ((ord($icc[36]) != 0x61) or (ord($icc[37]) != 0x63) or
                 (ord($icc[38]) != 0x73) or (ord($icc[39]) != 0x70)) {
                // invalid ICC profile
                $icc = false;
            }
        } else {
    ...
    public static function _parsepng($file) {
        ...
        fread($f, 4);
        $channels = ($ct == 2 ? 3 : 1);
        $parms = '/DecodeParms << /Predictor 15 /Colors ' . $channels .
