...
/**
 * @file Unicode data and font methods for TCPDF library.
 *
 * @author Nicola Asuni
...
class TCPDF_FONTS {
    ...
    /**
     * Static cache used for speed up uniord performances @protected
     */
    protected static $cache_uniord = array();

    /**
     * Convert and add the selected TrueType or Type1 font to the fonts folder
     * (that must be writeable).
     *
     * @param $fontfile (string) Font file (full path).
     * @param $fonttype (string) Font type. Leave empty for autodetect mode.
     *            Valid values are: TrueTypeUnicode, TrueType, Type1, CID0JP =
     *            CID-0 Japanese, CID0KR = CID-0 Korean, CID0CS = CID-0 Chinese
     *            Simplified, CID0CT = CID-0 Chinese Traditional.
     * @param $enc (string) Name of the encoding table to use. Leave empty for
     *            default mode. Omit this parameter for TrueType Unicode and
     *            symbolic fonts like Symbol or ZapfDingBats.
     * @param $flags (int) Unsigned 32-bit integer containing flags specifying
     *            various characteristics of the font (PDF32000:2008 - 9.8.2
     *            Font Descriptor Flags): +1 for fixed font; +4 for symbol or
     *            +32 for non-symbol; +64 for italic. Fixed and Italic mode are
     *            generally autodetected so you have to set it to 32 =
     *            non-symbolic font (default) or 4 = symbolic font.
     * @param $outpath (string) Output path for generated font files (must be
     *            writeable by the web server). Leave empty for default font
     *            folder.
     * @param $platid (int) Platform ID for CMAP table to extract (when building
     *            a Unicode font for Windows this value should be 3, for
     *            Macintosh should be 1).
     * @param $encid (int) Encoding ID for CMAP table to extract (when building
     *            a Unicode font for Windows this value should be 1, for
     *            Macintosh should be 0). When Platform ID is 3, legal values
     *            for Encoding ID are: 0=Symbol, 1=Unicode, 2=ShiftJIS, 3=PRC,
     *            4=Big5, 5=Wansung, 6=Johab, 7=Reserved, 8=Reserved,
     *            9=Reserved, 10=UCS-4.
     * @param $addcbbox (boolean) If true includes the character bounding box
     *            information on the php font file.
     * @param $link (boolean) If true link to system font instead of copying the
     *            font data (not transportable) - Note: do not work with Type1
     *            fonts.
     * @return (string) TCPDF font name or boolean false in case of error.
     * @author Nicola Asuni
     * @since 5.9.123 (2010-09-30) @public static
     */
    public static function addTTFfont($fontfile, $fonttype = '', $enc = '', 
        $flags = 32, $outpath = '', $platid = 3, $encid = 1, $addcbbox = false, 
        $link = false) {
        ...
        if (! isset($font_path_parts['filename'])) {
            $font_path_parts['filename'] = substr($font_path_parts['basename'], 
                0, - (strlen($font_path_parts['extension']) + 1));
        }
        ...
        if (@file_exists($outpath . $font_name . '.php')) {
            // this font already exist (delete it from fonts folder to rebuild
            // it)
            return $font_name;
        }
        ...
        if (($fmetric['type'] == 'TrueType') or ($fmetric['type'] == 'Type1')) {
            if (! empty($enc) and ($enc != 'cp1252') and
        ...
        if ($fmetric['type'] == 'Type1') {
            ...
            $encrypted = substr($font, (12 + $fmetric['size1']), 
                $fmetric['size2']);
            $data .= $encrypted;
            ...
            $fmetric['name'] = preg_replace('/[^a-zA-Z0-9_\-]/', '', 
                $matches[1]);
            ...
            preg_match('#/UnderlineThickness[\s]*([0-9\+\-]*)#', $font, 
                $matches);
            ...
            // get internal map
            $imap = array();
            if (preg_match_all('#dup[\s]([0-9]+)[\s]*/([^\s]*)[\s]put#sU', 
                $font, $fmap, PREG_SET_ORDER) > 0) {
                foreach ($fmap as $v) {
                    $imap[$v[2]] = $v[1];
                }
            }
            ...
            preg_match_all(
                '#/([A-Za-z0-9\.]*)[\s][0-9]+[\s]RD[\s](.*)[\s]ND#sU', $eplain, 
            ...
            foreach ($matches as $k => $v) {
                ...
                while ($i < $clen) {
                    if ($ccom[$i] < 32) {
                        ...
                        $cdec[$ck] = ((($ccom[$i] - 247) * 256) + $ccom[($i + 1)] +
                             108);
                        $i += 2;
                    } elseif (($ccom[$i] >= 251) and ($ccom[$i] <= 254)) {
                        $cdec[$ck] = ((- ($ccom[$i] - 251) * 256) -
                             $ccom[($i + 1)] - 108);
                        $i += 2;
                    } elseif ($ccom[$i] == 255) {
                        $sval = chr($ccom[($i + 1)]) . chr($ccom[($i + 2)]) .
                             chr($ccom[($i + 3)]) . chr($ccom[($i + 4)]);
            ...
            if ($fmetric['type'] != 'cidfont0') {
                if ($link) {
                    ...
                    // store compressed font
                    $fmetric['file'] .= '.z';
                    $fp = TCPDF_STATIC::fopenLocal($outpath . $fmetric['file'], 
                        'wb');
            ...
            for ($i = 0; $i < $numTables; ++ $i) {
                ...
                $offset += 4;
                $table[$tag] = array();
                $table[$tag]['checkSum'] = TCPDF_STATIC::_getULONG($font, 
                    $offset);
                $offset += 4;
            ...
            $offset += 2;
            // get the offsets to the locations of the glyphs in the font,
            // relative to the beginning of the glyphData table
            $indexToLoc = array();
            $offset = $table['loca']['offset'];
            if ($short_offset) {
                // short version
                $tot_num_glyphs = floor($table['loca']['length'] / 2); // numGlyphs
                                                                       // + 1
                for ($i = 0; $i < $tot_num_glyphs; ++ $i) {
                    $indexToLoc[$i] = TCPDF_STATIC::_getUSHORT($font, $offset) *
                         2;
                    if (isset($indexToLoc[($i - 1)]) &&
                ...
                // long version
                $tot_num_glyphs = floor($table['loca']['length'] / 4); // numGlyphs
                                                                       // + 1
                for ($i = 0; $i < $tot_num_glyphs; ++ $i) {
            ...
            for ($i = 0; $i < $numEncodingTables; ++ $i) {
                ...
                $encodingTables[$i]['encodingID'] = TCPDF_STATIC::_getUSHORT(
                    $font, $offset);
                $offset += 2;
                $encodingTables[$i]['offset'] = TCPDF_STATIC::_getULONG($font, 
                    $offset);
                $offset += 4;
            }
            // ---------- get os/2 metrics ----------
            ...
            // estimate StemV and StemH (400 = usWeightClass for Normal -
            // Regular font)
            ...
            $offset += 2;
            if ($fsType == 2) {
                // This Font cannot be modified, embedded or exchanged in any
                // manner without first obtaining permission of the legal owner.
                return false;
            }
            // ---------- get font name ----------
            $fmetric['name'] = '';
            ...
            for ($i = 0; $i < $numNameRecords; ++ $i) {
                $offset += 6; // skip Platform ID, Platform-specific encoding
                              // ID, Language ID.
                              // Name ID.
                ...
                if ($nameID == 6) {
                    ...
                    $offset += 2;
                    $offset = ($table['name']['offset'] + $stringStorageOffset +
                         $stringOffset);
            ...
            foreach ($encodingTables as $enctable) {
                ...
                if (($enctable['platformID'] == $platid) and
                    ...
                    switch ($format) {
                        ...
                        case 2:
                            { // Format 2: High-byte mapping through table
                                ...
                                $numSubHeaders = 0;
                                for ($i = 0; $i < 256; ++ $i) {
                                    // Array that maps high bytes to subHeaders:
                                    // value is subHeader index * 8.
                                    $subHeaderKeys[$i] = (TCPDF_STATIC::_getUSHORT(
                                        $font, $offset) / 8);
                                    $offset += 2;
                                ...
                                // the number of subHeaders is equal to the max
                                // of subHeaderKeys + 1
                                ++ $numSubHeaders;
                                ...
                                for ($k = 0; $k < $numSubHeaders; ++ $k) {
                                    $subHeaders[$k]['firstCode'] = TCPDF_STATIC::_getUSHORT(
                                        $font, $offset);
                                    $offset += 2;
                                    $subHeaders[$k]['entryCount'] = TCPDF_STATIC::_getUSHORT(
                                        $font, $offset);
                                    $offset += 2;
                                    $subHeaders[$k]['idDelta'] = TCPDF_STATIC::_getUSHORT(
                                        $font, $offset);
                                    $offset += 2;
                                    $subHeaders[$k]['idRangeOffset'] = TCPDF_STATIC::_getUSHORT(
                                        $font, $offset);
                                    $offset += 2;
                                ...
                                for ($i = 0; $i < 256; ++ $i) {
                                    ...
                                    if ($k == 0) {
                                        ...
                                        for ($j = $start_byte; $j < $end_byte; ++ $j) {
                                            ...
                                            $c = (($i << 8) + $j);
                                            $idRangeOffset = ($subHeaders[$k]['idRangeOffset'] +
                                                 $j -
                        ...
                        case 4:
                            { // Format 4: Segment mapping to delta values
                                $length = TCPDF_STATIC::_getUSHORT($font, 
                                    $offset);
                                $offset += 2;
                                ...
                                $offset += 2;
                                $offset += 6; // skip searchRange,
                                              // entrySelector, rangeShift
                                $endCount = array(); // array of end character
                                ...
                                $startCount = array(); // array of start
                                                       // character codes for
                                                       // each segment
                                for ($k = 0; $k < $segCount; ++ $k) {
                                ...
                                $idDelta = array(); // delta for all character
                                                    // codes in segment
                                for ($k = 0; $k < $segCount; ++ $k) {
                                ...
                                $idRangeOffset = array(); // Offsets into
                                                          // glyphIdArray or 0
                                for ($k = 0; $k < $segCount; ++ $k) {
                        ...
                        case 6:
                            { // Format 6: Trimmed table mapping
                                $offset += 4; // skip length and
                                              // version/language
                                $firstCode = TCPDF_STATIC::_getUSHORT($font, 
                                    $offset);
                                $offset += 2;
                                $entryCount = TCPDF_STATIC::_getUSHORT($font, 
                                    $offset);
                                $offset += 2;
                                for ($k = 0; $k < $entryCount; ++ $k) {
                                    $c = ($k + $firstCode);
                                    $g = TCPDF_STATIC::_getUSHORT($font, 
                                        $offset);
                                    $offset += 2;
                                    $ctg[$c] = $g;
                                }
                        ...
                        case 8:
                            { // Format 8: Mixed 16-bit and 32-bit coverage
                                $offset += 10; // skip reserved, length and
                                               // version/language
                                for ($k = 0; $k < 8192; ++ $k) {
                                    $is32[$k] = TCPDF_STATIC::_getBYTE($font, 
                                        $offset);
                                    ++ $offset;
                                }
                                $nGroups = TCPDF_STATIC::_getULONG($font, 
                                    $offset);
                                $offset += 4;
                                for ($i = 0; $i < $nGroups; ++ $i) {
                                    ...
                                    for ($k = $startCharCode; $k <= $endCharCode; ++ $k) {
                                        $is32idx = floor($c / 8);
                                        if ((isset($is32[$is32idx])) and (($is32[$is32idx] &
                                             (1 << (7 - ($c % 8)))) == 0)) {
                                            $c = $k;
                                        } else {
                                            // 32 bit format
                        ...
                        case 10:
                            { // Format 10: Trimmed array
                                $offset += 10; // skip reserved, length and
                                               // version/language
                                $startCharCode = TCPDF_STATIC::_getULONG($font, 
                                    $offset);
                                $offset += 4;
                                $numChars = TCPDF_STATIC::_getULONG($font, 
                                    $offset);
                                $offset += 4;
        ...
        if ($fmetric['type'] == 'Type1') {
            ...
            if ($fmetric['type'] == 'cidfont0') {
                ...
                // create CIDToGIDMap
                $cidtogidmap = str_pad('', 131072, "\x00"); // (256 * 256 * 2) =
                                                            // 131072
                foreach ($ctg as $cid => $gid) {
                    $cidtogidmap = self::updateCIDtoGIDmap($cidtogidmap, $cid, 
                        $ctg[$cid]);
                }
     ...
     * @param $subsetchars (array) Array of used characters (the glyphs to
     *            keep).
     * @return (string) A subset of TrueType font data without the unused
     *         glyphs.
     * @author Nicola Asuni
     * @since 5.2.000 (2010-06-02) @public static
     */
    public static function _getTrueTypeFontSubset($font, $subsetchars) {
        ...
        $short_offset = (TCPDF_STATIC::_getSHORT($font, $offset) == 0);
        $offset += 2;
        // get the offsets to the locations of the glyphs in the font, relative
        ...
        $subsetglyphs[0] = true; // character codes that do not correspond to
                                 // any glyph in the font should be mapped to
                                 // glyph index 0
        $offset = $table['cmap']['offset'] + 2;
        $numEncodingTables = TCPDF_STATIC::_getUSHORT($font, $offset);
        $offset += 2;
        $encodingTables = array();
        for ($i = 0; $i < $numEncodingTables; ++ $i) {
            $encodingTables[$i]['platformID'] = TCPDF_STATIC::_getUSHORT($font, 
                $offset);
            $offset += 2;
            $encodingTables[$i]['encodingID'] = TCPDF_STATIC::_getUSHORT($font, 
                $offset);
            $offset += 2;
            $encodingTables[$i]['offset'] = TCPDF_STATIC::_getULONG($font, 
                $offset);
            ...
            switch ($format) {
                ...
                case 2:
                    { // Format 2: High-byte mapping through table
                        ...
                        $numSubHeaders = 0;
                        for ($i = 0; $i < 256; ++ $i) {
                            // Array that maps high bytes to subHeaders: value
                            // is subHeader index * 8.
                            $subHeaderKeys[$i] = (TCPDF_STATIC::_getUSHORT(
                                $font, $offset) / 8);
                            $offset += 2;
                            if ($numSubHeaders < $subHeaderKeys[$i]) {
                        ...
                        // the number of subHeaders is equal to the max of
                        // subHeaderKeys + 1
                        ++ $numSubHeaders;
                        // read subHeader structures
                        $subHeaders = array();
                ...
                case 4:
                    { // Format 4: Segment mapping to delta values
                        ...
                        $segCount = floor(
                            TCPDF_STATIC::_getUSHORT($font, $offset) / 2);
                        $offset += 2;
                        $offset += 6; // skip searchRange, entrySelector,
                                      // rangeShift
                        $endCount = array(); // array of end character codes for
                                             // each segment
                        for ($k = 0; $k < $segCount; ++ $k) {
                            $endCount[$k] = TCPDF_STATIC::_getUSHORT($font, 
                                $offset);
                            $offset += 2;
                        }
                        $offset += 2; // skip reservedPad
                        $startCount = array(); // array of start character codes
                                               // for each segment
                        for ($k = 0; $k < $segCount; ++ $k) {
                            $startCount[$k] = TCPDF_STATIC::_getUSHORT($font, 
                                $offset);
                            $offset += 2;
                        }
                        $idDelta = array(); // delta for all character codes in
                                            // segment
                        for ($k = 0; $k < $segCount; ++ $k) {
                            $idDelta[$k] = TCPDF_STATIC::_getUSHORT($font, 
                                $offset);
                            $offset += 2;
                        }
                        $idRangeOffset = array(); // Offsets into glyphIdArray
                                                  // or 0
                        for ($k = 0; $k < $segCount; ++ $k) {
                            $idRangeOffset[$k] = TCPDF_STATIC::_getUSHORT($font, 
                                $offset);
                            $offset += 2;
                        }
                        ...
                        $glyphIdArray = array(); // glyph index array
                        for ($k = 0; $k < $gidlen; ++ $k) {
                            $glyphIdArray[$k] = TCPDF_STATIC::_getUSHORT($font, 
                                $offset);
                            $offset += 2;
                        }
                        for ($k = 0; $k < $segCount; ++ $k) {
                    ...
                    { // Format 8: Mixed 16-bit and 32-bit coverage
                        ...
                        for ($i = 0; $i < $nGroups; ++ $i) {
                            $startCharCode = TCPDF_STATIC::_getULONG($font, 
                                $offset);
                            $offset += 4;
                            $endCharCode = TCPDF_STATIC::_getULONG($font, 
                                $offset);
                            $offset += 4;
                            $startGlyphID = TCPDF_STATIC::_getULONG($font, 
                                $offset);
                            $offset += 4;
                            for ($k = $startCharCode; $k <= $endCharCode; ++ $k) {
                                $is32idx = floor($c / 8);
                                if ((isset($is32[$is32idx])) and (($is32[$is32idx] &
                                    ...
                                    // = 55232
                                    // SURROGATE_OFFSET = (0x10000 - (0xD800 <<
                                    // 10) - 0xDC00) = -56613888
                                    $c = ((55232 + ($k >> 10)) << 10) +
                                         (0xDC00 + ($k & 0x3FF)) - 56613888;
                                }
                                if (isset($subsetchars[$c])) {
                    ...
                    { // Format 12: Segmented coverage
                        ...
                        for ($k = 0; $k < $nGroups; ++ $k) {
                            ...
                            $endCharCode = TCPDF_STATIC::_getULONG($font, 
                                $offset);
                            $offset += 4;
                            $startGlyphCode = TCPDF_STATIC::_getULONG($font, 
                                $offset);
                            $offset += 4;
                            for ($c = $startCharCode; $c <= $endCharCode; ++ $c) {
                                if (isset($subsetchars[$c])) {
                ...
                if (isset($indexToLoc[$key])) {
                    ...
                    if ($numberOfContours < 0) { // composite glyph
                        ...
                        do {
                            ...
                            $offset += 2;
                            $glyphIndex = TCPDF_STATIC::_getUSHORT($font, 
                                $offset);
                            $offset += 2;
        ...
        for ($i = 0; $i < $tot_num_glyphs; ++ $i) {
            if (isset($subsetglyphs[$i])) {
                ...
                $glyf .= substr($font, ($glyf_offset + $indexToLoc[$i]), 
                    $length);
            } else {
                $length = 0;
            }
        ...
        // array of table names to preserve (loca and glyf tables will be added
        // later)
        // the cmap table is not needed and shall not be present, since the
        // mapping from character codes to glyph descriptions is provided
        // separately
        $table_names = array(
            'head', 
            'hhea', 
            ...
            'cvt ', 
            'fpgm', 
            'prep'); // minimum required table names
                     // get the tables to preserve
        $offset = 12;
        foreach ($table as $tag => $val) {
            if (in_array($tag, $table_names)) {
                $table[$tag]['data'] = substr($font, $table[$tag]['offset'], 
                    $table[$tag]['length']);
                if ($tag == 'head') {
                    // set the checkSumAdjustment to 0
                    $table[$tag]['data'] = substr($table[$tag]['data'], 0, 8) .
                         "\x0\x0\x0\x0" . substr($table[$tag]['data'], 12);
                }
                ...
                // check sum is not changed (so keep the following line
                // commented)
                // $table[$tag]['checkSum'] =
                // self::_getTTFtableChecksum($table[$tag]['data'],
                // $table[$tag]['length']);
            } else {
                unset($table[$tag]);
            }
        ...
        $table['loca']['checkSum'] = self::_getTTFtableChecksum(
            $table['loca']['data'], $table['loca']['length']);
        ...
        $table['glyf']['checkSum'] = self::_getTTFtableChecksum(
            $table['glyf']['data'], $table['glyf']['length']);
        // rebuild font
        $font = '';
        ...
        $checkSumAdjustment = 0xB1B0AFBA -
             self::_getTTFtableChecksum($font, strlen($font));
        $font = substr($font, 0, $table['head']['offset'] + 8) .
     ...
     * @author Nicola Asuni
     * @since 4.4.000 (2008-12-07) @public static
     */
     ...
     * @since 2.3.000 (2008-03-05) @public static
     */
     ...
     *
     * @param $c (int) UTF-8 value
     * @return Returns the specified character. @public static
     */
    ...
    /**
     * Converts array of UTF-8 characters to UTF16-BE string.<br> Based on:
     * http://www.faqs.org/rfcs/rfc2781.html <pre> Encoding UTF-16: Encoding of
     * a single character from an ISO 10646 character value to UTF-16 proceeds
     * as follows. Let U be the character number, no greater than 0x10FFFF. 1)
     * If U < 0x10000, encode U as a 16-bit unsigned integer and terminate. 2)
     * Let U' = U - 0x10000. Because U is less than or equal to 0x10FFFF, U'
     * must be less than or equal to 0xFFFFF. That is, U' can be represented in
     * 20 bits. 3) Initialize two 16-bit unsigned integers, W1 and W2, to 0xD800
     * and 0xDC00, respectively. These integers each have 10 bits free to encode
     * the character value, for a total of 20 bits. 4) Assign the 10 high-order
     * bits of the 20-bit U' to the 10 low-order bits of W1 and the 10 low-order
     * bits of U' to the 10 low-order bits of W2. Terminate. Graphically, steps
     * 2 through 4 look like: U' = yyyyyyyyyyxxxxxxxxxx W1 = 110110yyyyyyyyyy W2
     * = 110111xxxxxxxxxx </pre>
     *
     ...
     * @author Nicola Asuni
     * @since 2.1.000 (2008-01-08) @public static
     */
     ...
     * @since 4.5.037 (2009-04-07) @public static
     */
     ...
     * @param $unicode (boolean) True if we are in unicode mode, false
     *            otherwise.
     * @return Return part of a string @public static
     */
    public static function UTF8ArrSubString($strarr, $start = '', $end = '', 
        $unicode = true) {
        if (strlen($start) == 0) {
            $start = 0;
        }
     ...
     * @author Nicola Asuni
     * @since 5.9.123 (2011-09-29) @public static
     */
     ...
     * @param $fontdir (string) Font directory (set to false fto search on
     *            default directories)
     ...
     * @return array
     * @author Nicola Asuni
     * @since 4.8.023 (2010-01-15) @public static
     */
    ...
    /**
     * Converts UTF-8 character to integer value.<br> Uses the getUniord()
     * method if the value is not cached.
     *
     * @param $uch (string) character string to process.
     * @return integer Unicode value @public static
     */
    ...
    /**
     * Converts UTF-8 character to integer value.<br> Invalid byte sequences
     * will be replaced with 0xFFFD (replacement character)<br> Based on:
     * http://www.faqs.org/rfcs/rfc3629.html <pre> Char. number range | UTF-8
     * octet sequence (hexadecimal) | (binary)
     * --------------------+----------------------------------------------- 0000
     ...
     * UTF8-4 UTF8-1 = %x00-7F UTF8-2 = %xC2-DF UTF8-tail UTF8-3 = %xE0 %xA0-BF
     * UTF8-tail / %xE1-EC 2( UTF8-tail ) / %xED %x80-9F UTF8-tail / %xEE-EF 2(
     * UTF8-tail ) UTF8-4 = %xF0 %x90-BF 2( UTF8-tail ) / %xF1-F3 3( UTF8-tail )
     * / %xF4 %x80-8F 2( UTF8-tail ) UTF8-tail = %x80-BF
     * ---------------------------------------------------------------------
     * </pre>
     *
     * @param $uch (string) character string to process.
    ...
    public static function getUniord($uch) {
        ...
        $countbytes = 0;
        $numbytes = 1; // number of octetc needed to represent the UTF-8
                       // character
        $length = strlen($uch);
        for ($i = 0; $i < $length; ++ $i) {
            ...
            if ($countbytes == 0) { // get starting octect
                ...
                } elseif (($char >> 0x05) == 0x06) { // 2 bytes character (0x06
                                                     // = 110 BIN)
                    $bytes[] = ($char - 0xC0) << 0x06;
                    ++ $countbytes;
                    $numbytes = 2;
                } elseif (($char >> 0x04) == 0x0E) { // 3 bytes character (0x0E
                                                     // = 1110 BIN)
                    $bytes[] = ($char - 0xE0) << 0x0C;
                    ++ $countbytes;
                    $numbytes = 3;
                } elseif (($char >> 0x03) == 0x1E) { // 4 bytes character (0x1E
                                                     // = 11110 BIN)
            ...
            } elseif (($char >> 0x06) == 0x02) { // bytes 2, 3 and 4 must start
                                                 // with 0x02 = 10 BIN
                $bytes[] = $char - 0x80;
                ++ $countbytes;
                if ($countbytes == $numbytes) {
                    ...
                    if ((($char >= 0xD800) and ($char <= 0xDFFF)) or
                         ($char >= 0x10FFFF)) {
                        // The definition of UTF-8 prohibits encoding character
                        // numbers between
                        // U+D800 and U+DFFF, which are reserved for use with
                        // the UTF-16
                        // encoding form (as surrogate pairs) and do not
                        // directly represent
                        // characters.
    ...
    /**
     * Converts UTF-8 strings to codepoints array.<br> Invalid byte sequences
     * will be replaced with 0xFFFD (replacement character)<br>
     *
     * @param $str (string) string to process.
     * @param $isunicode (boolean) True when the documetn is in Unicode mode,
     *            false otherwise.
    ...
    public static function UTF8StringToArray($str, $isunicode = true, &$currentfont) {
        if ($isunicode) {
            ...
            $chars = TCPDF_STATIC::pregSplit('//', 'u', $str, - 1, 
                PREG_SPLIT_NO_EMPTY);
            $carr = array_map(
                array(
                    'TCPDF_FONTS', 
     ...
     * @param $currentfont (array) Reference to current font array.
     * @return string
     * @since 3.2.000 (2008-06-23) @public static
     */
    public static function UTF8ToLatin1($str, $isunicode = true, &$currentfont) {
        $unicode = self::UTF8StringToArray($str, $isunicode, $currentfont); // array
                                                                            // containing
                                                                            // UTF-8
                                                                            // unicode
                                                                            // values
     ...
     * @return string
     * @author Nicola Asuni
     * @since 1.53.0.TC005 (2005-01-05) @public static
     */
    public static function UTF8ToUTF16BE($str, $setbom = false, $isunicode = true, 
        &$currentfont) {
        if (! $isunicode) {
     ...
     * @param $isunicode (boolean) True if the document is in Unicode mode,
     *            false otherwise.
     * @param $currentfont (array) Reference to current font array.
     * @return string
     * @author Nicola Asuni
     * @since 2.1.000 (2008-01-08) @public static
     */
    public static function utf8StrRev($str, $setbom = false, $forcertl = false, 
        $isunicode = true, &$currentfont) {
        return self::utf8StrArrRev(
            self::UTF8StringToArray($str, $isunicode, $currentfont), $str, 
            $setbom, $forcertl, $isunicode, $currentfont);
    }

    /**
     * Reverse the RLT substrings array using the Bidirectional Algorithm
     * (http://unicode.org/reports/tr9/).
     *
     ...
     * @return string
     * @author Nicola Asuni
     * @since 4.9.000 (2010-03-27) @public static
     */
    public static function utf8StrArrRev($arr, $str = '', $setbom = false, 
        $forcertl = false, $isunicode = true, &$currentfont) {
     ...
     * @author Nicola Asuni
     * @since 2.4.000 (2008-03-06) @public static
     */
    public static function utf8Bidi($ta, $str = '', $forcertl = false, 
        $isunicode = true, &$currentfont) {
        ...
        // In the resolution of levels in rules I1 and I2, the maximum embedding
        // level of 62 can be reached.
        for ($i = 0; $i < $numchars; ++ $i) {
            if ($ta[$i] == TCPDF_FONT_DATA::$uni_RLE) {
                // X2. With each RLE, compute the least greater odd embedding
                // level.
                // a. If this new level would be valid, then this embedding code
                // is valid. Remember (push) the current embedding level and
                // override status. Reset the current level to this new level,
                // and reset the override status to neutral.
                // b. If the new level would not be valid, then this code is
                // invalid. Do not change the current level or override status.
                ...
                // X3. With each LRE, compute the least greater even embedding
                // level.
                // a. If this new level would be valid, then this embedding code
                // is valid. Remember (push) the current embedding level and
                // override status. Reset the current level to this new level,
                ...
                // X4. With each RLO, compute the least greater odd embedding
                // level.
                // a. If this new level would be valid, then this embedding code
                // is valid. Remember (push) the current embedding level and
                // override status. Reset the current level to this new level,
                // and reset the override status to right-to-left.
                // b. If the new level would not be valid, then this code is
                // invalid. Do not change the current level or override status.
                $next_level = $cel + ($cel % 2) + 1;
                if ($next_level < 62) {
                ...
                // X5. With each LRO, compute the least greater even embedding
                // level.
                // a. If this new level would be valid, then this embedding code
                // is valid. Remember (push) the current embedding level and
                // override status. Reset the current level to this new level,
                // and reset the override status to left-to-right.
                // b. If the new level would not be valid, then this code is
                // invalid. Do not change the current level or override status.
                $next_level = $cel + 2 - ($cel % 2);
                if ($next_level < 62) {
        ...
        // X8. All explicit directional embeddings and overrides are completely
          ...
          // the same level. For each run, determine the start-of-level-run
          // (sor) and end-of-level-run (eor) type, either L or R. This depends
          // on the higher of the two levels on either side of the boundary (at
          // the start or end of the paragraph, the level of the 'other' run is
          // the base embedding level). If the higher level is odd, the type is
          // R; otherwise, it is L.
          
        // 3.3.3 Resolving Weak Types
          // Weak types are now resolved one level run at a time. At level run
          // boundaries where the type of the character on the other side of the
          // boundary is required, the type assigned to sor or eor is used.
        ...
        $levcount = 0;
        for ($i = 0; $i < $numchars; ++ $i) {
            if (($levcount > 0) and (($i + 1) < $numchars) and
    ...
    // Arabic numbers act as if they were R in terms of their influence on
    // neutrals. Start-of-level-run (sor) and end-of-level-run (eor) are used at
    // level run boundaries.
    $prevlevel = - 1;
    $levcount = 0;
    for ($i = 0; $i < $numchars; ++ $i) {
        if (($levcount > 0) and (($i + 1) < $numchars) and
             ($chardata[($i + 1)]['level'] == $prevlevel)) {
            if (($chardata[$i]['type'] == 'N') and
             ($chardata[($i - 1)]['type'] == 'L') and
             ($chardata[($i + 1)]['type'] == 'L')) {
            $chardata[$i]['type'] = 'L';
        } elseif (($chardata[$i]['type'] == 'N') and (($chardata[($i - 1)]['type'] ==
             'R') or ($chardata[($i - 1)]['type'] == 'EN') or
             ($chardata[($i - 1)]['type'] == 'AN')) and (($chardata[($i + 1)]['type'] ==
             'R') or ($chardata[($i + 1)]['type'] == 'EN') or
             ($chardata[($i + 1)]['type'] == 'AN'))) {
            $chardata[$i]['type'] = 'R';
    ...
    } elseif (($levcount == 0) and (($i + 1) < $numchars) and
         ($chardata[($i + 1)]['level'] == $prevlevel)) {
        // first char
        if (($chardata[$i]['type'] == 'N') and ($chardata[$i]['sor'] == 'L') and
         ($chardata[($i + 1)]['type'] == 'L')) {
        $chardata[$i]['type'] = 'L';
    } elseif (($chardata[$i]['type'] == 'N') and (($chardata[$i]['sor'] == 'R') or
         ($chardata[$i]['sor'] == 'EN') or ($chardata[$i]['sor'] == 'AN')) and (($chardata[($i +
         1)]['type'] == 'R') or ($chardata[($i + 1)]['type'] == 'EN') or
         ($chardata[($i + 1)]['type'] == 'AN'))) {
        $chardata[$i]['type'] = 'R';
    } elseif ($chardata[$i]['type'] == 'N') {
        // N2. Any remaining neutrals take the embedding direction
     ...
     ((($i + 1) == $numchars) or
     (($i + 1) < $numchars) and ($chardata[($i + 1)]['level'] != $prevlevel))) {
    // last char
    if (($chardata[$i]['type'] == 'N') and ($chardata[($i - 1)]['type'] == 'L') and
     ($chardata[$i]['eor'] == 'L')) {
    $chardata[$i]['type'] = 'L';
} elseif (($chardata[$i]['type'] == 'N') and (($chardata[($i - 1)]['type'] == 'R') or
     ($chardata[($i - 1)]['type'] == 'EN') or
     ($chardata[($i - 1)]['type'] == 'AN')) and (($chardata[$i]['eor'] == 'R') or
     ($chardata[$i]['eor'] == 'EN') or ($chardata[$i]['eor'] == 'AN'))) {
    $chardata[$i]['type'] = 'R';
} elseif ($chardata[$i]['type'] == 'N') {
    // N2. Any remaining neutrals take the embedding direction
    $chardata[$i]['type'] = $chardata[$i]['sor'];
...
// of positional character shapes that depend on adjacent characters. Shaping is
// logically applied after the Bidirectional Algorithm is used and is limited to
...
$x = 0;
for ($i = 0; $i < $numchars; ++ $i) {
if ((TCPDF_FONT_DATA::$uni_type[$chardata[$i]['char']] == 'AL') or
     ($chardata[$i]['char'] == 32) or ($chardata[$i]['char'] == 8204)) {
    $charAL[$x] = $chardata[$i];
    $charAL[$x]['i'] = $i;
    $chardata[$i]['x'] = $x;
    ++ $x;
...
if (TCPDF_FONT_DATA::$uni_type[$thischar['char']] == 'AL') {
    ...
    if (($prevchar !== false) and ($nextchar !== false) and ((TCPDF_FONT_DATA::$uni_type[$prevchar['char']] ==
         'AL') or (TCPDF_FONT_DATA::$uni_type[$prevchar['char']] == 'NSM')) and ((TCPDF_FONT_DATA::$uni_type[$nextchar['char']] ==
         'AL') or (TCPDF_FONT_DATA::$uni_type[$nextchar['char']] == 'NSM')) and
         ($prevchar['type'] == $thischar['type']) and
         ($nextchar['type'] == $thischar['type']) and ($nextchar['char'] != 1567)) {
    ...
    } elseif (($nextchar !== false) and ((TCPDF_FONT_DATA::$uni_type[$nextchar['char']] ==
         'AL') or (TCPDF_FONT_DATA::$uni_type[$nextchar['char']] == 'NSM')) and
         ($nextchar['type'] == $thischar['type']) and ($nextchar['char'] != 1567)) {
    ...
    } elseif ((($prevchar !== false) and ((TCPDF_FONT_DATA::$uni_type[$prevchar['char']] ==
         'AL') or (TCPDF_FONT_DATA::$uni_type[$prevchar['char']] == 'NSM')) and
...
/*
 * Combining characters that can occur with Arabic Shadda (0651 HEX, 1617 DEC)
 * are replaced. Putting the combining mark and shadda in the same glyph allows
 * us to avoid the two marks overlapping each other in an illegible manner.
 */
for ($i = 0; $i < ($numchars - 1); ++ $i) {
if (($chardata2[$i]['char'] == 1617) and
 (isset(TCPDF_FONT_DATA::$uni_diacritics[($chardata2[$i + 1]['char'])]))) {
// check if the subtitution font is defined on current font
if (isset(
    $currentfont['cw'][(TCPDF_FONT_DATA::$uni_diacritics[($chardata2[$i + 1]['char'])])])) {
    $chardata2[$i]['char'] = false;
    $chardata2[$i + 1]['char'] = TCPDF_FONT_DATA::$uni_diacritics[($chardata2[$i +
         1]['char'])];
}
}
}
...
// line, including intermediate levels not actually present in the text, reverse
 ...
 * @return float value in points @public static
 */
