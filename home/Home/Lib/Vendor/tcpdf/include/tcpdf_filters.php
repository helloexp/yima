...
// -------------------------------------------------------------------
//
// Description : This is a PHP class for decoding common PDF filters (PDF
// 32000-2008 - 7.4 Filters).
//
// ============================================================+

/**
 * @file This is a PHP class for decoding common PDF filters (PDF 32000-2008 -
 * 7.4 Filters).<br>
 *
...
/**
 * @class TCPDF_FILTERS This is a PHP class for decoding common PDF filters (PDF
 * 32000-2008 - 7.4 Filters).<br>
 *
 * @package com.tecnick.tcpdf @brief This is a PHP class for decoding common PDF
 *          filters.
 * @version 1.0.001
...
class TCPDF_FILTERS {
    ...
    /**
     * Define a list of available filter decoders. @private static
     */
     ...
     * @since 1.0.000 (2011-05-23) @public static
     */
    ...
    public static function decodeFilterASCII85Decode($data) {
        ...
        for ($i = 0; $i < $data_length; ++ $i) {
            ...
            if ($char == 122) { // 'z'
                ...
                // the value represented by a group of 5 characters should never
                // be greater than 2^32 - 1
                $tuple += (($char - 33) * $pow85[$group_pos]);
                if ($group_pos == 4) {
                    $decoded .= chr($tuple >> 24) . chr($tuple >> 16) .
                         chr($tuple >> 8) . chr($tuple);
                    $tuple = 0;
                    $group_pos = 0;
                } else {
                    ++ $group_pos;
    ...
    /**
     * LZWDecode Decompresses data encoded using the LZW (Lempel-Ziv-Welch)
     * adaptive compression method, reproducing the original text or binary
     * data.
     *
     * @param $data (string) Data to decode.
     * @return Decoded data string.
        ...
        while (($data_length > 0) and
            ...
            if ($index == 256) { // clear-table marker
                ...
                if ($index < $dix) {
                    ...
                    $dic_val = $dictionary[$prev_index] .
                         $dictionary[$prev_index][0];
                    $decoded .= $dic_val;
                }
                // update dictionary
    ...
    /**
     * FlateDecode Decompresses data encoded using the zlib/deflate compression
    ...
    /**
     * RunLengthDecode Decompresses data encoded using a byte-oriented
    ...
    public static function decodeFilterRunLengthDecode($data) {
        ...
        while ($i < $data_length) {
            ...
            if ($byte == 128) {
                ...
                // literally during decompression
                $decoded .= substr($data, ($i + 1), ($byte + 1));
                // move to next block
                $i += ($byte + 2);
            } else {
                // if length is in the range 129 to 255,
                // the following single byte shall be copied 257 - length (2 to
                // 128) times during decompression
    ...
    /**
     * CCITTFaxDecode (NOT IMPLEMETED - RETURN AN EXCEPTION) Decompresses data
     * encoded using the CCITT facsimile standard, reproducing the original data
    ...
    /**
     * JBIG2Decode (NOT IMPLEMETED - RETURN AN EXCEPTION) Decompresses data
     * encoded using the JBIG2 standard, reproducing the original monochrome (1
     * bit per pixel) image data (or an approximation of that data).
     *
     * @param $data (string) Data to decode.
     * @return Decoded data string.
     * @since 1.0.000 (2011-05-23) @public static
    ...
    /**
     * DCTDecode (NOT IMPLEMETED - RETURN AN EXCEPTION) Decompresses data
     * encoded using a DCT (discrete cosine transform) technique based on the
     * JPEG standard, reproducing image sample data that approximates the
     * original data.
     *
     * @param $data (string) Data to decode.
     * @return Decoded data string.
    ...
    /**
     * JPXDecode (NOT IMPLEMETED - RETURN AN EXCEPTION) Decompresses data
     * encoded using the wavelet-based JPEG2000 standard, reproducing the
     * original image data.
     *
     * @param $data (string) Data to decode.
     * @return Decoded data string.
    ...
    /**
     * Crypt (NOT IMPLEMETED - RETURN AN EXCEPTION) Decrypts data encrypted by a
     * security handler, reproducing the data as it was before encryption.
     *
     * @param $data (string) Data to decode.
     * @return Decoded data string.
     * @since 1.0.000 (2011-05-23) @public static
