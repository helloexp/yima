...
// PDF417 (ISO/IEC 15438:2006) is a 2-dimensional stacked bar code created by
// Symbol Technologies in 1991.
...
/**
 * @file Class to create PDF417 barcode arrays for TCPDF class. PDF417 (ISO/IEC
 * 15438:2006) is a 2-dimensional stacked bar code created by Symbol
 * Technologies in 1991. (requires PHP bcmath extension)
 *
...
/**
 * @class PDF417 Class to create PDF417 barcode arrays for TCPDF class. PDF417
 * (ISO/IEC 15438:2006) is a 2-dimensional stacked bar code created by Symbol
...
class PDF417 {
    ...
    /**
     * Array of switching codes for Text Compaction Sub-Modes. @protected
     */
    ...
    /**
     * Clusters of codewords (0, 3, 6)<br/> Values are hex equivalents of binary
     * representation of bars (1 = bar, 0 = space).<br/> The codewords numbered
     * from 900 to 928 have special meaning, some enable to switch between modes
     * in order to optimise the code:<ul> <li>900 : Switch to "Text" mode</li>
     * <li>901 : Switch to "Byte" mode</li> <li>902 : Switch to "Numeric"
     * mode</li> <li>903 - 912 : Reserved</li> <li>913 : Switch to "Octet" only
     * for the next codeword</li> <li>914 - 920 : Reserved</li> <li>921 :
     * Initialization</li> <li>922 : Terminator codeword for Macro PDF control
     * block</li> <li>923 : Sequence tag to identify the beginning of optional
     * fields in the Macro PDF control block</li> <li>924 : Switch to "Byte"
     * mode (If the total number of byte is multiple of 6)</li> <li>925 :
     * Identifier for a user defined Extended Channel Interpretation (ECI)</li>
     * <li>926 : Identifier for a general purpose ECI format</li> <li>927 :
     * Identifier for an ECI of a character set or code page</li> <li>928 :
     * Macro marker codeword to indicate the beginning of a Macro PDF Control
     * Block</li> </ul> @protected
     */
     ...
     * correction; one sub array for each correction level (0-8). @protected
     */
     ...
     * @param $ecl (int) error correction level (0-8); default -1 = automatic
     *            correction level
     * @param $aspectratio (float) the width to height of the symbol (excluding
     *            quiet zones)
     * @param $macro (array) information for macro block @public
     */
    public function __construct($code, $ecl = -1, $aspectratio = 2, 
        ...
        if (! empty($macro)) {
            ...
            $macrocw[] = 928;
            // segment index
            $cw = $this->getCompaction(902, 
                sprintf('%05d', $macro['segment_index']), false);
            ...
            foreach ($optmodes as $k => $omode) {
                if (isset($macro['option_' . $k])) {
                    ...
                    $macrocw[] = $k;
                    if ($optsize[$k] == 2) {
                        $macro['option_' . $k] = sprintf('%05d', 
                            $macro['option_' . $k]);
                    } elseif ($optsize[$k] == 4) {
                        $macro['option_' . $k] = sprintf('%010d', 
                            $macro['option_' . $k]);
                    }
                    $cw = $this->getCompaction($omode, $macro['option_' . $k], 
                        false);
                    $macrocw = array_merge($macrocw, $cw);
                }
            }
        ...
        // Symbol Length Descriptor (number of data codewords including Symbol
        // Length Descriptor and pad codewords)
        $sld = $size - $errsize;
     ...
     * @return int error correction level @protected
     */
     ...
     *
     * @param $cw (array) array of codewords including Symbol Length Descriptor
     *            and pad
     * @param $ecl (int) error correction level 0-8
     * @return array of error correction codewords @protected
     */
    ...
    protected function getInputSequences($code) {
        ...
        foreach ($numseq[1] as $seq) {
            ...
            if ($seq[1] > 0) {
                ...
                $textseq = array();
                // get text sequences
                preg_match_all('/([\x09\x0a\x0d\x20-\x7e]{5,})/', $prevseq, 
                    $textseq, PREG_OFFSET_CAPTURE);
                $textseq[1][] = array(
                    '', 
                    strlen($prevseq));
                $txtoffset = 0;
                foreach ($textseq[1] as $txtseq) {
                    ...
                    if ($txtseq[1] > 0) {
                        ...
                        $prevtxtseq = substr($prevseq, $txtoffset, 
                            ($txtseq[1] - $txtoffset));
                        if (strlen($prevtxtseq) > 0) {
                            // add BYTE sequence
                            if ((strlen($prevtxtseq) == 1) and ((count(
                                $sequence_array) > 0) and ($sequence_array[(count(
                                $sequence_array) - 1)][0] == 900))) {
        ...
        switch ($mode) {
            case 900:
                { // Text Compaction mode latch
                    ...
                    $txtarr = array(); // array of characters and sub-mode
                                       // switching characters
                    $codelen = strlen($code);
                    for ($i = 0; $i < $codelen; ++ $i) {
                        $chval = ord($code{$i});
                        if (($k = array_search($chval, 
                            $this->textsubmodes[$submode])) !== false) {
                            ...
                            for ($s = 0; $s < 4; ++ $s) {
                                // search new sub-mode
                                if (($s != $submode) and
                                     (($k = array_search($chval, 
                                        $this->textsubmodes[$s])) !== false)) {
                                    // $s is the new submode
                                    if (((($i + 1) == $codelen) or
                                     ((($i + 1) < $codelen) and
                                     (array_search(ord($code{($i + 1)}), 
                                        $this->textsubmodes[$submode]) !== false))) and (($s ==
                                     3) or (($s == 0) and ($submode == 1)))) {
                                    // shift (temporary change only for this
                                    // char)
                                    if ($s == 3) {
