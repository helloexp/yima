...
/**
 * @file Class to create DataMatrix ECC 200 barcode arrays for TCPDF class.
 * DataMatrix (ISO/IEC 16022:2006) is a 2-dimensional bar code.
 *
...
/**
 * @class Datamatrix Class to create DataMatrix ECC 200 barcode arrays for TCPDF
 * class. DataMatrix (ISO/IEC 16022:2006) is a 2-dimensional bar code.
 *
 * @package com.tecnick.tcpdf
 * @author Nicola Asuni
 * @version 1.0.004
...
class Datamatrix {
    ...
    /**
     * Store last used encoding for data codewords. @protected
     */
    protected $last_enc = ENC_ASCII;

    /**
     * Table of Data Matrix ECC 200 Symbol Attributes:<ul> <li>total matrix rows
     * (including finder pattern)</li> <li>total matrix cols (including finder
     * pattern)</li> <li>total matrix rows (without finder pattern)</li>
     * <li>total matrix cols (without finder pattern)</li> <li>region data rows
     * (with finder pattern)</li> <li>region data col (with finder pattern)</li>
     * <li>region data rows (without finder pattern)</li> <li>region data col
     * (without finder pattern)</li> <li>horizontal regions</li> <li>vertical
     * regions</li> <li>regions</li> <li>data codewords</li> <li>error
     * codewords</li> <li>blocks</li> <li>data codewords per block</li>
     * <li>error codewords per block</li> </ul> @protected
     */
    ...
    /**
     * Map encodation modes whit character sets. @protected
     */
    ...
    /**
     * Basic set of characters for each encodation mode. @protected
     */
     ...
     *
     * @param $code (string) Code to represent using Datamatrix. @public
     */
    public function __construct($code) {
        ...
        if ($params[11] < $nd) {
            ...
            if ((($params[11] - $nd) > 1) and ($cw[($nd - 1)] != 254)) {
                if ($this->last_enc == ENC_EDF) {
                    ...
                    $cw[] = 124;
                    ++ $nd;
                } elseif (($this->last_enc != ENC_ASCII) and
        ...
        $cw = $this->getErrorCorrection($cw, $params[13], $params[14], 
            $params[15]);
     ...
     * @param $pp (int) The value of its prime modulus polynomial (301 for
     *            ECC200).
     * @return array data codewords + error codewords @protected
     */
    protected function getErrorCorrection($wd, $nb, $nd, $nc, $gf = 256, $pp = 301) {
        ...
        for ($b = 0; $b < $nb; ++ $b) {
            ...
            for ($i = 0; $i < $nd; ++ $i) {
                $k = ($we[0] ^ $block[$i]);
                for ($j = 0; $j < $nc; ++ $j) {
                    $we[$j] = ($we[($j + 1)] ^ $this->getGFProduct($k, 
                        $c[($nc - $j - 1)], $log, $alog, $gf));
                }
            }
     ...
     * @param $cwpos (int) Number of data codewords from the beginning of
     *            encoded data.
     * @return pad codeword @protected
     */
     ...
     * @return boolean true if the char is of the selected mode. @protected
     */
        ...
        switch ($mode) {
            ...
            case ENC_C40:
                { // Upper-case alphanumeric
                    $status = (($chr == 32) or (($chr >= 48) and ($chr <= 57)) or
                         (($chr >= 65) and ($chr <= 90)));
                    break;
                }
            case ENC_TXT:
            ...
            case ENC_BASE256:
                { // Function character (FNC1, Structured Append, Reader
                  // Program, or Code Page)
                    $status = (($chr == 232) or ($chr == 233) or ($chr == 234) or
                         ($chr == 241));
                    break;
                }
            case ENC_ASCII_EXT:
     ...
     * (Annex P - steps from J to S).
     *
     * @param $data (string) data to encode
     * @param $pos (int) current position
     * @param $mode (int) current encoding mode
     * @return int encoding mode @protected
     */
        ...
        while (true) {
            ...
            if (($pos + $charscount) == $data_length) {
                if ($numch[ENC_ASCII] <= ceil(
                    min($numch[ENC_C40], $numch[ENC_TXT], $numch[ENC_X12], 
                        $numch[ENC_EDF], $numch[ENC_BASE256]))) {
                    return ENC_ASCII;
                }
                if ($numch[ENC_BASE256] < ceil(
                    min($numch[ENC_ASCII], $numch[ENC_C40], $numch[ENC_TXT], 
                        $numch[ENC_X12], $numch[ENC_EDF]))) {
                    return ENC_BASE256;
                }
                if ($numch[ENC_EDF] < ceil(
                    min($numch[ENC_ASCII], $numch[ENC_C40], $numch[ENC_TXT], 
                        $numch[ENC_X12], $numch[ENC_BASE256]))) {
                    return ENC_EDF;
                }
                if ($numch[ENC_TXT] < ceil(
                    min($numch[ENC_ASCII], $numch[ENC_C40], $numch[ENC_X12], 
                        $numch[ENC_EDF], $numch[ENC_BASE256]))) {
                    return ENC_TXT;
                }
            ...
            // STEP O
            if ($this->isCharMode($chr, ENC_X12) or
            ...
            // STEP R
            if ($charscount >= 4) {
                if (($numch[ENC_ASCII] + 1) <= min($numch[ENC_C40], 
                    $numch[ENC_TXT], $numch[ENC_X12], $numch[ENC_EDF], 
                    $numch[ENC_BASE256])) {
                    return ENC_ASCII;
                }
                if ((($numch[ENC_BASE256] + 1) <= $numch[ENC_ASCII]) or (($numch[ENC_BASE256] +
                     1) < min($numch[ENC_C40], $numch[ENC_TXT], $numch[ENC_X12], 
                        $numch[ENC_EDF]))) {
                    return ENC_BASE256;
                }
                if (($numch[ENC_EDF] + 1) < min($numch[ENC_ASCII], 
                    $numch[ENC_C40], $numch[ENC_TXT], $numch[ENC_X12], 
                    $numch[ENC_BASE256])) {
                    return ENC_EDF;
                }
                if (($numch[ENC_TXT] + 1) < min($numch[ENC_ASCII], 
                    $numch[ENC_C40], $numch[ENC_X12], $numch[ENC_EDF], 
                    $numch[ENC_BASE256])) {
                    return ENC_TXT;
                }
                if (($numch[ENC_X12] + 1) < min($numch[ENC_ASCII], 
                    $numch[ENC_C40], $numch[ENC_TXT], $numch[ENC_EDF], 
                    $numch[ENC_BASE256])) {
                    return ENC_X12;
                }
                if (($numch[ENC_C40] + 1) < min($numch[ENC_ASCII], 
                    $numch[ENC_TXT], $numch[ENC_EDF], $numch[ENC_BASE256])) {
                    ...
                    if ($numch[ENC_C40] == $numch[ENC_X12]) {
                        ...
                        while ($k < $data_length) {
                            ...
                            if ($this->isCharMode($tmpchr, ENC_X12)) {
                                return ENC_X12;
                            } elseif (! ($this->isCharMode($tmpchr, ENC_X12) or
     ...
     * @return (int) Switch codeword. @protected
     */
    ...
    /**
     * Choose the minimum matrix size and return the max number of data
     * codewords.
     *
     * @param $numcw (int) Number of current codewords.
     * @return number of data codewords in matrix @protected
     */
    ...
    /**
     * Get high level encoding using the minimum symbol data characters for ECC
     * 200
     *
     * @param $data (string) data to encode
     * @return array of codewords @protected
     */
            ...
            switch ($enc) {
                case ENC_ASCII:
                    { // STEP B. While in ASCII encodation
                        if (($data_length > 1) and ($pos < ($data_length - 1)) and ($this->isCharMode(
                            ord($data[$pos]), ENC_ASCII_NUM) and $this->isCharMode(
                            ord($data[$pos + 1]), ENC_ASCII_NUM))) {
                            // 1. If the next data sequence is at least 2
                            // consecutive digits, encode the next two digits as
                            // a double digit in ASCII mode.
                            ...
                            if ($newenc != $enc) {
                                ...
                                if ($this->isCharMode($chr, ENC_ASCII_EXT)) {
                                    // 3. If the next data character is extended
                                    // ASCII (greater than 127) encode it in
                                    // ASCII mode first using the Upper Shift
                                    // (value 235) character.
                                    $cw[] = 235;
                                    $cw[] = ($chr - 127);
                                    $cw_num += 2;
                                } else {
                                    // 4. Otherwise process the next data
                                    // character in ASCII encodation.
                                    $cw[] = ($chr + 1);
                                    ++ $cw_num;
                                }
                            }
                    ...
                    { // ANSI X12
                        ...
                        do {
                            ...
                            if ($p >= 3) {
                                ...
                                $cw_num += 2;
                                $pos = $epos;
                                // 1. If the C40 encoding is at the point of
                                // starting a new double symbol character and if
                                // the look-ahead test (starting at step J)
                                // indicates another mode, switch to that mode.
                                $newenc = $this->lookAheadTest($data, $pos, 
                                    $enc);
                                if ($newenc != $enc) {
                                    // switch to new encoding
                                    $enc = $newenc;
                        ...
                        if ($p > 0) {
                            ...
                            if (($cwr == 1) and ($p == 1)) {
                                ...
                                // C40 value (data character) remains to be
                                // encoded
                                $c1 = array_shift($temp_cw);
                                -- $p;
                                $cw[] = ($chr + 1);
                                ...
                                // c. If two symbol characters remain and only
                                // one C40 value (data character) remains to be
                                // encoded
                                $c1 = array_shift($temp_cw);
                                -- $p;
                                $cw[] = 254;
                                ...
                                // b. If two symbol characters remain and two
                                // C40 values remain to be encoded
                                $c1 = array_shift($temp_cw);
                                $c2 = array_shift($temp_cw);
                                $p -= 2;
                                $tmp = ((1600 * $c1) + (40 * $c2) + 1);
                ...
                case ENC_EDF:
                    { // F. While in EDIFACT (EDF) encodation
                      // initialize temporary array with 0 length
                        ...
                        $newenc = $enc;
                        do {
                            // 2. process the next character in EDIFACT
                            // encodation.
                            $chr = ord($data[$epos]);
                            if ($this->isCharMode($chr, ENC_EDF)) {
                                ++ $epos;
                            ...
                            if (($field_length == 4) or ($epos == $data_length) or
                                 ! $this->isCharMode($chr, ENC_EDF)) {
                                if (($epos == $data_length) and
                                 ($field_length < 3)) {
                                $enc = ENC_ASCII;
                                $cw[] = $this->getSwitchEncodingCodeword($enc);
                                ++ $cw_num;
                            ...
                            $tcw = (($temp_cw[0] & 0x3F) << 2) +
                                 (($temp_cw[1] & 0x30) >> 4);
                            if ($tcw > 0) {
                                $cw[] = $tcw;
                                $cw_num ++;
                            }
                            $tcw = (($temp_cw[1] & 0x0F) << 4) +
                                 (($temp_cw[2] & 0x3C) >> 2);
                            if ($tcw > 0) {
                                $cw[] = $tcw;
                                $cw_num ++;
                            }
                            $tcw = (($temp_cw[2] & 0x03) << 6) +
                                 ($temp_cw[3] & 0x3F);
                            if ($tcw > 0) {
                                $cw[] = $tcw;
                                $cw_num ++;
                  ...
                  // initialize temporary array with 0 length
                    ...
                    while (($pos < $data_length) and ($field_length <= 1555)) {
                        ...
                        if ($newenc != $enc) {
                            ...
                            // 2. Otherwise, process the next character in Base
                            // 256 encodation.
                            $chr = ord($data[$pos]);
                            ++ $pos;
                            $temp_cw[] = $chr;
                    ...
                    // set field length
                    if ($field_length <= 249) {
                        $cw[] = $this->get255StateCodeword($field_length, 
                            ($cw_num + 1));
                        ++ $cw_num;
                    } else {
                        $cw[] = $this->get255StateCodeword(
                            (floor($field_length / 250) + 249), ($cw_num + 1));
                        $cw[] = $this->get255StateCodeword(
                            ($field_length % 250), ($cw_num + 2));
                        $cw_num += 2;
                    }
                    if (! empty($temp_cw)) {
                        // add B256 field
                        foreach ($temp_cw as $p => $cht) {
                            $cw[] = $this->get255StateCodeword($cht, 
                                ($cw_num + $p + 1));
                        }
                    }
                    break;
...
/**
 * Places "chr+bit" with appropriate wrapping within array[]. (Annex F - ECC 200
...
/**
 * Places the 8 bits of a utah-shaped symbol character. (Annex F - ECC 200
 * symbol character placement)
 *
 * @param $marr (array) Array of symbols.
 * @param $nrow (int) Number of rows.
...
/**
 * Places the 8 bits of the first special corner case. (Annex F - ECC 200 symbol
 * character placement)
 *
 * @param $marr (array) Array of symbols.
 * @param $nrow (int) Number of rows.
...
/**
 * Places the 8 bits of the second special corner case. (Annex F - ECC 200
 * symbol character placement)
 *
 * @param $marr (array) Array of symbols.
 * @param $nrow (int) Number of rows.
...
/**
 * Places the 8 bits of the third special corner case. (Annex F - ECC 200 symbol
 * character placement)
 *
 * @param $marr (array) Array of symbols.
 * @param $nrow (int) Number of rows.
...
/**
 * Places the 8 bits of the fourth special corner case. (Annex F - ECC 200
 * symbol character placement)
 *
 * @param $marr (array) Array of symbols.
 * @param $nrow (int) Number of rows.
...
protected function placeCornerD($marr, $nrow, $ncol, $chr) {
    $marr = $this->placeModule($marr, $nrow, $ncol, $nrow - 1, 0, $chr, 1);
    $marr = $this->placeModule($marr, $nrow, $ncol, $nrow - 1, $ncol - 1, $chr, 
        2);
