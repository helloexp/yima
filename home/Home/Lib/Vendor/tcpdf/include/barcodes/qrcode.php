...
/**
 * @file Class to create QR-code arrays for TCPDF class. QR Code symbol is a 2D
 * barcode that can be scanned by handy terminals such as a mobile phone with
 * CCD. The capacity of QR Code is up to 7000 digits or 4000 characters, and has
 * high robustness. This class supports QR Code model 2, described in JIS
 * (Japanese Industrial Standards) X0510:2004 or ISO/IEC 18004. Currently the
 * following features are not supported: ECI and FNC1 mode, Micro QR Code, QR
 * Code model 1, Structured mode. This class is derived from "PHP QR Code
 * encoder" by Dominik Dzienia (http://phpqrcode.sourceforge.net/) based on
 * "libqrencode C library 3.1.1." by Kentaro Fukuchi
 * (http://megaui.net/fukuchi/works/qrencode/index.en.html), contains
 * Reed-Solomon code written by Phil Karn, KA9Q. QR Code is registered trademark
 * of DENSO WAVE INCORPORATED (http://www.denso-wave.com/qrcode/index-e.html).
...
if (! defined('QRCODEDEFS')) {
    ...
    /**
     * Encoding mode numeric (0-9). 3 characters are encoded to 10bit length. In
     * theory, 7089 characters or less can be stored in a QRcode.
     */
    define('QR_MODE_NM', 0);
    ...
    // QRcode has a function of an error correcting for miss reading that white
    // is black.
    ...
    /**
     * Down point base value for case 4 mask pattern (ration of dark modules in
     * whole)
     */
    define('N4', 10);
...
if (! function_exists('str_split')) {
     ...
     * @return If the optional split_length parameter is specified, the returned
     *         array will be broken down into chunks with each being
     *         split_length in length, otherwise each chunk will be one
     *         character in length. FALSE is returned if split_length is less
     *         than 1. If the split_length length exceeds the length of string ,
     *         the entire string is returned as the first (and only) array
     *         element.
     */
...
/**
 * @class QRcode Class to create QR-code arrays for TCPDF class. QR Code symbol
 * is a 2D barcode that can be scanned by handy terminals such as a mobile phone
 * with CCD. The capacity of QR Code is up to 7000 digits or 4000 characters,
 * and has high robustness. This class supports QR Code model 2, described in
 * JIS (Japanese Industrial Standards) X0510:2004 or ISO/IEC 18004. Currently
 * the following features are not supported: ECI and FNC1 mode, Micro QR Code,
 * QR Code model 1, Structured mode. This class is derived from "PHP QR Code
 * encoder" by Dominik Dzienia (http://phpqrcode.sourceforge.net/) based on
 * "libqrencode C library 3.1.1." by Kentaro Fukuchi
 * (http://megaui.net/fukuchi/works/qrencode/index.en.html), contains
 * Reed-Solomon code written by Phil Karn, KA9Q. QR Code is registered trademark
...
class QRcode {
    ...
    /**
     * Barcode array to be returned which is readable by TCPDF. @protected
     */
     ...
     * version increases. So version 40 is 177*177 matrix. @protected
     */
    protected $version = 0;
    ...
    /**
     * Structured QR code (not supported yet). @protected
     */
    ...
    /**
     * X position of bit. @protected
     */
    protected $x;

    /**
     * Y position of bit. @protected
     */
    protected $y;
    ...
    /**
     * Error correction code. @protected
     */
    ...
    /**
     * Data length. @protected
     */
    protected $dataLength;

    /**
     * Error correction length. @protected
     */
    protected $eccLength;
    ...
    /**
     * Input data string. @protected
     */
    protected $dataStr = '';

    /**
     * Input items. @protected
     */
    protected $items;
    ...
    /**
     * Reed-Solomon items. @protected
     */
    protected $rsitems = array();

    /**
     * Array of frames. @protected
     */
    protected $frames = array();

    /**
     * Alphabet-numeric convesion table. @protected
     */
    ...
    /**
     * Array Table of the capacity of symbols. See Table 1 (pp.13) and Table
     * 12-16 (pp.30-36), JIS X0510:2004. @protected
     */
    ...
    /**
     * Array Length indicator. @protected
     */
    ...
    /**
     * Array Table of the error correction code (Reed-Solomon block). See Table
     * 12-16 (pp.30-36), JIS X0510:2004. @protected
     */
    ...
    /**
     * Array Positions of alignment patterns. This array includes only the
     * second and the third position of the alignment patterns. Rest of them can
     * be calculated from the distance between them. See Table 1 in Appendix E
     * (pp.71) of JIS X0510:2004. @protected
     */
    ...
    /**
     * Array Version information pattern (BCH coded). See Table 1 in Appendix D
     * (pp.68) of JIS X0510:2004. size: [QRSPEC_VERSION_MAX - 6] @protected
     */
    ...
    /**
     * Array Format information @protected
     */
     ...
     * @param $eclevel (string) error level: <ul><li>L : About 7% or less errors
     *            can be corrected.</li><li>M : About 15% or less errors can be
     *            corrected.</li><li>Q : About 25% or less errors can be
     *            corrected.</li><li>H : About 30% or less errors can be
     *            corrected.</li></ul> @public
     * @since 1.0.000
     */
    public function __construct($code, $eclevel = 'L') {
        ...
        $this->level = array_search($eclevel, 
            array(
                'L', 
                'M', 
                'Q', 
     ...
     *
     * @return array barcode array readable by TCPDF; @public
     */
    ...
    protected function init($spec) {
        ...
        for ($i = 0; $i < $endfor; ++ $i) {
            ...
            $this->rsblocks[$blockNo]['data'] = array_slice($this->datacode, 
                $dataPos);
            $this->rsblocks[$blockNo]['eccLength'] = $el;
            $ecc = $this->encode_rs_char($rs, $this->rsblocks[$blockNo]['data'], 
                $ecc);
        ...
        for ($i = 0; $i < $length; ++ $i) {
            ...
            if ($i & 1) {
                if (($i >= 3) and ($i < ($length - 2)) and
                    ...
                    if (($this->runLength[$i - 2] == $fact) and
                         ($this->runLength[$i - 1] == $fact) and
                         ($this->runLength[$i + 1] == $fact) and
                         ($this->runLength[$i + 2] == $fact)) {
                        if (($this->runLength[$i - 3] < 0) or
                         ($this->runLength[$i - 3] >= (4 * $fact))) {
                        $demerit += N3;
                    } elseif ((($i + 3) >= $length) or
                         ($this->runLength[$i + 3] >= (4 * $fact))) {
                        $demerit += N3;
                    }
                }
...
protected function eatNum() {
    ...
    $this->items = $this->appendNewInputItem($this->items, QR_MODE_NM, $run, 
...
protected function eatAn() {
    ...
    $this->items = $this->appendNewInputItem($this->items, QR_MODE_AN, $run, 
        str_split($this->dataStr));
    return $run;
}
...
protected function eatKanji() {
    ...
    $this->items = $this->appendNewInputItem($this->items, QR_MODE_KJ, $p, 
        str_split($this->dataStr));
    return $run;
}
...
protected function eat8() {
    ...
    $run = $p;
    $this->items = $this->appendNewInputItem($this->items, QR_MODE_8B, $run, 
        str_split($this->dataStr));
    return $run;
}
...
protected function encodeModeNum($inputitem, $version) {
    ...
    for ($i = 0; $i < $words; ++ $i) {
        ...
        $inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 10, 
            $val);
    }
...
protected function encodeModeAn($inputitem, $version) {
    ...
    for ($i = 0; $i < $words; ++ $i) {
        ...
        $inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 11, 
            $val);
    }
    if ($inputitem['size'] & 1) {
...
protected function encodeMode8($inputitem, $version) {
    ...
    for ($i = 0; $i < $inputitem['size']; ++ $i) {
        $inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 8, 
...
protected function encodeModeKanji($inputitem, $version) {
    ...
    for ($i = 0; $i < $inputitem['size']; $i += 2) {
        ...
        $h = ($val >> 8) * 0xc0;
        $val = ($val & 0xff) + $h;
        $inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 13, 
            $val);
    }
    return $inputitem;
}
...
protected function encodeModeStructure($inputitem) {
    ...
    $inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 4, 
...
protected function encodeBitStream($inputitem, $version) {
    ...
    if ($inputitem['size'] > $words) {
        ...
        $st2 = $this->newInputItem($inputitem['mode'], 
            $inputitem['size'] - $words, array_slice($inputitem['data'], $words));
        $st1 = $this->encodeBitStream($st1, $version);
        $st2 = $this->encodeBitStream($st2, $version);
        $inputitem['bstream'] = array();
        $inputitem['bstream'] = $this->appendBitstream($inputitem['bstream'], 
            $st1['bstream']);
        $inputitem['bstream'] = $this->appendBitstream($inputitem['bstream'], 
            $st2['bstream']);
    } else {
...
/**
 * Append data to an input object. The data is copied and appended to the input
 * object.
 *
...
protected function checkModeKanji($size, $data) {
    ...
    for ($i = 0; $i < $size; $i += 2) {
        ...
        if (($val < 0x8140) or (($val > 0x9ffc) and ($val < 0xe040)) or
             ($val > 0xebbf)) {
            return false;
        }
    }
...
protected function getMinimumVersion($size, $level) {
    ...
    // the size of input data is greater than QR capacity, try to lover the
 ...
 * @param $version (int) version
 * @param $level (int) error correction level
 * @param $spec (array) an array of ECC specification contains as following: {#
 *            of type1 blocks, # of data code, # of ecc code, # of type2 blocks,
 *            # of data code}
 * @return array spec
 */
...
/**
 * Return BCH encoded version information pattern that is used for the symbol of
 * version 7 or greater. Use lower 18 bits.
 *
 * @param $version (int) version
 ...
 * @return array Array of RS values:<ul><li>mm = Bits per symbol;</li><li>nn =
 *         Symbols per block;</li><li>alpha_to = log lookup table
 *         array;</li><li>index_of = Antilog lookup table array;</li><li>genpoly
 *         = Generator polynomial array;</li><li>nroots = Number of
 *         generator;</li><li>roots = number of parity symbols;</li><li>fcr =
 *         First consecutive root, index form;</li><li>prim = Primitive element,
 *         index form;</li><li>iprim = prim-th root of 1, index
 *         form;</li><li>pad = Padding bytes in shortened
 *         block;</li><li>gfpoly</ul>.
 */
protected function init_rs($symsize, $gfpoly, $fcr, $prim, $nroots, $pad) {
    foreach ($this->rsitems as $rs) {
        if (($rs['pad'] != $pad) or ($rs['nroots'] != $nroots) or
             ($rs['mm'] != $symsize) or ($rs['gfpoly'] != $gfpoly) or
             ($rs['fcr'] != $fcr) or ($rs['prim'] != $prim)) {
            continue;
        }
        return $rs;
    }
...
protected function encode_rs_char($rs, $data, $parity) {
    ...
    $ALPHA_TO = & $rs['alpha_to']; // the address of an array of NN elements to
                                   // convert Galois field elements in index
                                   // (log) form to polynomial form
    $INDEX_OF = & $rs['index_of']; // the address of an array of NN elements to
                                   // convert Galois field elements in
                                   // polynomial form to index (log) form
    $GENPOLY = & $rs['genpoly']; // an array of NROOTS+1 elements containing the
                                 // generator polynomial in index form
    $NROOTS = & $rs['nroots']; // the number of roots in the RS code generator
                               // polynomial, which is the same as the number of
                               // parity symbols in a block
    ...
    for ($i = 0; $i < ($NN - $NROOTS - $PAD); $i ++) {
        ...
        if ($feedback != $A0) {
            ...
            for ($j = 1; $j < $NROOTS; ++ $j) {
                $parity[$j] ^= $ALPHA_TO[$this->modnn($rs, 
