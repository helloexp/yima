...
/**
 * @file This is a command line script to generate TCPDF fonts.<br>
 *
...
foreach ($options['fonts'] as $font) {
    ...
    $fontname = TCPDF_FONTS::addTTFfont($fontfile, $options['type'], 
        $options['enc'], $options['flags'], $options['outpath'], 
        $options['platid'], $options['encid'], $options['addcbbox'], 
        $options['link']);
    if ($fontname === false) {
        $errors = true;
