...
/**
 * @file This is a PHP class for parsing PDF documents.<br>
 *
...
/**
 * @class TCPDF_PARSER This is a PHP class for parsing PDF documents.<br>
 *
 * @package com.tecnick.tcpdf @brief This is a PHP class for parsing PDF
 *          documents..
 * @version 1.0.15
...
class TCPDF_PARSER {
    ...
    /**
     * Raw content of the PDF document. @private
     */
    private $pdfdata = '';
    ...
    /**
     * Array of PDF objects. @protected
     */
    protected $objects = array();

    /**
     * Class object for decoding filters. @private
     */
    private $FilterDecoders;

    /**
     * Array of configuration parameters. @private
     */
    private $cfg = array(
     ...
     * @param $cfg (array) Array of configuration parameters: 'die_for_errors' :
     *            if true termitate the program execution in case of error,
     *            otherwise thows an exception; 'ignore_filter_decoding_errors'
     *            : if true ignore filter decoding errors;
     *            'ignore_missing_filter_decoders' : if true ignore missing
     *            filter decoding errors. @public
    ...
    public function __construct($data, $cfg = array()) {
        ...
        foreach ($this->xref['xref'] as $obj => $offset) {
            if (! isset($this->objects[$obj]) and ($offset > 0)) {
                ...
                $this->objects[$obj] = $this->getIndirectObject($obj, $offset, 
                    true);
            }
        }
        // release some memory
     ...
     *
     * @return (array) Array of parsed PDF document objects. @public
     ...
     * @return Array containing xref and trailer data. @protected
     * @since 1.0.000 (2011-05-24)
     */
    protected function getXrefData($offset = 0, $xref = array()) {
        if ($offset == 0) {
            ...
            if (preg_match_all(
                '/[\r\n]startxref[\s]*[\r\n]+([0-9]+)[\s]*[\r\n]+%%EOF/i', 
                $this->pdfdata, $matches, PREG_SET_ORDER, $offset) == 0) {
            ...
            $startxref = $offset;
        } elseif (preg_match('/([0-9]+[\s][0-9]+[\s]obj)/i', $this->pdfdata, 
     ...
     *
     * @param $startxref (int) Offset at which the xref section starts (position
     *            of the 'xref' keyword).
     * @param $xref (array) Previous xref array (if any).
     * @return Array containing xref and trailer data. @protected
    ...
    protected function decodeXref($startxref, $xref = array()) {
        $startxref += 4; // 4 is the length of the word 'xref'
                         // skip initial white space chars: \x00 null (NUL),
                         // \x09 horizontal tab (HT), \x0A line feed (LF), \x0C
                         // form feed (FF), \x0D carriage return (CR), \x20
                         // space (SP)
        $offset = $startxref +
        ...
        while (preg_match(
            ...
            if ($matches[3][0] == 'n') {
                // create unique object index: [object number]_[generation
                // number]
        ...
        // get trailer data
        if (preg_match('/trailer[\s]*<<(.*)>>/isU', $this->pdfdata, $matches, 
            ...
            if (! isset($xref['trailer']) or empty($xref['trailer'])) {
                ...
                $xref['trailer'] = array();
                // parse trailer_data
                if (preg_match('/Size[\s]+([0-9]+)/i', $trailer_data, $matches) >
                     0) {
                    $xref['trailer']['size'] = intval($matches[1]);
                }
                if (preg_match('/Root[\s]+([0-9]+)[\s]+([0-9]+)[\s]+R/i', 
                    $trailer_data, $matches) > 0) {
                    $xref['trailer']['root'] = intval($matches[1]) . '_' .
                         intval($matches[2]);
                }
                if (preg_match('/Encrypt[\s]+([0-9]+)[\s]+([0-9]+)[\s]+R/i', 
                    $trailer_data, $matches) > 0) {
                    $xref['trailer']['encrypt'] = intval($matches[1]) . '_' .
                         intval($matches[2]);
                }
                if (preg_match('/Info[\s]+([0-9]+)[\s]+([0-9]+)[\s]+R/i', 
                    $trailer_data, $matches) > 0) {
                    $xref['trailer']['info'] = intval($matches[1]) . '_' .
                         intval($matches[2]);
                }
                if (preg_match(
                    '/ID[\s]*[\[][\s]*[<]([^>]*)[>][\s]*[<]([^>]*)[>]/i', 
    ...
    protected function decodeXrefStream($startxref, $xref = array()) {
        ...
        $sarr = $xrefcrs[0][1];
        foreach ($sarr as $k => $v) {
            if (($v[0] == '/') and ($v[1] == 'Type') and (isset($sarr[($k + 1)]) and
                 ($sarr[($k + 1)][0] == '/') and ($sarr[($k + 1)][1] == 'XRef'))) {
                $valid_crs = true;
            } elseif (($v[0] == '/') and ($v[1] == 'Index') and
                 (isset($sarr[($k + 1)]))) {
                // first object number in the subsection
                $index_first = intval($sarr[($k + 1)][1][0][1]);
                // number of entries in the subsection
                $index_entries = intval($sarr[($k + 1)][1][1][1]);
            } elseif (($v[0] == '/') and ($v[1] == 'Prev') and
                 (isset($sarr[($k + 1)]) and ($sarr[($k + 1)][0] == 'numeric'))) {
                // get previous xref offset
                $prevxref = intval($sarr[($k + 1)][1]);
            } elseif (($v[0] == '/') and ($v[1] == 'W') and
                 (isset($sarr[($k + 1)]))) {
                // number of bytes (in the decoded stream) of the corresponding
                // field
                $wb = array();
                $wb[0] = intval($sarr[($k + 1)][1][0][1]);
                $wb[1] = intval($sarr[($k + 1)][1][1][1]);
                $wb[2] = intval($sarr[($k + 1)][1][2][1]);
            } elseif (($v[0] == '/') and ($v[1] == 'DecodeParms') and
                 (isset($sarr[($k + 1)][1]))) {
                $decpar = $sarr[($k + 1)][1];
                foreach ($decpar as $kdc => $vdc) {
                    if (($vdc[0] == '/') and ($vdc[1] == 'Columns') and (isset(
                        $decpar[($kdc + 1)]) and
                         ($decpar[($kdc + 1)][0] == 'numeric'))) {
                        $columns = intval($decpar[($kdc + 1)][1]);
                    } elseif (($vdc[0] == '/') and ($vdc[1] == 'Predictor') and (isset(
                        $decpar[($kdc + 1)]) and
                         ($decpar[($kdc + 1)][0] == 'numeric'))) {
                ...
                if (($v[0] == '/') and ($v[1] == 'Size') and (isset(
                    $sarr[($k + 1)]) and ($sarr[($k + 1)][0] == 'numeric'))) {
                    $xref['trailer']['size'] = $sarr[($k + 1)][1];
                } elseif (($v[0] == '/') and ($v[1] == 'Root') and
                     (isset($sarr[($k + 1)]) and ($sarr[($k + 1)][0] == 'objref'))) {
                    $xref['trailer']['root'] = $sarr[($k + 1)][1];
                } elseif (($v[0] == '/') and ($v[1] == 'Info') and
                     (isset($sarr[($k + 1)]) and ($sarr[($k + 1)][0] == 'objref'))) {
                    $xref['trailer']['info'] = $sarr[($k + 1)][1];
                } elseif (($v[0] == '/') and ($v[1] == 'Encrypt') and
                     (isset($sarr[($k + 1)]) and ($sarr[($k + 1)][0] == 'objref'))) {
                    $xref['trailer']['encrypt'] = $sarr[($k + 1)][1];
                } elseif (($v[0] == '/') and ($v[1] == 'ID') and
                     (isset($sarr[($k + 1)]))) {
                    $xref['trailer']['id'] = array();
        ...
        if ($valid_crs and isset($xrefcrs[1][3][0])) {
            ...
            foreach ($sdata as $k => $row) {
                ...
                for ($i = 1; $i <= $columns; ++ $i) {
                    ...
                    switch ($predictor) {
                        case 10:
                            { // PNG prediction (on encoding, PNG None on all
                              // rows)
                                $ddata[$k][$j] = $row[$i];
                                break;
                            }
                        case 11:
                            { // PNG prediction (on encoding, PNG Sub on all
                              // rows)
                                $ddata[$k][$j] = (($row[$i] + $row_left) & 0xff);
                                break;
                            }
                        case 12:
                            { // PNG prediction (on encoding, PNG Up on all
                              // rows)
                                $ddata[$k][$j] = (($row[$i] + $row_up) & 0xff);
                                break;
                            }
                        case 13:
                            { // PNG prediction (on encoding, PNG Average on all
                              // rows)
                                $ddata[$k][$j] = (($row[$i] +
                                     (($row_left + $row_up) / 2)) & 0xff);
                                break;
                            }
                        case 14:
                            { // PNG prediction (on encoding, PNG Paeth on all
                              // rows)
                              // initial estimate
                ...
                switch ($row[0]) {
                    ...
                    case 1:
                        { // (n) objects that are in use but are not compressed
                          // create unique object index: [object
                          // number]_[generation number]
                    ...
                    case 2:
                        { // compressed objects
                          // $row[1] = object number of the object stream in
                          // which this object is stored
                          // $row[2] = index of this object within the object
                          // stream
    ...
    protected function getRawObject($offset = 0) {
        ...
        $objval = ''; // object value to be returned
                      // skip initial white space chars: \x00 null (NUL), \x09
                      // horizontal tab (HT), \x0A line feed (LF), \x0C form
                      // feed (FF), \x0D carriage return (CR), \x20 space (SP)
        ...
        switch ($char) {
                ...
                { // \x29 RIGHT PARENTHESIS
                  // literal string object
                    ...
                    if ($char == '(') {
                        ...
                        $objval = substr($this->pdfdata, $offset, 
                            ($strpos - $offset - 1));
                        $offset = $strpos;
                    }
                    break;
            ...
            case '>':
                { // \x3E GREATER-THAN SIGN
                    if (isset($this->pdfdata{($offset + 1)}) and
                        ...
                        if (($char == '<') and
                             (preg_match(
                                ...
                                substr($this->pdfdata, $offset), $matches) == 1)) {
                            // remove white space characters
                            $objval = strtr($matches[1], "\x09\x0a\x0c\x0d\x20", 
                                '');
                            $offset += strlen($matches[0]);
                        } elseif (($endpos = strpos($this->pdfdata, '>', 
                            $offset)) !== FALSE) {
                            $offset = $endpos + 1;
                        }
                    }
                    ...
                    if (substr($this->pdfdata, $offset, 6) == 'endobj') {
                        ...
                        if (preg_match('/^([\r]?[\n])/isU', 
                            ...
                            if (preg_match(
                                ...
                                substr($this->pdfdata, $offset), $matches, 
                        ...
                        $objtype = 'endstream';
                        $offset += 9;
                    } elseif (preg_match('/^([0-9]+)[\s]+([0-9]+)[\s]+R/iU', 
                        substr($this->pdfdata, $offset, 33), $matches) == 1) {
                        // indirect object reference
                        $objtype = 'objref';
                        $offset += strlen($matches[0]);
                        $objval = intval($matches[1]) . '_' . intval(
                            $matches[2]);
                    } elseif (preg_match('/^([0-9]+)[\s]+([0-9]+)[\s]+obj/iU', 
                        substr($this->pdfdata, $offset, 33), $matches) == 1) {
                        // object start
                        $objtype = 'obj';
                        $objval = intval($matches[1]) . '_' . intval(
                            $matches[2]);
                        $offset += strlen($matches[0]);
                    } elseif (($numlen = strspn($this->pdfdata, '+-.0123456789', 
                        $offset)) > 0) {
                        // numeric object
                        $objtype = 'numeric';
     ...
     *
     * @param $obj_ref (string) Object number and generation number separated by
     *            underscore character.
     * @param $offset (int) Object offset.
     * @param $decoding (boolean) If true decode streams.
     * @return array containing object data. @protected
     * @since 1.0.000 (2011-05-24)
     */
    protected function getIndirectObject($obj_ref, $offset = 0, $decoding = true) {
        ...
        if (strpos($this->pdfdata, $objref, $offset) != $offset) {
            // an indirect reference to an undefined object shall be considered
        ...
        do {
            ...
            if ($decoding and ($element[0] == 'stream') and
                 (isset($objdata[($i - 1)][0])) and
                 ($objdata[($i - 1)][0] == '<<')) {
                $element[3] = $this->decodeStream($objdata[($i - 1)][1], 
                    $element[1]);
            }
            $objdata[$i] = $element;
            ++ $i;
    ...
    protected function getObjectVal($obj) {
        if ($obj[0] == 'objref') {
            ...
            if (isset($this->objects[$obj[1]])) {
                ...
                // parse new object
                $this->objects[$obj[1]] = $this->getIndirectObject($obj[1], 
                    $this->xref[$obj[1]], false);
                return $this->objects[$obj[1]];
            }
        }
        return $obj;
    ...
    protected function decodeStream($sdic, $stream) {
        ...
        foreach ($sdic as $k => $v) {
            if ($v[0] == '/') {
                if (($v[1] == 'Length') and (isset($sdic[($k + 1)])) and
                     ($sdic[($k + 1)][0] == 'numeric')) {
        ...
        foreach ($filters as $filter) {
            if (in_array($filter, TCPDF_FILTERS::getAvailableFilters())) {
                try {
                    ...
                    $emsg = $e->getMessage();
                    if ((($emsg[0] == '~') and
                         ! $this->cfg['ignore_missing_filter_decoders']) or (($emsg[0] !=
                         '~') and ! $this->cfg['ignore_filter_decoding_errors'])) {
