 ...
 * 修改日期：2010-11-23
 */
require_cache(dirname(__FILE__) . "/alipay_function.php");

class alipay_royalty {
    ...
    /**
     * 构造函数 从配置文件及入口文件中初始化变量 $parameter 需要签名的参数数组 $key 安全校验码 $sign_type 签名类型
     */
    function alipay_royalty($parameter, $key, $sign_type) {
        ...
        $this->mysign = build_mysign($this->parameter, $this->_key, 
            $this->sign_type);
    }
    ...
    /**
     * 构造表单提交HTML return 表单提交HTML文本
     */
    function build_form() {
        // GET方式传递
        $sHtml = "<form id='alipaysubmit' name='alipaysubmit' action='" .
             $this->gateway . "_input_charset=" .
             $this->parameter['_input_charset'] . "' method='get'>";
        // POST方式传递（GET与POST二必选一）
        // $sHtml = "<form id='alipaysubmit' name='alipaysubmit'
        // action='".$this->gateway."_input_charset=".$this->parameter['_input_charset']."'
        // method='post'>";
        ...
        $sHtml = $sHtml . "<input type='hidden' name='sign' value='" .
             $this->mysign . "'/>";
        $sHtml = $sHtml . "<input type='hidden' name='sign_type' value='" .
             $this->sign_type . "'/>";
    ...
    /**
     * 把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串 $array 需要拼接的数组 return 拼接完成以后的字符串
     */
    ...
    function create_url() {
        ...
        $url .= $arg . "&sign=" . $this->mysign . "&sign_type=" .
             $this->sign_type;
        return $url;
    }
