<?php
/*
 * 类名：alipay_notify 功能：付款过程中服务器通知类 详细：该页面是通知返回核心处理文件，不需要修改 版本：2.0 日期：2011-09-01
 * 说明：
 */
...
class alipay_notify {
    ...
    /**
     * 构造函数 从配置文件中初始化变量 $partner 合作身份者ID $key 安全校验码 $sign_type 签名类型
    ...
    /**
     * 对notify_url的认证 返回的验证结果：true/false
     */
    function notify_verify() {
        ...
        if (empty($_POST)) {
            ...
            $this->mysign = build_mysign($notifyarray, $this->_key, 
                $this->sign_type);
    ...
    /**
     * 对return_url的认证 return 验证结果：true/false
     */
    function return_verify() {
        ...
        if (empty($_GET)) {
            ...
            $this->mysign = build_mysign($sort_get, $this->_key, 
                $this->sign_type); // 生成签名结果
