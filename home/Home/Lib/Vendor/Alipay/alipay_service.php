<?php
/**
 * 类名：alipay_service 功能：支付宝Wap服务接口控制 详细：该页面是请求参数核心处理文件，不需要修改 版本：2.0
 * 日期：2011-09-01 '说明：
 */
...
class alipay_service {
    ...
    function mobile_merchant_paychannel($parameter, $key, $sign_type) {
        ...
        $this->req_data = create_linkstring($this->parameter) . '&sign=' .
             urlencode($this->mysign) . '&sign_type=' . $this->sign_type; // 配置post请求数据，注意sign签名需要urlencode
    ...
    function alipay_wap_trade_create_direct($parameter, $key, $sign_type) {
        ...
        $this->mysign = build_mysign($sort_array, $this->_key, $this->sign_type); // 生成签名
        $this->req_data = create_linkstring($this->parameter) . '&sign=' .
             urlencode($this->mysign); // 配置post请求数据，注意sign签名需要urlencode
                                      // Post提交请求
    ...
    function alipay_Wap_Auth_AuthAndExecute($parameter, $key) {
        ...
        $RedirectUrl = $this->gateway . create_linkstring($this->parameter) .
             '&sign=' . urlencode($this->mysign);
    ...
    /**
     * 返回token参数 参数 result 需要先urldecode
     */
...
{
            return getDataForXML($myArray['res_data'], 
                '/direct_trade_create_res/request_token'); // 返回token
        } else {
    ...
    /**
     * PHP Crul库 模拟Post提交至支付宝网关 如果使用Crul
     * 你需要改一改你的php.ini文件的设置，找到php_curl.dll去掉前面的";"就行了 返回 $data
     */
