...
class PHPExcel {
    ...
    /**
     * _macrosCode : all macros code (the vbaProject.bin file, this include
     * form, code, etc.), NULL if no macro
     *
     * @var binary
     */
    ...
    /**
     * _macrosCertificate : if macros are signed, contains
    ...
    /**
     * _ribbonXMLData : NULL if workbook is'nt Excel 2007 or not contain a
     * customized UI
     *
     * @var NULL|string
     */
    ...
    /**
     * _ribbonBinObjects : NULL if workbook is'nt Excel 2007 or not contain
     * embedded objects (picture(s)) for Ribbon Elements ignored if
     * $_ribbonXMLData is null
     *
     * @var NULL|array
     */
    ...
    /**
     * return the extension of a filename. Internal use for a array_map callback
     * (php<5.3 don't like lambda function)
     */
        ...
        switch ($What) {
            ...
            case 'types':
                if (is_array($this->_ribbonBinObjects) &&
                    ...
                    $ReturnData = array_unique(
                        array_map(
                            array(
                                $this, 
                                '_getExtensionOnly'), $tmpTypes));
                } else
                    $ReturnData = array(); // the caller want an array... not
                                               // null if empty
                break;
        }
        return $ReturnData;
    ...
    /**
     * Disconnect all worksheets from this PHPExcel workbook object, typically
     * so that the PHPExcel object can be unset
     */
    ...
    public function addExternalSheet(PHPExcel_Worksheet $pSheet, 
        ...
        // count how many cellXfs there are in this workbook currently, we will
        // need this below
...
public function garbageCollect() {
    ...
    // remove cellXfs without references and create mapping so we can update
    // xfIndex
    // for all cells and columns
    $countNeededCellXfs = 0;
    foreach ($this->_cellXfCollection as $index => $cellXf) {
        if ($countReferencesCellXf[$index] > 0 || $index == 0) { // we must
                                                                 // never remove
                                                                 // the first
                                                                 // cellXf
            ++ $countNeededCellXfs;
