 ...
 *            (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Style_NumberFormat extends PHPExcel_Style_Supervisor implements 
    ...
    private static $_dateFormatReplacements = array(
        ...
        // so we try to identify times be the inclusion of a : separator in the
        // mask
    ...
    /**
     * Search/replace values to convert Excel date/time format masks hours to
     * PHP format masks (24 hr clock)
     *
     * @var array
     */
    ...
    private static function _formatAsDate(&$value, &$format) {
        ...
        // OpenOffice.org uses upper-case number formats, e.g. 'YYYY', convert
        // to lower-case
    ...
    private static function _complexNumberFormatMask($number, $mask) {
        if (strpos($mask, '.') !== false) {
            ...
            $result2 = strrev(
                self::_complexNumberFormatMask(strrev($numbers[1]), 
                    strrev($masks[1])));
     ...
     */
    public static function toFormattedString($value = '0', 
        $format = PHPExcel_Style_NumberFormat::FORMAT_GENERAL, $callBack = null) {
        // For now we do not treat strings although section 4 of a format code
        // affects strings
        if (! is_numeric($value))
            return $value;
            
            // For 'General' format code, we just pass the value although this
            // is not entirely the way Excel does it,
        ...
        // Fetch the relevant section depending on whether number is positive,
        // negative, or zero?
        ...
        // formatted string
        if (preg_match('/^(\[\$[A-Z]*-[0-9A-F]*\])*[hmsdy]/i', $format)) { // datetime
                                                                           // format
            ...
            if ($format === self::FORMAT_CURRENCY_EUR_SIMPLE) {
                ...
                // In Excel formats, "_" is used to add spacing, which we can't
                // do in HTML
                $format = preg_replace('/_./', '', $format);
                
                // Some non-number characters are escaped with \, which we don't
                // need
                ...
                if (preg_match('/#?.*\?\/\?/', $format, $m)) {
                    ...
                    if (preg_match($number_regex, $m, $matches)) {
                        ...
                        $minWidth = strlen($left) + strlen($dec) + strlen(
                            $right);
                        if ($useThousands) {
