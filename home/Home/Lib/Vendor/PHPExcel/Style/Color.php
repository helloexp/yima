 ...
 *            (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Style_Color extends PHPExcel_Style_Supervisor implements 
    ...
    public function getStyleArray($array) {
        ...
        return $this->_parent->getStyleArray(
            array(
                $key => $array));
    }
     ...
     * $objPHPExcel->getActiveSheet()->getStyle('B2')->getFont()->getColor()->applyFromArray(
     * array('rgb' => '808080') ); </code>
     *
     ...
     * @param boolean $hex Flag indicating whether the component should be
     *            returned as a hex or a decimal value
     ...
     *
     * @param string $hex The colour as an RGBA or RGB value (e.g. FF00CCCC or
     *            CCDDEE)
     * @param float $adjustPercentage The percentage by which to adjust the
     *            colour as a float from -1 to 1
     * @return string The adjusted colour as an RGBA or RGB value (e.g. FF00CCCC
     *         or CCDDEE)
     */
    public static function changeBrightness($hex, $adjustPercentage) {
        ...
        $rgb = strtoupper(
            str_pad(dechex($red), 2, '0', 0) . str_pad(dechex($green), 2, '0', 
                0) . str_pad(dechex($blue), 2, '0', 0));
        return (($rgba) ? 'FF' : '') . $rgb;
    }
     ...
     * @param boolean $background Flag to indicate whether default background or
     *            foreground colour should be returned if the indexed colour
     *            doesn't exist
     * @return PHPExcel_Style_Color
     */
