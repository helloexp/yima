 ...
 *            (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Style extends PHPExcel_Style_Supervisor implements 
     ...
     *
     * @param boolean $isSupervisor Flag indicating if this is a supervisor or
     *            not Leave this value at default unless you understand exactly
     *            what its ramifications are
     * @param boolean $isConditional Flag indicating if this is a conditional
     *            style or not Leave this value at default unless you understand
    ...
    /**
     * Apply styles from array <code>
     * $objPHPExcel->getActiveSheet()->getStyle('B2')->applyFromArray( array(
     * 'font' => array( 'name' => 'Arial', 'bold' => true, 'italic' => false,
     * 'underline' => PHPExcel_Style_Font::UNDERLINE_DOUBLE, 'strike' => false,
     * 'color' => array( 'rgb' => '808080' ) ), 'borders' => array( 'bottom' =>
     * array( 'style' => PHPExcel_Style_Border::BORDER_DASHDOT, 'color' =>
     * array( 'rgb' => '808080' ) ), 'top' => array( 'style' =>
     * PHPExcel_Style_Border::BORDER_DASHDOT, 'color' => array( 'rgb' =>
     * '808080' ) ) ), 'quotePrefix' => true ) ); </code>
     *
    ...
    public function applyFromArray($pStyles = null, $pAdvanced = true) {
        if (is_array($pStyles)) {
            if ($this->_isSupervisor) {
                ...
                $rangeStart[0] = PHPExcel_Cell::columnIndexFromString(
                    $rangeStart[0]) - 1;
                $rangeEnd[0] = PHPExcel_Cell::columnIndexFromString(
                    $rangeEnd[0]) - 1;
                ...
                if ($pAdvanced && isset($pStyles['borders'])) {
                    ...
                    // 'allborders' is a shorthand property for 'outline' and
                    // 'inside' and
                    ...
                    // 'outline' is a shorthand property for 'top', 'right',
                    // 'bottom', 'left'
                    // it applies to components that have not been set
                    // explicitly
                    if (isset($pStyles['borders']['outline'])) {
                        ...
                        unset($pStyles['borders']['outline']); // not needed any
                                                                   // more
                    }
                    
                    // 'inside' is a shorthand property for 'vertical' and
                    // 'horizontal'
                    // it applies to components that have not been set
                    // explicitly
                    ...
                    // width and height characteristics of selection, 1, 2, or 3
                    // (for 3 or more)
                    ...
                    for ($x = 1; $x <= $xMax; ++ $x) {
                        ...
                        $colStart = ($x == 3) ? PHPExcel_Cell::stringFromColumnIndex(
                            $rangeEnd[0]) : PHPExcel_Cell::stringFromColumnIndex(
                            $rangeStart[0] + $x - 1);
                        
                        // end column index for region
                        $colEnd = ($x == 1) ? PHPExcel_Cell::stringFromColumnIndex(
                            $rangeStart[0]) : PHPExcel_Cell::stringFromColumnIndex(
                            $rangeEnd[0] - $xMax + $x);
                        ...
                        for ($y = 1; $y <= $yMax; ++ $y) {
                            ...
                            $rowStart = ($y == 3) ? $rangeEnd[1] : $rangeStart[1] +
                                 $y - 1;
                            
                            // end row index for region
                            $rowEnd = ($y == 1) ? $rangeStart[1] : $rangeEnd[1] -
                                 $yMax + $y;
                            ...
                            // should take the 'inside' border properties if
                            // they have been set
                            foreach ($innerEdges as $innerEdge) {
                                switch ($innerEdge) {
                                    case 'top':
                                    case 'bottom':
                                        // should pick up 'horizontal' border
                                        // property if set
                                        if (isset(
                                    ...
                                    case 'left':
                                    case 'right':
                                        // should pick up 'vertical' border
                                        // property if set
                                        if (isset(
                    ...
                    case 'ROW':
                        ...
                        for ($row = $rangeStart[1]; $row <= $rangeEnd[1]; ++ $row) {
                            if ($this->getActiveSheet()
                                ->getRowDimension($row)
                                ->getXfIndex() == null) {
                                $oldXfIndexes[0] = true; // row without explicit
                                                             // style should be
                                                             // formatted based on
                                                             // default style
                            } else {
                ...
                // clone each of the affected styles, apply the style array, and
                // add the new styles to the workbook
                ...
                foreach ($oldXfIndexes as $oldXfIndex => $dummy) {
                    ...
                    if ($existingStyle = $workbook->getCellXfByHashCode(
                        $newStyle->getHashCode())) {
                ...
                switch ($selectionType) {
                    case 'COLUMN':
                        for ($col = $rangeStart[0]; $col <= $rangeEnd[0]; ++ $col) {
                            $columnDimension = $this->getActiveSheet()->getColumnDimensionByColumn(
                                $col);
                            $oldXfIndex = $columnDimension->getXfIndex();
                            $columnDimension->setXfIndex(
                    ...
                    case 'ROW':
                        for ($row = $rangeStart[1]; $row <= $rangeEnd[1]; ++ $row) {
                            $rowDimension = $this->getActiveSheet()->getRowDimension(
                                $row);
                            $oldXfIndex = $rowDimension->getXfIndex() === null ? 0 : $rowDimension->getXfIndex(); // row
                                                                                                                  // without
                                                                                                                  // explicit
                                                                                                                  // style
                                                                                                                  // should
                    ...
                    case 'CELL':
                        for ($col = $rangeStart[0]; $col <= $rangeEnd[0]; ++ $col) {
                            for ($row = $rangeStart[1]; $row <= $rangeEnd[1]; ++ $row) {
                                $cell = $this->getActiveSheet()->getCellByColumnAndRow(
                                    $col, $row);
                ...
                if (array_key_exists('numberformat', $pStyles)) {
                    $this->getNumberFormat()->applyFromArray(
                        $pStyles['numberformat']);
                }
                if (array_key_exists('protection', $pStyles)) {
                    $this->getProtection()->applyFromArray(
                        $pStyles['protection']);
                }
    ...
    public function getConditionalStyles() {
        return $this->getActiveSheet()->getConditionalStyles(
            $this->getActiveCell());
    }
    ...
    public function setConditionalStyles($pValue = null) {
        if (is_array($pValue)) {
            $this->getActiveSheet()->setConditionalStyles(
                $this->getSelectedCells(), $pValue);
        }
        return $this;
    }
    ...
    public function getHashCode() {
        ...
        return md5(
            $this->_fill->getHashCode() . $this->_font->getHashCode() .
                 $this->_borders->getHashCode() .
                 $this->_alignment->getHashCode() .
                 $this->_numberFormat->getHashCode() . $hashConditionals .
                 $this->_protection->getHashCode() .
                 ($this->_quotePrefix ? 't' : 'f') . __CLASS__);
    }
