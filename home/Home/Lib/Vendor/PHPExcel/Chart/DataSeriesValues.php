...
class PHPExcel_Chart_DataSeriesValues {
    ...
    public function __construct($dataType = self::DATASERIES_TYPE_NUMBER, $dataSource = null, $formatCode = null, 
        $pointCount = 0, $dataValues = array(), $marker = null) {
     ...
     *
     * @param string $dataType Datatype of this data series Typical values are:
    ...
    public function setDataType($dataType = self::DATASERIES_TYPE_NUMBER) {
        if (! in_array($dataType, self::$_dataTypeValues)) {
            throw new PHPExcel_Chart_Exception(
     ...
     * @param boolean $refreshDataSource TRUE - refresh the value of _dataSource
     *            based on the values of $dataValues FALSE - don't change the
     *            value of _dataSource
     * @return PHPExcel_Chart_DataSeriesValues
     */
    public function setDataValues($dataValues = array(), $refreshDataSource = TRUE) {
        $this->_dataValues = PHPExcel_Calculation_Functions::flattenArray(
            $dataValues);
    ...
    public function refresh(PHPExcel_Worksheet $worksheet, $flatten = TRUE) {
        if ($this->_dataSource !== NULL) {
            ...
            $newDataValues = PHPExcel_Calculation::_unwrapResult(
                $calcEngine->_calculateFormulaValue('=' . $this->_dataSource, 
                    NULL, $worksheet->getCell('A1')));
            if ($flatten) {
                $this->_dataValues = PHPExcel_Calculation_Functions::flattenArray(
                    $newDataValues);
                ...
                $dimensions = PHPExcel_Cell::rangeDimension(
                    str_replace('$', '', $cellRange));
