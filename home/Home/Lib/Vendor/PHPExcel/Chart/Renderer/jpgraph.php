...
class PHPExcel_Chart_Renderer_jpgraph {
    ...
    private function _formatPointMarker($seriesPlot, $markerID) {
        ...
        if (is_null($markerID)) {
            ...
            $seriesPlot->mark->SetType(
                self::$_markSet[$plotMarkKeys[self::$_plotMark ++]]);
        } elseif ($markerID !== 'none') {
            // Use specified plot marker (if it exists)
            if (isset(self::$_markSet[$markerID])) {
                $seriesPlot->mark->SetType(self::$_markSet[$markerID]);
            } else {
                // If the specified plot marker doesn't exist, use default plot
                // marker (next marker in the series)
                self::$_plotMark %= count(self::$_markSet);
                $seriesPlot->mark->SetType(
                    self::$_markSet[$plotMarkKeys[self::$_plotMark ++]]);
            }
    ...
    private function _formatDataSetLabels($groupID, $datasetLabels, $labelCount, 
        $rotation = '') {
            ...
            if (is_array($datasetLabel)) {
                ...
                if (! is_null($datasetLabelFormatCode)) {
                    $datasetLabels[$i] = PHPExcel_Style_NumberFormat::toFormattedString(
                        $datasetLabel, $datasetLabelFormatCode);
                }
            }
            ++ $testCurrentIndex;
        }
    ...
    private function _percentageSumCalculation($groupID, $seriesCount) {
        // Adjust our values to a percentage value across all series in the
        // group
    ...
    private function _renderPlotLine($groupID, $filled = false, $combination = false, 
        $dimensions = '2d') {
        ...
        for ($i = 0; $i < $seriesCount; ++ $i) {
            ...
            if ($filled) {
                ...
                $seriesPlot->SetFillColor(
                    self::$_colourSet[self::$_plotColour ++]);
            } else {
    ...
    private function _renderPlotScatter($groupID, $bubble) {
        ...
        for ($i = 0; $i < $seriesCount; ++ $i) {
            ...
            if ($scatterStyle == 'lineMarker') {
                ...
                $seriesPlot->link->SetColor(
                    self::$_colourSet[self::$_plotColour]);
            } elseif ($scatterStyle == 'smoothMarker') {
                $spline = new Spline($dataValuesY, $dataValuesX);
                list ($splineDataY, $splineDataX) = $spline->Get(
                    count($dataValuesX) * self::$_width / 20);
    ...
    private function _renderPieChart($groupCount, $dimensions = '2d', 
        $doughnut = False, $multiplePlots = False) {
        ...
        for ($groupID = 0; $groupID < $iLimit; ++ $groupID) {
            ...
            $seriesCount = $this->_chart->getPlotArea()
                ...
                ->getPlotSeriesCount();
            $seriesPlots = array();
            // For pie charts, we only display the first series: doughnut charts
            // generally display all series
            ...
            for ($j = 0; $j < $jLimit; ++ $j) {
                ...
                if (count($datasetLabels) > 0)
                    $seriesPlot->SetLabels(
                        array_fill(0, count($datasetLabels), ''));
                if ($dimensions != '3d') {
    ...
    private function _renderCombinationChart($groupCount, $dimensions, 
        $outputDestination) {
        require_once ('jpgraph_line.php');
        require_once ('jpgraph_bar.php');
        require_once ('jpgraph_scatter.php');
