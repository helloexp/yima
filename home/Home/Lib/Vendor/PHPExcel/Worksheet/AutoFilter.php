...
class PHPExcel_Worksheet_AutoFilter {
    ...
    public function setRange($pRange = '') {
        ...
        if (empty($pRange)) {
            ...
            // Discard any column rules that are no longer valid within this
            // range
    ...
    /**
     * Get a specified AutoFilter Column Offset within the defined AutoFilter
     * range
     *
     * @param string $pColumn Column name (e.g. A)
     * @throws PHPExcel_Exception
     * @return integer The offset of the specified column within the autofilter
     *         range
     */
    ...
    public function getColumn($pColumn) {
        ...
        if (! isset($this->_columns[$pColumn])) {
            $this->_columns[$pColumn] = new PHPExcel_Worksheet_AutoFilter_Column(
                $pColumn, $this);
        }
     ...
     *
     * @param integer $pColumnOffset Column offset within range (starting from
     *            0)
     * @throws PHPExcel_Exception
     * @return PHPExcel_Worksheet_AutoFilter_Column
     */
     ...
     * Set AutoFilter
     *
     * @param PHPExcel_Worksheet_AutoFilter_Column|string $pColumn A simple
     *            string containing a Column ID like 'A' is permitted
     * @throws PHPExcel_Exception
     * @return PHPExcel_Worksheet_AutoFilter
     */
    ...
    /**
     * Shift an AutoFilter Column Rule to a different column Note: This method
     * bypasses validation of the destination column to ensure it is within this
     * AutoFilter range. Nor does it verify whether any column rule already
     * exists at $toColumn, but will simply overrideany existing value. Use with
     * caution.
     *
    ...
    private static function _filterTestInCustomDataSet($cellValue, $ruleSet) {
        ...
        foreach ($dataSet as $rule) {
            if (is_numeric($rule['value'])) {
                ...
                // String values are always tested for equality, factoring in
                // for wildcards (hence a regexp test)
                $retVal = preg_match('/^' . $rule['value'] . '$/i', $cellValue);
            }
            // If there are multiple conditions, then we need to test both using
            // the appropriate join operator
            switch ($join) {
    ...
    /**
     * Test if cell date value is matches a set of values defined by a set of
     * months
     *
     * @param mixed $cellValue
    ...
    /**
     * Search/Replace arrays to convert Excel wildcard syntax to a regexp syntax
     * for preg_matching
     *
     * @var array
     */
    ...
    /**
     * Convert a dynamic rule daterange to a custom filter range expression for
     * ease of calculation
     *
    ...
    private function _dynamicFilterDateRange($dynamicRuleType, &$filterColumn) {
        ...
        // Calculate start/end dates for the required date range based on
        // current date
        ...
        switch ($dynamicRuleType) {
            ...
            case PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_NEXTQUARTER:
                ...
                $maxVal = (int) PHPExcel_Shared_Date::PHPtoExcel(
                    gmmktime(0, 0, 0, date('t', $baseDate), 
                        (1 + $thisQuarter) * 3, date('Y', $baseDate)));
                ++ $maxVal;
                $val = (int) PHPExcel_Shared_Date::PHPToExcel(
                    gmmktime(0, 0, 0, 1, 1 + $thisQuarter * 3, 
                        date('Y', $baseDate)));
                break;
    ...
    private function _calculateTopTenValue($columnID, $startRow, $endRow, 
        $ruleType, $ruleValue) {
    ...
    public function showHideRows() {
        ...
        // echo 'AutoFilter Heading Row ',$rangeStart[1],' is always
        // SHOWN',PHP_EOL;
        ...
        foreach ($this->_columns as $columnID => $filterColumn) {
            ...
            switch ($filterColumn->getFilterType()) {
                ...
                case PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_CUSTOMFILTER:
                    ...
                    foreach ($rules as $rule) {
                        ...
                        if (! is_numeric($ruleValue)) {
                            // Convert to a regexp allowing for regexp reserved
                            // characters, wildcards and escaped wildcards
                ...
                case PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_DYNAMICFILTER:
                    $ruleValues = array();
                    foreach ($rules as $rule) {
                        // We should only ever have one Dynamic Filter Rule
                        // anyway
                        ...
                        $operator = ($dynamicRuleType ===
                             PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_ABOVEAVERAGE) ? PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_GREATERTHAN : PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_LESSTHAN;
                        $ruleValues[] = array(
                            'operator' => $operator, 
                            'value' => $average);
                        $columnFilterTests[$columnID] = array(
                        ...
                        if ($dynamicRuleType{0} == 'M' ||
                             $dynamicRuleType{0} == 'Q') {
                            // Month or Quarter
                            sscanf($dynamicRuleType, '%[A-Z]%d', $periodType, 
                                $period);
                            if ($periodType == 'M') {
                                $ruleValues = array(
                                    $period);
            ...
            case PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_TOPTENFILTER:
                ...
                $operator = ($toptenRuleType ==
                     PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_TOP) ? PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_GREATERTHANOREQUAL : PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_LESSTHANOREQUAL;
                $ruleValues[] = array(
                    'operator' => $operator, 
                    'value' => $maxVal);
                $columnFilterTests[$columnID] = array(
    ...
    //
    // Execute the column tests for each row in the autoFilter range to
    // determine show/hide,
    for ($row = $rangeStart[1] + 1; $row <= $rangeEnd[1]; ++ $row) {
        ...
        foreach ($columnFilterTests as $columnID => $columnFilterTest) {
            ...
            // If filter test has resulted in FALSE, exit the loop straightaway
            // rather than running any more tests
            if (! $result)
                break;
        }
        // Set show/hide for the row based on the result of the autoFilter
        // result
    ...
    foreach ($vars as $key => $value) {
        if (is_object($value)) {
            ...
            foreach ($value as $k => $v) {
                $this->{$key}[$k] = clone $v;
                // attach the new cloned Column to this new cloned Autofilter
                // object
...
/**
 * toString method replicates previous behavior by returning the range if object
 * is referenced as a property of its parent.
 */
