...
class PHPExcel_Worksheet_AutoFilter_Column {
     ...
     */
    public function __construct($pColumn, 
    ...
    public function getRule($pIndex) {
        if (! isset($this->_ruleset[$pIndex])) {
            $this->_ruleset[$pIndex] = new PHPExcel_Worksheet_AutoFilter_Column_Rule(
                $this);
        }
     ...
     */
    public function addRule(PHPExcel_Worksheet_AutoFilter_Column_Rule $pRule, 
        $returnRule = TRUE) {
    ...
    /**
     * Delete a specified AutoFilter Column Rule If the number of rules is
     * reduced to 1, then we reset And/Or logic to Or
     *
     * @param integer $pIndex Rule index in the ruleset array
     * @return PHPExcel_Worksheet_AutoFilter_Column
     */
    public function deleteRule($pIndex) {
        if (isset($this->_ruleset[$pIndex])) {
            ...
            // If we've just deleted down to a single rule, then reset And/Or
            // joining to Or
        ...
        foreach ($vars as $key => $value) {
            if (is_object($value)) {
                ...
                foreach ($value as $k => $v) {
                    $this->$key[$k] = clone $v;
                    // attach the new cloned Rule to this new cloned Autofilter
                    // Cloned object
