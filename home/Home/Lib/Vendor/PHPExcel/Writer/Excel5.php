 ...
 *            (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Writer_Excel5 extends PHPExcel_Writer_Abstract implements 
    ...
    public function save($pFilename = null) {
        ...
        $this->_writerWorkbook = new PHPExcel_Writer_Excel5_Workbook(
            $this->_phpExcel, $this->_str_total, $this->_str_unique, 
        ...
        // build Escher objects. Escher objects for workbooks needs to be build
        // before Escher object for workbook.
        ...
        for ($i = 0; $i < $countSheets; ++ $i) {
            foreach ($this->_writerWorksheets[$i]->_phpSheet->getCellCollection() as $cellID) {
                $cell = $this->_writerWorksheets[$i]->_phpSheet->getCell(
                    $cellID);
                $cVal = $cell->getValue();
                if ($cVal instanceof PHPExcel_RichText) {
                    ...
                    foreach ($elements as $element) {
                        if ($element instanceof PHPExcel_RichText_Run) {
                            ...
                            $this->_writerWorksheets[$i]->_fntHashIndex[$font->getHashCode()] = $this->_writerWorkbook->_addFont(
                                $font);
                        }
                    }
                }
        ...
        $OLE = new PHPExcel_Shared_OLE_PPS_File(
            PHPExcel_Shared_OLE::Asc2Ucs($workbookStreamName));
        
        // Write the worksheet streams before the global workbook stream,
        // because the byte sizes of these are needed in the global workbook
        // stream
        ...
        if (isset($this->_documentSummaryInformation) &&
            ...
            $OLE_DocumentSummaryInformation = new PHPExcel_Shared_OLE_PPS_File(
                PHPExcel_Shared_OLE::Asc2Ucs(
                    chr(5) . 'DocumentSummaryInformation'));
            $OLE_DocumentSummaryInformation->append(
    ...
    private function _buildWorksheetEschers() {
        ...
        foreach ($this->_phpExcel->getAllsheets() as $sheet) {
            ...
            if (count($sheet->getDrawingCollection()) == 0 && empty(
                $filterRange)) {
                continue;
            }
            ...
            if (! empty($filterRange)) {
                ...
                while ($iInc <= $iNumColEnd) {
                    ...
                    $spContainer->setOPT(0x007F, 0x01040104); // Protection ->
                                                              // fLockAgainstGrouping
                    $spContainer->setOPT(0x00BF, 0x00080008); // Text ->
                                                              // fFitTextToShape
                    $spContainer->setOPT(0x01BF, 0x00010000); // Fill Style ->
                                                              // fNoFillHitTest
                    $spContainer->setOPT(0x01FF, 0x00080000); // Line Style ->
                                                              // fNoLineDrawDash
                    $spContainer->setOPT(0x03BF, 0x000A0000); // Group Shape ->
                                                              // fPrint
    ...
    private function _buildWorkbookEscher() {
        ...
        $dggContainer->setCSpSaved($totalCountShapes + $countDrawings); // total
                                                                        // number
                                                                        // of
                                                                        // shapes
                                                                        // incl.
        ...
        foreach ($this->_phpExcel->getAllsheets() as $sheet) {
            foreach ($sheet->getDrawingCollection() as $drawing) {
                if ($drawing instanceof PHPExcel_Worksheet_Drawing) {
                    ...
                    list ($imagesx, $imagesy, $imageFormat) = getimagesize(
                        $filename);
                    
                    switch ($imageFormat) {
                        
                        case 1: // GIF, not supported by BIFF8, we convert to
                                // PNG
    ...
    private function _writeDocumentSummaryInformation() {
        ...
        // GKPIDDSI_VERSION :Version of the application that wrote the property
        // storage
        $dataSection[] = array(
            'summary' => array(
                'pack' => 'V', 
        ...
        foreach ($dataSection as $dataProp) {
            ...
            if ($dataProp['type']['data'] == 0x02) { // 2 byte signed integer
            ...
            } elseif ($dataProp['type']['data'] == 0x03) { // 4 byte signed
                                                           // integer
                $dataSection_Content .= pack('V', $dataProp['data']['data']);
                ...
                $dataSection_Content_Offset += 4 + 4;
            } elseif ($dataProp['type']['data'] == 0x1E) { // null-terminated
                                                           // string prepended
                                                           // by dword string
                                                           // length
                                                           // Null-terminated
            ...
            } elseif ($dataProp['type']['data'] == 0x40) { // Filetime (64-bit
                                                           // value representing
                                                           // the number of
                                                           // 100-nanosecond
                                                           // intervals since
    ...
    private function _writeSummaryInformation() {
        ...
        if ($this->_phpExcel->getProperties()->getTitle()) {
            ...
            $dataSection[] = array(
                ...
                'type' => array(
                    'pack' => 'V', 
                    'data' => 0x1E),  // null-terminated string prepended by
                                     // dword string
                                     // length
                'data' => array(
                    'data' => $dataProp, 
        ...
        if ($this->_phpExcel->getProperties()->getCreated()) {
            ...
            $dataSection[] = array(
                ...
                'type' => array(
                    'pack' => 'V', 
                    'data' => 0x40),  // Filetime (64-bit value representing the
                                     // number of
                                     // 100-nanosecond intervals since January
                                     // 1, 1601)
                'data' => array(
