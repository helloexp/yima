...
// Original file header of PEAR::Spreadsheet_Excel_Writer_Parser (used as the
// base for this class):
// -----------------------------------------------------------------------------------------
...
class PHPExcel_Writer_Excel5_Parser {
    ...
    // *:/\?[]
    // Moreover, there are valid sheet title characters that cannot occur in
    // unquoted form (there may be more?)
    // +-% '^&<>=,;#()"{}
    ...
    public function __construct() {
        ...
        $this->_initializeHashes(); // Initialize the hashes: ptg's and
                                    // function's ptg's
    ...
    function _initializeHashes() {
        ...
        //
        // The following hash was generated by "function_locale.pl" in the
        // distro.
    ...
    function _convert($token) {
        if (preg_match("/\"([^\"]|\"\"){0,255}\"/", $token)) {
            ...
            // or 'Sheet1'!$A$1 or 'Sheet1:Sheet2'!$A$1
        } elseif (preg_match(
            ...
            // match external ranges like Sheet1!A1:B2 or Sheet1:Sheet2!A1:B2 or
            ...
            // match error codes
        } elseif (preg_match("/^#[A-Z0\/]{3,5}[!?]{1}$/", $token) or
             $token == '#N/A') {
    ...
    function _convertNumber($num) {
        ...
        if ((preg_match("/^\d+$/", $num)) and ($num <= 65535)) {
            ...
            if (PHPExcel_Writer_Excel5_BIFFwriter::getByteOrder()) { // if it's
                                                                     // Big
                                                                     // Endian
                $num = strrev($num);
            }
    ...
    /**
     * Convert an Excel 3d range such as "Sheet1!A1:D4" or "Sheet1:Sheet2!A1:D4"
     * to a ptgArea3d.
     *
     * @access private
    ...
    function _convertRange3d($token) {
        // $class = 0; // formulas like Sheet1!$A$1:$A$2 in list type data
        // validation need this class (0x3B)
        ...
        if (preg_match("/^(\\$)?[A-Ia-i]?[A-Za-z](\\$)?(\d+)$/", $cell1)) {
            ...
            list ($row1, $col1, $row2, $col2) = $this->_rangeToPackedRange(
                $cell1 . ':' . $cell2);
        }
    ...
    /**
     * Convert an Excel 3d reference such as "Sheet1!A1" or "Sheet1:Sheet2!A1"
     * to a ptgRef3d.
     *
     * @access private
     * @param string $cell An Excel cell reference
    ...
    /**
     * Convert the sheet name part of an external reference, for example
     * "Sheet1" or "Sheet1:Sheet2", to a packed structure.
     *
     * @access private
    ...
    function _packExtRef($ext_ref) {
        $ext_ref = preg_replace("/^'/", '', $ext_ref); // Remove leading ' if
                                                       // any.
        $ext_ref = preg_replace("/'$/", '', $ext_ref); // Remove trailing ' if
                                                       // any.
    ...
    /**
     * Look up the REF index that corresponds to an external sheet name (or
     * range). If it doesn't exist yet add it to the workbook's references
    ...
    function _getRefIndex($ext_ref) {
        ...
        $ext_ref = preg_replace("/'$/", '', $ext_ref); // Remove trailing ' if
                                                       // any.
        $ext_ref = str_replace('\'\'', '\'', $ext_ref); // Replace escaped ''
                                                        // with '
    ...
    /**
     * This method is used to update the array of sheet names. It is called by
     * the addWorksheet() method of the PHPExcel_Writer_Excel5_Workbook class.
     *
     * @access public
    ...
    /**
     * pack() row range into the required 3 or 4 byte format. Just using maximum
     * col/rows, which is probably not the correct solution
     *
     * @access private
    ...
    /**
     * Convert an Excel cell reference such as A1 or $B2 or C$3 or $D$4 to a
     * zero indexed row and column number. Also returns two (0,1) values to
     * indicate whether the row or column are relative references.
     *
     * @access private
...
elseif (is_numeric($token) and (! is_numeric($token . $this->_lookahead) or
             ($this->_lookahead == '')) and ($this->_lookahead != '!') and
             ($this->_lookahead != ':')) {
            return $token;
...
elseif (preg_match("/^[A-Z0-9\xc0-\xdc\.]+$/i", $token) and
             ($this->_lookahead == "(")) {
            return $token;
...
/**
 * It parses a condition. It assumes the following rule: Cond -> Expr [(">" |
 * "<") Expr]
 *
 * @access private
...
/**
 * It parses a expression. It assumes the following rule: Expr -> Term [("+" |
 * "-") Term] -> "string" -> "-" Term : Negative value -> "+" Term : Positive
 * value -> Error code
 *
 * @access private
 * @return mixed The parsed ptg'd tree on success
...
/**
 * It parses a term. It assumes the following rule: Term -> Fact [("*" | "/")
 * Fact]
 *
 * @access private
 * @return mixed The parsed ptg'd tree on success
...
/**
 * It parses a factor. It assumes the following rule: Fact -> ( Expr ) | CellRef
 * | CellRange | Number | Function
 *
 * @access private
 * @return mixed The parsed ptg'd tree on success
 */
...
if (preg_match('/^\$?[A-Ia-i]?[A-Za-z]\$?[0-9]+$/', $this->_current_token)) {
    ...
    $this->_advance();
    return $result;
} // If it's an external reference (Sheet1!A1 or Sheet1:Sheet2!A1 or Sheet1!$A$1
  // or Sheet1:Sheet2!$A$1)
elseif (preg_match(
    "/^" . self::REGEX_SHEET_TITLE_UNQUOTED . "(\:" .
         self::REGEX_SHEET_TITLE_UNQUOTED .
    ...
    $this->_advance();
    return $result;
} // If it's an external reference ('Sheet1'!A1 or 'Sheet1:Sheet2'!A1 or
  // 'Sheet1'!$A$1 or 'Sheet1:Sheet2'!$A$1)
elseif (preg_match(
    "/^'" . self::REGEX_SHEET_TITLE_QUOTED . "(\:" .
         self::REGEX_SHEET_TITLE_QUOTED .
...
elseif (preg_match(
    '/^(\$)?[A-Ia-i]?[A-Za-z](\$)?[0-9]+:(\$)?[A-Ia-i]?[A-Za-z](\$)?[0-9]+$/', 
    $this->_current_token) or preg_match(
    '/^(\$)?[A-Ia-i]?[A-Za-z](\$)?[0-9]+\.\.(\$)?[A-Ia-i]?[A-Za-z](\$)?[0-9]+$/', 
    $this->_current_token)) {
    // must be an error?
    $result = $this->_createTree($this->_current_token, '', '');
    $this->_advance();
    return $result;
} // If it's an external range (Sheet1!A1:B2 or Sheet1:Sheet2!A1:B2 or
  // Sheet1!$A$1:$B$2 or Sheet1:Sheet2!$A$1:$B$2)
elseif (preg_match(
    "/^" . self::REGEX_SHEET_TITLE_UNQUOTED . "(\:" .
         self::REGEX_SHEET_TITLE_UNQUOTED .
    ...
    $this->_advance();
    return $result;
} // If it's an external range ('Sheet1'!A1:B2 or 'Sheet1'!A1:B2 or
  // 'Sheet1'!$A$1:$B$2 or 'Sheet1'!$A$1:$B$2)
elseif (preg_match(
    "/^'" . self::REGEX_SHEET_TITLE_QUOTED . "(\:" .
         self::REGEX_SHEET_TITLE_QUOTED .
         ")?'\!\\$?([A-Ia-i]?[A-Za-z])?\\$?[0-9]+:\\$?([A-Ia-i]?[A-Za-z])?\\$?[0-9]+$/u", 
...
throw new PHPExcel_Writer_Exception(
    "Syntax error: " . $this->_current_token . ", lookahead: " .
         $this->_lookahead . ", current char: " . $this->_current_char);
}

/**
 * It parses a function call. It assumes the following rule: Func -> ( Expr
 * [,Expr]* )
 *
 * @access private
 * @return mixed The parsed ptg'd tree on success
...
// If fixed number of args eg. TIME($i,$j,$k). Check that the number of args is
// valid.
 ...
 * would use in a HP calculator stack). The following tree: + / \ 2 3 produces:
 * "23+" The following tree: + / \ 3 * / \ 6 A1 produces: "36A1*+" In fact all
 * operands, functions, references, etc... are written as ptg's
 *
 * @access public
...
if (preg_match("/^[A-Z0-9\xc0-\xdc\.]+$/", $tree['value']) and
     ...
     ! preg_match("/^[A-Ia-i]?[A-Za-z](\d+)\.\.[A-Ia-i]?[A-Za-z](\d+)$/", 
        $tree['value']) and ! is_numeric($tree['value']) and
