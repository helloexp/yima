...
// Original file header of PEAR::Spreadsheet_Excel_Writer_Workbook (used as the
// base for this class):
// -----------------------------------------------------------------------------------------
// /*
...
class PHPExcel_Writer_Excel5_Workbook extends PHPExcel_Writer_Excel5_BIFFwriter {
     ...
     */
    public function __construct(PHPExcel $phpExcel = null, &$str_total, &$str_unique, 
        &$str_table, &$colors, $parser) {
    ...
    public function addXfWriter($style, $isStyleXf = false) {
        ...
        // Background colors, best to treat these after the font so black will
        // come after white in custom palette
        $xfWriter->setFgColor(
            $this->_addColor(
                $style->getFill()
                    ->getStartColor()
        ...
        // Add the number format if it is not a built-in one and not already
        // added
     ...
     * storage.
     *
     * @param array $pWorksheetSizes The sizes in bytes of the binary worksheet
     *            streams
    ...
    public function writeWorkbook($pWorksheetSizes = null) {
        ...
        // Prepare part 3 of the workbook global stream, what goes after the
        // SHEET records
        $part3 = '';
        ...
        /*
         * TODO: store external SUPBOOK records and XCT and CRN records in case
         * of external references for BIFF8
         */
    ...
    function _calcSheetOffsets() {
        ...
        foreach ($this->_phpExcel->getWorksheetIterator() as $sheet) {
            $offset += $boundsheet_length + strlen(
    ...
    private function _writeNames() {
        ...
        for ($i = 0; $i < $total_worksheets; ++ $i) {
            ...
            if ($sheetSetup->isPrintAreaSet()) {
                ...
                $print_colmin = PHPExcel_Cell::columnIndexFromString(
                    $printArea[0][0]) - 1;
                $print_colmax = PHPExcel_Cell::columnIndexFromString(
                    $printArea[1][0]) - 1;
...
$rowmin, $rowmax, $colmin, $colmax);
            ...
            } else if ($sheetSetup->isColumnsToRepeatAtLeftSet() ||
                 $sheetSetup->isRowsToRepeatAtTopSet()) {
...
$rowmin, $rowmax, $colmin, $colmax);
    ...
    /**
     * Writes all the DEFINEDNAME records (BIFF8). So far this is only used for
     * repeating rows/columns (print titles) and print areas
     */
    private function _writeAllDefinedNamesBiff8() {
        ...
        if (count($this->_phpExcel->getNamedRanges()) > 0) {
            ...
            foreach ($namedRanges as $namedRange) {
                ...
                $range = PHPExcel_Cell::buildRange($range); // e.g.
                                                            // Sheet1!$A$1:$B$2
                ...
                try {
                    ...
                    if ($namedRange->getLocalOnly()) {
                        // local scope
                        $scope = $this->_phpExcel->getIndex(
                            $namedRange->getScope()) + 1;
                    } else {
                        // global scope
                        $scope = 0;
                    }
                    $chunk .= $this->writeData(
                        $this->_writeDefinedNameBiff8($namedRange->getName(), 
        ...
        for ($i = 0; $i < $total_worksheets; ++ $i) {
            ...
            if ($sheetSetup->isColumnsToRepeatAtLeftSet() &&
                ...
                $chunk .= $this->writeData(
                    $this->_writeDefinedNameBiff8(pack('C', 0x07), $formulaData, 
                        $i + 1, true));
        ...
        for ($i = 0; $i < $total_worksheets; ++ $i) {
            ...
            if ($sheetSetup->isPrintAreaSet()) {
                ...
                // store the DEFINEDNAME record
                $chunk .= $this->writeData(
                    $this->_writeDefinedNameBiff8(pack('C', 0x06), $formulaData, 
                        $i + 1, true));
            }
        }
        ...
        for ($i = 0; $i < $total_worksheets; ++ $i) {
            ...
            if (! empty($autoFilterRange)) {
                ...
                $chunk .= $this->writeData(
                    $this->_writeShortNameBiff8($name, $i + 1, $rangeBounds, 
                        true));
            }
        }
     ...
     * @param string $sheetIndex 1-based sheet index the defined name applies
     *            to. 0 = global
     * @param boolean $isBuiltIn Built-in name?
     * @return string Complete binary record data
     */
    private function _writeDefinedNameBiff8($name, $formulaData, $sheetIndex = 0, 
        $isBuiltIn = false) {
        $record = 0x0018;
     ...
     * @return string Complete binary record data
     */
    private function _writeShortNameBiff8($name, $sheetIndex = 0, $rangeBounds, 
        $isHidden = false) {
        $record = 0x0018;
        ...
        $extra = pack('Cvvvvv', 0x3B, $sheetIndex - 1, $rangeBounds[0][1] - 1, 
            $rangeBounds[1][1] - 1, $rangeBounds[0][0] - 1, 
            $rangeBounds[1][0] - 1);
    ...
    private function _writeWindow1() {
        ...
        // not supported by PHPExcel, so there is only one selected sheet, the
                         // active
        ...
        $header = pack("vv", $record, $length);
        $data = pack("vvvvvvvvv", $xWn, $yWn, $dxWn, $dyWn, $grbit, $itabCur, 
            $itabFirst, $ctabsel, $wTabRatio);
        $this->_append($header . $data);
    }
    ...
    private function _writeDatemode() {
        ...
        $f1904 = (PHPExcel_Shared_Date::getExcelCalendar() ==
             PHPExcel_Shared_Date::CALENDAR_MAC_1904) ? 1 : 0; // Flag
                                                              // for
                                                              // 1904
                                                              // date
                                                              // system
     ...
     * references in the workbook. Excel only stores references to external
     * sheets that are used in NAME. The workbook NAME record is required to
     * define the print area and the repeat rows and columns. A similar method
     * is used in Worksheet.php for a slightly different purpose.
     *
     ...
     * rows and columns. A similar method is used in Worksheet.php for a
     ...
     * print area, repeat rows only and repeat columns only.
     *
     ...
     */
    private function _writeNameShort($index, $type, $rowmin, $rowmax, $colmin, 
        $colmax) {
     ...
     * repeat rows and columns when both are specified. This shares a lot of
     * code with _writeNameShort() but we use a separate method to keep the code
     * clean. Code abstraction for reuse can be carried too far, and I should
     * know. ;-)
     *
     * @param integer $index Sheet index
     * @param integer $type Built-in name type
     ...
     * @param integer $colmax End column
     */
    private function _writeNameLong($index, $type, $rowmin, $rowmax, $colmin, 
        $colmax) {
        $record = 0x0018; // Record identifier
    ...
    /**
     * Handling of the SST continue blocks is complicated by the need to include
     * an additional continuation byte depending on whether the string is split
     * between blocks or whether it starts at the beginning of the block. (There
     * are also additional complications that will arise later when/if Rich
     * Strings are supported). The Excel documentation says that the SST record
     * should be followed by an EXTSST record. The EXTSST record is a hash table
     * that is used to optimise access to SST. However, despite the
     * documentation it doesn't seem to be required so we will ignore it.
     *
    ...
    private function _writeSharedStringsTable() {
        ...
        foreach (array_keys($this->_str_table) as $string) {
            ...
            while ($finished === false) {
                ...
                // normally, there will be only one cycle, but if string cannot
                // immediately be written as is
                // there will be need for more than one cylcle, if string longer
                // than one record data block, there
                // may be need for even more cycles
                
                if (strlen($recordData) + strlen($string) <= $continue_limit) {
                    // then we can write the string (or remainder of string)
                    // without any problems
                    $recordData .= $string;
                    
                    if (strlen($recordData) + strlen($string) == $continue_limit) {
                        // we close the record data block, and initialize a new
                        // one
                    ...
                    // special treatment writing the string (or remainder of the
                    // string)
                    // If the string is very long it may need to be written in
                    ...
                    // uncompressed: 2 byte string length length field + 1 byte
                    // option flags + 2 byte character
                    // compressed: 2 byte string length length field + 1 byte
                    // option flags + 1 byte character
                    $min_space_needed = ($encoding == 1) ? 5 : 4;
                    
                    // We have two cases
                    // 1. space remaining is less than minimum space needed
                    // here we must waste the space remaining and move to next
                    // record data block
                    // 2. space remaining is greater than or equal to minimum
                    // space needed
                    // here we write as much as we can in the current block,
                    // then move to next record data block
                    ...
                    if ($space_remaining < $min_space_needed) {
                        ...
                        // initialize effective remaining space, for Unicode
                        // strings this may need to be reduced by 1, see below
                        $effective_space_remaining = $space_remaining;
                        
                        // for uncompressed strings, sometimes effective space
                        ...
                        $string = substr($string, $effective_space_remaining); // for
                                                                               // next
                                                                               // cycle
                                                                               // in
                                                                               // while
                        ...
                        // start new record data block with the repeated option
                        // flags
        ...
        // if there was no need for any continue records, this will be the for
        // SST record data block itself
        ...
        foreach ($recordDatas as $i => $recordData) {
            // first block should have the SST record header, remaing should
            // have CONTINUE header
