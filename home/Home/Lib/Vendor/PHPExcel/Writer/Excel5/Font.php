...
class PHPExcel_Writer_Excel5_Font {
    ...
    public function writeFont() {
        ...
        $bCharSet = PHPExcel_Shared_Font::getCharsetFromFontName(
            $this->_font->getName()); // Character
                                      // set
        ...
        $data = pack("vvvvvCCCC", $this->_font->getSize() * 20, // Fontsize (in
                                                                // twips)
        $grbit, $icv, // Colour
self::_mapBold($this->_font->getBold()), // Font weight
$sss, // Superscript/Subscript
self::_mapUnderline($this->_font->getUnderline()), $bFamily, $bCharSet, 
            $reserved);
        $data .= PHPExcel_Shared_String::UTF8toBIFF8UnicodeShort(
            $this->_font->getName());
