...
// Original file header of PEAR::Spreadsheet_Excel_Writer_Worksheet (used as the
// base for this class):
// -----------------------------------------------------------------------------------------
// /*
...
class PHPExcel_Writer_Excel5_Worksheet extends PHPExcel_Writer_Excel5_BIFFwriter {
     ...
     * @param boolean $preCalculateFormulas Flag indicating whether formulas
    ...
    public function __construct(&$str_total, &$str_unique, &$str_table, &$colors, 
        ...
        $this->_countCellStyleXfs = count(
            $phpSheet->getParent()->getCellStyleXfCollection());
    }

    /**
     * Add data to the beginning of the workbook (note the reverse order) and to
    ...
    function close() {
        ...
        // Column dimensions
        if (($defaultWidth = $_phpSheet->getDefaultColumnDimension()->getWidth()) <
             0) {
        ...
        for ($i = 0; $i <= $maxCol; ++ $i) {
            ...
            if (isset($columnDimensions[$columnLetter])) {
                ...
                $xfIndex = $columnDimension->getXfIndex() + 15; // there are 15
                                                                    // cell style
                                                                    // Xfs
            }
        ...
        foreach ($_phpSheet->getCellCollection() as $cellID) {
            ...
            if ($cVal instanceof PHPExcel_RichText) {
                // $this->_writeString($row, $column, $cVal->getPlainText(),
                // $xfIndex);
                $arrcRun = array();
                $str_len = PHPExcel_Shared_String::CountCharacters(
                    $cVal->getPlainText(), 'UTF-8');
                $str_pos = 0;
    ...
    /**
     * Write a cell range address in BIFF8 always fixed range See section 2.5.14
     * in OpenOffice.org's Documentation of the Microsoft Excel File Format
     *
    ...
    private function _writeBIFF8CellRangeAddressFixed($range = 'A1') {
        ...
        $firstCellCoordinates = PHPExcel_Cell::coordinateFromString($firstCell); // e.g.
                                                                                 // array(0,
                                                                                 // 1)
        $lastCellCoordinates = PHPExcel_Cell::coordinateFromString($lastCell); // e.g.
                                                                               // array(1,
                                                                               // 6)
    ...
    /**
     * Retrieves data from memory in one chunk, or from disk in $buffer sized
     * chunks.
     *
     ...
     */
    function setOutline($visible = true, $symbols_below = true, $symbols_right = true, 
        $auto_style = false) {
    ...
    /**
     * Write a double to the specified row and column (zero indexed). An integer
     * can be written as a double. Excel will display an integer. $format is
     * optional. Returns 0 : normal termination -2 : row or column out of range
     *
    ...
    /**
     * Write a LABELSST record or a LABEL record. Which one depends on BIFF
     * version It differs from _writeString by the writing of rich text strings.
     *
     * @param int $row Row index (0-based)
     * @param int $col Column index (0-based)
     * @param string $str The string
    ...
    /**
     * Write a string to the specified row and column (zero indexed). NOTE:
     * there is an Excel 5 defined limit of 255 characters. $format is optional.
     * Returns 0 : normal termination -2 : row or column out of range -3 : long
     * string truncated to 255 chars
     *
     * @access public
     * @param integer $row Zero indexed row
     * @param integer $col Zero indexed column
    ...
    /**
     * Write a string to the specified row and column (zero indexed). This is
     * the BIFF8 version (no 255 chars limit). $format is optional. Returns 0 :
     * normal termination -2 : row or column out of range -3 : long string
    ...
    /**
     * Writes a note associated with the cell given by the row and column. NOTE
     * records don't have a length limit.
     *
     * @param integer $row Zero indexed row
     * @param integer $col Zero indexed column
    ...
    /**
     * Write a blank cell to the specified row and column (zero indexed). A
     * blank cell is used to specify formatting without adding a string or a
     * number. A blank cell without a format serves no purpose. Therefore, we
     * don't write a BLANK record unless a format is specified. Returns 0 :
     * normal termination (including no format) -1 : insufficient number of
     * arguments -2 : row or column out of range
     *
     * @param integer $row Zero indexed row
     * @param integer $col Zero indexed column
    ...
    /**
     * Write a boolean or an error type to the specified row and column (zero
     * indexed)
     *
     * @param int $row Row index (0-based)
     * @param int $col Column index (0-based)
    ...
    /**
     * Write a formula to the specified row and column (zero indexed). The
     * textual representation of the formula is passed to the parser in
     * Parser.php which returns a packed binary string. Returns 0 : normal
     * termination -1 : formula errors (bad formula) -2 : row or column out of
     * range
     *
     * @param integer $row Zero indexed row
     * @param integer $col Zero indexed column
    ...
    private function _writeFormula($row, $col, $formula, $xfIndex, 
        ...
        // Initialize possible additional value for STRING record that should be
                          // written after the FORMULA record?
        $stringValue = null;
        ...
        try {
            ...
            $data = pack("vvv", $row, $col, $xfIndex) . $num .
     ...
     * an alternative string is specified. The label is written using the
     * _writeString() method. Therefore the 255 characters string limit applies.
     * $string and $format are optional. The hyperlink can be to a http, ftp,
     * mail, internal sheet (not yet), or external directory url. Returns 0 :
     * normal termination -2 : row or column out of range -3 : long string
     * truncated to 255 chars
     *
     ...
     * hyperlink to be written. These are either, Web (http, ftp, mailto),
     * Internal (Sheet1!A1) or external ('c:\temp\foo.xls#Sheet1!A1').
     *
     * @access private
     * @see _writeUrl()
    ...
    /**
     * Used to write http, ftp and mailto hyperlinks. The link type ($options)
     * is 0x03 is the same as absolute dir ref without sheet. However it is
     * differentiated by the $unknown2 data stream.
     *
     * @access private
     * @see _writeUrl()
     * @param integer $row1 Start row
     ...
     * ignore these cases for the sake of simpler code.
     *
     * @access private
     * @see _writeUrl()
     * @param integer $row1 Start row
    ...
    function _writeUrlExternal($row1, $col1, $row2, $col2, $url) {
        ...
        if (preg_match('[^external:\\\\]', $url)) {
            return; // ($this->_writeUrlExternal_net($row1, $col1, $row2, $col2,
                        // $url, $str, $format));
        }
        ...
        $data = pack("vvvv", $row1, $row2, $col1, $col2) . $unknown1 . $link_type .
             $unknown2 . $up_count . $dir_short_len . $dir_short . $unknown3 .
             $stream_len; /*
                          * . $dir_long_len . $unknown4 . $dir_long . $sheet_len
                          * . $sheet ;
                          */
     ...
     * @param integer $height Height we are giving to the row. Use null to set
     *            XF without setting height
     * @param integer $xfIndex The optional cell style Xf index to apply to the
     *            columns
    ...
    private function _writeRow($row, $height, $xfIndex, $hidden = false, $level = 0) {
        ...
        // Use _writeRow($row, null, $XF) to set XF format without setting
        // height
        if ($height != null) {
        ...
        // Set the options flags. fUnsynced is used to show that the font and
        // row
        // heights are not compatible. This is usually the case for WriteExcel.
        // The collapsed flag 0x10 doesn't seem to be used to indicate that a
        // row
        ...
        $header = pack("vv", $record, $length);
        $data = pack("vvvvvvvv", $row, $colMic, $colMac, $miyRw, $irwMac, 
            $reserved, $grbit, $ixfe);
        $this->_append($header . $data);
    }
    ...
    private function _writeDimensions() {
        ...
        $data = pack('VVvvv', $this->_firstRowIndex, $this->_lastRowIndex + 1, 
            $this->_firstColumnIndex, $this->_lastColumnIndex + 1, 0x0000); // reserved
    ...
    private function _writeWindow2() {
        ...
        $fFrozenNoSplit = 0; // 0 - bit
                             ...
                             // therefore sheet is only selected if it is the
                             // active sheet
        $fSelected = ($this->_phpSheet ===
             $this->_phpSheet->getParent()->getActiveSheet()) ? 1 : 0;
        $fPaged = 1; // 2
    ...
    /**
     * Write BIFF record COLINFO to define column widths Note: The SDK says the
     * record length is 0x0B but Excel writes a 0x0C length record.
     *
     * @param array $col_array This is the only parameter received and is
     *            composed of the following: 0 => First formatted column, 1 =>
     *            Last formatted column, 2 => Col width (8.43 is Excel default),
     *            3 => The optional XF format of the column, 4 => Option flags.
    ...
    private function _writeColinfo($col_array) {
        ...
        $header = pack("vv", $record, $length);
        $data = pack("vvvvvv", $colFirst, $colLast, $coldx, $ixfe, $grbit, 
            $reserved);
        $this->_append($header . $data);
    }
    ...
    private function _writeSelection() {
        ...
        $selectedCells = PHPExcel_Cell::splitRange(
            $this->_phpSheet->getSelectedCells());
        ...
        $colFirst = PHPExcel_Cell::columnIndexFromString($colFirst) - 1; // base
                                                                         // 0
                                                                         // column
                                                                         // index
        ...
        $colLast = PHPExcel_Cell::columnIndexFromString($colLast) - 1; // base 0
                                                                       // column
                                                                       // index
        ...
        $header = pack("vv", $record, $length);
        $data = pack("CvvvvvvCC", $pnn, $rwAct, $colAct, $irefAct, $cref, 
    ...
    private function _writeMergedCells() {
        ...
        // counter for number of merged cells written in record currently being
        // written
        $j = 0;
        ...
        foreach ($mergeCells as $mergeCell) {
            ...
            // flush record if we have reached limit for number of merged cells,
            // or reached final merged cell
...
$this->_colors[$this->_phpSheet->getTabColor()->getRGB()], // color
                                                                   // index
        0x0000); // unused
    ...
    private function _writeSheetProtection() {
        ...
        $options = (int) ! $this->_phpSheet->getProtection()->getObjects() |
             ...
             (int) ! $this->_phpSheet->getProtection()->getInsertRows() << 6 | (int) ! $this->_phpSheet->getProtection()->getInsertHyperlinks() <<
             7 |
             (int) ! $this->_phpSheet->getProtection()->getDeleteColumns() << 8 |
             (int) ! $this->_phpSheet->getProtection()->getDeleteRows() << 9 | (int) ! $this->_phpSheet->getProtection()->getSelectLockedCells() <<
             10 | (int) ! $this->_phpSheet->getProtection()->getSort() << 11 |
             (int) ! $this->_phpSheet->getProtection()->getAutoFilter() << 12 |
             (int) ! $this->_phpSheet->getProtection()->getPivotTables() << 13 | (int) ! $this->_phpSheet->getProtection()->getSelectUnlockedCells() <<
             14;
...
0x0000); // not used
    ...
    /**
     * Write BIFF record RANGEPROTECTION Openoffice.org's Documentaion of the
     * Microsoft Excel File Format uses term RANGEPROTECTION for these records
     * Microsoft Office Excel 97-2007 Binary File Format Specification uses term
     * FEAT for these records
     */
    private function _writeRangeProtection() {
        foreach ($this->_phpSheet->getProtectedCells() as $range => $password) {
            ...
            $recordData = pack('vvVVvCVvVv', 0x0868, 0x00, 0x0000, 0x0000, 0x02, 
                0x0, 0x0000, $cref, 0x0000, 0x00);
            
            foreach ($cellRanges as $cellRange) {
                $recordData .= $this->_writeBIFF8CellRangeAddressFixed(
                    $cellRange);
            }
     ...
     * references in a worksheet. Excel only stores references to external
     * sheets that are used in formulas. For simplicity we store references to
     * all the sheets in the workbook regardless of whether they are used or
     * not. This reduces the overall complexity and eliminates the need for a
     * two way dialogue between the formula parser the worksheet objects.
     *
     ...
     * formulas. A formula references a sheet name via an index. Since we store
     * a reference to all of the external worksheets the EXTERNSHEET index is
     * the same as the worksheet index.
     *
    ...
    private function _writeExternsheet($sheetname) {
        ...
        // References to the current sheet are encoded differently to references
                          // to
                          // external sheets.
                          //
    ...
    /**
     * Writes the Excel BIFF PANE record. The panes can either be frozen or
     * thawed (unfrozen). Frozen panes are specified in terms of an integer
     * number of rows and columns. Thawed panes are specified in terms of
     * Excel's units for rows and columns.
     */
    ...
    private function _writeSetup() {
        ...
        $iPaperSize = $this->_phpSheet->getPageSetup()->getPaperSize(); // Paper
                                                                        // size
        
        $iScale = $this->_phpSheet->getPageSetup()->getScale() ? $this->_phpSheet->getPageSetup()->getScale() : 100; // Print
                                                                                                                     // scaling
                                                                                                                     // factor
        
        $iPageStart = 0x01; // Starting page number
        $iFitWidth = (int) $this->_phpSheet->getPageSetup()->getFitToWidth(); // Fit
                                                                              // to
                                                                              // number
                                                                              // of
                                                                              // pages
                                                                              // wide
        $iFitHeight = (int) $this->_phpSheet->getPageSetup()->getFitToHeight(); // Fit
                                                                                // to
                                                                                // number
                                                                                // of
                                                                                // pages
        ...
        $numHdr = $this->_phpSheet->getPageMargins()->getHeader(); // Header
                                                                   // Margin
        
        $numFtr = $this->_phpSheet->getPageMargins()->getFooter(); // Footer
                                                                   // Margin
    ...
    private function _writeHeader() {
        ...
        /*
         * removing for now // need to fix character count (multibyte!) if
         * (strlen($this->_phpSheet->getHeaderFooter()->getOddHeader()) <= 255)
         * { $str = $this->_phpSheet->getHeaderFooter()->getOddHeader(); //
         * header string } else { $str = ''; }
         */
    ...
    private function _writeFooter() {
        ...
        /*
         * removing for now // need to fix character count (multibyte!) if
         * (strlen($this->_phpSheet->getHeaderFooter()->getOddFooter()) <= 255)
         * { $str = $this->_phpSheet->getHeaderFooter()->getOddFooter(); } else
         * { $str = ''; }
         */
    ...
    private function _writeHcenter() {
        ...
        $fHCenter = $this->_phpSheet->getPageSetup()->getHorizontalCentered() ? 1 : 0; // Horizontal
                                                                                       // centering
    ...
    private function _writeVcenter() {
        ...
        $fVCenter = $this->_phpSheet->getPageSetup()->getVerticalCentered() ? 1 : 0; // Horizontal
                                                                                     // centering
    ...
    private function _writeMarginLeft() {
        ...
        $margin = $this->_phpSheet->getPageMargins()->getLeft(); // Margin in
                                                                 // inches
    ...
    private function _writeMarginRight() {
        ...
        $margin = $this->_phpSheet->getPageMargins()->getRight(); // Margin in
                                                                  // inches
    ...
    private function _writeMarginTop() {
        ...
        $margin = $this->_phpSheet->getPageMargins()->getTop(); // Margin in
                                                                // inches
    ...
    private function _writeMarginBottom() {
        ...
        $margin = $this->_phpSheet->getPageMargins()->getBottom(); // Margin in
                                                                   // inches
    ...
    /**
     * Write the PRINTGRIDLINES BIFF record. Must be used in conjunction with
     * the GRIDSET record.
     */
    private function _writePrintGridlines() {
        ...
        $fPrintGrid = $this->_phpSheet->getPrintGridlines() ? 1 : 0; // Boolean
                                                                     // flag
    ...
    private function _writeAutoFilterInfo() {
        ...
        $rangeBounds = PHPExcel_Cell::rangeBoundaries(
            $this->_phpSheet->getAutoFilter()->getRange());
     ...
     * where Excel outline symbols are displayed. The visibility of the gutters
     * is controlled by a flag in WSBOOL.
     *
     * @see _writeWsbool()
     */
        ...
        foreach ($this->_phpSheet->getBreaks() as $cell => $breakType) {
            ...
            switch ($breakType) {
                case PHPExcel_Worksheet::BREAK_COLUMN:
                    ...
                    $vbreaks[] = PHPExcel_Cell::columnIndexFromString(
                        $coordinates[0]) - 1;
                    break;
    ...
    private function _writePassword() {
        ...
        $wPassword = hexdec($this->_phpSheet->getProtection()->getPassword()); // Encoded
                                                                               // password
     ...
     * @param integer $x The horizontal position (offset) of the image inside
     *            the cell.
     * @param integer $y The vertical position (offset) of the image inside the
     *            cell.
     * @param float $scale_x The horizontal scale
     * @param float $scale_y The vertical scale
     */
    function insertBitmap($row, $col, $bitmap, $x = 0, $y = 0, $scale_x = 1, 
        $scale_y = 1) {
        $bitmap_array = (is_resource($bitmap) ? $this->_processBitmapGd($bitmap) : $this->_processBitmap(
            $bitmap));
     ...
     * by the OBJ record. +------------+------------+ | A | B |
     * +-----+------------+------------+ | |(x1,y1) | | | 1 |(A1)._______|______
     * | | | | | | | | | | | +-----+----| BITMAP |-----+ | | | | | | 2 |
     * |______________. | | | | (B2)| | | | (x2,y2)| +----
     * +------------+------------+ Example of a bitmap that covers some of the
     * area from cell A1 to cell B2. Based on the width and height of the bitmap
     * we need to calculate 8 vars: $col_start, $row_start, $col_end, $row_end,
     * $x1, $y1, $x2, $y2. The width and height of the cells are also variable
     * and have to be taken into account. The values of $col_start and
     * $row_start are passed in from the calling function. The values of
     * $col_end and $row_end are calculated by subtracting the width and height
     * of the bitmap from the width and height of the underlying cells. The
     * vertices are expressed as a percentage of the underlying cell width as
     * follows (rhs values are in pixels): x1 = X / W *1024 y1 = Y / H *256 x2 =
     * (X-1) / W *1024 y2 = (Y-1) / H *256 Where: X is distance from the left
     * side of the underlying cell Y is distance from the top of the underlying
     * cell W is the width of the cell H is the height of the cell The SDK
     * incorrectly states that the height should be expressed as a percentage of
     * 1024.
     *
     * @access private
    ...
    function _positionImage($col_start, $row_start, $x1, $y1, $width, $height) {
        ...
        if ($y1 >=
             PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_start + 1)) {
            $y1 = 0;
        }
        ...
        // image
        while ($height >=
             PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)) {
        ...
        $x2 = $width /
             PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, 
                PHPExcel_Cell::stringFromColumnIndex($col_end)) * 1024; // Distance
                                                                        // to
                                                                        // right
                                                                        // side
                                                                        // of
        ...
        $this->_writeObjPicture($col_start, $x1, $row_start, $y1, $col_end, $x2, 
            $row_end, $y2);
    }

    /**
     * Store the OBJ record that precedes an IMDATA record. This could be
     * generalise to support other Excel objects.
     *
     ...
     */
    private function _writeObjPicture($colL, $dxL, $rwT, $dyT, $colR, $dxR, $rwB, 
        $dyB) {
    ...
    private function _writeMsoDrawing() {
        ...
        if (isset($this->_escher)) {
            ...
            $spOffsets[0] = 0;
            $nm = count($spOffsets) - 1; // number of shapes excluding first
                                         // shape
            for ($i = 1; $i <= $nm; ++ $i) {
                // MSODRAWING record
...
$i, // object id number, Excel seems to use 1-based index,
                        // local for the sheet
...
0x0000); // length of ftEnd data
    ...
    private function _writeDataValidity() {
        ...
        if (! empty($dataValidationCollection)) {
            ...
            $grbit = 0x0000; // Prompt box at cell, no cached validity data at
                             // DV records
            $horPos = 0x00000000; // Horizontal position of prompt box, if fixed
                                  // position
            $verPos = 0x00000000; // Vertical position of prompt box, if fixed
                                  // position
            $objId = 0xFFFFFFFF; // Object identifier of drop down arrow object,
                                 // or -1 if not visible
            ...
            foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
                ...
                // prompt title
                $promptTitle = $dataValidation->getPromptTitle() !== '' ? $dataValidation->getPromptTitle() : chr(
                    0);
                $data .= PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong(
                    $promptTitle);
                
                // error title
                $errorTitle = $dataValidation->getErrorTitle() !== '' ? $dataValidation->getErrorTitle() : chr(
                    0);
                $data .= PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong(
                    $errorTitle);
                
                // prompt text
                $prompt = $dataValidation->getPrompt() !== '' ? $dataValidation->getPrompt() : chr(
                    0);
                $data .= PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($prompt);
                
                // error text
                $error = $dataValidation->getError() !== '' ? $dataValidation->getError() : chr(
                    0);
    ...
    private function _writeCFRule(PHPExcel_Style_Conditional $conditional) {
        ...
        if ($numConditions == 1) {
            ...
            $operand2 = null;
        } else if ($numConditions == 2 && ($conditional->getOperatorType() ==
        ...
        if ($bFormatBorder == 1) {
            ...
            // @todo _writeCFRule() => $blockLineStyle => Index Color for left
            // line
            // @todo _writeCFRule() => $blockLineStyle => Index Color for right
            // line
            // @todo _writeCFRule() => $blockLineStyle => Top-left to
            // bottom-right on/off
            // @todo _writeCFRule() => $blockLineStyle => Bottom-left to
            // top-right on/off
            $blockColor = 0;
            // @todo _writeCFRule() => $blockColor => Index Color for top line
            // @todo _writeCFRule() => $blockColor => Index Color for bottom
            // line
            // @todo _writeCFRule() => $blockColor => Index Color for diagonal
            // line
            switch ($conditional->getStyle()
                ->getBorders()
                ->getDiagonal()
