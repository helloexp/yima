...
// Original file header of PEAR::Spreadsheet_Excel_Writer_Format (used as the
// base for this class):
// -----------------------------------------------------------------------------------------
// /*
...
class PHPExcel_Writer_Excel5_Xf {
    ...
    function writeXf() {
        ...
        if ($this->_isStyleXf) {
            ...
            $style = self::_mapLocked(
                $this->_style->getProtection()->getLocked());
            $style |= self::_mapHidden(
                $this->_style->getProtection()->getHidden()) << 1;
        }
        ...
        $atr_bdr = (self::_mapBorderStyle(
            $this->_style->getBorders()
                ->getBottom()
                ->getBorderStyle()) || self::_mapBorderStyle(
            $this->_style->getBorders()
                ->getTop()
                ->getBorderStyle()) || self::_mapBorderStyle(
            $this->_style->getBorders()
        ...
        $align = $this->_mapHAlign(
            $this->_style->getAlignment()
        ...
        $data .= pack("CCC", 
            self::_mapTextRotation(
                $this->_style->getAlignment()->getTextRotation()), $biff8_options, 
            $used_attrib);
    ...
    /**
     * Sets the index to the number format record It can be date, time,
     * currency, etc...
     *
     * @access public
    ...
    private static $_mapFillType = array(
        ...
        PHPExcel_Style_Fill::FILL_GRADIENT_LINEAR => 0x00,  // does not exist in
                                                           // BIFF8
