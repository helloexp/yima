...
class PHPExcel_Writer_PDF {
    ...
    /**
     * Instantiate a new renderer of the configured type within this container
     * class
     *
    ...
    public function __construct(PHPExcel $phpExcel) {
        ...
        if (is_null($pdfLibraryName)) {
            throw new PHPExcel_Writer_Exception(
                "PDF Rendering library path has not been defined.");
        }
        $includePath = str_replace('\\', '/', get_include_path());
        $rendererPath = str_replace('\\', '/', $pdfLibraryPath);
        if (strpos($rendererPath, $includePath) === false) {
            set_include_path(
                get_include_path() . PATH_SEPARATOR . $pdfLibraryPath);
        }
    ...
    /**
     * Magic method to handle direct calls to the configured PDF renderer
     * wrapper class.
     *
     * @param string $name Renderer library method name
     * @param mixed[] $arguments Array of arguments to pass to the renderer
     *            method
