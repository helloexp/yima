 ...
 *            (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Writer_Excel2007 extends PHPExcel_Writer_Abstract implements 
    PHPExcel_Writer_IWriter {

    /**
     * Pre-calculate formulas Forces PHPExcel to recalculate all formulae in a
     * workbook when saving, so that the pre-calculated values are immediately
     * available to MS Excel or other office spreadsheet viewer when opening the
     * file Overrides the default TRUE for this specific writer for performance
     * reasons
     *
     * @var boolean
     */
    ...
    public function save($pFilename = null) {
        if ($this->_spreadSheet !== NULL) {
            ...
            $saveDebugLog = PHPExcel_Calculation::getInstance(
                $this->_spreadSheet)->getDebugLog()->getWriteDebugLog();
            PHPExcel_Calculation::getInstance($this->_spreadSheet)->getDebugLog()->setWriteDebugLog(
                FALSE);
            ...
            for ($i = 0; $i < $this->_spreadSheet->getSheetCount(); ++ $i) {
                $this->_stringTable = $this->getWriterPart('StringTable')->createStringTable(
                    $this->_spreadSheet->getSheet($i), $this->_stringTable);
            }
            
            // Create styles dictionaries
            $this->_styleHashTable->addFromSource(
            ...
            $this->_fillHashTable->addFromSource(
                $this->getWriterPart('Style')
                    ->allFills($this->_spreadSheet));
            $this->_fontHashTable->addFromSource(
                $this->getWriterPart('Style')
                    ->allFonts($this->_spreadSheet));
            $this->_bordersHashTable->addFromSource(
                $this->getWriterPart('Style')
            ...
            $this->_drawingHashTable->addFromSource(
            ...
            // Retrieve OVERWRITE and CREATE constants from the instantiated zip
            // class
            // This method of accessing constant values from a dynamic class
            // should work with all appropriate versions of PHP
            ...
            if ($objZip->open($pFilename, $zipOverWrite) !== true) {
                if ($objZip->open($pFilename, $zipCreate) !== true) {
                    throw new PHPExcel_Writer_Exception(
                        "Could not open " . $pFilename . " for writing.");
                }
            }
            ...
            // if hasMacros, add the vbaProject.bin file, Certificate file(if
            // exists)
            if ($this->_spreadSheet->hasMacros()) {
                $macrosCode = $this->_spreadSheet->getMacrosCode();
                if (! is_null($macrosCode)) { // we have the code ?
                    $objZip->addFromString('xl/vbaProject.bin', $macrosCode); // allways
                                                                              // in
                                                                              // 'xl',
                                                                              // allways
                                                                              // named
                                                                              // vbaProject.bin
                    if ($this->_spreadSheet->hasMacrosCertificate()) { // signed
                                                                       // macros
                                                                       // ?
                                                                       // Yes :
                                                                       // add
            ...
            // a custom UI in this workbook ? add it ("base" xml and additional
            // objects (pictures) and rels)
            if ($this->_spreadSheet->hasRibbon()) {
                $tmpRibbonTarget = $this->_spreadSheet->getRibbonXMLData(
                    'target');
                ...
                if ($this->_spreadSheet->hasRibbonBinObjects()) {
                    ...
                    $ribbonBinObjects = $this->_spreadSheet->getRibbonBinObjects(
                        'data'); // the
                                 // files
                                 // to
                                 // write
                    ...
                    $objZip->addFromString(
                        $tmpRootPath . '_rels/' . basename($tmpRibbonTarget) .
                             '.rels', 
            ...
            $customPropertiesPart = $this->getWriterPart('DocProps')->writeDocPropsCustom(
                $this->_spreadSheet);
            ...
            for ($i = 0; $i < $this->_spreadSheet->getSheetCount(); ++ $i) {
                ...
                $objZip->addFromString(
                    ...
                    $this->getWriterPart('Rels')
                        ...
                        $this->_spreadSheet->getSheet($i), ($i + 1), 
                        $this->_includeCharts));
                ...
                if (($drawingCount > 0) || ($chartCount > 0)) {
                    ...
                    $objZip->addFromString(
                        ...
                        $this->getWriterPart('Rels')
                            ...
                            $this->_spreadSheet->getSheet($i), $chartRef1, 
                            $this->_includeCharts));
            ...
            for ($i = 0; $i < $this->getDrawingHashTable()->count(); ++ $i) {
                if ($this->getDrawingHashTable()->getByIndex($i) instanceof PHPExcel_Worksheet_Drawing) {
                    ...
                    if (strpos($imagePath, 'zip://') !== false) {
                        ...
                        $imageContents = $imageZip->getFromName(
                            $imagePathSplitted[1]);
                        $imageZip->close();
                        unset($imageZip);
                    } else {
                    ...
                    $objZip->addFromString(
                        'xl/media/' . str_replace(' ', '_', 
                            $this->getDrawingHashTable()
                                ->getByIndex($i)
