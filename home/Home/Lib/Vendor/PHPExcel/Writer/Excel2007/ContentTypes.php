...
class PHPExcel_Writer_Excel2007_ContentTypes extends PHPExcel_Writer_Excel2007_WriterPart {
    ...
    public function writeContentTypes(PHPExcel $pPHPExcel = null, 
        ...
        for ($i = 0; $i < $sheetCount; ++ $i) {
            ...
            // We need a drawing relationship for the worksheet if we have
            // either drawings or charts
            ...
            // If we have charts, then we need a chart relationship for every
            // individual chart
            if ($chartCount > 0) {
        ...
        for ($i = 0; $i < $mediaCount; ++ $i) {
            ...
            if (! isset($aMediaContentTypes[$extension])) {
                ...
                $this->_writeDefaultContentType($objWriter, $extension, 
                    $mimeType);
            }
        }
        if ($pPHPExcel->hasRibbonBinObjects()) { // Some additional objects in
                                                 // the ribbon ?
                                                 // we need to write "Extension"
                                                 // but not already write for
                                                 // media content
            $tabRibbonTypes = array_diff(
                $pPHPExcel->getRibbonBinObjects('types'), 
                array_keys($aMediaContentTypes));
            foreach ($tabRibbonTypes as $aRibbonType) {
                $mimeType = 'image/.' . $aRibbonType; // we wrote $mimeType like
                                                      // customUI Editor
                $this->_writeDefaultContentType($objWriter, $aRibbonType, 
                    $mimeType);
            }
        }
        $sheetCount = $pPHPExcel->getSheetCount();
            ...
            if (count(
                ...
                foreach ($pPHPExcel->getSheet()
                    ...
                    if (! isset(
                        ...
                        $this->_writeDefaultContentType($objWriter, 
                            strtolower($image->getExtension()), 
                            $aMediaContentTypes[strtolower(
                                $image->getExtension())]);
                    }
                }
            }
    ...
    private function _writeDefaultContentType(
        PHPExcel_Shared_XMLWriter $objWriter = null, $pPartname = '', 
        $pContentType = '') {
