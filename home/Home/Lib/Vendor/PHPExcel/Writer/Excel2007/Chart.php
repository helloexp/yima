...
class PHPExcel_Writer_Excel2007_Chart extends PHPExcel_Writer_Excel2007_WriterPart {
    ...
    public function writeChart(PHPExcel_Chart $pChart = null) {
        ...
        $this->_writePlotArea($pChart->getPlotArea(), $pChart->getXAxisLabel(), 
            $pChart->getYAxisLabel(), $objWriter, $pChart->getWorksheet());
    ...
    private function _writePlotArea(PHPExcel_Chart_PlotArea $plotArea, 
        ...
        PHPExcel_Chart_Title $yAxisLabel = NULL, $objWriter, 
        ...
        if (($chartType !== PHPExcel_Chart_DataSeries::TYPE_PIECHART) &&
            ...
            if ($chartType === PHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
                $this->_writeValAx($objWriter, $plotArea, $xAxisLabel, 
                    $chartType, $id1, $id2, $catIsMultiLevelSeries);
            } else {
     ...
     * @throws PHPExcel_Writer_Exception
     */
    private function _writeCatAx($objWriter, PHPExcel_Chart_PlotArea $plotArea, 
        $xAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries) {
        ...
        if (! is_null($xAxisLabel)) {
            ...
            $objWriter->writeRawData(
                PHPExcel_Shared_String::ControlCharacterPHP2OOXML($caption));
            $objWriter->endElement();
     ...
     * @throws PHPExcel_Writer_Exception
     */
    private function _writeValAx($objWriter, PHPExcel_Chart_PlotArea $plotArea, 
        $yAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries) {
     ...
     * @param PHPExcel_Shared_XMLWriter $objWriter XML Writer
     * @param boolean &$catIsMultiLevelSeries Is category a multi-series
     *            category
     ...
     * @throws PHPExcel_Writer_Exception
     */
    private function _writePlotGroup($plotGroup, $groupType, $objWriter, 
        &$catIsMultiLevelSeries, &$valIsMultiLevelSeries, &$plotGroupingType, 
        PHPExcel_Worksheet $pSheet) {
        ...
        // Get these details before the loop, because we can use the count to
        // check for varyColors
 ...
 * @throws PHPExcel_Writer_Exception
 */
private function _writePlotSeriesValues($plotSeriesValues, $objWriter, 
    $groupType, $dataType = 'str', PHPExcel_Worksheet $pSheet) {
