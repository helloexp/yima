 ...
 *            (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Writer_HTML extends PHPExcel_Writer_Abstract implements 
    ...
    public function save($pFilename = null) {
        ...
        PHPExcel_Calculation::getInstance($this->_phpExcel)->getDebugLog()->setWriteDebugLog(
            FALSE);
     ...
     * Set sheet index
     *
     * @param boolean $pValue Flag indicating whether the sheet navigation block
    ...
    public function generateSheetData() {
        ...
        foreach ($sheets as $sheet) {
            ...
            $dimension[0][0] = PHPExcel_Cell::columnIndexFromString(
                $dimension[0][0]) - 1;
            $dimension[1] = PHPExcel_Cell::coordinateFromString($dimension[1]);
            $dimension[1][0] = PHPExcel_Cell::columnIndexFromString(
                $dimension[1][0]) - 1;
            ...
            while ($row ++ < $rowMax) {
                ...
                if (! isset(
                    $this->_isSpannedRow[$sheet->getParent()->getIndex($sheet)][$row])) {
                    // Start a new rowData
                    $rowData = array();
                    ...
                    while ($column ++ < $dimension[1][0]) {
                        ...
                        if ($sheet->cellExistsByColumnAndRow($column, $row)) {
                            $rowData[$column] = PHPExcel_Cell::stringFromColumnIndex(
                                $column) . $row;
                        } else {
                            $rowData[$column] = '';
                        }
        ...
        if (count($sheets) > 1) {
            ...
            foreach ($sheets as $sheet) {
                $html .= '  <li class="sheet' . $sheetId . '"><a href="#sheet' .
                     $sheetId . '">' . $sheet->getTitle() . '</a></li>' . PHP_EOL;
                ++ $sheetId;
            }
    ...
    private function _extendRowsForChartsAndImages(PHPExcel_Worksheet $pSheet, 
        $row) {
        $rowMax = $row;
        $colMax = 'A';
        if ($this->_includeCharts) {
            foreach ($pSheet->getChartCollection() as $chart) {
                if ($chart instanceof PHPExcel_Chart) {
                    ...
                    $chartTL = PHPExcel_Cell::coordinateFromString(
                        $chartCoordinates['cell']);
                    $chartCol = PHPExcel_Cell::columnIndexFromString(
                        $chartTL[0]);
                    if ($chartTL[1] > $rowMax) {
                        $rowMax = $chartTL[1];
                        if ($chartCol >
        ...
        foreach ($pSheet->getDrawingCollection() as $drawing) {
            if ($drawing instanceof PHPExcel_Worksheet_Drawing) {
                $imageTL = PHPExcel_Cell::coordinateFromString(
                    $drawing->getCoordinates());
                $imageCol = PHPExcel_Cell::columnIndexFromString($imageTL[0]);
                if ($imageTL[1] > $rowMax) {
                    $rowMax = $imageTL[1];
                    if ($imageCol > PHPExcel_Cell::columnIndexFromString(
                        $colMax)) {
                        $colMax = $imageTL[0];
                    }
                }
            ...
            if ($drawing instanceof PHPExcel_Worksheet_Drawing) {
                if ($drawing->getCoordinates() == $coordinates) {
                    ...
                    $html .= '<img style="position: absolute; z-index: 1; left: ' .
                         ...
                         $drawing->getHeight() . 'px;" src="' . $imageData .
                         '" border="0" />';
                    $html .= '</div>';
                }
            }
            ...
            if ($chart instanceof PHPExcel_Chart) {
                ...
                if ($chartCoordinates['cell'] == $coordinates) {
                    ...
                    if ($fp = fopen($chartFileName, "rb", 0)) {
                        ...
                        // into chunks according to RFC 2045 semantics
                        $base64 = chunk_split(base64_encode($picture));
                        $imageData = 'data:' . $imageDetails['mime'] . ';base64,' .
                             $base64;
                        ...
                        $html .= '<img style="position: absolute; z-index: 1; left: ' .
                             ...
                             $imageDetails[0] . 'px; height: ' . $imageDetails[1] .
                             'px;" src="' . $imageData . '" border="0" />' .
                             PHP_EOL;
                        $html .= '</div>';
    ...
    public function generateStyles($generateSurroundingHTML = true) {
        ...
        foreach ($css as $styleName => $styleDefinition) {
            if ($styleName != 'html') {
                $html .= '	  ' . $styleName . ' { ' .
                     $this->_assembleCSS($styleDefinition) . ' }' . PHP_EOL;
            }
        }
     ...
     * Build CSS styles
     *
     * @param boolean $generateSurroundingHTML Generate surrounding HTML style?
     *            (html { })
     * @return array
     * @throws PHPExcel_Writer_Exception
     */
        ...
        foreach ($sheets as $sheet) {
            ...
            foreach ($sheet->getColumnDimensions() as $columnDimension) {
                if (($width = PHPExcel_Shared_Drawing::cellDimensionToPixels(
                    $columnDimension->getWidth(), $this->_defaultFont)) >= 0) {
                    ...
                    $css['table.sheet' . $sheetIndex . ' col.col' . $column]['width'] = $width .
                         'pt';
                    
                    if ($columnDimension->getVisible() === false) {
                        $css['table.sheet' . $sheetIndex . ' col.col' . $column]['visibility'] = 'collapse';
                        $css['table.sheet' . $sheetIndex . ' col.col' . $column]['*display'] = 'none'; // target
                                                                                                           // IE6+7
                    }
                }
            }
            ...
            foreach ($sheet->getRowDimensions() as $rowDimension) {
                ...
                $css['table.sheet' . $sheetIndex . ' tr.row' . $row]['height'] = $pt_height .
                     'pt';
                if ($rowDimension->getVisible() === false) {
        ...
        if ($textAlign = $this->_mapHAlign($pStyle->getHorizontal())) {
            ...
            if (in_array($textAlign, 
                array(
                    'left', 
                    'right')))
                $css['padding-' . $textAlign] = (string) ((int) $pStyle->getIndent() *
                     9) . 'px';
        }
    ...
    private function _createCSSStyleBorders(PHPExcel_Style_Borders $pStyle) {
        ...
        // Create CSS
        $css['border-bottom'] = $this->_createCSSStyleBorder(
            $pStyle->getBottom());
    ...
    private function _createCSSStyleBorder(PHPExcel_Style_Border $pStyle) {
        // Create CSS
        // $css = $this->_mapBorderStyle($pStyle->getBorderStyle()) . ' #' .
        // $pStyle->getColor()->getRGB();
        // Create CSS - add !important to non-none border styles for merged
        // cells
    ...
    private function _createCSSStyleFill(PHPExcel_Style_Fill $pStyle) {
        ...
        // Create CSS
        $value = $pStyle->getFillType() == PHPExcel_Style_Fill::FILL_NONE ? 'white' : '#' .
     ...
     *
     * @param PHPExcel_Worksheet $pSheet The worksheet for the table we are
     *            writing
     * @return string
     * @throws PHPExcel_Writer_Exception
     */
    private function _generateTableHeader($pSheet) {
        ...
        if (! $this->_useInlineCss) {
            ...
            $html .= '	<table border="0" cellpadding="0" cellspacing="0" id="sheet' .
                 $sheetIndex . '" class="sheet' . $sheetIndex . $gridlines . '">' .
                 PHP_EOL;
        } else {
            $style = isset($this->_cssStyles['table']) ? $this->_assembleCSS(
                $this->_cssStyles['table']) : '';
     ...
     * @throws PHPExcel_Writer_Exception
     */
    private function _generateRow(PHPExcel_Worksheet $pSheet, $pValues = null, 
        $pRow = 0) {
        if (is_array($pValues)) {
            // Construct HTML
            $html = '';
            ...
            if (! $this->_useInlineCss) {
                ...
                $style = isset(
                    $this->_cssStyles['table.sheet' . $sheetIndex . ' tr.row' .
                         $pRow]) ? $this->_assembleCSS(
                    $this->_cssStyles['table.sheet' . $sheetIndex . ' tr.row' .
                     $pRow]) : '';
            ...
            if ($cell instanceof PHPExcel_Cell) {
                ...
                if (! $this->_useInlineCss) {
                    ...
                    if (isset(
                        $this->_cssStyles['td.style' . $cell->getXfIndex()])) {
                        $cssClass = array_merge($cssClass, 
                            $this->_cssStyles['td.style' . $cell->getXfIndex()]);
                    }
                    
                    // General horizontal alignment: Actual horizontal alignment
                    // depends on dataType
                    $sharedStyle = $pSheet->getParent()->getCellXfByIndex(
                        $cell->getXfIndex());
    ...
    $rowspan = 1;
    if (isset(
        $this->_isBaseCell[$pSheet->getParent()->getIndex($pSheet)][$pRow + 1][$colNum])) {
        $spans = $this->_isBaseCell[$pSheet->getParent()->getIndex($pSheet)][$pRow +
             1][$colNum];
        $rowSpan = $spans['rowspan'];
        $colSpan = $spans['colspan'];
        
        // Also apply style from last cell in merge to fix borders -
        // relies on !important for non-none border declarations in
        // _createCSSStyleBorder
        $endCellCoord = PHPExcel_Cell::stringFromColumnIndex(
            $colNum + $colSpan - 1) . ($pRow + $rowSpan);
        if (! $this->_useInlineCss) {
    ...
    if ($writeCell) {
        ...
        if (! $this->_useInlineCss) {
            ...
            // **
            // We must explicitly write the width of the <td> element because
            // TCPDF
            ...
            // We must also explicitly write the height of the <td> element
            // because TCPDF
            // does not recognize e.g. <tr style="height:50pt">
            if (isset(
                $this->_cssStyles['table.sheet' . $sheetIndex . ' tr.row' . $pRow]['height'])) {
                $height = $this->_cssStyles['table.sheet' . $sheetIndex .
                     ' tr.row' . $pRow]['height'];
...
/**
 * Calculate information about HTML colspan and rowspan which is not always the
 * same as Excel's
 */
    ...
    while ($r ++ < $lr) {
        ...
        while ($c ++ < $lc) {
            if (! ($c == $fc && $r == $fr)) {
                ...
                // upper-left is the base cell that should hold the
                // colspan/rowspan attribute
                $this->_isBaseCell[$sheetIndex][$r][$c] = array(
                    'xlrowspan' => $lr - $fr + 1,  // Excel rowspan
                    'rowspan' => $lr - $fr + 1,  // HTML rowspan, value may
                                                // change
                    'xlcolspan' => $lc - $fc + 1,  // Excel colspan
                    'colspan' => $lc - $fc + 1); // HTML colspan, value may
                                                     // change
...
// Identify which rows should be omitted in HTML. These are the rows where all
// the cells
...
// For each of the omitted rows we found above, the affected rowspans should be
// subtracted by 1
...
$left = PHPExcel_Shared_String::FormatNumber(
$pSheet->getPageMargins()->getLeft()) . 'in; ';
$htmlPage .= 'left-margin: ' . $left;
$htmlBody .= 'left-margin: ' . $left;
$right = PHPExcel_Shared_String::FormatNumber(
$pSheet->getPageMargins()->getRight()) . 'in; ';
$htmlPage .= 'right-margin: ' . $right;
$htmlBody .= 'right-margin: ' . $right;
$top = PHPExcel_Shared_String::FormatNumber($pSheet->getPageMargins()->getTop()) .
 'in; ';
$htmlPage .= 'top-margin: ' . $top;
$htmlBody .= 'top-margin: ' . $top;
$bottom = PHPExcel_Shared_String::FormatNumber(
$pSheet->getPageMargins()->getBottom()) . 'in; ';
