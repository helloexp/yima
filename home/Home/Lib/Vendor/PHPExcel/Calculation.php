...
class PHPExcel_Calculation {
    ...
    // Function (allow for the old @ symbol that could be used to prefix a
    // function, but we'll ignore it)
    ...
    /**
     * List of instances of the calculation engine that we've instantiated for
    ...
    /**
     * List of operators that can be used within formulae The true/false value
     * indicates whether it is a binary operator or a unary operator
     *
     * @access private
     * @var array
     */
    ...
    /**
     * Flag to determine how formula errors should be handled If true, then a
     * user error will be triggered If false, then an exception will be thrown
     *
     * @access public
     * @var boolean
     *
    ...
    /**
     * Error message for any error that was raised/thrown by the calculation
     * engine
     *
     * @access public
     * @var string
    ...
    /**
     * Current iteration counter for cyclic formulae If the value is 0 (or less)
     * then cyclic formulae will throw an exception, otherwise they will iterate
     * to the limit defined here before returning a result
     *
     * @var integer
     *
     */
    ...
    /**
     * List of available locale settings Note that this is read for the locale
     * subdirectory only when requested
     *
     * @var string[]
     *
     */
    ...
    /**
     * Excel constant string translations to their PHP equivalents Constant
     * conversion from text name/value to actual (datatyped) value
     *
     * @var string[]
     *
     */
     ...
     *
     * @access public
     * @param PHPExcel $workbook Injected workbook for working with a PHPExcel
     *            object, or NULL to create a standalone claculation engine
     ...
     *
     * @access public
     * @param PHPExcel $workbook Injected workbook identifying the instance to
     *            unset
     */
    ...
    /**
     * Flush the calculation cache for any existing instance of this class but
     * only if a PHPExcel_Calculation instance exists
     *
     * @access public
     * @return null
     */
    ...
    public function setLocale($locale = 'en_us') {
        ...
        if (in_array($language, self::$_validLocaleLanguages)) {
            ...
            self::$_localeBoolean = array(
                ...
                'FALSE' => 'FALSE', 
                'NULL' => 'NULL');
            // Default is English, if user isn't requesting english, then read
            // the necessary data from the locale files
            if ($locale != 'en_us') {
                ...
                if (! file_exists($functionNamesFile)) {
                    // If there isn't a locale specific function file, look for
                    // a language specific function file
                    $functionNamesFile = PHPEXCEL_ROOT . 'PHPExcel' .
                         DIRECTORY_SEPARATOR . 'locale' . DIRECTORY_SEPARATOR .
                ...
                // Retrieve the list of locale or language specific function
                // names
                $localeFunctions = file($functionNamesFile, 
                    FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
                foreach ($localeFunctions as $localeFunction) {
                    list ($localeFunction) = explode('##', $localeFunction); // Strip
                                                                             // out
                                                                             // comments
                ...
                // Default the TRUE and FALSE constants to the locale names of
                ...
                if (file_exists($configFile)) {
                    ...
                    foreach ($localeSettings as $localeSetting) {
                        list ($localeSetting) = explode('##', $localeSetting); // Strip
                                                                               // out
                                                                               // comments
                        if (strpos($localeSetting, '=') !== FALSE) {
                            ...
                            switch ($settingName) {
                                case 'ARGUMENTSEPARATOR':
                                    self::$_localeArgumentSeparator = trim(
                                        $settingValue);
                                    break;
                            }
                        }
    ...
    // function setLocale()
    public static function _translateSeparator($fromSeparator, $toSeparator, 
        $formula, &$inBraces) {
    ...
    private static function _translateFormula($from, $to, $formula, 
        $fromSeparator, $toSeparator) {
        // Convert any Excel function names to the required language
        if (self::$_localeLanguage !== 'en_us') {
            $inBraces = FALSE;
            // If there is the possibility of braces within a quoted string,
            // then we don't treat those as matrix indicators
            if (strpos($formula, '"') !== FALSE) {
                // So instead we skip replacing in any quoted strings by only
                // replacing in every other array element after we've exploded
                // the formula
                ...
                // If there's no quoted strings, then we do a simple
                // count/replace
        ...
        if (self::$functionReplaceFromExcel === NULL) {
            ...
            foreach (array_keys(self::$_localeFunctions) as $excelFunctionName) {
                self::$functionReplaceFromExcel[] = '/(@?[^\w\.])' .
                     preg_quote($excelFunctionName) . '([\s]*\()/Ui';
            }
            foreach (array_keys(self::$_localeBoolean) as $excelBoolean) {
                self::$functionReplaceFromExcel[] = '/(@?[^\w\.])' .
                     preg_quote($excelBoolean) . '([^\w\.])/Ui';
            }
        }
    ...
    public function _translateFormulaToEnglish($formula) {
        if (self::$functionReplaceFromLocale === NULL) {
            ...
            foreach (array_values(self::$_localeFunctions) as $localeFunctionName) {
                self::$functionReplaceFromLocale[] = '/(@?[^\w\.])' .
                     preg_quote($localeFunctionName) . '([\s]*\()/Ui';
            }
        ...
        if (self::$functionReplaceToExcel === NULL) {
            ...
            foreach (array_keys(self::$_localeFunctions) as $excelFunctionName) {
                self::$functionReplaceToExcel[] = '$1' . trim(
                    $excelFunctionName) . '$2';
            }
            foreach (array_keys(self::$_localeBoolean) as $excelBoolean) {
    ...
    /**
     * Calculate cell value (using formula from a cell ID) Retained for backward
     * compatibility
     *
     * @access public
     ...
     * @access public
     * @param PHPExcel_Cell $pCell Cell to calculate
     * @param Boolean $resetLog Flag indicating whether the debug log should be
     *            reset or not
     * @return mixed
     * @throws PHPExcel_Calculation_Exception
     */
    public function calculateCellValue(PHPExcel_Cell $pCell = NULL, $resetLog = TRUE) {
        ...
        try {
            $result = self::_unwrapResult(
                $this->_calculateFormulaValue($pCell->getValue(), 
        ...
        if ((is_array($result)) &&
            ...
            if (count($testResult) != 1) {
                // If keys are numeric, then it's a matrix result rather than a
                // cell range result, so we permit it
                $r = array_keys($result);
                $r = array_shift($r);
                if (! is_numeric($r)) {
                    return PHPExcel_Calculation_Functions::VALUE();
     ...
     * @throws PHPExcel_Calculation_Exception
     */
    public function calculateFormula($formula, $cellID = NULL, 
        ...
        // Disable calculation cacheing because it only applies to cell
        // calculations, not straight formulae
    ...
    public function getValueFromCache($worksheetName, $cellID, &$cellValue) {
        ...
        if (($this->_calculationCacheEnabled) &&
            ...
            $this->_debugLog->writeDebugLog('Retrieving value for cell ', 
                $worksheetName, '!', $cellID, ' from cache');
     ...
     * @throws PHPExcel_Calculation_Exception
     */
    public function _calculateFormulaValue($formula, $cellID = null, 
        PHPExcel_Cell $pCell = null) {
        $cellValue = '';
        ...
        if (($cellID !== NULL) &&
             ($this->getValueFromCache($wsTitle, $cellID, $cellValue))) {
            return $cellValue;
        }
        
        if (($wsTitle{0} !== "\x00") &&
             ($this->_cyclicReferenceStack->onStack($wsTitle . '!' . $cellID))) {
        ...
        $cellValue = $this->_processTokenStack(
            $this->_parseFormula($formula, $pCell), $cellID, $pCell);
     ...
     * @param integer $resize Flag indicating whether the matrices should be
     *            resized to match and (if so), whether the smaller dimension
     *            should grow or the larger should shrink. 0 = no resize 1 =
     *            shrink to fit 2 = extend to fit
     */
    private static function _checkMatrixOperands(&$operand1, &$operand2, 
        $resize = 1) {
        // Examine each of the two operands, and turn them into an array if they
        // aren't one already
        // Note that this function should only be called if one or both of the
        // operand is already an array
        if (! is_array($operand1)) {
            list ($matrixRows, $matrixColumns) = self::_getMatrixDimensions(
                $operand2);
        ...
        list ($matrix1Rows, $matrix1Columns) = self::_getMatrixDimensions(
            $operand1);
        list ($matrix2Rows, $matrix2Columns) = self::_getMatrixDimensions(
            $operand2);
        ...
        if ($resize == 2) {
            // Given two matrices of (potentially) unequal size, convert the
            // smaller in each dimension to match the larger
            self::_resizeMatricesExtend($operand1, $operand2, $matrix1Rows, 
                $matrix1Columns, $matrix2Rows, $matrix2Columns);
        } elseif ($resize == 1) {
            // Given two matrices of (potentially) unequal size, convert the
            // larger in each dimension to match the smaller
    ...
    /**
     * Read the dimensions of a matrix, and re-index it with straight numeric
     * keys starting from row 0, column 0
     *
     * @param mixed &$matrix matrix operand
     * @return array An array comprising the number of rows, and number of
     *         columns
     */
    ...
    private static function _resizeMatricesShrink(&$matrix1, &$matrix2, 
        $matrix1Rows, $matrix1Columns, $matrix2Rows, $matrix2Columns) {
    ...
    /**
     * Format details of an operand for display in the log (based on operand
     * type)
     *
    ...
    /**
     * Format type and details of an operand for display in the log (based on
     * operand type)
     *
     * @param mixed $value First matrix operand
     * @return mixed
    ...
    private static function _convertMatrixReferences($formula) {
        ...
        if (strpos($formula, '{') !== FALSE) {
            ...
            if (strpos($formula, '"') !== FALSE) {
                ...
                // the formula
                $temp = explode('"', $formula);
                // Open and Closed counts used for trapping mismatched braces in
                // the formula
    ...
    // Array key is the operator, the value indicates whether this is a left or
    ...
    // These operators work on two values, but always return a
    // boolean result
    ...
    private function _parseFormula($formula, PHPExcel_Cell $pCell = NULL) {
        ...
        // If we're using cell caching, then $pCell may well be flushed back to
        // the cache (which detaches the parent worksheet),
        // so we store the parent worksheet so that we can re-attach it when
        // necessary
        $pCellParent = ($pCell !== NULL) ? $pCell->getWorksheet() : NULL;
        ...
        $output = array();
        $expectingOperator = FALSE; // We use this test in syntax-checking the
                                    // expression to determine when a
                                    // - is a negation or + is a positive
                                    // operator rather than an operation
        $expectingOperand = FALSE; // We use this test in syntax-checking the
                                   // expression to determine whether an operand
                                   // should be null in a function call
                                   // The guts of the lexical parser
                                   // Loop through the formula extracting each
                                   // operator and operand in turn
        while (TRUE) {
            // echo 'Assessing Expression '.substr($formula, $index),PHP_EOL;
            $opCharacter = $formula{$index}; // Get the first character of the
                                             // value at the current index
                                             // position
                                             // echo 'Initial character of
                                             // expression block is
                                             // '.$opCharacter,PHP_EOL;
            if ((isset(self::$_comparisonOperators[$opCharacter])) &&
                ...
                // echo 'Initial character of expression block is comparison
                // operator '.$opCharacter.PHP_EOL;
            }
            
            // Find out if we're currently at the beginning of a number,
            // variable, cell reference, function, parenthesis or operand
            $isOperandOrFunction = preg_match($regexpMatchString, 
                substr($formula, $index), $match);
            // echo '$isOperandOrFunction is '.(($isOperandOrFunction) ? 'True'
            // : 'False').PHP_EOL;
            // var_dump($match);
            
            if ($opCharacter == '-' && ! $expectingOperator) { // Is it a
                                                               // negation
                                                               // instead of a
                                                               // minus?
                                                               // echo 'Element
                                                               // is a Negation
                                                               // operator',PHP_EOL;
                $stack->push('Unary Operator', '~'); // Put a negation on the
                                                     // stack
                ++ $index; // and drop the negation symbol
            } elseif ($opCharacter == '%' && $expectingOperator) {
                // echo 'Element is a Percentage operator',PHP_EOL;
                $stack->push('Unary Operator', '%'); // Put a percentage on the
                                                     // stack
                ++ $index;
            } elseif ($opCharacter == '+' && ! $expectingOperator) { // Positive
                                                                     // (unary
                                                                     // plus
                                                                     // rather
                                                                     // than
                ...
                ++ $index; // Drop the redundant plus symbol
                 ...
                 (! $isOperandOrFunction)) { // We
                                            ...
                                            // legal
                return $this->_raiseFormulaError(
                    "Formula Error: Illegal character '~'"); // on
                                                                 // the
                                                                 // stack
                                                                 // but
                                                                 // not
                                                                 ...
                                                                 // input
                                                                 // expression
            } elseif ((isset(self::$_operators[$opCharacter]) or
                 $isOperandOrFunction) && $expectingOperator) { // Are
                                                               // we
                                                               // putting
                                                               // an
                                                               // operator
                ...
                while ($stack->count() > 0 && ($o2 = $stack->last()) &&
                 isset(self::$_operators[$o2['value']]) && @(self::$_operatorAssociativity[$opCharacter] ? self::$_operatorPrecedence[$opCharacter] <
                 self::$_operatorPrecedence[$o2['value']] : self::$_operatorPrecedence[$opCharacter] <=
                 self::$_operatorPrecedence[$o2['value']])) {
                    $output[] = $stack->pop(); // Swap operands and higher
                                                   // precedence operators from the
                                                   // stack to the output
            }
            $stack->push('Binary Operator', $opCharacter); // Finally put our
                                                           // current operator
                                                           // onto the stack
            ++ $index;
            $expectingOperator = FALSE;
        } elseif ($opCharacter == ')' && $expectingOperator) { // Are we
                                                               // expecting to
                                                               // close a
                                                               // parenthesis?
                                                               // echo 'Element
            ...
            while (($o2 = $stack->pop()) && $o2['value'] != '(') { // Pop off
                                                                   // the stack
                                                                   // back to
                                                                   // the last (
                if ($o2 === NULL)
            ...
            if (preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/i', 
                ...
                $functionName = $matches[1]; // Get the function name
                                             // echo 'Closed Function is
                                             // '.$functionName,PHP_EOL;
                $d = $stack->pop();
                $argumentCount = $d['value']; // See how many arguments there
                                              // were (argument count is the
                                              // next value stored on the stack)
                                              // if ($argumentCount == 0) {
                                              // echo 'With no
                                              // arguments',PHP_EOL;
                                              ...
                                              // } else {
                                              // echo 'With '.$argumentCount.'
                                              // arguments',PHP_EOL;
                                              // }
                $output[] = $d; // Dump the argument count on the output
                $output[] = $stack->pop(); // Pop the function and push onto the
                                           // output
                ...
                } else { // did we somehow push a non-function on the stack?
                ...
                if (is_numeric($expectedArgumentCount)) {
                    if ($expectedArgumentCount < 0) {
                        // echo '$expectedArgumentCount is between 0 and
                        // '.abs($expectedArgumentCount),PHP_EOL;
                        ...
                        // echo '$expectedArgumentCount is numeric
                        // '.$expectedArgumentCount,PHP_EOL;
            ...
            ++ $index;
        } elseif ($opCharacter == ',') { // Is this the separator for function
                                         // arguments?
                                         // echo 'Element is a Function argument
                                         // separator',PHP_EOL;
            while (($o2 = $stack->pop()) && $o2['value'] != '(') { // Pop off
                                                                   // the stack
                                                                   // back to
                ...
                if ($o2 === NULL)
                    ...
                    $output[] = $o2; // pop the argument expression stuff and
                                         // push onto the output
            }
            // If we've a comma when we're expecting an operand, then what we
            // actually have is a null operand;
            ...
            $d = $stack->pop();
            $stack->push($d['type'], ++ $d['value'], $d['reference']); // increment
                                                                       // the
                                                                       // argument
                                                                       // count
            $stack->push('Brace', '('); // put the ( back on, we'll need to pop
                                        // back to it again
            $expectingOperator = FALSE;
            ...
            $stack->push('Brace', '(');
            ++ $index;
        } elseif ($isOperandOrFunction && ! $expectingOperator) { // do we now
                                                                  // have a
            ...
            $val = $match[1];
            $length = strlen($val);
            // echo 'Element with value '.$val.' is an Operand, Variable,
            // Constant, String, Number, Cell Reference or Function<br />';
            ...
            if (preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/i', 
                ...
                if (isset(self::$_PHPExcelFunctions[strtoupper($matches[1])]) ||
                     isset(self::$_controlFunctions[strtoupper($matches[1])])) { // it's
                                                                                // a
                                                                                // function
                ...
                // Watch for this case-change when modifying to allow cell
                // references in different worksheets...
                // Should only be applied to the actual cell column, not the
                // worksheet name
                
                // If the last entry on the stack was a : operator, then we have
                // a cell range reference
                $testPrevOp = $stack->last(1);
                if ($testPrevOp['value'] == ':') {
                    // If we have a worksheet reference, then we're playing with
                    // a 3D reference
                    if ($matches[2] == '') {
                        // Otherwise, we 'inherit' the worksheet reference from
                        // the start cell reference
                        // The start of the cell range reference should be the
                        // last entry in $output
                        $startCellRef = $output[count($output) - 1]['value'];
                        preg_match(
                            '/^' . self::CALCULATION_REGEXP_CELLREF . '$/i', 
                            $startCellRef, $startMatches);
                        if ($startMatches[2] > '') {
                ...
                // $expectingOperator = FALSE;
                     ...
                     // echo 'Element is a Variable, Constant, String, Number or
                     // Boolean<br />';
                     // If the last entry on the stack was a : operator, then we
                     // may have a row or column range reference
                $testPrevOp = $stack->last(1);
                if ($testPrevOp['value'] == ':') {
                    ...
                    if ((is_integer($startRowColRef)) && (ctype_digit($val)) &&
                        ...
                        // Row range
                        $endRowColRef = ($pCellParent !== NULL) ? $pCellParent->getHighestColumn() : 'XFD'; // Max
                                                                                                            // 16,384
                                                                                                            // columns
                                                                                                            // for
                                                                                                            // Excel2007
                        $output[count($output) - 1]['value'] = $rangeWS1 . 'A' .
                             $startRowColRef;
                        $val = $rangeWS2 . $endRowColRef . $val;
                    } elseif ((ctype_alpha($startRowColRef)) &&
                         (ctype_alpha($val)) && (strlen($startRowColRef) <= 3) &&
                         (strlen($val) <= 3)) {
                        // Column range
                        $endRowColRef = ($pCellParent !== NULL) ? $pCellParent->getHighestRow() : 1048576; // Max
                                                                                                           // 1,048,576
                                                                                                           // rows
                                                                                                           // for
                                                                                                           // Excel2007
                        $output[count($output) - 1]['value'] = $rangeWS1 .
                             strtoupper($startRowColRef) . '1';
                ...
                if ($opCharacter == '"') {
                    ...
                    $val = self::_wrapResult(
                        str_replace('""', '"', self::_unwrapResult($val)));
                    ...
                    trim(strtoupper($val)), self::$_localeBoolean)) !== FALSE) {
        ...
        if ($formula{$index} == ' ') {
            ...
            // If we're expecting an operator, but only have a space between the
            // previous and next operands (and both are
            ...
            $stack->push('Binary Operator', '|'); // Put an Intersect Operator
                                                  // on the stack
            $expectingOperator = FALSE;
        }
    }
...
while (($op = $stack->pop()) !== NULL) { // pop everything off the stack and
                                         // push onto output
    if ((is_array($op) && $op['value'] == '(') || ($op === '('))
        return $this->_raiseFormulaError("Formula Error: Expecting ')'"); // if
                                                                              // there
                                                                              // are
                                                                              // any
                                                                              // opening
...
private function _processTokenStack($tokens, $cellID = NULL, 
PHPExcel_Cell $pCell = NULL) {
if ($tokens == FALSE)
    return FALSE;
    
    // If we're using cell caching, then $pCell may well be flushed back to the
    // cache (which detaches the parent cell collection),
    // so we store the parent cell collection so that we can re-attach it when
    // necessary
...
foreach ($tokens as $tokenData) {
    ...
    // if the token is a binary operator, pop the top two values off the stack,
    // do the operation, and push the result back on the stack
    if (isset(self::$_binaryOperators[$token])) {
        ...
        switch ($token) {
            ...
            case ':': // Range
                ...
                if ($sheet1 == $sheet2) {
                    if ($operand1Data['reference'] === NULL) {
                        if ((trim($operand1Data['value']) != '') &&
                            ...
                            $operand1Data['reference'] = $pCell->getColumn() .
                             $operand1Data['value'];
                ...
                if ($operand2Data['reference'] === NULL) {
                    if ((trim($operand2Data['value']) != '') &&
                        ...
                        $operand2Data['reference'] = $pCell->getColumn() .
                         $operand2Data['value'];
            ...
            $cellRef = PHPExcel_Cell::stringFromColumnIndex(min($oCol)) .
                 min($oRow) . ':' .
        ...
        break;
    case '+': // Addition
        $this->_executeNumericBinaryOperation($cellID, $operand1, $operand2, 
            $token, 'plusEquals', $stack);
        break;
    case '-': // Subtraction
        $this->_executeNumericBinaryOperation($cellID, $operand1, $operand2, 
            $token, 'minusEquals', $stack);
        break;
    case '*': // Multiplication
        $this->_executeNumericBinaryOperation($cellID, $operand1, $operand2, 
            $token, 'arrayTimesEquals', $stack);
        break;
    case '/': // Division
        $this->_executeNumericBinaryOperation($cellID, $operand1, $operand2, 
            $token, 'arrayRightDivide', $stack);
        break;
    case '^': // Exponential
        $this->_executeNumericBinaryOperation($cellID, $operand1, $operand2, 
            $token, 'power', $stack);
    ...
    case '&': // Concatenation
              ...
              // both as matrices
              // (converting the other operand to a matrix if need be); then
        ...
        if ((is_array($operand1)) || (is_array($operand2))) {
            ...
            try {
                ...
                // Perform the required operation against the operand 1 matrix,
                // passing in operand 2
    ...
    case '|': // Intersect
        ...
        foreach (array_keys($rowIntersect) as $row) {
            ...
            foreach ($rowIntersect[$row] as $col => $data) {
                ...
                $cellIntersect[$row] = array_intersect_key($operand1[$row], 
                    $operand2[$row]);
            }
        }
...
if (is_array($arg)) {
    ...
    $this->_executeNumericBinaryOperation($cellID, $multiplier, $arg, '*', 
        'arrayTimesEquals', $stack);
}
} elseif (preg_match('/^' . self::CALCULATION_REGEXP_CELLREF . '$/i', $token, 
$matches)) {
$cellRef = NULL;
...
if (isset($matches[8])) {
    ...
    if ($pCell === NULL) {
        ...
        if ($matches[2] > '') {
            ...
            if ((strpos($matches[2], '[') !== FALSE) ||
                ...
                // It's a Reference to an external workbook (not currently
                // supported)
                return $this->_raiseFormulaError(
            ...
            if ($pCellParent !== NULL) {
                $cellValue = $this->extractCellRange($cellRef, $pCellWorksheet, 
                    FALSE);
            } else {
                return $this->_raiseFormulaError(
                    'Unable to access Cell Reference');
            ...
            $this->_debugLog->writeDebugLog('Evaluation Result for cells ', 
                $cellRef, ' is ', $this->_showTypeDetails($cellValue));
        }
    }
} else {
    ...
    if ($pCell === NULL) {
        ...
        if ($matches[2] > '') {
            ...
            // echo '$cellRef='.$cellRef.' in worksheet '.$matches[2].'<br />';
            $this->_debugLog->writeDebugLog('Evaluating Cell ', $cellRef, 
                ' in worksheet ', $matches[2]);
            if ($pCellParent !== NULL) {
...
$stack->push('Value', $cellValue, $cellRef);
    ...
    // if the token is a function, pop arguments off the stack, hand them to the
    // function, and push the result back on
} elseif (preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/i', $token, 
$matches)) {
...
if ((isset(self::$_PHPExcelFunctions[$functionName])) ||
     (isset(self::$_controlFunctions[$functionName]))) { // function
    if (isset(self::$_PHPExcelFunctions[$functionName])) {
        $functionCall = self::$_PHPExcelFunctions[$functionName]['functionCall'];
        $passByReference = isset(
            self::$_PHPExcelFunctions[$functionName]['passByReference']);
        $passCellReference = isset(
            self::$_PHPExcelFunctions[$functionName]['passCellReference']);
    } elseif (isset(self::$_controlFunctions[$functionName])) {
        $functionCall = self::$_controlFunctions[$functionName]['functionCall'];
        $passByReference = isset(
            self::$_controlFunctions[$functionName]['passByReference']);
        $passCellReference = isset(
            self::$_controlFunctions[$functionName]['passCellReference']);
    }
    ...
    // if (($argCount == 1) && (is_array($args[1])) && ($functionName !=
    // 'MKMATRIX')) {
    // $operand1 = $args[1];
    // $this->_debugLog->writeDebugLog('Argument is a matrix: ',
    ...
    // foreach($args as $arg) {
    // $this->_debugLog->writeDebugLog('Evaluating ',
    // self::_localeFunc($functionName), '( ', $this->_showValue($arg), ' )');
    // $r = call_user_func_array($functionCall,$arg);
    // $this->_debugLog->writeDebugLog('Evaluation Result for ',
    // self::_localeFunc($functionName), '() function call is ',
    // $this->_showTypeDetails($r));
    // $result[$row][] = $r;
    // }
    // ++$row;
    // } else {
    // $this->_debugLog->writeDebugLog('Evaluating ',
    // self::_localeFunc($functionName), '( ', $this->_showValue($args), ' )');
...
// if the token is a number, boolean, string or an Excel error, push it onto the
// stack
if (isset(self::$_ExcelConstants[strtoupper($token)])) {
    $excelConstant = strtoupper($token);
    // echo 'Token is a PHPExcel constant: '.$excelConstant.'<br />';
    $stack->push('Constant Value', self::$_ExcelConstants[$excelConstant]);
    $this->_debugLog->writeDebugLog('Evaluating Constant ', $excelConstant, 
...
} elseif (preg_match('/^' . self::CALCULATION_REGEXP_NAMEDRANGE . '$/i', $token, 
    $matches)) {
    ...
    $this->_debugLog->writeDebugLog('Evaluation Result for named range ', 
        $namedRange, ' is ', $this->_showTypeDetails($cellValue));
...
// when we're out of tokens, the stack should have a single element, the final
// result
if ($stack->count() != 1)
...
// if ((is_array($output)) && (self::$returnArrayAsType !=
...
// Numbers, matrices and booleans can pass straight through, as they're already
// valid
if (is_string($operand)) {
// We only need special validations for the operand if it is a string
// Start by stripping off the quotation marks we use to identify true excel
// string values internally
if ($operand > '' && $operand{0} == '"') {
$operand = self::_unwrapResult($operand);
}
// If the string is a numeric value, we treat it as a numeric, so no further
// testing
if (! is_numeric($operand)) {
// If not a numeric, test to see if the value is an Excel error, and so can't be
// used in normal binary operations
if ($operand > '' && $operand{0} == '#') {
    ...
    // If not a numeric or a fraction, then it's a text string, and so can't be
    // used in mathematical binary operations
...
// return a true if the value of the operand is one that we can use in normal
// binary operations
return TRUE;
}
// function _validateBinaryOperand()
private function _executeBinaryComparisonOperation($cellID, $operand1, $operand2, 
$operation, &$stack, $recursingArrays = FALSE) {
...
foreach ($operand1 as $x => $operandData) {
    $this->_debugLog->writeDebugLog('Evaluating Comparison ', 
        ...
        $this->_showValue($operand2));
    $this->_executeBinaryComparisonOperation($cellID, $operandData, $operand2, 
        $operation, $stack);
    $r = $stack->pop();
    $result[$x] = $r['value'];
}
...
foreach ($operand2 as $x => $operandData) {
    ...
    $this->_executeBinaryComparisonOperation($cellID, $operand1, $operandData, 
        $operation, $stack);
    $r = $stack->pop();
    $result[$x] = $r['value'];
}
...
foreach ($operand1 as $x => $operandData) {
    ...
    $this->_executeBinaryComparisonOperation($cellID, $operandData, 
        $operand2[$x], $operation, $stack, TRUE);
    $r = $stack->pop();
    $result[$x] = $r['value'];
}
}
...
$useLowercaseFirstComparison = is_string($operand1) && is_string($operand2) && PHPExcel_Calculation_Functions::getCompatibilityMode() ==
 PHPExcel_Calculation_Functions::COMPATIBILITY_OPENOFFICE;
...
/**
 * Compare two strings in the same way as strcmp() except that lowercase come
...
private function _executeNumericBinaryOperation($cellID, $operand1, $operand2, 
...
return FALSE;
    ...
    // If either of the operands is a matrix, we need to treat them both as
    // matrices
    // (converting the other operand to a matrix if need be); then perform the
    // required
    // matrix operation
if ((is_array($operand1)) || (is_array($operand2))) {
...
// Convert operand 1 from a PHP array to a matrix
$matrix = new PHPExcel_Shared_JAMA_Matrix($operand1);
// Perform the required operation against the operand 1 matrix, passing in
// operand 2
 ...
 * @param boolean $resetLog Flag indicating whether calculation log should be
 *            reset or not
 * @return mixed Array of values in range if range contains more than one
 *         element. Otherwise, a single value is returned.
 * @throws PHPExcel_Calculation_Exception
 */
public function extractCellRange(&$pRange = 'A1', PHPExcel_Worksheet $pSheet = NULL, 
$resetLog = TRUE) {
// Return value
$returnValue = array();
...
list ($pSheetName, $pRange) = PHPExcel_Worksheet::extractSheetTitle($pRange, 
    true);
 ...
 * @throws PHPExcel_Calculation_Exception
 */
public function extractNamedRange(&$pRange = 'A1', PHPExcel_Worksheet $pSheet = NULL, 
$resetLog = TRUE) {
// Return value
$returnValue = array();
...
if (ctype_alpha($splitRange[0][0])) {
    ...
    // echo $pRange.') is in sheet
    // '.$namedRange->getWorksheet()->getTitle().'<br />';
...
list ($currentCol, $currentRow) = PHPExcel_Cell::coordinateFromString(
    $aReferences[0]);
$cellValue = NULL;
if ($pSheet->cellExists($aReferences[0])) {
    $returnValue[$currentRow][$currentCol] = $pSheet->getCell($aReferences[0])->getCalculatedValue(
...
foreach ($aReferences as $reference) {
    ...
    list ($currentCol, $currentRow) = PHPExcel_Cell::coordinateFromString(
        $reference);
    // echo 'NAMED RANGE: $currentCol='.$currentCol.'
...
$returnValue[$functionName] = new PHPExcel_Calculation_Function(
    $function['category'], $functionName, $function['functionCall']);
}
}
