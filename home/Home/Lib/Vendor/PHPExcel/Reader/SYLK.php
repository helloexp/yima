...
class PHPExcel_Reader_SYLK extends PHPExcel_Reader_Abstract implements 
    ...
    public function listWorksheetInfo($pFilename) {
        ...
        while (($rowData = fgets($fileHandle)) !== FALSE) {
            ...
            // explode each row at semicolons while taking into account that
            // literal semicolon (;)
            ...
            if ($dataType == 'C') {
                ...
                foreach ($rowData as $rowDatum) {
                    ...
                    $worksheetInfo[0]['totalRows'] = max(
                        $worksheetInfo[0]['totalRows'], $rowIndex);
                    $worksheetInfo[0]['lastColumnIndex'] = max(
    ...
    public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel) {
        ...
        while (($rowData = fgets($fileHandle)) !== FALSE) {
            ...
            if ($dataType == 'P') {
                    ...
                    switch ($rowDatum{0}) {
                        case 'P':
                            $formatArray['numberformat']['code'] = str_replace(
                                $fromFormats, $toFormats, substr($rowDatum, 1));
                            break;
                        case 'E':
                        case 'F':
                        ...
                        case 'E':
                            ...
                            foreach ($temp as &$value) {
                                ...
                                if ($key = ! $key) {
                                    ...
                                    // Reverse the matches array, otherwise all
                                    // our offsets will become incorrect if we
                                    // modify our way
                                    // through the formula from left to right.
                                    // Reversing means that we work right to
                                    // left.through
                                    // the formula
                                    $cellReferences = array_reverse(
                                        $cellReferences);
                                    // Loop through each R1C1 style reference in
                                    // turn, converting it to its A1 style
                                    // equivalent,
                                    // then modify the formula to use that new
                                    // reference
                                    foreach ($cellReferences as $cellReference) {
                                        ...
                                        // Empty C reference is the current
                                        // column
                                        if ($columnReference == '')
                                        ...
                                        $A1CellReference = PHPExcel_Cell::stringFromColumnIndex(
                                            $columnReference - 1) . $rowReference;
                ...
                $columnLetter = PHPExcel_Cell::stringFromColumnIndex(
                    $column - 1);
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum{0}) {
                        ...
                        case 'P':
                            ...
                            break;
                        case 'W':
                            list ($startCol, $endCol, $columnWidth) = explode(
                                ' ', substr($rowDatum, 1));
                            break;
                        case 'S':
                            $styleSettings = substr($rowDatum, 1);
                ...
                if ($columnWidth > '') {
                    if ($startCol == $endCol) {
                        $startCol = PHPExcel_Cell::stringFromColumnIndex(
                            $startCol - 1);
                        $objPHPExcel->getActiveSheet()
                        ...
                        $startCol = PHPExcel_Cell::stringFromColumnIndex(
                            $startCol - 1);
                        $endCol = PHPExcel_Cell::stringFromColumnIndex(
                            $endCol - 1);
                        $objPHPExcel->getActiveSheet()
                            ->getColumnDimension($startCol)
                            ->setWidth($columnWidth);
