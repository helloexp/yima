...
class PHPExcel_Reader_Excel2003XML extends PHPExcel_Reader_Abstract implements 
    ...
    public function listWorksheetNames($pFilename) {
        ...
        if (! $this->canRead($pFilename)) {
            throw new PHPExcel_Reader_Exception(
                $pFilename . " is an Invalid Spreadsheet file.");
        }
        ...
        foreach ($xml_ss->Worksheet as $worksheet) {
            ...
            $worksheetNames[] = self::_convertStringEncoding(
                (string) $worksheet_ss['Name'], $this->_charSet);
        }
    ...
    public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel) {
        ...
        if (isset($xml->DocumentProperties[0])) {
            foreach ($xml->DocumentProperties[0] as $propertyName => $propertyValue) {
                switch ($propertyName) {
                    case 'Title':
                        $docProps->setTitle(
                            self::_convertStringEncoding($propertyValue, 
                                $this->_charSet));
                        break;
                    case 'Subject':
                        $docProps->setSubject(
        ...
        if (isset($xml->CustomDocumentProperties)) {
            foreach ($xml->CustomDocumentProperties[0] as $propertyName => $propertyValue) {
                $propertyAttributes = $propertyValue->attributes(
                    $namespaces['dt']);
                ...
                switch ($styleType) {
                    case 'Alignment':
                        foreach ($styleAttributes as $styleAttributeKey => $styleAttributeValue) {
                            // echo $styleAttributeKey.' =
                            // '.$styleAttributeValue.'<br />';
                    ...
                    case 'Borders':
                        foreach ($styleData->Border as $borderStyle) {
                            $borderAttributes = $borderStyle->attributes(
                                $namespaces['ss']);
                            $thisBorder = array();
                        ...
                        foreach ($styleAttributes as $styleAttributeKey => $styleAttributeValue) {
                            ...
                            switch ($styleAttributeKey) {
                                ...
                                case 'Size':
                                    ...
                                    break;
                                case 'Color':
                                    $this->_styles[$styleID]['font']['color']['rgb'] = substr(
                                        $styleAttributeValue, 1);
                                    break;
                                case 'Bold':
                                ...
                                case 'Underline':
                                    if (self::identifyFixedStyleValue(
                                        $underlineStyles, $styleAttributeValue)) {
                    ...
                    case 'Interior':
                        foreach ($styleAttributes as $styleAttributeKey => $styleAttributeValue) {
                            ...
                            switch ($styleAttributeKey) {
                                case 'Color':
                                    $this->_styles[$styleID]['fill']['color']['rgb'] = substr(
                                        $styleAttributeValue, 1);
                                    break;
                            }
                        }
        ...
        foreach ($xml_ss->Worksheet as $worksheet) {
            ...
            if (isset($worksheet_ss['Name'])) {
                $worksheetName = self::_convertStringEncoding(
                    (string) $worksheet_ss['Name'], $this->_charSet);
                // Use false for $updateFormulaCellReferences to prevent
                // adjustment of worksheet references in
                // formula cells... during the load, all formulae should be
                // correct, and we're simply bringing
            ...
            if (isset($worksheet->Table->Column)) {
                foreach ($worksheet->Table->Column as $columnData) {
                    ...
                    if (isset($columnData_ss['Index'])) {
                        $columnID = PHPExcel_Cell::stringFromColumnIndex(
                            $columnData_ss['Index'] - 1);
                    }
                    if (isset($columnData_ss['Width'])) {
                        $columnWidth = $columnData_ss['Width'];
                        // echo '<b>Setting column width for '.$columnID.' to
                        // '.$columnWidth.'</b><br />';
                ...
                foreach ($worksheet->Table->Row as $rowData) {
                    ...
                    foreach ($rowData->Cell as $cell) {
                        ...
                        if ($this->getReadFilter() !== NULL) {
                            if (! $this->getReadFilter()->readCell($columnID, 
                                $rowID, $worksheetName)) {
                                continue;
                            }
                        }
                            ...
                            if (isset($cell_ss['MergeAcross'])) {
                                $columnTo = PHPExcel_Cell::stringFromColumnIndex(
                                    PHPExcel_Cell::columnIndexFromString(
                                        $columnID) + $cell_ss['MergeAcross'] - 1);
                            }
                            $rowTo = $rowID;
                        ...
                        if (isset($cell_ss['Formula'])) {
                            ...
                            if (isset($cell_ss['ArrayRange'])) {
                                ...
                                // echo "found an array formula at
                                // ".$columnID.$rowID."<br />";
                            }
                        ...
                        if (isset($cell->Data)) {
                            ...
                            $cellData_ss = $cellData->attributes(
                                $namespaces['ss']);
                            if (isset($cellData_ss['Type'])) {
                                $cellDataType = $cellData_ss['Type'];
                                switch ($cellDataType) {
                                    /*
                                     * const TYPE_STRING = 's'; const
                                     * TYPE_FORMULA = 'f'; const TYPE_NUMERIC =
                                     * 'n'; const TYPE_BOOL = 'b'; const
                                     * TYPE_NULL = 'null'; const TYPE_INLINE =
                                     * 'inlineStr'; const TYPE_ERROR = 'e';
                                     */
                                    case 'String':
                                        $cellValue = self::_convertStringEncoding(
                                            $cellValue, $this->_charSet);
                                    ...
                                    case 'DateTime':
                                        $type = PHPExcel_Cell_DataType::TYPE_NUMERIC;
                                        $cellValue = PHPExcel_Shared_Date::PHPToExcel(
                                            strtotime($cellValue));
                                        break;
                                    case 'Error':
                            ...
                            if ($hasCalculatedValue) {
                                ...
                                $columnNumber = PHPExcel_Cell::columnIndexFromString(
                                    $columnID);
                                if (substr($cellDataFormula, 0, 3) == 'of:') {
                                    ...
                                    foreach ($temp as &$value) {
                                        ...
                                        if ($key = ! $key) {
                                            ...
                                            // otherwise all our offsets will
                                            // become incorrect if we modify our
                                            // way
                                            // through the formula from left to
                                            // right. Reversing means that we
                                            // work right to left.through
                                            // the formula
                                            $cellReferences = array_reverse(
                                                $cellReferences);
                                            // Loop through each R1C1 style
                                            // reference in turn, converting it
                                            // to its A1 style equivalent,
                                            // then modify the formula to use
                                            // that new reference
                                            foreach ($cellReferences as $cellReference) {
                                                ...
                                                if ($rowReference == '')
                                                    $rowReference = $rowID;
                                                    // Bracketed R references
                                                    // are relative to the
                                                    // current
                                                    // row
                                                if ($rowReference{0} == '[')
                                                    $rowReference = $rowID + trim(
                                                        $rowReference, '[]');
                            ...
                            // echo 'Cell '.$columnID.$rowID.' is a '.$type.'
                            // with a value of '.(($hasCalculatedValue) ?
                            // $cellDataFormula : $cellValue).'<br />';
                            //
                            $objPHPExcel->getActiveSheet()
                        ...
                        if (isset($cell->Comment)) {
                            ...
                            $commentAttributes = $cell->Comment->attributes(
                                $namespaces['ss']);
                            $author = 'unknown';
                            ...
                            // $annotation =
                            // str_replace('html:','',substr($node,49,-10));
                            // echo $annotation,'<br />';
                            ...
                            $objPHPExcel->getActiveSheet()
                                ...
                                ->setAuthor(
                                self::_convertStringEncoding($author, 
                                    $this->_charSet))
                        ...
                        if (($cellIsSet) && (isset($cell_ss['StyleID']))) {
                            ...
                            // echo 'Cell style for '.$columnID.$rowID.' is
                            // '.$style.'<br />';
                            if ((isset($this->_styles[$style])) &&
                                ...
                                // echo '<br />';
                                if (! $objPHPExcel->getActiveSheet()->cellExists(
                                    $columnID . $rowID)) {
                                    $objPHPExcel->getActiveSheet()
                                        ->getCell($columnID . $rowID)
                                        ->setValue(NULL);
