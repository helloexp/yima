 ...
 *            (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Reader_Gnumeric extends PHPExcel_Reader_Abstract implements 
    ...
    public function listWorksheetNames($pFilename) {
        ...
        $xml = new XMLReader();
        $xml->open('compress.zlib://' . realpath($pFilename), null, 
        ...
        while ($xml->read()) {
            if ($xml->name == 'gnm:SheetName' &&
                ...
                // break out of the loop once we've got our sheet names rather
                // than parse the entire file
                break;
            }
        }
    ...
    public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel) {
        ...
        // echo '</pre><hr />';
        //
        $xml = simplexml_load_string($gFileData, 'SimpleXMLElement', 
            PHPExcel_Settings::getLibXmlLoaderOptions());
        ...
        // Document Properties are held differently, depending on the version of
        // Gnumeric
        ...
        foreach ($gnmXML->Sheets->Sheet as $sheet) {
            ...
            if ((! $this->_readDataOnly) && (isset($sheet->PrintInformation))) {
                if (isset($sheet->PrintInformation->Margins)) {
                    foreach ($sheet->PrintInformation->Margins->children('gnm', 
                        TRUE) as $key => $margin) {
            ...
            foreach ($sheet->Styles->StyleRegion as $styleRegion) {
                ...
                if (($styleAttributes['startRow'] <= $maxRow) &&
                    ...
                    $startColumn = PHPExcel_Cell::stringFromColumnIndex(
                        (int) $styleAttributes['startCol']);
                    $startRow = $styleAttributes['startRow'] + 1;
                    
                    $endColumn = ($styleAttributes['endCol'] > $maxCol) ? $maxCol : (int) $styleAttributes['endCol'];
                    $endColumn = PHPExcel_Cell::stringFromColumnIndex(
                        $endColumn);
                    ...
                    // We still set the number format mask for date/time values,
                    // even if _readDataOnly is true
                    if ((! $this->_readDataOnly) || (PHPExcel_Shared_Date::isDateTimeFormatCode(
                        ...
                        // If _readDataOnly is false, we set all formatting
                        // information
                        if (! $this->_readDataOnly) {
                            ...
                            $styleArray['alignment']['wrap'] = ($styleAttributes['WrapText'] ==
                                 '1') ? True : False;
                            $styleArray['alignment']['shrinkToFit'] = ($styleAttributes['ShrinkToFit'] ==
                                 '1') ? True : False;
                            $styleArray['alignment']['indent'] = (intval(
                                $styleAttributes["Indent"]) > 0) ? $styleAttributes["indent"] : 0;
                            
                            $RGB = self::_parseGnumericColour(
                                $styleAttributes["Fore"]);
                            $styleArray['font']['color']['rgb'] = $RGB;
                            $RGB = self::_parseGnumericColour(
                                $styleAttributes["Back"]);
                            $shade = $styleAttributes["Shade"];
                            if (($RGB != '000000') || ($shade != '0')) {
                                $styleArray['fill']['color']['rgb'] = $styleArray['fill']['startcolor']['rgb'] = $RGB;
                                $RGB2 = self::_parseGnumericColour(
                                    $styleAttributes["PatternColor"]);
                            ...
                            $styleArray['font']['size'] = intval(
                                $fontAttributes['Unit']);
                            $styleArray['font']['bold'] = ($fontAttributes['Bold'] ==
                                 '1') ? True : False;
                            $styleArray['font']['italic'] = ($fontAttributes['Italic'] ==
                                 '1') ? True : False;
                            $styleArray['font']['strike'] = ($fontAttributes['StrikeThrough'] ==
                                 '1') ? True : False;
            ...
            if ((! $this->_readDataOnly) && (isset($sheet->Cols))) {
                ...
                foreach ($sheet->Cols->ColInfo as $columnOverride) {
                    ...
                    $hidden = ((isset($columnAttributes['Hidden'])) &&
                         ($columnAttributes['Hidden'] == '1')) ? true : false;
            ...
            if ((! $this->_readDataOnly) && (isset($sheet->Rows))) {
                ...
                foreach ($sheet->Rows->RowInfo as $rowOverride) {
                    ...
                    $hidden = ((isset($rowAttributes['Hidden'])) &&
                         ($rowAttributes['Hidden'] == '1')) ? true : false;
            ...
            foreach ($gnmXML->Names->Name as $namedRange) {
                ...
                if ($worksheet = $objPHPExcel->getSheetByName($range[0])) {
                    ...
                    $objPHPExcel->addNamedRange(
                        new PHPExcel_NamedRange($name, $worksheet, 
                            $extractedRange));
                }
            }
        }
