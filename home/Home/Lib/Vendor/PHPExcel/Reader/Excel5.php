<?php
/**
 * PHPExcel Copyright (c) 2006 - 2014 PHPExcel This library is free software;
...
/**
 * PHPExcel_Reader_Excel5 This class uses {@link
 * http://sourceforge.net/projects/phpexcelreader/parseXL}
 *
 * @category PHPExcel
 * @package PHPExcel_Reader_Excel5
 * @copyright Copyright (c) 2006 - 2014 PHPExcel
...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements 
    ...
    /**
     * Workbook stream data. (Includes workbook globals substream as well as
     * sheet substreams)
     *
     * @var string
     */
     ...
     * (Excel 5.0 - Excel 95) For BIFF8 (Excel 97 - Excel 2003) this will always
     * have the value 'UTF-16LE'
     *
     * @var string
     */
    private $_codepage;
    ...
    /**
     * Fit printout to number of pages? (in sheet currently being read). See
     * SHEETPR record.
     *
     * @var boolean
     */
    ...
    /**
     * Mapping of XF index (that is a style XF) to final index in cellStyleXf
     * collection
     *
     * @var array
     */
    ...
    /**
     * The shared formulas in a sheet. One SHAREDFMLA record contributes with
     * one value.
     *
     * @var array
     */
    ...
    /**
     * The shared formula parts in a sheet. One FORMULA record contributes with
     * one value if it refers to a shared formula.
     *
     * @var array
     */
    ...
    /**
     * Reads names of the worksheets from a file, without parsing the whole file
                ...
                switch ($code) {
                    ...
                    case self::XLS_Type_LABEL:
                        ...
                        $tmpInfo['totalRows'] = max($tmpInfo['totalRows'], 
                            $rowIndex);
                        $tmpInfo['lastColumnIndex'] = max(
                            $tmpInfo['lastColumnIndex'], $columnIndex);
                        break;
                    case self::XLS_Type_BOF:
                        $this->_readBof();
                        break;
    ...
    public function load($pFilename) {
        ...
        if (! $this->_readDataOnly) {
            $this->_phpExcel->removeCellStyleXfByIndex(0); // remove the default
                                                           // style
            $this->_phpExcel->removeCellXfByIndex(0); // remove the default
                                                          // style
        }
        ...
        // Read the Additional document summary information stream (containing
        ...
        // Cannot be resolved already in XF record, because PALETTE record comes
        // afterwards
        if (! $this->_readDataOnly) {
            ...
            foreach ($this->_phpExcel->getCellXfCollection() as $objStyle) {
                ...
                if (isset($diagonal->colorIndex)) {
                    $borderDiagonalColor = self::_readColor(
                        $diagonal->colorIndex, $this->_palette, $this->_version);
        ...
        foreach ($this->_sheets as $sheet) {
            ...
            // Use false for $updateFormulaCellReferences to prevent adjustment
            // of worksheet references in formula
            // cells... during the load, all formulae should be correct, and
            // we're simply bringing the worksheet
            ...
            if (! $this->_readDataOnly && $this->_drawingData) {
                ...
                // get all spContainers in one long array, so they can be mapped
                // to OBJ records
            ...
            foreach ($this->_objs as $n => $obj) {
                ...
                if (isset($allSpContainers[$n + 1]) &&
                    ...
                    // we skip all spContainers that are a part of a group shape
                    // since we cannot yet handle those
                    ...
                    list ($endColumn, $endRow) = PHPExcel_Cell::coordinateFromString(
                        $spContainer->getEndCoordinates());
                    ...
                    $width = PHPExcel_Shared_Excel5::getDistanceX(
                        $this->_phpSheet, $startColumn, $startOffsetX, 
                        $endColumn, $endOffsetX);
                    $height = PHPExcel_Shared_Excel5::getDistanceY(
                        $this->_phpSheet, $startRow, $startOffsetY, $endRow, 
                        $endOffsetY);
                    
                    // calculate offsetX and offsetY of the shape
                    $offsetX = $startOffsetX * PHPExcel_Shared_Excel5::sizeCol(
                        $this->_phpSheet, $startColumn) / 1024;
                    $offsetY = $startOffsetY * PHPExcel_Shared_Excel5::sizeRow(
                        $this->_phpSheet, $startRow) / 256;
                            ...
                            if ($blip = $BSE->getBlip()) {
                                ...
                                switch ($blipType) {
                                    case PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG:
                                        ...
                                        $drawing->setMimeType(
                                            PHPExcel_Worksheet_MemoryDrawing::MIMETYPE_JPEG);
                                        break;
                                    
                                    case PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG:
                                        $drawing->setRenderingFunction(
                                            PHPExcel_Worksheet_MemoryDrawing::RENDERING_PNG);
                                        $drawing->setMimeType(
                                            PHPExcel_Worksheet_MemoryDrawing::MIMETYPE_PNG);
                                        break;
                                }
            ...
            if ($this->_version == self::XLS_BIFF8) {
                foreach ($this->_sharedFormulaParts as $cell => $baseCell) {
                    ...
                    if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell(
                        $column, $row, $this->_phpSheet->getTitle())) {
                        $formula = $this->_getFormulaFromStructure(
                            $this->_sharedFormulas[$baseCell], $cell);
            ...
            if ($definedName['isBuiltInName']) {
                switch ($definedName['name']) {
                    ...
                    case pack('C', 0x06):
                        ...
                        $ranges = explode(',', $definedName['formula']); // FIXME:
                                                                         // what
                                                                         // if
                                                                         // sheetname
                                                                         // contains
                        ...
                        foreach ($ranges as $range) {
                            ...
                            $explodes = explode('!', $range); // FIXME: what if
                                                              // sheetname
                                                              // contains
                                                              // exclamation
                                                              // mark?
                        ...
                        if ($docSheet = $this->_phpExcel->getSheetByName(
                            $sheetName)) {
                            $docSheet->getPageSetup()->setPrintArea(
                                implode(',', $extractedRanges)); // C7:J66,A1:IV2
                        }
                        break;
                ...
                if (count($explodes) == 2) {
                    if (($docSheet = $this->_phpExcel->getSheetByName(
                        $explodes[0])) || ($docSheet = $this->_phpExcel->getSheetByName(
                        trim($explodes[0], "'")))) {
                        $extractedRange = $explodes[1];
                        ...
                        $this->_phpExcel->addNamedRange(
                            new PHPExcel_NamedRange(
                                (string) $definedName['name'], $docSheet, 
    ...
    private function _readRecordData($data, $pos, $len) {
        ...
        if ($this->_encryption == self::MS_BIFF_CRYPTO_RC4) {
            ...
            // Spin an RC4 decryptor to the right spot. If we have a decryptor
            // sitting
            // at a point earlier in the current block, re-use it as we can save
            // some time.
    ...
    private function _loadOLE($pFilename) {
        ...
        $this->_documentSummaryInformation = $ole->getStream(
    ...
    private function _readSummaryInformation() {
        ...
        // offset: 28; size: 16; first section's class id: e0 85 9f f2 f9 4f 68
        // 10 ab 91 08 00 2b 27 b3 d9
        // offset: 44; size: 4
        ...
        for ($i = 0; $i < $countProperties; ++ $i) {
            ...
            switch ($type) {
                ...
                case 0x1E: // null-terminated string prepended by dword string
                           // length
                ...
                case 0x40: // Filetime (64-bit value representing the number of
                           // 100-nanosecond intervals since January 1, 1601)
                           // PHP-time
                    $value = PHPExcel_Shared_OLE::OLE2LocalDate(
                        substr($this->_summaryInformation, 
    ...
    private function _readDocumentSummaryInformation() {
        ...
        // offset: 28; size: 16; first section's class id: 02 d5 cd d5 9c 2e 1b
        // 10 93 97 08 00 2b 2c f9 ae
        ...
        for ($i = 0; $i < $countProperties; ++ $i) {
            ...
            // Use value of property id as appropriate
            // offset: 60 + 8 * $i; size: 4; offset from beginning of section
            // (48)
            ...
            switch ($type) {
                ...
                case 0x40: // Filetime (64-bit value representing the number of
                           ...
                           // PHP-Time
                    $value = PHPExcel_Shared_OLE::OLE2LocalDate(
                        substr($this->_documentSummaryInformation, 
                            $secOffset + 4 + $offset, 8));
                    break;
    ...
    /**
     * The NOTE record specifies a comment associated with a particular cell. In
     * Excel 95 (BIFF7) and earlier versions, this record stores a note (cell
     * note). This feature was significantly enhanced in Excel 97.
     */
    private function _readNote() {
        ...
        if ($this->_version == self::XLS_BIFF8) {
            ...
            if ($cellAddress == '$B$65536') {
                ...
                // note from the previous cell annotation. We're not yet
                // handling this, so annotations longer than the
                // max 2048 bytes will probably throw a wobbly.
                $row = self::_GetInt2d($recordData, 0);
                $extension = true;
                $cellAddress = array_pop(
                    array_keys($this->_phpSheet->getComments()));
            }
            // echo 'Note Address=',$cellAddress,'<br />';
            ...
            if ($extension) {
                // Concatenate this extension with the currently set comment for
                // the cell
                $comment = $this->_phpSheet->getComment($cellAddress);
                $commentText = $comment->getText()->getPlainText();
                $comment->setText(
                    $this->_parseRichText($commentText . $noteText));
            } else {
    ...
    private function _readTextObject() {
        ...
        // cbRuns: 2 bytes; length of the formatting (in the second continue
        // record)
        // followed by the continuation records containing the actual text and
        // formatting
        ...
        $this->_textObjects[$this->textObjRef] = array(
            'text' => substr($text["recordData"], $text["spliceOffsets"][0] + 1, 
                $cchText), 
            'format' => substr($text["recordData"], $text["spliceOffsets"][1], 
                $cbRuns), 
            'alignment' => $grbitOpts, 
            'rotation' => $rot);
    ...
    /**
     * FILEPASS This record is part of the File Protection Block. It contains
     * information about the read/write password of the file. All record
     * contents following this record will be encrypted. -- "OpenOffice.org's
     * Documentation of the Microsoft Excel File Format" The decryption
     * functions and objects used from here on in are based on the source of
    ...
    private function _readFilepass() {
        ...
        if (! $this->_verifyPassword('VelvetSweatshop', 
        ...
        $this->_encryptionStartPos = $this->_pos +
             self::_GetInt2d($this->_data, $this->_pos + 2);
    }
     ...
     * @return bool Success
     */
    private function _verifyPassword($password, $docid, $salt_data, 
    ...
    /**
     * CODEPAGE This record stores the text encoding used to write byte strings,
     * stored as MS Windows code page identifier. -- "OpenOffice.org's
    ...
    /**
     * DATEMODE This record specifies the base date for displaying date values.
     * All dates are stored as count of days past this base date. In BIFF2-BIFF4
     * this record is part of the Calculation Settings Block. In BIFF5-BIFF8 it
     * is stored in the Workbook Globals Substream. -- "OpenOffice.org's
     * Documentation of the Microsoft Excel File Format"
     */
    ...
    /**
     * FORMAT This record contains information about a number format. All FORMAT
     * records occur together in a sequential list. In BIFF2-BIFF4 other records
     * referencing a FORMAT record contain a zero-based index into this list.
     * From BIFF5 on the FORMAT record contains the index itself that will be
     * used by other records. -- "OpenOffice.org's Documentation of the
    ...
    /**
     * XF - Extended Format This record contains formatting information for
     * cells, rows, columns or styles. According to
     * http://support.microsoft.com/kb/147732 there are always at least 15 cell
     * style XF and 1 cell XF. Inspection of Excel files generated by MS Office
     * Excel shows that XF records 0-14 are cell style XF and XF record 15 is a
     * cell XF We only read the first cell style XF and skip the remaining cell
     * style XF records We read all cell XF records. -- "OpenOffice.org's
     * Documentation of the Microsoft Excel File Format"
     */
    private function _readXf() {
        ...
        if (! $this->_readDataOnly) {
            ...
            } elseif (($code = PHPExcel_Style_NumberFormat::builtInFormatCode(
                $numberFormatIndex)) !== '') {
            ...
            switch ($vertAlign) {
                case 0:
                    $objStyle->getAlignment()->setVertical(
            ...
            if ($this->_version == self::XLS_BIFF8) {
                ...
                // offset: 8; size: 1; Indentation, shrink to cell size, and
                // text direction
                ...
                if ($bordersLeftStyle = self::_mapBorderStyle(
                    (0x0000000F & self::_GetInt4d($recordData, 10)) >> 0)) {
                    $objStyle->getBorders()
                        ->getLeft()
                        ->setBorderStyle($bordersLeftStyle);
                }
                // bit: 7-4; mask: 0x000000F0; right style
                if ($bordersRightStyle = self::_mapBorderStyle(
                    (0x000000F0 & self::_GetInt4d($recordData, 10)) >> 4)) {
                    $objStyle->getBorders()
                        ->getRight()
                        ->setBorderStyle($bordersRightStyle);
                }
                // bit: 11-8; mask: 0x00000F00; top style
                if ($bordersTopStyle = self::_mapBorderStyle(
                    (0x00000F00 & self::_GetInt4d($recordData, 10)) >> 8)) {
                    $objStyle->getBorders()
                        ->getTop()
                ...
                // bit: 30; mask: 0x40000000; 1 = diagonal line from top left to
                // right bottom
                $diagonalDown = (0x40000000 & self::_GetInt4d($recordData, 10)) >>
                     30 ? true : false;
                
                // bit: 31; mask: 0x80000000; 1 = diagonal line from bottom left
                // to top right
                $diagonalUp = (0x80000000 & self::_GetInt4d($recordData, 10)) >>
                     31 ? true : false;
                
                if ($diagonalUp == false && $diagonalDown == false) {
                    $objStyle->getBorders()->setDiagonalDirection(
                        PHPExcel_Style_Borders::DIAGONAL_NONE);
                ...
                if ($fillType = self::_mapFillPattern(
                    (0xFC000000 & self::_GetInt4d($recordData, 14)) >> 26)) {
                ...
                $objStyle->getFill()->setFillType(
                    self::_mapFillPattern(
                        (0x003F0000 & $borderAndBackground) >> 16));
            ...
            if ($isCellStyleXf) {
                ...
                $this->_mapCellXfIndex[$this->_xfIndex] = count(
                    $this->_phpExcel->getCellXfCollection()) - 1;
            }
                ...
                switch ($extType) {
                    case 4: // fill start color
                        ...
                        if ($xclfType == 2) {
                            ...
                            if (isset($this->_mapCellXfIndex[$ixfe])) {
                                $fill = $this->_phpExcel->getCellXfByIndex(
                                    $this->_mapCellXfIndex[$ixfe])->getFill();
                                $fill->getStartColor()->setRGB($rgb);
                                unset($fill->startcolorIndex); // normal color
                                                                   // index does not
                                                                   // apply, discard
                            }
                        }
                    ...
                    case 5: // fill end color
                        ...
                        if ($xclfType == 2) {
                            ...
                            if (isset($this->_mapCellXfIndex[$ixfe])) {
                                ...
                                unset($fill->endcolorIndex); // normal color
                                                                 // index does not
                                                                 // apply, discard
                            }
                        }
                    ...
                    case 7: // border color top
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->_mapCellXfIndex[$ixfe])) {
                                $top = $this->_phpExcel->getCellXfByIndex(
                                    $this->_mapCellXfIndex[$ixfe])
                                    ->getBorders()
                                    ->getTop();
                                $top->getColor()->setRGB($rgb);
                                unset($top->colorIndex); // normal color index
                                                             // does not apply,
                                                             // discard
                            }
                        }
                    ...
                    case 8: // border color bottom
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->_mapCellXfIndex[$ixfe])) {
                                $bottom = $this->_phpExcel->getCellXfByIndex(
                                    $this->_mapCellXfIndex[$ixfe])
                                    ->getBorders()
                                    ->getBottom();
                                $bottom->getColor()->setRGB($rgb);
                                unset($bottom->colorIndex); // normal color
                                                                // index does not
                                                                // apply, discard
                            }
                        }
                    ...
                    case 9: // border color left
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->_mapCellXfIndex[$ixfe])) {
                                $left = $this->_phpExcel->getCellXfByIndex(
                                    $this->_mapCellXfIndex[$ixfe])
                                    ->getBorders()
                                    ->getLeft();
                                $left->getColor()->setRGB($rgb);
                                unset($left->colorIndex); // normal color index
                                                              // does not apply,
                                                              // discard
                            }
                        }
                    ...
                    case 10: // border color right
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->_mapCellXfIndex[$ixfe])) {
                                $right = $this->_phpExcel->getCellXfByIndex(
                                    $this->_mapCellXfIndex[$ixfe])
                                    ->getBorders()
                                    ->getRight();
                                $right->getColor()->setRGB($rgb);
                                unset($right->colorIndex); // normal color index
                                                               // does not apply,
                                                               // discard
                            }
                        }
                    ...
                    case 11: // border color diagonal
                        ...
                        if ($xclfType == 2) {
                            ...
                            if (isset($this->_mapCellXfIndex[$ixfe])) {
                                ...
                                unset($diagonal->colorIndex); // normal color
                                                                  // index does not
                                                                  // apply, discard
                            }
                        }
                    ...
                    case 13: // font color
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->_mapCellXfIndex[$ixfe])) {
                                $font = $this->_phpExcel->getCellXfByIndex(
                                    $this->_mapCellXfIndex[$ixfe])->getFont();
                                $font->getColor()->setRGB($rgb);
                                unset($font->colorIndex); // normal color index
                                                              // does not apply,
                                                              // discard
                            }
                        }
    ...
    /**
     * SHEET This record is located in the Workbook Globals Substream and
     * represents a sheet inside the workbook. One SHEET record is written for
     * each sheet. It stores the sheet name and a stream offset to the BOF
    ...
    private function _readSheet() {
        ...
        // offset: 0; size: 4; absolute stream position of the BOF record of the
        // sheet
        // NOTE: not encrypted
    ...
    private function _readExternalBook() {
        ...
        if (strlen($recordData) > 4) {
            ...
            // offset: 2; size: var; encoded URL without sheet name (Unicode
            // string, 16-bit length)
            $encodedUrlString = self::_readUnicodeStringLong(
                substr($recordData, 2));
            $offset += $encodedUrlString['size'];
            
            // offset: var; size: var; list of $nm sheet names (Unicode strings,
            // 16-bit length)
            $externalSheetNames = array();
            for ($i = 0; $i < $nm; ++ $i) {
                $externalSheetNameString = self::_readUnicodeStringLong(
            ...
            for ($i = 0; $i < $nm; ++ $i) {
                $this->_ref[] = array(
                    ...
                    'externalBookIndex' => self::_GetInt2d($recordData, 
                        2 + 6 * $i), 
                    // offset: 4 + 6 * $i; index to first sheet in EXTERNALBOOK
                    // record
                    'firstSheetIndex' => self::_GetInt2d($recordData, 
                        4 + 6 * $i), 
                    // offset: 6 + 6 * $i; index to last sheet in EXTERNALBOOK
                    // record
    ...
    /**
     * DEFINEDNAME This record is part of a Link Table. It contains the name and
     * the token array of an internal defined name. Token arrays of defined
     * names contain tokens with aberrant token classes. -- "OpenOffice.org's
     * Documentation of the Microsoft Excel File Format"
     */
    private function _readDefinedName() {
        ...
        if ($this->_version == self::XLS_BIFF8) {
            ...
            // offset: 4; size: 2; size of the formula data (it can happen that
            // this is zero)
            // note: there can also be additional data, this is not included in
            // $flen
    ...
    /**
     * SST - Shared String Table This record contains a list of all strings used
     * anywhere in the workbook. Each string occurs only once. The workbook uses
     * indexes into the list to reference the strings. -- "OpenOffice.org's
     * Documentation of the Microsoft Excel File Format"
     */
    private function _readSst() {
        ...
        for ($i = 0; $i < $nm; ++ $i) {
            ...
            if ($pos + $len <= $limitpos) {
                ...
                $charsLeft = $numChars -
                     (($isCompressed) ? $bytesRead : ($bytesRead / 2));
                ...
                while ($charsLeft > 0) {
                    ...
                    // look up next limit position, in case the string span more
                    // than one continue record
                    foreach ($spliceOffsets as $spliceOffset) {
                        if ($pos < $spliceOffset) {
                            $limitpos = $spliceOffset;
                            break;
        ...
        // _getSplicedRecordData() takes care of moving current position in data
        // stream
    }
    ...
    private function _readPrintGridlines() {
        ...
        if ($this->_version == self::XLS_BIFF8 && ! $this->_readDataOnly) {
            // offset: 0; size: 2; 0 = do not print sheet grid lines; 1 = print
            // sheet gridlines
    ...
    private function _readSheetPr() {
        ...
        // bit: 8; mask: 0x100; 0 = scale printout in percent, 1 = fit printout
        // to number of pages
    ...
    private function _readHeader() {
        ...
        if (! $this->_readDataOnly) {
            ...
            if ($recordData) {
                ...
                $this->_phpSheet->getHeaderFooter()->setOddHeader(
                    $string['value']);
                $this->_phpSheet->getHeaderFooter()->setEvenHeader(
                    $string['value']);
            }
        }
    }
    ...
    private function _readFooter() {
        ...
        if (! $this->_readDataOnly) {
            ...
            if ($recordData) {
                ...
                $this->_phpSheet->getHeaderFooter()->setOddFooter(
                    $string['value']);
                $this->_phpSheet->getHeaderFooter()->setEvenFooter(
                    $string['value']);
            }
        }
    }
    ...
    private function _readHcenter() {
        ...
        if (! $this->_readDataOnly) {
            // offset: 0; size: 2; 0 = print sheet left aligned, 1 = print sheet
            // centered horizontally
            $isHorizontalCentered = (bool) self::_GetInt2d($recordData, 0);
            
            $this->_phpSheet->getPageSetup()->setHorizontalCentered(
    ...
    private function _readVcenter() {
        ...
        if (! $this->_readDataOnly) {
            // offset: 0; size: 2; 0 = print sheet aligned at top page border, 1
            // = print sheet vertically centered
            $isVerticalCentered = (bool) self::_GetInt2d($recordData, 0);
            
            $this->_phpSheet->getPageSetup()->setVerticalCentered(
                $isVerticalCentered);
        }
    }
    ...
    private function _readLeftMargin() {
        ...
        if (! $this->_readDataOnly) {
            // offset: 0; size: 8
            $this->_phpSheet->getPageMargins()->setLeft(
                self::_extractNumber($recordData));
        }
    }
    ...
    private function _readRightMargin() {
        ...
        if (! $this->_readDataOnly) {
            // offset: 0; size: 8
            $this->_phpSheet->getPageMargins()->setRight(
                self::_extractNumber($recordData));
        }
    }
    ...
    private function _readTopMargin() {
        ...
        if (! $this->_readDataOnly) {
            // offset: 0; size: 8
            $this->_phpSheet->getPageMargins()->setTop(
                self::_extractNumber($recordData));
        }
    }
    ...
    private function _readBottomMargin() {
        ...
        if (! $this->_readDataOnly) {
            // offset: 0; size: 8
            $this->_phpSheet->getPageMargins()->setBottom(
                self::_extractNumber($recordData));
        }
    }
    ...
    private function _readPageSetup() {
        ...
        if (! $this->_readDataOnly) {
            ...
            // offset: 6; size: 2; fit worksheet width to this number of pages,
            // 0 = use as many as needed
            $fitToWidth = self::_GetInt2d($recordData, 6);
            
            // offset: 8; size: 2; fit worksheet height to this number of pages,
            // 0 = use as many as needed
            ...
            if (! $isNotInit) {
                ...
                $this->_phpSheet->getPageSetup()->setFitToPage(
    ...
    /**
     * PROTECT - Sheet protection (BIFF2 through BIFF8) if this record is
     * omitted, then it also means no sheet protection
     */
    ...
    private function _readPassword() {
        ...
        if (! $this->_readDataOnly) {
            ...
            $password = strtoupper(dechex(self::_GetInt2d($recordData, 0))); // the
                                                                             // hashed
                                                                             // password
    ...
    private function _readColInfo() {
        ...
        if (! $this->_readDataOnly) {
            ...
            // offset: 4; size: 2; width of the column in 1/256 of the width of
                                                   // the zero character
            ...
            // bit: 10-8; mask: 0x0700; outline level of the columns (0 = no
            // outline)
            ...
            for ($i = $fc; $i <= $lc; ++ $i) {
                if ($lc == 255 || $lc == 256) {
                    $this->_phpSheet->getDefaultColumnDimension()->setWidth(
                        $width / 256);
                    break;
                }
                $this->_phpSheet->getColumnDimensionByColumn($i)->setWidth(
                    $width / 256);
                $this->_phpSheet->getColumnDimensionByColumn($i)->setVisible(
                    ! $isHidden);
                $this->_phpSheet->getColumnDimensionByColumn($i)->setOutlineLevel(
                    $level);
                $this->_phpSheet->getColumnDimensionByColumn($i)->setCollapsed(
                    $isCollapsed);
                $this->_phpSheet->getColumnDimensionByColumn($i)->setXfIndex(
                    $this->_mapCellXfIndex[$xfIndex]);
            }
        }
    }

    /**
     * ROW This record contains the properties of a single row in a sheet. Rows
    ...
    private function _readRow() {
        ...
        if (! $this->_readDataOnly) {
            ...
            // offset: 2; size: 2; index to column of the first cell which is
            // described by a cell record
            
            // offset: 4; size: 2; index to column of the last cell which is
            // described by a cell record, increased by 1
            
            // offset: 6; size: 2;
            
            // bit: 14-0; mask: 0x7FFF; height of the row, in twips = 1/20 of a
            // point
            ...
            if (! $useDefaultHeight) {
                $this->_phpSheet->getRowDimension($r + 1)->setRowHeight(
                    $height / 20);
            }
            ...
            // bit: 4; mask: 0x00000010; 1 = outline group start or ends here...
            // and is collapsed
            $isCollapsed = (0x00000010 & self::_GetInt4d($recordData, 12)) >> 4;
            $this->_phpSheet->getRowDimension($r + 1)->setCollapsed(
                $isCollapsed);
            ...
            // bit: 27-16; mask: 0x0FFF0000; only applies when hasExplicitFormat
            // = 1; index to XF record
            $xfIndex = (0x0FFF0000 & self::_GetInt4d($recordData, 12)) >> 16;
            
            if ($hasExplicitFormat) {
                $this->_phpSheet->getRowDimension($r + 1)->setXfIndex(
                    $this->_mapCellXfIndex[$xfIndex]);
            }
        }
    }
    ...
    /**
     * Read RK record This record represents a cell that contains an RK value
     * (encoded integer or floating-point value). If a floating-point value
     * cannot be encoded to an RK value, a NUMBER record will be written. This
     * record replaces the record INTEGER written in BIFF2. -- "OpenOffice.org's
     * Documentation of the Microsoft Excel File Format"
     */
    private function _readRk() {
        ...
        // Read cell?
        if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell(
            $columnString, $row + 1, $this->_phpSheet->getTitle())) {
    ...
    /**
     * Read LABELSST record This record represents a cell that contains a
     * string. It replaces the LABEL record and RSTRING record used in
     * BIFF2-BIFF5. -- "OpenOffice.org's Documentation of the Microsoft Excel
     * File Format"
     */
                        ...
                        if ($i == 0) { // first text run, no style
                            ...
                            if (isset($fmtRuns[$i - 1])) {
                                if ($fmtRuns[$i - 1]['fontIndex'] < 4) {
                                    ...
                                    // omitted in all BIFF versions for some
                                    // strange reason
                                    // check the OpenOffice documentation of the
                                    // FONT record
                                    $fontIndex = $fmtRuns[$i - 1]['fontIndex'] -
                                         1;
                                }
                                $textRun->setFont(
                                    clone $this->_objFonts[$fontIndex]);
                            }
                        }
                    }
                }
     ...
     * cells. All cells are located in the same row. -- "OpenOffice.org's
     * Documentation of the Microsoft Excel File Format"
     */
    private function _readMulRk() {
        ...
        for ($i = 0; $i < $columns; ++ $i) {
            ...
            if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell(
                ...
                $numValue = self::_GetIEEE754(
                    self::_GetInt4d($recordData, $offset + 2));
                $cell = $this->_phpSheet->getCell($columnString . ($row + 1));
                if (! $this->_readDataOnly) {
                    // add style
                    $cell->setXfIndex($this->_mapCellXfIndex[$xfIndex]);
    ...
    /**
     * Read NUMBER record This record represents a cell that contains a
     * floating-point value. -- "OpenOffice.org's Documentation of the Microsoft
     * Excel File Format"
     */
     ...
     * is a string This record contains the token array and the result of a
     * formula cell. -- "OpenOffice.org's Documentation of the Microsoft Excel
     * File Format"
     */
    private function _readFormula() {
        ...
        // offset: 14: size: 2; option flags, recalculate always, recalculate on
        // open etc.
        ...
        // the formula data may be ordinary formula data, therefore we need to
        // check
        ...
        if ($isPartOfSharedFormula) {
            // part of shared formula which means there will be a formula with a
            // tExp token and nothing else
        ...
        if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell(
            ...
            if (! $isPartOfSharedFormula) {
                ...
                if ($this->_version == self::XLS_BIFF8) {
                    // do nothing at this point, formula id added later in the
                    // code
                } else {
                    $cell->setValueExplicit($value, $dataType);
                }
    ...
    /**
     * Read a SHAREDFMLA record. This function just stores the binary shared
     * formula in the reader, which usually contains relative references. These
     * will be used to construct the formula in each shared formula part after
     * the sheet is read.
     */
    private function _readSharedFmla() {
        ...
        // offset: 0, size: 6; cell range address of the area used by the shared
        // formula, not used for anything
        $cellRange = substr($recordData, 0, 6);
        $cellRange = $this->_readBIFF5CellRangeAddressFixed($cellRange); // note:
                                                                         // even
                                                                         // BIFF8
                                                                         // uses
                                                                         // BIFF5
    ...
    /**
     * Read a STRING record from current stream position and advance the stream
     * pointer to next record This record is used for storing result from
     * FORMULA record when it is a string, and it occurs directly after the
     * FORMULA record
     *
    ...
    /**
     * Read BOOLERR record This record represents a Boolean value or error value
     * cell. -- "OpenOffice.org's Documentation of the Microsoft Excel File
     * Format"
     */
     ...
     * All cells are located in the same row -- "OpenOffice.org's Documentation
     * of the Microsoft Excel File Format"
     */
        ...
        if (! $this->_readDataOnly) {
            for ($i = 0; $i < $length / 2 - 3; ++ $i) {
                ...
                if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell(
                    ...
                    $this->_phpSheet->getCell($columnString . ($row + 1))->setXfIndex(
                        $this->_mapCellXfIndex[$xfIndex]);
                }
            }
        }
     ...
     * In BIFF8 it is usually replaced by the LABELSST record. Excel still uses
     * this record, if it copies unformatted text cells to the clipboard. --
     * "OpenOffice.org's Documentation of the Microsoft Excel File Format"
     */
    ...
    private function _readObj() {
        ...
        // for now, we are just interested in the second subrecord containing
        // the object type
    ...
    private function _readWindow2() {
        ...
        if ($this->_version === self::XLS_BIFF8) {
            // offset: 8; size: 2; not used
            // offset: 10; size: 2; cached magnification factor in page break
            // preview (in percent); 0 = Default (60%)
            // offset: 12; size: 2; cached magnification factor in normal view
            // (in percent); 0 = Default (100%)
        ...
        // bit: 6; mask: 0x0040; 0 = columns from left to right, 1 = columns
        // from right to left
        ...
        if ($isActive) {
            $this->_phpExcel->setActiveSheetIndex(
                $this->_phpExcel->getIndex($this->_phpSheet));
        }
        ...
        if ($this->_phpSheet->getSheetView()->getView() !==
            ...
            if ($this->_version === self::XLS_BIFF8) {
                ...
                $this->_phpSheet->getSheetView()->setZoomScaleNormal(
    ...
    private function _readPageLayoutView() {
        ...
        // var_dump(unpack("vrt/vgrbitFrt/V2reserved/vwScalePLV/vgrbit",
        // $recordData));
        ...
        if ($fPageLayoutView === 1) {
            $this->_phpSheet->getSheetView()->setView(
                PHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT);
    ...
    private function _readScl() {
        ...
        $this->_phpSheet->getSheetView()->setZoomScale(
            $numerator * 100 / $denumerator);
    }
    ...
    private function _readPane() {
        ...
        if (! $this->_readDataOnly) {
            ...
            if ($this->_frozen) {
                // frozen panes
                $this->_phpSheet->freezePane(
                    PHPExcel_Cell::stringFromColumnIndex($px) . ($py + 1));
            } else {
    ...
    private function _readSelection() {
        ...
        if (! $this->_readDataOnly) {
            ...
            // offset: 5; size: 2; index into the following cell range list to
            // the
            ...
            $cellRangeAddressList = $this->_readBIFF5CellRangeAddressList($data); // note:
                                                                                  // also
                                                                                  // BIFF8
                                                                                  // uses
                                                                                  // BIFF5
            ...
            // first row '1' + last row '16384' indicates that full column is
            // selected (apparently also in BIFF8!)
            if (preg_match('/^([A-Z]+1\:[A-Z]+)16384$/', $selectedCells)) {
                $selectedCells = preg_replace('/^([A-Z]+1\:[A-Z]+)16384$/', 
                    '${1}1048576', $selectedCells);
            }
            
            // first row '1' + last row '65536' indicates that full column is
            // selected
            ...
            // first column 'A' + last column 'IV' indicates that full row is
            // selected
    ...
    private function _includeCellRangeFiltered($cellRangeAddress) {
        ...
        if ($this->getReadFilter() !== NULL) {
            ...
            $rangeBoundaries = PHPExcel_Cell::getRangeBoundaries(
                $cellRangeAddress);
            $rangeBoundaries[1][0] ++;
    ...
    /**
     * MERGEDCELLS This record contains the addresses of merged cell ranges in
     * the current sheet. -- "OpenOffice.org's Documentation of the Microsoft
     * Excel File Format"
     */
    private function _readMergedCells() {
        ...
        if ($this->_version == self::XLS_BIFF8 && ! $this->_readDataOnly) {
            $cellRangeAddressList = $this->_readBIFF8CellRangeAddressList(
                $recordData);
    ...
    private function _readHyperLink() {
        ...
        if (! $this->_readDataOnly) {
            // offset: 0; size: 8; cell range address of all cells containing
            // this hyperlink
            try {
            ...
            // bit: 0; mask: 0x00000001; 0 = no link or extant, 1 = file link or
            // URL
            $isFileLinkOrUrl = (0x00000001 & self::_GetInt2d($recordData, 28)) >>
                 0;
            
            // bit: 1; mask: 0x00000002; 0 = relative path, 1 = absolute path or
            // URL
            ...
            // bit: 7; mask: 0x00000080; 0 = no target frame, 1 = has target
            // frame
            ...
            if ($hasDesc) {
                ...
                // offset: 36; size: var; character array of description text,
                // no Unicode string header, always 16-bit characters, zero
                // terminated
                $desc = self::_encodeUTF16(
                    substr($recordData, 36, 2 * ($dl - 1)), false);
                $offset += 4 + 2 * $dl;
            }
            if ($hasFrame) {
            ...
            switch ($hyperlinkType) {
                case 'URL':
                    ...
                    $offset += 16;
                    // offset: var; size: 4; size (in bytes) of character array
                    // of the URL including trailing zero word
                    $us = self::_GetInt4d($recordData, $offset);
                    $offset += 4;
                    // offset: var; size: $us; character array of the URL, no
                    // Unicode string header, always 16-bit characters,
                    // zero-terminated
                    $url = self::_encodeUTF16(
                        substr($recordData, $offset, $us - 2), false);
                    $url .= $hasText ? '#' : '';
                    $offset += $us;
                    break;
                ...
                case 'local':
                    ...
                    // file path and name, including trailing zero word
                    $sl = self::_GetInt4d($recordData, $offset);
                    $offset += 4;
                    
                    // offset: var; size: sl; character array of the shortened
                    // file path and name in 8.3-DOS-format (compressed Unicode
                    // string)
                    $shortenedFilePath = substr($recordData, $offset, $sl);
                    $shortenedFilePath = self::_encodeUTF16($shortenedFilePath, 
                        true);
                    $shortenedFilePath = substr($shortenedFilePath, 0, - 1); // remove
                                                                             // trailing
                                                                             // zero
                    ...
                    // extended file path
                    // offset: var; size: 4; size of the following file link
                    // field including string lenth mark
                    $sz = self::_GetInt4d($recordData, $offset);
                    $offset += 4;
                    
                    // only present if $sz > 0
                    if ($sz > 0) {
                        // offset: var; size: 4; size of the character array of
                        // the extended file path and name
                        ...
                        $extendedFilePath = self::_encodeUTF16(
                            $extendedFilePath, false);
                        $offset += $xl;
                    }
                    
                    // construct the path
                    $url = str_repeat('..\\', $upLevelCount);
                    $url .= ($sz > 0) ? $extendedFilePath : $shortenedFilePath; // use
                                                                                // extended
                                                                                // path
                                                                                // if
                                                                                // available
                ...
                case 'UNC':
                    // section 5.58.4: Hyperlink to a File with UNC (Universal
            ...
            if ($hasText) {
                // offset: var; size: 4; character count of text mark including
                // trailing zero word
                $tl = self::_GetInt4d($recordData, $offset);
                $offset += 4;
                // offset: var; size: var; character array of the text mark
                // without the # sign, no Unicode header, always 16-bit
                // characters, zero-terminated
                $text = self::_encodeUTF16(
                    substr($recordData, $offset, 2 * ($tl - 1)), false);
                $url .= $text;
            }
    ...
    private function _readDataValidation() {
        ...
        // bit: 7; mask: 0x00000080; 1= formula is explicit (only applies to
        // list)
        ...
        // bit: 9; mask: 0x00000200; 1= suppress drop down arrow in list type
        // validity
        ...
        // offset: var; size: $sz1; formula data for first condition (without
        // size field)
        ...
        // offset: var; size: $sz2; formula data for second condition (without
        // size field)
    ...
    private function _readSheetLayout() {
        ...
        if (! $this->_readDataOnly) {
            ...
            // Excel 2003 uses size of 0x14 (documented), Excel 2007 uses size
            // of 0x28 (not documented?)
            ...
            switch ($sz) {
                ...
                case 0x28:
                    // TODO: Investigate structure for .xls SHEETLAYOUT record
                    // as saved by MS Office Excel 2007
                    return;
                    break;
            }
        }
    ...
    private function _readSheetProtection() {
        ...
        // offset: 12; size: 2; Shared feature type index (2=Enhanced Protetion,
        // 4=SmartTag)
        ...
        // bit: 0; mask 0x0001; 1 = user may edit objects, 0 = users must not
        // edit objects
    ...
    /**
     * Read RANGEPROTECTION record Reading of this record is based on Microsoft
     * Office Excel 97-2000 Binary File Format Specification, where it is
     * referred to as FEAT record
     */
    private function _readRangeProtection() {
        ...
        if (! $this->_readDataOnly) {
            ...
            for ($i = 0; $i < $cref; ++ $i) {
                try {
                    $cellRange = $this->_readBIFF8CellRangeAddressFixed(
                        substr($recordData, 27 + 8 * $i, 8));
                } catch (PHPExcel_Exception $e) {
                    return;
                }
    ...
    private function _readImData() {
        ...
        switch ($cf) {
            case 0x09: // Windows bitmap format
                       // BITMAPCOREINFO
                       // 1. BITMAPCOREHEADER
                       // offset: 0; size: 4; bcSize, Specifies the number of
                       // bytes required by the structure
                $bcSize = self::_GetInt4d($iData, 0);
                // var_dump($bcSize);
                
                // offset: 4; size: 2; bcWidth, specifies the width of the
                // bitmap, in pixels
                $bcWidth = self::_GetInt2d($iData, 4);
                // var_dump($bcWidth);
                
                // offset: 6; size: 2; bcHeight, specifies the height of the
                // bitmap, in pixels.
                $bcHeight = self::_GetInt2d($iData, 6);
                // var_dump($bcHeight);
                $ih = imagecreatetruecolor($bcWidth, $bcHeight);
                
                // offset: 8; size: 2; bcPlanes, specifies the number of planes
                // for the target device. This value must be 1
                
                // offset: 10; size: 2; bcBitCount specifies the number of
                // bits-per-pixel. This value must be 1, 4, 8, or 24
    ...
    /**
     * Read a free CONTINUE record. Free CONTINUE record may be a camouflaged
     * MSODRAWING record When MSODRAWING data on a sheet exceeds 8224 bytes,
     * CONTINUE records are used instead. Undocumented. In this case, we must
     * treat the CONTINUE record as a MSODRAWING record
     */
    private function _readContinue() {
        ...
        // this is in case a free CONTINUE record occurs in other circumstances
        // we are unaware of
        ...
        // check if record data is at least 4 bytes long, otherwise there is no
        // chance this is MSODRAWING data
        if ($length < 4) {
            // move stream pointer to next record
            $this->_pos += 4 + $length;
    ...
    /**
     * Reads a record from current position in data stream and continues reading
     * data as long as CONTINUE records are found. Splices the record data
     * pieces and returns the combined string as if record data is in one piece.
     * Moves to next current position in data stream to start of next record
     * different from a CONtINUE record
     *
     * @return array
     */
    ...
    /**
     * Take array of tokens together with additional data for formula and return
     * human readable formula
     *
     * @param array $tokens
     * @param array $additionalData Additional binary data going with the
     *            formula
     * @param string $baseCell Base cell, only needed when formula contains
     *            tRefN tokens, e.g. with shared formulas
     * @return string Human readable formula
    ...
    private function _createFormulaFromTokens($tokens, $additionalData) {
        ...
        foreach ($tokens as $token) {
            // initialize spaces
            $space0 = isset($space0) ? $space0 : ''; // spaces before next
                                                     // token, not tParen
            $space1 = isset($space1) ? $space1 : ''; // carriage returns before
                                                     // next token, not tParen
            $space2 = isset($space2) ? $space2 : ''; // spaces before opening
                                                     // parenthesis
            $space3 = isset($space3) ? $space3 : ''; // carriage returns before
                                                     // opening parenthesis
            $space4 = isset($space4) ? $space4 : ''; // spaces before closing
                                                     // parenthesis
            $space5 = isset($space5) ? $space5 : ''; // carriage returns before
                                                     // closing parenthesis
            ...
            switch ($token['name']) {
                ...
                case 'tAttrSpace': // space / carriage return
                                   // space will be used when next token
                                   // arrives, do not alter formulaString stack
                ...
                case 'tFuncV': // function with variable number of arguments
                    if ($token['data']['function'] != '') {
                        ...
                        $formulaStrings[] = "$space1$space0{$token['data']['function']}(" .
                             implode(',', $ops) . ")";
                        unset($space0, $space1);
                    } else {
                        // add-in function
                ...
                case 'tArray': // array constant
                    $constantArray = self::_readBIFF8ConstantArray(
                        $additionalData);
                    $formulaStrings[] = $space1 . $space0 .
                         $constantArray['value'];
                    $additionalData = substr($additionalData, 
                        $constantArray['size']); // bite
                                                 // of
                                                 // chunk
                                                 // of
                                                 // additional
        ...
        switch ($id) {
            ...
            case 0x19: // Special attribute
                ...
                switch (ord($formulaData[1])) {
                    ...
                    case 0x04:
                        $name = 'tAttrChoose';
                        // offset: 2; size: 2; number of choices in the CHOOSE
                        // function ($nc, number of parameters decreased by 1)
            ...
            case 0x1F: // number
                ...
                $data = str_replace(',', '.', (string) $data); // in case
                                                               // non-English
                                                               // locale
                break;
            ...
            case 0x65:
                $name = 'tArea';
                $size = 9;
                $data = $this->_readBIFF8CellRangeAddress(
                    substr($formulaData, 1, 8));
                break;
            ...
            case 0x66:
                ...
                $size = 7 + $subSize;
                $data = $this->_getFormulaFromData(
                    substr($formulaData, 7, $subSize));
                break;
            ...
            case 0x69:
                ...
                $size = 3 + $subSize;
                $data = $this->_getFormulaFromData(
                    substr($formulaData, 3, $subSize));
                break;
            
            case 0x2C: // Relative 2d cell reference reference, used in shared
                       // formulas and some other places
            case 0x4C:
            case 0x6C:
                $name = 'tRefN';
                $size = 5;
                $data = $this->_readBIFF8CellAddressB(
                    substr($formulaData, 1, 4), $baseCell);
                break;
            ...
            case 0x6D:
                $name = 'tAreaN';
                $size = 9;
                $data = $this->_readBIFF8CellRangeAddressB(
                    substr($formulaData, 1, 8), $baseCell);
                break;
            ...
            case 0x7A:
                ...
                try {
                    ...
                    $sheetRange = $this->_readSheetRangeByRefIndex(
                        self::_GetInt2d($formulaData, 1));
                    // offset: 3; size: 4; cell address
                    $cellAddress = $this->_readBIFF8CellAddress(
                        substr($formulaData, 3, 4));
            ...
            case 0x7B:
                ...
                try {
                    ...
                    $cellRangeAddress = $this->_readBIFF8CellRangeAddress(
                        substr($formulaData, 3, 8));
            ...
            default:
                throw new PHPExcel_Reader_Exception(
                    'Unrecognized token ' . sprintf('%02X', $id) . ' in formula');
                break;
        }
    ...
    private function _readBIFF8CellAddress($cellAddressStructure) {
        // offset: 0; size: 2; index to row (0... 65535) (or offset (-32768...
        // 32767))
        ...
        $column = PHPExcel_Cell::stringFromColumnIndex(
            0x00FF & self::_GetInt2d($cellAddressStructure, 2));
        
        // bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column
        // index)
     ...
     * negative values for row and column to indicate offsets from a base cell
     * section 3.3.4
     *
     * @param string $cellAddressStructure
     * @param string $baseCell Base cell, only needed when formula contains
     ...
     * @return string
     */
    private function _readBIFF8CellAddressB($cellAddressStructure, 
        $baseCell = 'A1') {
        list ($baseCol, $baseRow) = PHPExcel_Cell::coordinateFromString(
            $baseCell);
    ...
    /**
     * Reads a cell range address in BIFF5 e.g. 'A2:B6' or 'A1' always fixed
     * range section 2.5.14
     *
    ...
    /**
     * Reads a cell range address in BIFF8 e.g. 'A2:B6' or 'A1' always fixed
     * range section 2.5.14
     *
     * @param string $subData
     * @return string
    ...
    /**
     * Reads a cell range address in BIFF8 e.g. 'A2:B6' or '$A$2:$B$6' there are
     * flags indicating whether column/row index is relative section 3.3.4
     *
     * @param string $subData
     * @return string
     */
    private function _readBIFF8CellRangeAddress($subData) {
        ...
        // offset: 4; size: 2; index to first column or column offset + relative
        // flags
        
        // bit: 7-0; mask 0x00FF; column index
        $fc = PHPExcel_Cell::stringFromColumnIndex(
            0x00FF & self::_GetInt2d($subData, 4));
        ...
        // offset: 6; size: 2; index to last column or column offset + relative
        // flags
        
        // bit: 7-0; mask 0x00FF; column index
        $lc = PHPExcel_Cell::stringFromColumnIndex(
            0x00FF & self::_GetInt2d($subData, 6));
    ...
    /**
     * Reads a cell range address in BIFF8 for shared formulas. Uses positive
     * and negative values for row and column to indicate offsets from a base
     * cell section 3.3.4
     *
     * @param string $subData
    ...
    private function _readBIFF8CellRangeAddressB($subData, $baseCell = 'A1') {
        ...
        // offset: 2; size: 2; relative index to first row (0... 65535) should
                                                 // be treated as offset
                                                 // (-32768... 32767)
    ...
    private function _readBIFF8CellRangeAddressList($subData) {
        ...
        for ($i = 0; $i < $nm; ++ $i) {
            $cellRangeAddresses[] = $this->_readBIFF8CellRangeAddressFixed(
                substr($subData, $offset, 8));
            $offset += 8;
        }
    ...
    private function _readBIFF5CellRangeAddressList($subData) {
        ...
        for ($i = 0; $i < $nm; ++ $i) {
            $cellRangeAddresses[] = $this->_readBIFF5CellRangeAddressFixed(
                substr($subData, $offset, 6));
            $offset += 6;
        }
    ...
    /**
     * Get a sheet range like Sheet1:Sheet3 from REF index Note: If there is
     * only one sheet in the range, one gets e.g Sheet1 It can also happen that
     * the REF structure uses the -1 (FFFF) code to indicate deleted sheets, in
     * which case an PHPExcel_Reader_Exception is thrown
     *
     * @param int $index
     * @return string|false
     * @throws PHPExcel_Reader_Exception
        ...
        if (isset($this->_ref[$index])) {
            ...
            switch ($type) {
                case 'internal':
                    ...
                    // if there are special characters, we need to enclose the
                    // range in single-quotes
                    // todo: check if we have identified the whole set of
                    // special characters
                    // it seems that the following characters are not accepted
                    // for sheet names
    ...
    private static function _readBIFF8ConstantArray($arrayData) {
        ...
        for ($r = 1; $r <= $nr + 1; ++ $r) {
            ...
            $matrixChunks[] = implode(',', $items); // looks like e.g.
                                                        // '1,"hello"'
        }
    ...
    /**
     * read BIFF8 constant value which may be 'Empty Value', 'Number', 'String
     * Value', 'Boolean Value', 'Error Value' section 2.5.7 returns e.g.
    ...
    /**
     * Extract RGB color OpenOffice.org's Documentation of the Microsoft Excel
     * File Format, section 2.5.4
     *
    ...
    /**
     * Extracts an Excel Unicode short string (8-bit string length) OpenOffice
     * documentation: 2.5.3 function will automatically find out where the
     * Unicode string ends.
     *
     * @param string $subData
     * @return array
    ...
    /**
     * Extracts an Excel Unicode long string (16-bit string length) OpenOffice
     * documentation: 2.5.3 this function is under construction, needs to
     * support rich text, and Asian phonetic settings
     *
     * @param string $subData
     * @return array
     */
    ...
    /**
     * Read Unicode string with no string length field, but with known character
     * count this function is under construction, needs to support rich text,
     * and Asian phonetic settings OpenOffice.org's Documentation of the
     * Microsoft Excel File Format, section 2.5.3
     *
     * @param string $subData
    ...
    private static function _readUnicodeString($subData, $characterCount) {
        ...
        // bit: 0; mask: 0x01; character compression (0 = compressed 8-bit, 1 =
        // uncompressed 16-bit)
        ...
        return array(
            ...
            'size' => $isCompressed ? 1 + $characterCount : 1 +
                 2 * $characterCount); // the size in bytes including the option
                                          // flags
     ...
     * explicit string tokens in formulas. Example: hello"world -->
     * "hello""world"
     *
    ...
    /**
     * Convert UTF-16 string in compressed notation to uncompressed form. Only
     * used for BIFF8.
     *
     * @param string $string
     * @return string
    ...
    public static function _GetInt4d($data, $pos) {
        ...
        // Hacked by Andreas Rehm 2006 to ensure correct result of the <<24
        // block on 32 and 64bit systems
    ...
    private static function _mapBuiltInColor($color) {
        switch ($color) {
            ...
            case 0x40:
                return array(
                    'rgb' => '000000'); // system window text color
            case 0x41:
                return array(
                    'rgb' => 'FFFFFF'); // system window background color
            default:
                return array(
                    'rgb' => '000000');
        }
