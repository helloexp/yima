 ...
 *            (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Reader_HTML extends PHPExcel_Reader_Abstract implements 
    ...
    private $_formats = array(
        ...
        'a' => array(
            'font' => array(
                'underline' => true, 
                'color' => array(
                    'argb' => PHPExcel_Style_Color::COLOR_BLUE))),  // Blue
                                                                   // underlined
        'hr' => array(
            'borders' => array(
                'bottom' => array(
    ...
    protected function _isValidFormat() {
        // Reading 2048 bytes should be enough to validate that the format is
        // HTML
    ...
    // Data Array used for testing only, should write to PHPExcel object on
    // completion of tests
    ...
    private function _flushCell($sheet, $column, $row, &$cellContent) {
        if (is_string($cellContent)) {
            ...
            if (trim($cellContent) > '') {
                ...
                // ... we return the cell so we can mess about with styles more
                // easily
    ...
    private function _processDomElement(DOMNode $element, $sheet, &$row, 
        &$column, &$cellContent) {
        foreach ($element->childNodes as $child) {
            if ($child instanceof DOMText) {
                $domText = preg_replace('/\s+/', ' ', trim($child->nodeValue));
                if (is_string($cellContent)) {
                    // simply append the text if the cell content is a plain
                    // text string
                    $cellContent .= $domText;
                } else {
                    // but if we have a rich text run instead, we need to append
                    // it correctly
                    // TODO
                }
                ...
                foreach ($child->attributes as $attribute) {
                    // echo '<b>ATTRIBUTE: </b>' , $attribute->name , ' => ' ,
                    // $attribute->value , '<br />';
                ...
                switch ($child->nodeName) {
                    case 'meta':
                        foreach ($attributeArray as $attributeName => $attributeValue) {
                            switch ($attributeName) {
                                case 'content':
                                    // TODO
                                    // Extract character set, so we can convert
                    ...
                    case 'br':
                        if ($this->_tableLevel > 0) {
                            ...
                            // Otherwise flush our existing content and move the
                            // row cursor on
                            $this->_flushCell($sheet, $column, $row, 
                                $cellContent);
                            ++ $row;
                    ...
                    case 'tr':
                        ...
                        $cellContent = '';
                        // echo 'START OF TABLE ' , $this->_tableLevel , '
                        // ROW<br />';
                        $this->_processDomElement($child, $sheet, $row, $column, 
                            $cellContent);
    ...
    public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel) {
        ...
        if (! $this->_isValidFormat()) {
            ...
            throw new PHPExcel_Reader_Exception(
                $pFilename . " is an Invalid HTML file.");
        }
