...
class PHPExcel_Worksheet implements PHPExcel_IComparable {
    ...
    public function __construct(PHPExcel $pParent = null, $pTitle = 'Worksheet') {
        ...
        $this->_cellCollection = PHPExcel_CachedObjectStorageFactory::getInstance(
            $this);
        ...
        // Default column dimension
        $this->_defaultColumnDimension = new PHPExcel_Worksheet_ColumnDimension(
            NULL);
    ...
    /**
     * Disconnect all cells from this PHPExcel_Worksheet object, typically so
     * that the worksheet object can be unset
     */
    public function disconnectCells() {
        ...
        // detach ourself from the workbook, so that it can then delete this
     ...
     */
    function __destruct() {
        PHPExcel_Calculation::getInstance($this->_parent)->clearCalculationCacheForWorksheet(
            $this->_title);
    ...
    private static function _checkSheetCodeName($pValue) {
        ...
        // Some of the printable ASCII characters are invalid: * : / \ ? [ ] and
        // first and last characters cannot be a "'"
     ...
     * @param int|null $iChartIndex Index where chart should go (0,1,..., or
     *            null for last)
     * @return PHPExcel_Chart
     */
    public function addChart(PHPExcel_Chart $pChart = null, $iChartIndex = null) {
        ...
        if (is_null($iChartIndex)) {
            ...
            array_splice($this->_chartCollection, $iChartIndex, 0, 
                array(
                    $pChart));
        }
     ...
     *
     * @return string String containing the dimension of this worksheet that
        ...
        if (! empty($autoSizes)) {
            ...
            foreach ($this->getCellCollection(false) as $cellID) {
                ...
                if (isset(
                    ...
                    if (! isset(
                        $isMergeCell[$this->_cellCollection->getCurrentAddress()])) {
                        // Calculated value
                        // To formatted string
     ...
     * @param string $updateFormulaCellReferences boolean Flag indicating
     *            whether cell references in formulae should be updated to
     *            reflect the new sheet name. This should be left as the default
     *            true, unless you are certain that no formula cells on any
     *            worksheet contain references to this worksheet
     * @return PHPExcel_Worksheet
     */
    public function setTitle($pValue = 'Worksheet', $updateFormulaCellReferences = true) {
        ...
        if ($this->_parent) {
            ...
            PHPExcel_Calculation::getInstance($this->_parent)->renameCalculationCacheForWorksheet(
                $oldTitle, $newTitle);
            if ($updateFormulaCellReferences)
                PHPExcel_ReferenceHelper::getInstance()->updateNamedFormulas(
                    $this->_parent, $oldTitle, $newTitle);
        }
     ...
     *            or the highest data column of any row if no row number is
     *            passed
     ...
     *            column, or the highest row of any column if no column letter
     *            is passed
     ...
     *
     * @param string $column Return the highest data row for the specified
     *            column, or the highest data row of any column if no column
     *            letter is passed
     ...
     * @param bool $returnCell Return the worksheet (false, default) or the cell
     *            (true)
     * @return PHPExcel_Worksheet|PHPExcel_Cell Depending on the last parameter
     *         being specified
     */
     ...
     *         being specified
     */
    public function setCellValueByColumnAndRow($pColumn = 0, $pRow = 1, 
        $pValue = null, $returnCell = false) {
    ...
    public function setCellValueExplicit($pCoordinate = 'A1', $pValue = null, 
        ...
        // Set value
        $cell = $this->getCell($pCoordinate)->setValueExplicit($pValue, 
            $pDataType);
        return ($returnCell) ? $cell : $this;
    }
     ...
     *         being specified
     */
    public function setCellValueExplicitByColumnAndRow($pColumn = 0, $pRow = 1, 
        $pValue = null, $pDataType = PHPExcel_Cell_DataType::TYPE_STRING, $returnCell = false) {
    ...
    public function getCell($pCoordinate = 'A1') {
        ...
        if (strpos($pCoordinate, '!') !== false) {
            $worksheetReference = PHPExcel_Worksheet::extractSheetTitle(
                $pCoordinate, true);
            return $this->_parent->getSheetByName($worksheetReference[0])->getCell(
        ...
        if ((! preg_match(
            ...
            $pCoordinate, $matches)) && (preg_match(
    ...
    private function _createNewCell($pCoordinate) {
        ...
        if ($rowDimension !== NULL && $rowDimension->getXfIndex() > 0) {
            // then there is a row dimension with explicit style, assign it to
            // the cell
    ...
    public function cellExists($pCoordinate = 'A1') {
        ...
        if (strpos($pCoordinate, '!') !== false) {
            $worksheetReference = PHPExcel_Worksheet::extractSheetTitle(
                $pCoordinate, true);
            return $this->_parent->getSheetByName($worksheetReference[0])->cellExists(
                $worksheetReference[1]);
        }
            ...
            if ($namedRange !== NULL) {
                ...
                if ($this->getHashCode() !=
                    ...
                    if (! $namedRange->getLocalOnly()) {
                        return $namedRange->getWorksheet()->cellExists(
                            $pCoordinate);
                    } else {
                        throw new PHPExcel_Exception(
    ...
    public function getRowDimension($pRow = 1, $create = TRUE) {
        ...
        if (! isset($this->_rowDimensions[$pRow])) {
            if (! $create)
                return NULL;
            $this->_rowDimensions[$pRow] = new PHPExcel_Worksheet_RowDimension(
                $pRow);
    ...
    public function getColumnDimension($pColumn = 'A', $create = TRUE) {
        ...
        if (! isset($this->_columnDimensions[$pColumn])) {
            if (! $create)
                return NULL;
            $this->_columnDimensions[$pColumn] = new PHPExcel_Worksheet_ColumnDimension(
                $pColumn);
    ...
    /**
     * Set shared cell style to a range of cells Please note that this will
     * overwrite existing cell styles for cells in range!
     *
     * @deprecated
     *
     * @param PHPExcel_Style $pSharedCellStyle Cell style to share
     * @param string $pRange Range of cells (i.e. "A1:B10"), or just one cell
     *            (i.e. "A1")
     * @throws PHPExcel_Exception
     * @return PHPExcel_Worksheet
     */
    public function setSharedStyle(PHPExcel_Style $pSharedCellStyle = null, 
        $pRange = '') {
    ...
    /**
     * Duplicate cell style to a range of cells Please note that this will
     * overwrite existing cell styles for cells in range!
     *
    ...
    public function duplicateStyle(PHPExcel_Style $pCellStyle = null, $pRange = '') {
        ...
        if ($existingStyle = $this->_parent->getCellXfByHashCode(
        ...
        for ($col = $rangeStart[0]; $col <= $rangeEnd[0]; ++ $col) {
            for ($row = $rangeStart[1]; $row <= $rangeEnd[1]; ++ $row) {
                $this->getCell(
                    PHPExcel_Cell::stringFromColumnIndex($col - 1) . $row)->setXfIndex(
                    $xfIndex);
            }
        }
    ...
    /**
     * Duplicate conditional style to a range of cells Please note that this
     * will overwrite existing cell styles for cells in range!
     *
     * @param array of PHPExcel_Style_Conditional $pCellStyle Cell style to
     *            duplicate
     * @param string $pRange Range of cells (i.e. "A1:B10"), or just one cell
     *            (i.e. "A1")
     * @throws PHPExcel_Exception
    ...
    /**
     * Duplicate cell style array to a range of cells Please note that this will
     * overwrite existing cell styles for cells in range, if they are in the
     * styles array. For example, if you decide to set a range of cells to font
     * bold, only include font bold in the styles array.
     *
     * @deprecated
     *
     ...
     * @return PHPExcel_Worksheet
     */
    public function setBreakByColumnAndRow($pColumn = 0, $pRow = 1, 
        $pBreak = PHPExcel_Worksheet::BREAK_NONE) {
        return $this->setBreak(
            PHPExcel_Cell::stringFromColumnIndex($pColumn) . $pRow, $pBreak);
    }
     ...
     * @return PHPExcel_Worksheet
     */
    public function mergeCellsByColumnAndRow($pColumn1 = 0, $pRow1 = 1, 
        $pColumn2 = 0, $pRow2 = 1) {
     ...
     * @return PHPExcel_Worksheet
     */
    public function unmergeCellsByColumnAndRow($pColumn1 = 0, $pRow1 = 1, 
        $pColumn2 = 0, $pRow2 = 1) {
        $cellRange = PHPExcel_Cell::stringFromColumnIndex($pColumn1) . $pRow1 .
             ':' . PHPExcel_Cell::stringFromColumnIndex($pColumn2) . $pRow2;
    ...
    /**
     * Set merge cells array for the entire sheet. Use instead mergeCells() to
     * merge a single cell range.
     *
     * @param array
     */
     ...
     * @param boolean $pAlreadyHashed If the password has already been hashed,
     *            set this to true
     * @throws PHPExcel_Exception
     * @return PHPExcel_Worksheet
     */
    public function protectCells($pRange = 'A1', $pPassword = '', 
     ...
     * @return PHPExcel_Worksheet
     */
    public function protectCellsByColumnAndRow($pColumn1 = 0, $pRow1 = 1, 
        $pColumn2 = 0, $pRow2 = 1, $pPassword = '', $pAlreadyHashed = false) {
        $cellRange = PHPExcel_Cell::stringFromColumnIndex($pColumn1) . $pRow1 .
             ':' . PHPExcel_Cell::stringFromColumnIndex($pColumn2) . $pRow2;
     ...
     * @return PHPExcel_Worksheet
     */
    public function unprotectCellsByColumnAndRow($pColumn1 = 0, $pRow1 = 1, 
        $pColumn2 = 0, $pRow2 = 1, $pPassword = '', $pAlreadyHashed = false) {
        $cellRange = PHPExcel_Cell::stringFromColumnIndex($pColumn1) . $pRow1 .
             ':' . PHPExcel_Cell::stringFromColumnIndex($pColumn2) . $pRow2;
     ...
     * Set AutoFilter
     *
     * @param PHPExcel_Worksheet_AutoFilter|string $pValue A simple string
     *            containing a Cell range like 'A1:E10' is permitted for
     ...
     * @return PHPExcel_Worksheet
     */
    public function setAutoFilterByColumnAndRow($pColumn1 = 0, $pRow1 = 1, 
        $pColumn2 = 0, $pRow2 = 1) {
     ...
     * Freeze Pane
     *
     * @param string $pCell Cell (i.e. A2) Examples: A2 will freeze the rows
     *            above cell A2 (i.e row 1) B1 will freeze the columns to the
     *            left of cell B1 (i.e column A) B2 will freeze the rows above
     *            and to the left of cell A2 (i.e row 1 and column A)
     * @throws PHPExcel_Exception
     * @return PHPExcel_Worksheet
     */
     ...
     * Insert a new column, updating all possible related data
     *
     * @param int $pBefore Insert before this one (numeric column coordinate of
     *            the cell)
     * @param int $pNumCols Number of columns to insert
     * @throws PHPExcel_Exception
     * @return PHPExcel_Worksheet
    ...
    public function insertNewColumnBeforeByIndex($pBefore = 0, $pNumCols = 1) {
        if ($pBefore >= 0) {
            return $this->insertNewColumnBefore(
                PHPExcel_Cell::stringFromColumnIndex($pBefore), $pNumCols);
        } else {
            throw new PHPExcel_Exception(
     ...
     * Remove a column, updating all possible related data
     *
     * @param int $pColumn Remove starting with this one (numeric column
    ...
    public function removeColumnByIndex($pColumn = 0, $pNumCols = 1) {
        if ($pColumn >= 0) {
            return $this->removeColumn(
                PHPExcel_Cell::stringFromColumnIndex($pColumn), $pNumCols);
        } else {
            throw new PHPExcel_Exception(
     ...
     *
     * @param string $pCoordinate Cell range, examples: 'A1', 'B2:G5', 'A:C',
     *            '3:6'
     * @throws PHPExcel_Exception
     * @return PHPExcel_Worksheet
     */
     ...
     * @return PHPExcel_Worksheet
     */
    public function fromArray($source = null, $nullValue = null, $startCell = 'A1', 
                ...
                foreach ($rowData as $cellValue) {
                    if ($strictNullComparison) {
                        if ($cellValue !== $nullValue) {
                            // Set cell value
                            $this->getCell($currentColumn . $startRow)->setValue(
                                $cellValue);
                        }
                    } else {
                        if ($cellValue != $nullValue) {
     ...
     *            columns indexed by number counting from zero True - Return
     *            rows and columns indexed by their actual row and column IDs
     * @return array
     */
    public function rangeToArray($pRange = 'A1', $nullValue = null, 
            ...
            for ($col = $minCol; $col != $maxCol; ++ $col) {
                ...
                if ($this->_cellCollection->isDataSet($col . $row)) {
                    ...
                    if ($cell->getValue() !== null) {
                        ...
                        if ($formatData) {
                            $style = $this->_parent->getCellXfByIndex(
                                $cell->getXfIndex());
    ...
    public function toArray($nullValue = null, $calculateFormulas = true, 
        ...
        // Return
        return $this->rangeToArray('A1:' . $maxCol . $maxRow, $nullValue, 
            $calculateFormulas, $formatData, $returnCellRef);
    }
    ...
    public function garbageCollect() {
        ...
        if ($highestColumn < 0) {
            ...
            $this->_cachedHighestColumn = PHPExcel_Cell::stringFromColumnIndex(
                -- $highestColumn);
        }
    ...
    /**
     * Extract worksheet title from range. Example:
     * extractSheetTitle("testSheet!A1") ==> 'A1' Example:
     * extractSheetTitle("'testSheet 1'!A1", true) ==> array('testSheet 1',
     * 'A1');
     *
     ...
     * @return PHPExcel_Worksheet
     */
    public function setHyperlink($pCellCoordinate = 'A1', 
     ...
     * @return PHPExcel_Worksheet
     */
    public function setDataValidation($pCellCoordinate = 'A1', 
    ...
    /**
     * Accepts a range, returning it as a range that falls within the current
     * highest row and column of the worksheet
     *
     * @param string $range
     ...
     *
     * @param null|string Same rule as Title minus space not allowed (but, like
     *            Excel, change silently space to underscore)
     * @return objWorksheet
     * @throws PHPExcel_Exception
     */
    public function setCodeName($pValue = null) {
        ...
        $pValue = str_replace(' ', '_', $pValue); // Excel does this
                                                  // automatically without
                                                  // flinching, we are doing the
                                                  // same
                                                  // Syntax check
        ...
        // We use the same code that setTitle to find a valid codeName else not
        // using a space (Excel don't like) but a '_'
        ...
        if ($this->getParent()) {
            ...
            if ($this->getParent()->sheetCodeNameExists($pValue)) {
                ...
                $i = 1;
                while ($this->getParent()->sheetCodeNameExists(
                    $pValue . '_' . $i)) {
                    ++ $i;
                    if ($i == 10) {
                        if (PHPExcel_Shared_String::CountCharacters($pValue) > 28) {
                ...
                $pValue = $pValue . '_' . $i; // ok, we have a valid name
                                                  // codeName is'nt used in formula
