...
class trendClass {
    ...
    /**
     * Cached results for each method when trying to identify which provides the
     * best fit
     *
    ...
    public static function calculate($trendType = self::TREND_BEST_FIT, $yValues, $xValues = array(), 
        $const = True) {
        ...
        $key = md5(
            $trendType . $const . serialize($yValues) . serialize($xValues));
        ...
        switch ($trendType) {
            ...
            case self::TREND_POWER:
                if (! isset(self::$_trendCache[$key])) {
                    ...
                    self::$_trendCache[$key] = new $className($yValues, $xValues, 
                        $const);
                }
            ...
            case self::TREND_POLYNOMIAL_6:
                if (! isset(self::$_trendCache[$key])) {
                    ...
                    self::$_trendCache[$key] = new PHPExcel_Polynomial_Best_Fit(
                        $order, $yValues, $xValues, $const);
                }
                return self::$_trendCache[$key];
                break;
            case self::TREND_BEST_FIT:
            case self::TREND_BEST_FIT_NO_POLY:
                // If the request is to determine the best fit regression, then
                // we test each trend line in turn
                // Start by generating an instance of each available trend
                // method
                ...
                if ($trendType != self::TREND_BEST_FIT_NO_POLY) {
                    foreach (self::$_trendTypePolyOrders as $trendMethod) {
                        ...
                        $bestFit[$trendMethod] = new PHPExcel_Polynomial_Best_Fit(
                            $order, $yValues, $xValues, $const);
                ...
                // Determine which of our trend lines is the best fit, and then
                // we return the instance of that trend class
                arsort($bestFitValue);
