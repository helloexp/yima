...
/*
 * For futur use define( 'PCLZIP_CB_PRE_LIST', 78005 ); define(
 * 'PCLZIP_CB_POST_LIST', 78006 ); define( 'PCLZIP_CB_PRE_DELETE', 78007 );
...
class PclZip {
    ...
    function create($p_filelist) {
        ...
        foreach ($v_att_list as $v_entry) {
            $v_result = $this->privFileDescrParseAtt($v_entry, 
                $v_filedescr_list[], $v_options, $v_supported_attributes);
            if ($v_result != 1) {
                return 0;
            }
        }
    ...
    function privParseOptions(&$p_options_list, $p_size, &$v_result_list, 
        ...
        while ($i < $p_size) {
            ...
            switch ($p_options_list[$i]) {
                ...
                // ----- Look for options that request an array of string for
                // value
                case PCLZIP_OPT_BY_NAME:
                    ...
                    // ----- Get the value
                    if (is_string($p_options_list[$i + 1])) {
                        $v_result_list[$p_options_list[$i]][0] = $p_options_list[$i +
                             1];
                ...
                // ----- Look for options that request an EREG or PREG
                // expression
                case PCLZIP_OPT_BY_EREG:
                    // ereg() is deprecated starting with PHP 5.3. Move
                    // PCLZIP_OPT_BY_EREG
                ...
                case PCLZIP_OPT_BY_INDEX:
                    ...
                    // each index item in the list must be a couple with a start
                    // and
        ...
        */
        ...
        if ($v_requested_options !== false) {
            for ($key = reset($v_requested_options); $key = key(
                $v_requested_options); $key = next($v_requested_options)) {
    ...
    // or a string to be added as file. For any other type of files (link,
    // other)
            ...
            if ($v_descr['type'] == 'folder') {
                ...
                if ($v_folder_handler = @opendir($v_descr['filename'])) {
                    while (($v_item_handler = @readdir($v_folder_handler)) !==
                        ...
                        $v_dirlist_descr[$v_dirlist_nb]['filename'] = $v_descr['filename'] .
                             '/' . $v_item_handler;
                        
                        // ----- Look for different stored filename
                        // Because the name of the folder was changed, the name
                        // of the
...
function privOpenFd($p_mode) {
    ...
    if (($this->zip_fd = @fopen($this->zipname, $p_mode)) == 0) {
        // ----- Error log
        PclZip::privErrorLog(PCLZIP_ERR_READ_OPEN_FAIL, 
            'Unable to open archive \'' . $this->zipname . '\' in ' . $p_mode .
                 ' mode');
...
// Description :
// $p_add_dir and $p_remove_dir will give the ability to memorize a path which
// is
// different from the real path of the file. This is usefull if you want to have
// PclTar
...
function privAddList($p_filedescr_list, &$p_result_list, &$p_options) {
    ...
    // ----- Create the central dir footer
    if (($v_result = $this->privWriteCentralHeader($v_count, $v_size, $v_offset, 
        $v_comment)) != 1) {
        // ----- Reset the file list
        unset($v_header_list);
...
function privAddFileList($p_filedescr_list, &$p_result_list, &$p_options) {
    ...
    for ($j = 0; ($j < sizeof($p_filedescr_list)) && ($v_result == 1); $j ++) {
        ...
        $p_filedescr_list[$j]['filename'] = PclZipUtilTranslateWinPath(
            $p_filedescr_list[$j]['filename'], false);
        ...
        if (($p_filedescr_list[$j]['type'] == 'file') ||
             ($p_filedescr_list[$j]['type'] == 'virtual_file') || (($p_filedescr_list[$j]['type'] ==
             'folder') && (! isset($p_options[PCLZIP_OPT_REMOVE_ALL_PATH]) ||
...
function privAddFile($p_filedescr, &$p_header, &$p_options) {
    ...
    /*
     * TBC : Removed if (isset($p_filedescr['stored_filename'])) {
     * $v_stored_filename = $p_filedescr['stored_filename']; } else {
    ...
    if (isset($p_options[PCLZIP_CB_PRE_ADD])) {
        ...
        // header.
        // eval('$v_result =
        // '.$p_options[PCLZIP_CB_PRE_ADD].'(PCLZIP_CB_PRE_ADD,
        // $v_local_header);');
    ...
    if (isset($p_options[PCLZIP_CB_POST_ADD])) {
        ...
        // header.
        // eval('$v_result =
        // '.$p_options[PCLZIP_CB_POST_ADD].'(PCLZIP_CB_POST_ADD,
        // $v_local_header);');
...
// ----- Packed data
$v_binary_data = pack("VvvvvvvVVVvvvvvVV", 0x02014b50, $p_header['version'], 
    $p_header['version_extracted'], $p_header['flag'], $p_header['compression'], 
    $v_mtime, $v_mdate, $p_header['crc'], $p_header['compressed_size'], 
...
// $p_remove_path : Path to remove (from the file memorized path) while writing
// the
         ...
         (! $v_extract); $j ++) {
            ...
            if (substr($p_options[PCLZIP_OPT_BY_NAME][$j], - 1) == "/") {
                ...
                if ((strlen($v_header['stored_filename']) >
                 strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) && (substr(
                    $v_header['stored_filename'], 0, 
 ...
 * ($p_options[PCLZIP_OPT_BY_EREG] != "")) { if
 * (ereg($p_options[PCLZIP_OPT_BY_EREG], $v_header['stored_filename'])) {
 * $v_extract = true; } }
 */
...
elseif ((isset($p_options[PCLZIP_OPT_EXTRACT_IN_OUTPUT])) &&
    ...
    // ----- Extracting the file
    $v_result1 = $this->privExtractFile($v_header, $p_path, $p_remove_path, 
        $p_remove_all_path, $p_options);
    if ($v_result1 < 1) {
        $this->privCloseFd();
        $this->privSwapBackMagicQuotes();
...
$p_options[PCLZIP_OPT_EXTRACT_DIR_RESTRICTION], $p_entry['filename']);
if ($v_inclusion == 0) {
...
// ----- Look if the extracted file is older
    ...
    if ((isset($p_options[PCLZIP_OPT_REPLACE_NEWER])) &&
         ($p_options[PCLZIP_OPT_REPLACE_NEWER] === true)) {} else {
...
if ($p_entry['compression'] == 0) {
    ...
    while ($v_size != 0) {
        ...
        $v_buffer = @fread($this->zip_fd, $v_read_size);
        /*
         * Try to speed up the code $v_binary_data = pack('a'.$v_read_size,
         * $v_buffer); @fwrite($v_dest_file, $v_binary_data, $v_read_size);
         */
    ...
    if ((! isset($p_options[PCLZIP_OPT_TEMP_FILE_OFF])) && (isset(
        $p_options[PCLZIP_OPT_TEMP_FILE_ON]) || (isset(
...
// $v_local_header);');
$v_result = $p_options[PCLZIP_CB_POST_EXTRACT](PCLZIP_CB_POST_EXTRACT, 
$v_local_header);
...
$v_binary_data = pack('va1a1Va1a1', 0x8b1f, Chr($p_entry['compression']), 
...
$p_header['mtime'] = @mktime($v_hour, $v_minute, $v_seconde, $v_month, $v_day, 
$v_year);
} else {
$p_header['mtime'] = time();
}
...
// ----- First try : look if this is an archive with no commentaries (most of
// the time)
...
// Note we mask the old value down such that once shifted we can never end up
// with more than a 32bit number
// Otherwise on systems where we have 64bit integers the check below for the
// magic number will fail.
...
$v_header_list[$v_nb_extracted])) != 1) {
// ----- Close the zip file
$this->privCloseFd();
 ...
 * else if ( (isset($p_options[PCLZIP_OPT_BY_EREG])) &&
 * ($p_options[PCLZIP_OPT_BY_EREG] != "")) { if
 * (ereg($p_options[PCLZIP_OPT_BY_EREG],
 * $v_header_list[$v_nb_extracted]['stored_filename'])) { $v_found = true; } }
 */
...
// ----- Create the central dir footer
if (($v_result = $v_temp_zip->privWriteCentralHeader(sizeof($v_header_list), 
...
// ----- Copy the files from the archive to the temporary file
// TBC : Here I should better append the file and go back to erase the central
// dir
$v_size = $v_central_dir['offset'];
while ($v_size != 0) {
$v_read_size = ($v_size < PCLZIP_READ_BLOCK_SIZE ? $v_size : PCLZIP_READ_BLOCK_SIZE);
...
// ----- Create the central dir footer
if (($v_result = $this->privWriteCentralHeader(
$v_central_dir['entries'] + $v_central_dir_to_add['entries'], $v_size, $v_offset, 
$v_comment)) != 1) {
$this->privCloseFd();
$p_archive_to_add->privCloseFd();
@fclose($v_zip_temp_fd);
...
if ((($v_prefix == 'PCLZIP_OPT') || ($v_prefix == 'PCLZIP_CB_') ||
 ($v_prefix == 'PCLZIP_ATT')) && ($v_list[$v_key] == $p_option)) {
return $v_key;
}
}
