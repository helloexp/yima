...
class PHPExcel_Shared_OLE_PPS_Root extends PHPExcel_Shared_OLE_PPS {
    ...
    public function __construct($time_1st, $time_2nd, $raChild) {
        ...
        parent::__construct(null, PHPExcel_Shared_OLE::Asc2Ucs('Root Entry'), 
            PHPExcel_Shared_OLE::OLE_PPS_TYPE_ROOT, null, null, null, $time_1st, 
            $time_2nd, null, $raChild);
    }

    /**
     * Method for saving the whole OLE container (including files). In fact, if
     * called with an empty argument (or '-'), it saves to a temporary file and
     * then outputs it's contents to stdout. If a resource pointer to a stream
     * created by fopen() is passed it will be used, but you have to close such
     * stream by yourself.
     *
     * @param string|resource $filename The name of the file or stream where to
     *            save the OLE container.
     * @access public
    ...
    public function save($filename) {
        ...
        $this->_BIG_BLOCK_SIZE = pow(2, 
            ((isset($this->_BIG_BLOCK_SIZE)) ? self::_adjust2(
                $this->_BIG_BLOCK_SIZE) : 9));
        $this->_SMALL_BLOCK_SIZE = pow(2, 
            ((isset($this->_SMALL_BLOCK_SIZE)) ? self::_adjust2(
        ...
        $aList = array();
        PHPExcel_Shared_OLE_PPS::_savePpsSetPnt($aList, 
            array(
                $this));
    ...
    public function _calcSize(&$raList) {
        ...
        $iSlCnt = floor(
            $this->_BIG_BLOCK_SIZE / PHPExcel_Shared_OLE::OLE_LONG_INT_SIZE);
    ...
    public function _saveHeader($iSBDcnt, $iBBcnt, $iPPScnt) {
        ...
        $iBdCnt = floor(($iAll + $iBdCntW) / $iBlCnt) +
        ...
        if ($iBdCnt > $i1stBdL) {
            while (1) {
                ...
                ++ $iAllW;
                $iBdCntW = floor($iAllW / $iBlCnt) + (($iAllW % $iBlCnt) ? 1 : 0);
                $iBdCnt = floor(($iAllW + $iBdCntW) / $iBlCnt) +
                     ((($iAllW + $iBdCntW) % $iBlCnt) ? 1 : 0);
        ...
        fwrite($FILE, 
            "\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1" . "\x00\x00\x00\x00" .
                 ...
                 pack("v", 0x3b) . pack("v", 0x03) . pack("v", - 2) .
                 pack("v", 9) . pack("v", 6) . pack("v", 0) . "\x00\x00\x00\x00" .
                 "\x00\x00\x00\x00" . pack("V", $iBdCnt) .
                 pack("V", $iBBcnt + $iSBDcnt) . // ROOT
                                                // START
                pack("V", 0) . pack("V", 0x1000) . pack("V", $iSBDcnt ? 0 : - 2) . // Small
                                                                                   // Block
                                                                                   // Depot
                pack("V", $iSBDcnt));
    ...
    public function _saveBigData($iStBlk, &$raList) {
        ...
        for ($i = 0; $i < $iCount; ++ $i) {
            if ($raList[$i]->Type != PHPExcel_Shared_OLE::OLE_PPS_TYPE_DIR) {
                $raList[$i]->Size = $raList[$i]->_DataLen();
                if (($raList[$i]->Size >=
                     PHPExcel_Shared_OLE::OLE_DATA_SIZE_SMALL) || (($raList[$i]->Type ==
    ...
    public function _saveBbd($iSbdSize, $iBsize, $iPpsCnt) {
        ...
        if ($iBdCnt > $i1stBdL) {
            while (1) {
                ...
                ++ $iAllW;
                $iBdCntW = floor($iAllW / $iBbCnt) + (($iAllW % $iBbCnt) ? 1 : 0);
                $iBdCnt = floor(($iAllW + $iBdCntW) / $iBbCnt) +
                     ((($iAllW + $iBdCntW) % $iBbCnt) ? 1 : 0);
