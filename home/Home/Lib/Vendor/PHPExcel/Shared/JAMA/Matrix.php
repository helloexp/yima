...
/*
 * Matrix class @author Paul Meagher @author Michael Bommarito @author Lukasz
 * Karapuda @author Bartek Matosiuk @version 1.8 @license PHP v3.0 @see
...
class PHPExcel_Shared_JAMA_Matrix {
    ...
    /**
     * Polymorphic constructor As PHP has no support for polymorphic
     * constructors, we hack our own sort of polymorphism using func_num_args,
     * func_get_arg, and gettype. In essence, we're just implementing a simple
     * RTTI filter and calling the appropriate constructor.
     */
                ...
                case 'array,integer':
                    ...
                    if (($this->m * $this->n) == count($args[0])) {
                        ...
                        throw new PHPExcel_Calculation_Exception(
                            self::ArrayLengthException);
                    }
                    break;
                default:
                    throw new PHPExcel_Calculation_Exception(
                        self::PolymorphicArgumentException);
                    break;
            }
        } else {
            throw new PHPExcel_Calculation_Exception(
            ...
            for ($i = 0; $i < $this->m; ++ $i) {
                for ($j = 0; $j < $this->n; ++ $j) {
                    ...
                    if ((is_string($this->A[$i][$j])) &&
                        ...
                        $validValues &= PHPExcel_Shared_String::convertToNumberIfFraction(
                            $this->A[$i][$j]);
                    }
            ...
            switch ($match) {
                case 'object':
                    ...
                    if ($this->n == $B->m) {
                        ...
                        throw new PHPExcel_Calculation_Exception(
                            JAMAError(MatrixDimensionMismatch));
                    }
                    break;
                case 'array':
