...
class PHPExcel_Shared_Font {
    ...
    /**
     * How wide is a default column for a given default font and size? Empirical
     * data found by inspecting real Excel files and reading off the pixel width
     ...
     * ttf files. There should be a trailing slash. Typical locations on variout
     * some platforms: <ul> <li>C:/Windows/Fonts/</li>
     * <li>/usr/share/fonts/truetype/</li> <li>~/.fonts/</li> </ul>
     *
     * @param string $pValue
     */
    ...
    /**
     * Calculate an (approximate) OpenXML column width, based on font size and
     * text contained
     *
     ...
     */
    public static function calculateColumnWidth(PHPExcel_Style_Font $font, 
        $cellText = '', $rotation = 0, PHPExcel_Style_Font $defaultFont = null) {
        ...
        try {
            ...
            $columnWidth += ceil(
                self::getTextWidthPixelsExact('0', $font, 0) * 1.07); // pixels
                                                                          // incl.
                                                                          // padding
        ...
        $columnWidth = PHPExcel_Shared_Drawing::pixelsToCellDimension(
            $columnWidth, $defaultFont);
    ...
    /**
     * Get GD text width in pixels for a string of text in a certain font at a
    ...
    public static function getTextWidthPixelsExact($text, 
        PHPExcel_Style_Font $font, $rotation = 0) {
    ...
    /**
     * Get approximate width in pixels for a string of text in a certain font at
     * a certain rotation angle
     *
    ...
    public static function getTextWidthPixelsApprox($columnText, 
        PHPExcel_Style_Font $font = null, $rotation = 0) {
        $fontName = $font->getName();
        $fontSize = $font->getSize();
        
        // Calculate column width in pixels. We assume fixed glyph width. Result
        // varies with font name and size.
        switch ($fontName) {
            case 'Calibri':
                // value 8.26 was found via interpolation by inspecting real
                // Excel files with Calibri 11 font.
                $columnWidth = (int) (8.26 *
                     PHPExcel_Shared_String::CountCharacters($columnText));
                $columnWidth = $columnWidth * $fontSize / 11; // extrapolate
                                                              // from font size
                break;
            
            case 'Arial':
                // value 7 was found via interpolation by inspecting real Excel
                // files with Arial 10 font.
                $columnWidth = (int) (7 *
                     PHPExcel_Shared_String::CountCharacters($columnText));
                $columnWidth = $columnWidth * $fontSize / 10; // extrapolate
                                                              // from font size
                break;
            
            case 'Verdana':
                // value 8 was found via interpolation by inspecting real Excel
                // files with Verdana 10 font.
                $columnWidth = (int) (8 *
                     PHPExcel_Shared_String::CountCharacters($columnText));
                $columnWidth = $columnWidth * $fontSize / 10; // extrapolate
                                                              // from font size
    ...
    /**
     * Get the effective column width for columns without a column dimension or
     * column with width -1 For example, for Calibri 11 this is 9.140625 (64 px)
     *
     * @param PHPExcel_Style_Font $font The workbooks default font
     * @param boolean $pPixels true = return column width in pixels, false =
     *            return in OOXML units
     * @return mixed Column width
     */
    public static function getDefaultColumnWidthByFont(PHPExcel_Style_Font $font, 
        $pPixels = false) {
        if (isset(
            self::$defaultColumnWidths[$font->getName()][$font->getSize()])) {
            ...
            // We don't have data for this particular font and size, use
            // approximation by
    ...
    /**
     * Get the effective row height for rows without a row dimension or rows
     * with height -1 For example, for Calibri 11 this is 15 points
     *
     * @param PHPExcel_Style_Font $font The workbooks default font
