...
class PHPExcel_Shared_Date {
    ...
    /*
     * Names of the months of the year, indexed by shortname Planned usage for
     * locale settings @public @var string[]
     */
    ...
    /*
     * Base calendar year to use for calculations @private @var int
     */
     ...
     * @param boolean $adjustToTimezone Flag indicating whether $dateValue
     *            should be treated as a UST timestamp, or adjusted to UST
     ...
     *            should be treated as a UST timestamp, or adjusted to UST
     * @param string $timezone The timezone for finding the adjustment from UST
     * @return mixed Excel date/time value or boolean FALSE on failure
     */
    public static function PHPToExcel($dateValue = 0, $adjustToTimezone = FALSE, 
        $timezone = NULL) {
     ...
     */
    public static function FormattedPHPToExcel($year, $month, $day, $hours = 0, 
        $minutes = 0, $seconds = 0) {
        if (self::$_excelBaseDate == self::CALENDAR_WINDOWS_1900) {
            //
            // Fudge factor for the erroneous fact that the year 1900 is treated
            // as a Leap Year in MS Excel
        ...
        // Calculate the Julian Date, then subtract the Excel base date (JD
        // 2415020 = 31-Dec-1899 Giving Excel Date of 0)
        $century = substr($year, 0, 2);
        $decade = substr($year, 2, 2);
        $excelDate = floor((146097 * $century) / 4) + floor(
            (1461 * $decade) / 4) + floor((153 * $month + 2) / 5) + $day + 1721119 -
             $my_excelBaseDate + $excel1900isLeapYear;
    ...
    public static function isDateTimeFormatCode($pFormatCode = '') {
        if (strtolower($pFormatCode) ===
            ...
            // "General" contains an epoch letter 'e', so we trap for it
            // explicitly here (case-insensitive check)
            return FALSE;
        ...
        // Typically number, currency or accounting (or occasionally fraction)
        // formats
        if ((substr($pFormatCode, 0, 1) == '_') ||
             (substr($pFormatCode, 0, 2) == '0 ')) {
            return FALSE;
        }
        // Try checking for any of the date formatting characters that don't
        // appear within square braces
        if (preg_match(
            '/(^|\])[^\[]*[' . self::$possibleDateFormatCharacters . ']/i', 
            $pFormatCode)) {
            // We might also have a format mask containing quoted strings...
            // we don't want to test for any of our characters within the quoted
            // blocks
            if (strpos($pFormatCode, '"') !== FALSE) {
                $segMatcher = FALSE;
                foreach (explode('"', $pFormatCode) as $subVal) {
                    // Only test in alternate array entries (the non-quoted
                    // blocks)
     ...
     *
     * @param string $dateValue Examples: '2009-12-31', '2009-12-31 15:59',
     *            '2009-12-31 15:59:10'
