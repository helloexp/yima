 ...
 */
defined('IDENTIFIER_OLE') || define('IDENTIFIER_OLE', 
...
class PHPExcel_Shared_OLERead {
    ...
    // Minimum size of a standard stream = 4096 bytes, streams smaller than this
    ...
    public function read($sFileName) {
        ...
        // Don't bother reading the whole file until we know it's a valid OLE
        // file
        ...
        // SecID of the first sector of the MSAT (or -2 if no additional sectors
        // are used)
        ...
        if ($this->numExtensionBlocks != 0) {
            $bbdBlocks = (self::BIG_BLOCK_SIZE - self::BIG_BLOCK_DEPOT_BLOCKS_POS) /
                 4;
        }
    ...
    public function getStream($stream) {
        ...
        if ($this->props[$stream]['size'] < self::SMALL_BLOCK_THRESHOLD) {
            $rootdata = $this->_readData(
                $this->props[$this->rootentry]['startBlock']);
    ...
    private function _readPropertySets() {
        ...
        while ($offset < $entryLen) {
            ...
            // sectorID of first sector or short sector, if this entry refers to
            // a stream (the case with workbook)
            // sectorID of first sector of the short-stream container stream, if
            // this entry is root entry
