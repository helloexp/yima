...
class PHPExcel_Calculation_TextData {
    ...
    private static function _uniord($c) {
        ...
        if (ord($c{0}) >= 252 && ord($c{0}) <= 253)
            return (ord($c{0}) - 252) * 1073741824 +
                 (ord($c{1}) - 128) * 16777216 + (ord($c{2}) - 128) * 262144 +
    ...
    public static function CHARACTER($character) {
        $character = PHPExcel_Calculation_Functions::flattenSingleValue(
            $character);
    ...
    public static function TRIMNONPRINTABLE($stringValue = '') {
        $stringValue = PHPExcel_Calculation_Functions::flattenSingleValue(
            $stringValue);
    ...
    public static function ASCIICODE($characters) {
        if (($characters === NULL) || ($characters === ''))
            return PHPExcel_Calculation_Functions::VALUE();
        $characters = PHPExcel_Calculation_Functions::flattenSingleValue(
            $characters);
    ...
    /**
     * DOLLAR This function converts a number to text using currency format,
     * with the decimals rounded to the specified place. The format used is
     * $#,##0.00_);($#,##0.00)..
     *
     * @param float $value The value to format
     * @param int $decimals The number of digits to display to the right of the
     *            decimal point. If decimals is negative, number is rounded to
     *            the left of the decimal point. If you omit decimals, it is
     *            assumed to be 2
     * @return string
     */
    public static function DOLLAR($value = 0, $decimals = 2) {
        $value = PHPExcel_Calculation_Functions::flattenSingleValue($value);
        $decimals = is_null($decimals) ? 0 : PHPExcel_Calculation_Functions::flattenSingleValue(
            $decimals);
    ...
    public static function SEARCHSENSITIVE($needle, $haystack, $offset = 1) {
        ...
        $haystack = PHPExcel_Calculation_Functions::flattenSingleValue(
            $haystack);
        ...
        if (! is_bool($needle)) {
            ...
            if (($offset > 0) &&
                 (PHPExcel_Shared_String::CountCharacters($haystack) > $offset)) {
    ...
    public static function FIXEDFORMAT($value, $decimals = 2, $no_commas = FALSE) {
        $value = PHPExcel_Calculation_Functions::flattenSingleValue($value);
        $decimals = PHPExcel_Calculation_Functions::flattenSingleValue(
            $decimals);
        $no_commas = PHPExcel_Calculation_Functions::flattenSingleValue(
            $no_commas);
    ...
    public static function LOWERCASE($mixedCaseString) {
        $mixedCaseString = PHPExcel_Calculation_Functions::flattenSingleValue(
            $mixedCaseString);
     ...
     * @return string
     */
    public static function SUBSTITUTE($text = '', $fromText = '', $toText = '', 
        $instance = 0) {
        $text = PHPExcel_Calculation_Functions::flattenSingleValue($text);
        $fromText = PHPExcel_Calculation_Functions::flattenSingleValue(
            $fromText);
        $toText = PHPExcel_Calculation_Functions::flattenSingleValue($toText);
        $instance = floor(
            PHPExcel_Calculation_Functions::flattenSingleValue($instance));
    ...
    public static function RETURNSTRING($testValue = '') {
        $testValue = PHPExcel_Calculation_Functions::flattenSingleValue(
            $testValue);
