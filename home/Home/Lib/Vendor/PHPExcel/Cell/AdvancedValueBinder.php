 ...
 *            (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Cell_AdvancedValueBinder extends PHPExcel_Cell_DefaultValueBinder implements 
    ...
    public function bindValue(PHPExcel_Cell $cell, $value = null) {
        ...
        if ($dataType === PHPExcel_Cell_DataType::TYPE_STRING &&
            ...
            if (preg_match(
                '/^' . preg_quote($currencyCode) . ' *(\d{1,3}(' .
                     preg_quote($thousandsSeparator) . '\d{3})*|(\d+))(' .
                     preg_quote($decimalSeparator) . '\d{2})?$/', $value)) {
                // Convert value to number
                $value = (float) trim(
                    str_replace(
                        array(
            ...
            // Check for datetime, e.g. '2008-12-31', '2008-12-31 15:59',
            // '2008-12-31 15:59:10'
