...
/**
 * Class to create QR-code arrays for TCPDF class. QR Code symbol is a 2D
 ...
 * @author Nicola Asuni
 * @copyright 2010 Nicola Asuni - Tecnick.com S.r.l (www.tecnick.com) Via Della
 *            Pace, 11 - 09044 - Quartucciu (CA) - ITALY - www.tecnick.com -
 *            info@tecnick.com
...
if (! class_exists('QRcode', false)) {
    ...
    if (! function_exists('str_split')) {
         ...
         *         one character in length. FALSE is returned if split_length is
         *         less than 1. If the split_length length exceeds the length of
         *         string , the entire string is returned as the first (and
         *         only) array element.
         */
    ...
    /**
     * Class to create QR-code arrays for TCPDF class. QR Code symbol is a 2D
     * barcode that can be scanned by handy terminals such as a mobile phone
     * with CCD. The capacity of QR Code is up to 7000 digits or 4000
     * characters, and has high robustness. This class supports QR Code model 2,
     * described in JIS (Japanese Industrial Standards) X0510:2004 or ISO/IEC
     * 18004. Currently the following features are not supported: ECI and FNC1
     * mode, Micro QR Code, QR Code model 1, Structured mode. This class is
     * derived from "PHP QR Code encoder" by Dominik Dzienia
     * (http://phpqrcode.sourceforge.net/) based on "libqrencode C library
     * 3.1.1." by Kentaro Fukuchi
     * (http://megaui.net/fukuchi/works/qrencode/index.en.html), contains
     * Reed-Solomon code written by Phil Karn, KA9Q. QR Code is registered
     * trademark of DENSO WAVE INCORPORATED
     * (http://www.denso-wave.com/qrcode/index-e.html). Please read comments on
     * this class source file for full copyright and license information.
     *
     * @name QRcode
     * @package com.tecnick.tcpdf
     * @abstract Class for generating QR-code array for TCPDF.
     ...
     *            Della Pace, 11 - 09044 - Quartucciu (CA) - ITALY -
    ...
    class QRcode {
        ...
        /**
         *
         * @var QR code version. Size of QRcode is defined as version. Version
         *      is from 1 to 40. Version 1 is 21*21 matrix. And 4 modules
         *      increases whenever 1 version increases. So version 40 is 177*177
         *      matrix.
         * @access protected
         */
        protected $version = 0;
        ...
        /**
         *
         * @var array Table of the capacity of symbols See Table 1 (pp.13) and
        ...
        /**
         *
         * @var array Positions of alignment patterns. This array includes only
         *      the second and the third position of the alignment patterns.
        ...
        /**
         *
         * @var array Version information pattern (BCH coded). See Table 1 in
         *      Appendix D (pp.68) of JIS X0510:2004. size: [QRSPEC_VERSION_MAX
         *      - 6]
         * @access protected
         */
         ...
         * @param string $eclevel error level: <ul><li>L : About 7% or less
         *            errors can be corrected.</li><li>M : About 15% or less
         *            errors can be corrected.</li><li>Q : About 25% or less
         *            errors can be corrected.</li><li>H : About 30% or less
         *            errors can be corrected.</li></ul>
         * @access public
         * @since 1.0.000
         */
        ...
        /**
         * Write Format Information on frame and returns the number of black
         * bits
         *
    ...
    protected function encodeModeNum($inputitem, $version) {
        ...
        if ($inputitem['size'] - $words * 3 == 1) {
            ...
            $inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 7, 
                $val);
        }
        return $inputitem;
    }
    ...
    protected function encodeModeAn($inputitem, $version) {
        ...
        if ($inputitem['size'] & 1) {
            ...
            $inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 6, 
                $val);
        }
        return $inputitem;
    }
    ...
    protected function encodeBitStream($inputitem, $version) {
        ...
        if ($inputitem['size'] > $words) {
            ...
            $inputitem['bstream'] = $this->appendBitstream(
                $inputitem['bstream'], $st1['bstream']);
            $inputitem['bstream'] = $this->appendBitstream(
                $inputitem['bstream'], $st2['bstream']);
        } else {
            switch ($inputitem['mode']) {
                ...
                case QR_MODE_KJ:
                    {
                        $inputitem = $this->encodeModeKanji($inputitem, 
                            $version);
                        break;
                    }
                case QR_MODE_ST:
    ...
    /**
     * Append data to an input object. The data is copied and appended to the
     * input object.
     *
     ...
     * @param int $version version
     * @param int $level error correction level
     * @param array $spec an array of ECC specification contains as following:
     *            {# of type1 blocks, # of data code, # of ecc code, # of type2
     *            blocks, # of data code}
     * @return array spec
     */
     ...
     *         array;</li><li>nroots = Number of generator;</li><li>roots =
     *         number of parity symbols;</li><li>fcr = First consecutive root,
     *         index form;</li><li>prim = Primitive element, index
     *         form;</li><li>iprim = prim-th root of 1, index form;</li><li>pad
     *         = Padding bytes in shortened block;</li><li>gfpoly</ul>.
     */
    ...
    protected function encode_rs_char($rs, $data, $parity) {
        ...
        $ALPHA_TO = & $rs['alpha_to']; // the address of an array of NN elements
                                       // to convert Galois field elements in
                                       // index (log) form to polynomial form
        $INDEX_OF = & $rs['index_of']; // the address of an array of NN elements
                                       // to convert Galois field elements in
                                       // polynomial form to index (log) form
        $GENPOLY = & $rs['genpoly']; // an array of NROOTS+1 elements containing
                                     // the generator polynomial in index form
        $NROOTS = & $rs['nroots']; // the number of roots in the RS code
                                   // generator polynomial, which is the same as
                                   // the number of parity symbols in a block
