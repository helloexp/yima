...
function form_change(num){
	...
	$("#addmul1").click(function(){
		...
		var	addcon=[
				...
				'<a href="javascript:void(0)" class="input-addone fn" id="addone_'+thisid+'" data-type="addmu11" data-num="5" data-rel="'+thisid+'"><i></i>增加1条答案（最多50条）</span></a>',
				'</div>',
				'<div class="del">',
	...
	$("#addmul2").click(function(){
		...
		var	addcon=[
				...
				'<a href="javascript:void(0)" class="input-addone fn" data-type="addmu12" data-num="5" data-rel="'+thisid+'"><i></i>增加1条答案（最多50条）</span></a>',
				'</div>',
				'<div class="del">',
	...
	$("body").on("click",".btn-down",function(){
		if($(this).closest(".global-input2-choose").index()>=$(".global-input2-choose").length+3){
			...
			var objc = $("#iphonePreview-four #choseq_"+_this);
...
function PreviewbmAdd(){
	...
	$(".wap_check_title").keyup(function(){
		...
		if(name.substr(0, 7) == 'defined'){
			...
			$("#iphonePreview-four "+$sel).text($(this).val()+"：");
