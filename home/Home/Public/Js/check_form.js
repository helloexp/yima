...
var check_form=function(formname,opt)
{
	...
	function cf_trim(obj,flag){
		if(flag == 1)
		{
			var str = obj.val();
			str = $.trim(str);
			try{obj.val(str);}catch(e){};//file时会出错
		}
		return true;
	}
	//判断是否为空
	...
	function cf_charset( obj, para )
	{
		...
		for(var l=0;l<p_arrlen;l++ )
		{
			if( p_arr[l]=='en' ) {c_rule += "a-zA-Z";para_arr[l]='字母';}
			else if( p_arr[l]=='num' ) {c_rule += "0-9";para_arr[l]='整数';}
			else if( p_arr[l]=='fl' ) {c_rule	+= "0-9\\.0-9";para_arr[l]='小数';}
			else if( p_arr[l]=='cn' ) {c_rule	+= "\\u4E00-\\u9FA5";para_arr[l]='中文';}
			else if( p_arr[l]=='ul' ) {c_rule	+= "_";para_arr[l]='下划线';}
		}
