		...
		enableDnd: function(jq, id){
			...
			return jq.each(function(){
				...
				$(".datagrid-btable").each(function(){
					if($(this).find(".datagrid-btable").length==0){
						$(this).find("tr[node-id]").addClass("disabled");
					}else{
						$(this).find(">tbody>tr[node-id]").each(function(){
							$(this).addClass("canaccept")
						});
					}
				});
				$("tr.canaccept[node-id]").each(function(index, element) {
...
//						e.data.offsetHeight = 0;
				...
				$("tr.canaccept[node-id]").droppable({
					accept:'tr[node-id]',
					...
					onDrop:function(e,source){
						...
						$.ajax({
							type: "post",
							url: "{:U('Wfx/Fxgl/freeMovement')}",
							data: {"newParentId":dRow[opts.idField],"id":sRow[opts.idField]}, 
							beforeSend: function() {
								 Dialoading("正在更新");
							},
							success: function() {
