...
requirejs.config({
	baseUrl: "Home/Public/Label/Js/assets/Js/thirdparty",
	paths: {
		"jquery": "jquery",
...
requirejs(['jquery'], function($) {
	$(document).ready(function(e) {
		var loading = {
			tips:function(msg){
				...
				if($('#loadingBox').length==0){
					$("body").append('<section class="loadingBg" id="loadingBox"><section class="fullHtml loadingBox '+cl+'"><i></i>'+msg+'</section></section>');
				}
				$('#loadingBox').show();
			},
			close:function(time){
		...
		$(".icon-nav,.icon-out,.icon-home,.sideNav .sideNavCon dl dd,.nav dd,.wxPay,.alPay,.cameraVoucher,.list dd,.btn a,.dateNav li,dl.more").on("touchstart",function(){
		...
		$(".icon-nav,.icon-out,.icon-home,.sideNav .sideNavCon dl dd,.nav dd,.wxPay,.alPay,.cameraVoucher,.list dd,.btn a,.dateNav li,dl.more").on("touchend",function(){
			$(".touch").removeClass("touch");
		});
		$("a").on("click",function(){
			if($(this).attr("href")){
				var url = $(this).attr("href").toString().indexOf("php");
			}else{
				var url = 0;
			}
			if(url>=1){
		...
		if($("#listPage").length>=1){
			...
			var nScrollHeight = $("#wrapper").height()+$("#header").height();
			if($(".nomoredate").length>=1){
				$(".nomoredate").show();
			}else{
				$("#listPage").after('<div class="nomoredate">加载中...</div>');
				$(".nomoredate").show();
			}
			if(nScrollHeight < divHeight) {
				$(".nomoredate").text("没有更多数据了");
			}
			setTimeout(function(){
				$(".nomoredate").hide();
			},2000)
			$(window).scroll(function(){
				...
				var divHeight = $("body").height();
				var nScrollHeight = $("#wrapper").height()+$("#header").height()-80;
				var nScrollTop = $(this).scrollTop();
				if(nScrollTop + divHeight >= nScrollHeight) {
					$(".nomoredate").show();
					var box = $("#listPage").closest("div").find(".list dl");
					var url = $("#listPage").attr("href");
					$("#listPage").load(url+ " " + 'dd',function(response,status,xhr){
						if(status=="success"){
							if($("#listPage").html()==""){
								$(".nomoredate").text("没有更多数据了");
								flag = true;
								...
								listPage = url.split(/^(.*?page=)/)[2]*1;
								listPage++;
								var newurl = url.split(/^(.*?page=)/)[1]+listPage;
								$("#listPage").attr("href",newurl);
								var html = $("#listPage").html();
								box.append(html);
								setTimeout(function(){
									$(".nomoredate").hide();
								},2000)
		...
		var datelist = $(".list").find("dl dd").length;
		if(datelist==0){
			var html = '<div class="nodate"><i></i></div>';
			$(".list").html(html);
			$(".nomoredate").hide();
		};
    });
