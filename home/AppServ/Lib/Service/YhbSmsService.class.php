...
class YhbSmsService {
    ...
    public function smssend($mobile, $content, $extnum = '') {
        $data = array_merge($this->public_data, array(
                        'mobile'  => $mobile,
                        'content' => $content,
                        'extnum'  => $extnum,
    ...
    public function sendBarcodeSms($barID) {
        $barInfo = M('tbarcode_trace')->where(array(
                'id' => $barID,
        ))->find();
        if (!$barInfo) {
            throw_exception('未找到凭证');
        }
        $tpl       = '您已获得“[GOODS_NAME]”、有效期[BEGIN_TIME]-[END_TIME]，串码标号：[ASS_NUMBER]，优惠券详情请查看【翼惠宝】';
        $batchInfo = M('tbatch_info')->where(array(
                'id' => $barInfo['b_id'],
        ))->find();
        if (!$batchInfo) {
            throw_exception('未找到奖品信息');
        }
        $tpl = str_ireplace(array(
                '[GOODS_NAME]',
                '[BEGIN_TIME]',
                '[END_TIME]',
                '[ASS_NUMBER]',
                        ...
                        dateformat($barInfo['begin_time'], 'Y/n/j'),
                        dateformat($barInfo['end_time'], 'Y/n/j'),
                        $barInfo['assist_number'],
                ), $tpl);
        $res = Yhb_sms($barInfo['phone_no'], $tpl);
        if (!$res) {
            throw_exception("翼蕙宝短信接口发送失败");
        } else {
            return true;
        }

        /*
         * if (is_production()) { return $this->smssend($barInfo['phone_no'],
         * $tpl); } else { //通知支撑 $TransactionID = date("YmdHis") .
         * mt_rand(100000, 999999); //请求单号 //请求参数 $req_array = array(
         * 'NotifyReq' => array( 'TransactionID' => $TransactionID, 'ISSPID' =>
         * C('MOBILE_ISSPID'), 'SystemID' => C('ISS_SYSTEM_ID'), 'SendLevel' =>
         * '1', 'Recipients' => array( 'Number' => $barInfo['phone_no'] //手机号 ),
         * 'SendClass' => 'MMS', 'MessageText' => $tpl,//短信内容 'Subject' => '',
         * 'ActivityID' => C('MOBILE_ACTIVITYID'), 'ChannelID' => '',
         * 'ExtentCode' => '' ) ); $RemoteRequest = D('RemoteRequest',
         * 'Service'); $resp_array = $RemoteRequest->requestIssServ($req_array);
         * $ret_msg = $resp_array['NotifyRes']['Status']; if (!$resp_array ||
