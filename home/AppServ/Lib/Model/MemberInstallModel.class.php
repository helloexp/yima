 ...
 */
class MemberInstallModel extends Model {
    ...
    public function MemberAddOne($memberArr) {
        ...
        $list = M("tmember_cards")->where(array(
                'node_id'        => $memberArr['node_id'],
                'acquiesce_flag' => 1,
        ))->find();
        if (empty($list)) {
            log_write("无默认会员卡" . $memberArr['node_id']);
    ...
    // 查询会员是否领取过富文本活动积分
    public function receiveTextPoint($openId, $nodeId, $mId) {
        ...
        $memberInfo = M("tmember_info")->where(array(
                'node_id'   => $nodeId,
                'wx_openid' => $openId,
        ))->find();
        if ($memberInfo) {
        ...
        $integralStatus = $integralPoint->integralPointChange('14', $integralNum, $memberInfo['id'], $nodeId, $mId, '');
        ...
        // 新增行为数据
        $res = D("MemberBehavior")->addBehaviorType($memberInfo['id'], $nodeId, 16, $integralNum, $mId);
        if ($res === false) {
    ...
    public function configurationIntegralValue($nodeId, $mId) {
        // 标准校验营销活动
        $powers = M("tnode_info")->where(array(
                'node_id' => $nodeId,
        ))->getField('pay_module');
        if (empty($powers)) {
        ...
        $configData = M("tmarketing_info")->where(array(
                'node_id' => $nodeId,
                'id'      => $mId,
        ))->find();
        if (empty($configData)) {
    ...
    public function getMembers($node_id) {
        $result = M('tmember_info')->where(array(
                'node_id' => $node_id,
                'status'  => 0,
        ))->select();
        if (!$result) {
    ...
    public function getMemberDetailMsg($member_id) {
        $result = M()->table("tmember_info i")->join('tcity_code c ON i.citycode = c.path')->field('i.*, c.province, c.city, c.town')->where(array(
                'i.id'     => $member_id,
                'i.status' => 0,
        ))->find();
    ...
    public function getMemberCodeFlag($node_id, $member_num) {
        $result = M("tmember_qrcode")->where(array(
    ...
    public function wxTermMemberFlag(
            $node_id,
            $condition,
            $nom,
            $isAdd = false,
        ...
        if (!$result) {
            if ($isAdd) {
                $data = array(
                        ...
                        'add_time'   => date('YmdHis'),
                        'request_id' => '',
                        'channel_id' => isset($option['channel_id']) ? $option['channel_id'] : 0,
                        'batch_id' => isset($option['batch_id']) ? $option['batch_id'] : 0
                );
                if ($nom == 1) {
                    $data['phone_no']   = $condition;
                    $data['mwx_openid'] = $option['openid'];
                    $list               = M("tmember_cards")->where(array(
                            'node_id'        => $node_id,
                            'acquiesce_flag' => 1,
                    ))->find();
                    if (empty($list)) {
                        log_write("无默认会员卡" . $node_id);
    ...
    public function getMemberCards($node_id) {
        $memCard = M('tmember_cards')->where(array(
                'node_id' => $node_id,
        ))->select();
        if (!$memCard) {
            return false;
    ...
    public function getMemberIdToCard($node_id, $member_id) {
        ...
        $result = M('tmember_cards')->where(array(
                'id' => $memberInfo['card_id'],
        ))->find();
        if (!$result) {
            return false;
        }
    ...
    public function makeMemberCardNum($node_id, $member_id) {
        $mem_num = M("tmember_info")->where(array(
                'id' => $member_id,
        ))->getField("member_num");
        if ($mem_num) {
            return false;
        ...
        $result = M('tsequence')->where(array(
                'name' => 'member_' . $node_id,
        ))->count();
        if ($result == 0) {
            $result = M('tsequence')->add(array(
        ...
        $res = M('tmember_info')->where(array(
                'id' => $member_id,
        ))->save(array(
    ...
    public function getMemberInstallShow($node_id) {
        $result = M('tmember_center_config')->where(array(
                'node_id' => $node_id,
        ))->find();
        if (!$result) {
            return false;
    ...
    public function getBatchData($batchIdStr) {
        $result = M('tmarketing_info')->where(array(
    ...
    public function getBatchDataWap($batchIdStr) {
        $result = M('tmarketing_info')->where(array(
                '_string' => 'id in (' . $batchIdStr . ')',
        ))->field('id, batch_type, name, wap_title, wap_info')->select();
        ...
        foreach ($result as $k => $v) {
            $ret                    = M("tbatch_channel")->where(array(
                    'batch_type' => $v['batch_type'],
                    'batch_id'   => $v['id'],
                    'status'     => '1',
            ))->field('id label_id')->order('add_time desc')->select();
    ...
    public function getRedData($redIdStr) {
        $result = M()->table("tbonus_info b")->join("tbonus_detail d on b.id = d.bonus_id")->join('tgoods_info g on b.id = g.bonus_id')->where(array(
                        '_string' => 'b.id in (' . $redIdStr . ')',
                ))->field('b.id,b.bonus_page_name, b.bonus_num, d.get_num ,d.amount , g.goods_name,g.goods_image')->select();
    ...
    public function getListLabels($node_id) {
        $result = M("tmember_label")->where(array(
                'node_id' => $node_id,
        ))->select();
    ...
    public function getMemberLabels($node_id, $member_id) {
        $result = M()->table("tmember_label_ex e")->join("tmember_label l on e.label_id = l.id")->field("l.*")->where(array(
                "e.node_id"   => $node_id,
    ...
    public function judgedLabelFlag($node_id, $label_name) {
        $lFlag = M("tmember_label")->where(array(
                'node_id'    => $node_id,
    ...
    public function getLabelId($node_id, $batch_id, $batch_type) {
        $batchchannel_id = M("tbatch_channel")->where(array(
                "batch_id" => $batch_id,
                "node_id"  => $node_id,
        ))->order('add_time desc')->field("id, status")->find();
        if (!$batchchannel_id) {
            $sns_type   = CommonConst::SNS_TYPE_MEMSTORENAV;
            $channel_id = 0;
            $channel    = M("tchannel")->where(array(
                    "sns_type" => $sns_type,
                    "node_id"  => $node_id,
            ))->getField("id,status");
            if (!$channel) {
            ...
            // if($batchchannel_id['status'] == '2') {
            $ret = M("tbatch_channel")->where(array(
                    'id' => $batchchannel_id['id'],
            ))->save(array(
                    'end_time' => date('YmdHis', strtotime("+1 year")),
                    'status'   => 1,
            ));
    ...
    public function getCheckinInfo($memberId) {
        ...
        if ($lastCheckInDay != '') {
            if (date('Ymd', strtotime('+1 day', strtotime($lastCheckInDay))) == date('Ymd') || date('Ymd',
                            strtotime($lastCheckInDay)) == date('Ymd')
            ) {
        ...
        $return['todayChecked'] = date('Ymd', strtotime($lastCheckInDay)) == date('Ymd');
    ...
    public function checkIn($nodeId, $memberId) {
        // 判断是否签到是否开启
        $integralConfig = M('tintegral_node_config')->where("node_id = '$nodeId'")->find();
        ...
        // 连续签到
        if (date('Ymd', strtotime('+1 day', strtotime($lastCheckInDay))) == date('Ymd')) {
            // todo:确认连续签到逻辑。。。。
            $sign_count = $sign_count + 1;
            if ($sign_count == 7) {
                $point    = $integralConfig['7day_sign_rate'];
                $con_sign = '1';
                $res      = D("MemberBehavior")->addBehaviorType($memberId, $nodeId, 8, $point);
                if ($res === false) {
                    M()->rollback();
                    $this->error("行为增加失败!");
                }
                ...
                $res   = D("MemberBehavior")->addBehaviorType($memberId, $nodeId, 7, $point);
                if ($res === false) {
                    M()->rollback();
                    $this->error("行为增加失败!");
                }
        ...
        // 赠送积分
        $flag = $pointModel->integralPointChange(CommonConst::INTEGRAL_TYPE5, $point, $memberId, $nodeId, $con_sign);
        if ($flag === false) {
            $this->rollback();
    ...
    public function getCheckDaysByMonth($memberId, $month) {
        ...
        $result = M('tintegral_point_trace')->where($map)->field('trace_time')->select();
    ...
    public function resendBarcode($requestId, $newPhone) {
        $barcodeInfo = M('tbarcode_trace')->where(array(
                'request_id' => $requestId,
        ))->field('node_id, phone_no')->find();
        if (!$barcodeInfo) {
            return false;
        }

        if ($barcodeInfo['phone_no'] != $newPhone && $barcodeInfo['phone_no'] == '13900000000') {
            $flag = M('tbarcode_trace')->where(array(
                    'request_id' => $requestId,
            ))->save(array(
                    'phone_no' => $newPhone,
            ));
            ...
            $result = $this->wxTermMemberFlag($barcodeInfo['node_id'], $newPhone, 1, true, array(
                            'channel_id' => $barcodeInfo['channel_id'],
                    ));

            if ($result === false) {
                log_write("getMemberInfo fail! node_id[{$barcodeInfo['node_id']}] condition[{$newPhone}]");

                return true;
            }

            $behaviorModel = D('MemberBehavior', 'Model');
            $behaviorModel->addBehaviorData($result['id'], $barcodeInfo['node_id'], 1, 1);
            log_write("===MEM_DEBUG===记录会员行为数据member_id[{$result['id']}],node_id[{$barcodeInfo['node_id']}],1,1");
    ...
    public function receiveIntegal($nodeId, $integalGetId, $newPhone) {
        $integalInfo = M('tintegal_get_detail')->where(array(
                'id'      => $integalGetId,
                'node_id' => $nodeId,
        ))->find();
        if (!$integalInfo) {
        ...
        $result = $this->wxTermMemberFlag($nodeId, $newPhone, 1, true, array(
                        'channel_id' => $integalInfo['channel_id'],
                        'batch_id' => $integalInfo['m_id']
                ));

        if ($result === false) {
            log_write("getMemberInfo fail! node_id[{$nodeId}] condition[{$newPhone}]");

            return false;
        }

        $flag = M('tintegal_get_detail')->where(array(
                'id'      => $integalGetId,
                'node_id' => $nodeId,
        ))->save(array(
                'member_id' => $result['id'],
        ...
        $rs = D('IntegralPointTrace')->integralPointChange(12, $integalInfo['integral_num'], $result['id'], $nodeId,
                $integalGetId);
        if ($rs === false) {
            log_write("===MEM_DEBUG===增加积分失败：" . print_r(array(
                            12,
        ...
        $bflag = D('MemberBehavior', 'Model')->addBehaviorType($result['id'], $nodeId, 15, $integalInfo['integral_num'],
                $integalInfo['m_id']);
        if ($bflag === false) {
            $this->log("===MEM_DEBUG===记录会员行为数据失败[抽奖领取积分]member_id[{$result['id']}],node_id[{$nodeId}],1");
        }
    ...
    public function receiveBonus($nodeId, $bonsUseDetailId, $newPhone) {
        $bonsInfo = M('tbonus_use_detail')->where(array(
                'id'      => $bonsUseDetailId,
                'node_id' => $nodeId,
        ))->find();
        if (!$bonsInfo) {
        ...
        $channelId = M('tcj_trace')->where(array(
                'request_id' => $bonsInfo['request_id'],
        ))->getField('channel_id');
        $result    = $this->wxTermMemberFlag($nodeId, $newPhone, 1, true, array(
        ...
        if ($result === false) {
            log_write("receiveBonus fail! node_id[{$nodeId}] condition[{$newPhone}]");

            return false;
        }

        $flag = M('tbonus_use_detail')->where(array(
                'id'      => $bonsUseDetailId,
                'node_id' => $nodeId,
        ))->save(array(
                'phone' => $newPhone,
        ...
        $bflag = D('MemberBehavior', 'Model')->addBehaviorType($result['id'], $nodeId, 14, '', $bonsUseDetailId);
        if ($bflag === false) {
            $this->log("===MEM_DEBUG===记录会员行为数据失败[抽奖领取红包]member_id[{$result['id']}],node_id[{$nodeId}],1");
        }
    ...
    public function orderInfoSelectPointOne($orderId, $nodeId) {
        ...
        $orderInfo = M()->table("ttg_order_info a")->field('a.order_amt,a.order_phone,a.receiver_type,b.change_num')->join('tintegral_point_trace b on b.relation_id=a.order_id')->where(array(
                'a.node_id'  => $nodeId,
                'a.order_id' => $orderId,
        ))->find();
        if (!$orderInfo) {
    ...
    public function orderPay($orderId, $nodeId, $labelId) {
        log_write("访问进来了,orderId" . $orderId . "nodeId" . $nodeId . "labelId" . $labelId);
        if (empty($orderId)) {
            log_write("订单号为空");
        ...
        if ($orderInfo['change_num']) {
            log_write("该订单号已经赠送过积分,订单号：" . $orderId . "商户号" . $nodeId);
        ...
        $memberInfo = $this->telTermMemberFlag($nodeId, $orderInfo['order_phone']);
        if (!$memberInfo) {
            // 新增会员
            $res = $this->wxTermMemberFlag($nodeId, $orderInfo['order_phone'], 1, true, array(
                            'channel_id' => $labelId,
                    ));
            if ($res === false) {
        ...
        if ($orderInfo['order_amt'] >= 1) {
            // 开始算积分，查询是否开设积分营销
            $IntegralConfigNodeModel = new IntegralConfigNodeModel();
            $integralConfig          = $IntegralConfigNodeModel->checkIntegralConfig($nodeId);
            ...
            if ($integralConfig) {
                if ($integralConfig['shop_online_flag'] == '1') {
                    ...
                    $pointStatus        = $IntegralPointTrace->integralPointChange('1', $changeNum, $memberInfo['id'],
                            $nodeId, $orderId, '');
        ...
        // 增加行为数据
        $behaviorRes = D("MemberBehavior")->addBehaviorType($memberInfo['id'], $nodeId, 4, $changeNum, $orderId,
                $orderInfo['order_amt']);
    ...
    public function cacheCityData($province, $city = '', $town = '') {
        ...
        if (!$mp) {
            $pList = M('tcity_code')->field("province_code, province")->where(array(
                    'city_level' => '1',
            ))->select();
            if (!$pList || $province == '') {
                return $cityCode;
        ...
        if (!$mc) {
            $cList = M('tcity_code')->field("province_code, city_code, city")->where(array(
                    'city_level' => '2',
            ))->select();
            if (!$cList || $city == '') {
                return $cityCode;
        ...
        foreach ($mc as $kc => $vc) {
            if ($cityCode == $vc['province_code'] && stristr($vc['city'], $city) !== false) {
                $cityCode .= $vc['city_code'];
                $mt = S('mem_town_' . $vc['city_code']);
                if (!$mt) {
                    $tList = M('tcity_code')->field("province_code, city_code, town_code, town")->where(array(
                            'city_level'    => '3',
                ...
                foreach ($mt as $kt => $vt) {
                    if ($cityCode == $vt['province_code'] . $vt['city_code'] && stristr($vt['town'], $town) !== false) {
    ...
    public function deductionMemberPoint($node_id, $phone, $point, $relation_id) {
        ...
        $flag     = $integral->integralPointChange(17, $point, $member['id'], $node_id, $relation_id);
        if ($flag === false) {
            log_write("积分扣减，积分扣减失败,会员id:" . $member['id'] . "，订单号:" . $relation_id . "，返还积分:" . $point);
    ...
    public function backMemberPoint($node_id, $phone, $relation_id) {
        ...
        $ptDFlag = M('tintegral_point_trace')->where(array(
                'type'        => 17,
                'member_id'   => $member['id'],
                'relation_id' => $relation_id,
        ))->count();
        if ($ptDFlag < 1) {
            log_write("积分退还，未找到原始扣减流水,会员id:" . $member['id'] . "，订单号:" . $relation_id);

            return false;
        }

        $ptBFlag = M('tintegral_point_trace')->where(array(
                'type'        => 18,
                'member_id'   => $member['id'],
                'relation_id' => $relation_id,
        ))->find();
        if (!$ptBFlag) {
            log_write("积分退还，【重复调用】扣减积分已经返还,会员id:" . $member['id'] . "，订单号:" . $relation_id);

            return false;
        }

        $integral = D('IntegralPointTrace', 'Model');
        $flag     = $integral->integralPointChange(18, $ptBFlag['change_num'], $member['id'], $node_id, $relation_id);
        if ($flag === false) {
            log_write("积分退还，积分返还失败,会员id:" . $member['id'] . "，订单号:" . $relation_id . "，返还积分:" . $point);
