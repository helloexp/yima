...
abstract class ThinkOauth{
     ...
     */
    public static function getInstance($type, $token = null,$conf=null) {
    	$name = ucfirst(strtolower($type)) . 'SDK';
    	require_once "sdk/{$name}.class.php";
    	if (class_exists($name)) {
    		return new $name($token,$conf);
    	} else {
	 ...
	 * 初始化配置
	 */
	private function config($conf=null){
				
		$config = C("THINK_SDK_{$this->Type}");
		if(!empty($conf)){
			$this->Authorize = $conf['authorize'];
		}else{
	 ...
	 * 请求code 
	 */
	public function getRequestCodeURL($conf=null){
	 ...
	 */
	public function getAccessToken($code, $extend = null,$conf=null){
		$this->config($conf);
		$params = array(
				'client_id'     => $this->AppKey,
