...
class Db {
    ...
    /**
     * 是否需要从主库查询 (sql在一个事务中或者 为 select _nextval 都需要到主库)
     * @author   Jeff Liu<liuwy@imagec.com.cn>
     *
     * @param      $sql
     *
     ...
     * @return bool
     */
    public function needQueryFromMaster($sql, $forceQueryMaster = false)
    {
        $queryMaster = $forceQueryMaster;
        if (!$queryMaster) {
            $queryMaster = $this->transTimes > 0; //如果在事务中，强制链接master

            if (!$queryMaster) { //没有处在一个事务中，查看是否 有 select _nextval(*) 语句，如果有的话 强制转到master  这个语句为包含update操作
                $pattern = '/SELECT\s+_nextval\(/i';
    ...
    public function getLinkConf($linkNum)
    {
        return isset($this->linkInfo[$linkNum]['config']) ? $this->linkInfo[$linkNum]['config'] : '';
    }
     ...
     * @param null $useTime
     */
    protected function debug($numRows = null, $error = null, $useTime = null) {
        ...
        if (C('DB_SQL_LOG')) {
            G('queryEndTime');
            trace($this->queryStr .'[numRows: '.$numRows.']'.'[error:'.$error.']'.' [ RunTime:'.G('queryStartTime','queryEndTime',6).'s ]','','SQL');
        }
        if (!is_null($useTime)) {
            $msg .= 'RunTime:'.$useTime .'s';
        } else {
            $msg .= 'RunTime:'.G('queryStartTime','queryEndTime',6) . 's';
        }
        if(function_exists('log_write')) {
            //log_write($msg, Log::SQL, 'finalsql');
        }
    }
    ...
    protected function parseJoin($join) {
		...
		$joinStr = preg_replace_callback("/__([A-Z_-]+)__/sU",function($matches){return C("DB_PREFIX").strtolower($matches[0]);},$joinStr);
        return $joinStr;
    }
    ...
    public function select($options=array()) {
        ...
        if (isset($options['forceQueryMaster'])) {
            $forceQueryMaster = $options['forceQueryMaster'];
        } else {
            $forceQueryMaster = C('DB_RW_DEFAULT_W');
            $forceQueryMaster = (bool) $forceQueryMaster;
        }
        $result   = $this->query($sql,$this->parseBind(!empty($options['bind'])?$options['bind']:array()), $forceQueryMaster);
    ...
    public function query($str, $bind = array(), $forceQueryMaster = true){
        return new Exception(__METHOD__ .' is not implemented yet!');
    }
