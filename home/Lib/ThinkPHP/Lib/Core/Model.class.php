    ...
    protected function _checkTableInfo() {
        ...
        if(empty($this->fields)) {
            ...
            if(C('DB_FIELDS_CACHE')) {
                $fieldsCachedFileName = $this->getFieldCachedName();
                $fields = F('_fields/'.$fieldsCachedFileName);
                if($fields) {
...
//        if (in_array($tableNameTmp, ['SELECT', 'INSERT', 'UPDATE', 'DELETE', 'SHOW', 'CREATE', 'CALL'])) {//应对 M()->table('select * from xxxx') 这类变态写法
//            return false;
//        }
        if (stripos($tableName, 't') === 0 || strtolower($tableName) === 'index_count') { //所有的表都是t开头
            return true;
        }
        return false;
    }
    ...
    public function flush() {
        ...
        $needGetFields = $this->needGetFields($this->getTableName());
        $fields = false;
        if ($needGetFields) {
        ...
        if(C('DB_FIELDS_CACHE')){
            // 永久缓存数据表信息
            $fieldsCachedFileName = $this->getFieldCachedName();
            F('_fields/'.$fieldsCachedFileName,$this->fields);
        }


    }

    private function getFieldCachedName()
    {
        $tableName = '';
        preg_match('|\w+|', $this->name, $tableNameList); //去掉表别名
        ...
        return strtolower($db.'.'.$tableName);
    }
    ...
    public function add($data='',$options=array(),$replace=false) {
        ...
        if ($error) {
            log_write('add sql:'.$this->_sql() .' db error:'.$error);
        }
        return $result;
    }
    // 插入数据前的回调方法
    ...
    public function addAll($dataList,$options=array(),$replace=false){
        ...
        $error = $this->getDbError();
        if ($error) {
            log_write('addAll sql:'.$this->_sql() .' db error:'.$error);
        }
        return $result;
    }
    ...
    public function selectAdd($fields='',$table='',$options=array()) {
        ...
        if(false === $result = $this->db->selectInsert($fields?$fields:$options['field'],$table?$table:$this->getTableName(),$options)){
            ...
            $error = $this->getDbError();
            if ($error) {
                log_write('select sql:'.$this->_sql() .' db error:'.$error);
            }
            return false;
        }else {
            // 插入成功
    ...
    public function save($data='',$options=array()) {
        ...
        $error = $this->getDbError();
        if ($error) {
            log_write('save sql:'.$this->_sql() .' db error:'.$error);
        }
     ...
     * @return Model
     */
    public function selectDbType($master = false)
    {
        $this->options['forceQueryMaster'] = $master;
        return $this;
    }

    public function selectSlave()
    {
        return $this->selectDbType(false);
    }

    public function selectMaster()
    {
        return $this->selectDbType(true);
    }

    public function getCurrentLinkConf()
    {
        return $this->db->getLinkConf($this->db->getLinkNum());
    }
