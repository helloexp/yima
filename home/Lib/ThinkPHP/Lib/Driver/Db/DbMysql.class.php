...
class DbMysql extends Db{
    ...
    public function connect($config='',$linkNum=0,$force=false) {
        if (!isset($this->linkInfo[$linkNum]) || $this->linkInfo[$linkNum]['config'] != $config) {
            ...
            if($pconnect) {
                $mysql = mysql_pconnect( $host, $config['username'], $config['password'],131072);
            }else{
                $mysql = mysql_connect( $host, $config['username'], $config['password'],true,131072);
            }
            if ( !$mysql || (!empty($config['database']) && !mysql_select_db($config['database'], $mysql)) ) {
                throw_exception(mysql_error());
            }
            $dbVersion = mysql_get_server_info($mysql);

            //使用UTF8存取数据库
            mysql_query("SET NAMES '".C('DB_CHARSET')."'", $mysql);
            mysql_query("SET character_set_client=".C('DB_CHARSET'), $mysql);
            mysql_query("SET character_set_results=".C('DB_CHARSET'), $mysql);

            //设置 sql_model
            if($dbVersion >'5.0.1'){
                mysql_query("SET sql_mode=''", $mysql);
            }
            // 标记连接成功
            $this->connected    =   true;
            // 注销数据库连接配置信息
            if(1 != C('DB_DEPLOY_TYPE')) unset($this->config);
            $this->linkInfo[$linkNum]['link'] = $mysql;
    ...
    public function connectOld($config='',$linkNum=0,$force=false) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            $this->setLinkConf(array('host' => $host, 'user' => $config['username'], 'pass' => $config['password']), $linkNum);
    ...
    private function getAll() {
        ...
        if($this->numRows >0) {
            while($row = mysql_fetch_assoc($this->queryID)){
                ...
                if (is_array($row) && $changeKeyCase && !isset($row['Field'])) { //排除 show column from 语句结果
                    foreach ($row as $index => $item) {
                        if ($this->needChangeCase($index)) {
                            $row[strtolower($index)] = $item;
                        }
                    }
                }
                $result[]   =   $row;
